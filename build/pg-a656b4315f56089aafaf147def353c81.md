...
### The Off-Policy Gradient Estimator
...
The full trajectory weight $\prod_{l=0}^T \rho_l$ is replaced by a single per-step weight $\rho_t$, dramatically reducing variance.

#### Control Variates

The control variate technique applies directly to the off-policy estimator. Subtracting a baseline $b(s_t)$ from the return preserves unbiasedness:

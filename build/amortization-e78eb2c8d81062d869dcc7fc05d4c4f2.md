...
DDPG inherits the overestimation bias from DQN's use of the max operator in Bellman targets. TD3 {cite}`fujimoto2018addressing` addresses this through three modifications to the DDPG template, following similar principles to Double DQN but adapted for continuous actions:

1. **Twin Q-networks (Clipped Double Q-learning)**: Learn two independent critics $q^A(s,a; \boldsymbol{\theta}^A)$ and $q^B(s,a; \boldsymbol{\theta}^B)$, use the minimum for targets: $\min(q^A, q^B)$. This extends Double DQN's idea but with explicit twin networks.

2. **Target policy smoothing**: Add clipped noise to the target policy's actions $\pi_{\boldsymbol{w}_{\text{target}}}(s') + \varepsilon$ when computing targets. This regularization prevents the policy from exploiting narrow peaks in the Q-function.

3. **Delayed policy updates**: Update the actor less frequently than the critics (every $d$ steps instead of every step). This reduces per-update error by letting the critic converge before the actor adapts.
...
5. **while** training **do**
    ...
    7. For each sampled transition $(s_i,a_i,r_i,s_i')$:
        ...
        2. $y_i \leftarrow r_i + \gamma \min\big(q^A(s'_i, \tilde{a}_i; \boldsymbol{\theta}^A_{\text{target}}), q^B(s'_i, \tilde{a}_i; \boldsymbol{\theta}^B_{\text{target}})\big)$ (clipped double Q)
    8. $\boldsymbol{\theta}^A_{t+1} \leftarrow \boldsymbol{\theta}^A_t - \alpha_q \nabla_{\boldsymbol{\theta}} \frac{1}{b}\sum_{i=1}^b(q^A(s_i,a_i;\boldsymbol{\theta}^A_t) - y_i)^2$
    9. $\boldsymbol{\theta}^B_{t+1} \leftarrow \boldsymbol{\theta}^B_t - \alpha_q \nabla_{\boldsymbol{\theta}} \frac{1}{b}\sum_{i=1}^b(q^B(s_i,a_i;\boldsymbol{\theta}^B_t) - y_i)^2$
    10. **// Delayed policy update and target network updates**
...
The algorithm maintains the DDPG structure but with key differences in target computation and update frequency. Lines 7.1-7.2 implement the three core modifications: smoothing adds noise to target actions, the min operation over twin critics reduces overestimation, and line 11 gates policy updates to occur only every $d$ steps (typically $d=2$). Both critics are updated every step (lines 8-9) using the same shared target from the min operation, ensuring they remain calibrated to each other. The EMA updates (lines 11.2-11.4) provide smoother target evolution than DDPG's hard updates, following the same principle as Algorithm {prf:ref}`nfqi-flattened-ema`.

TD3 also simplifies exploration by replacing DDPG's Ornstein-Uhlenbeck process with uncorrelated Gaussian noise $\varepsilon \sim \mathcal{N}(0, \sigma_{\text{explore}}^2)$ in line 3. This eliminates the need to tune multiple OU parameters ($\theta, \mu, \sigma$) while providing equally effective exploration. 

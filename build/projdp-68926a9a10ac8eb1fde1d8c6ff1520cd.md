...
Having chosen our basis and defined the residual, we must decide how to make the residual "close to zero." The key idea, as discussed in the introduction, is to test the residual using **weighted integrals**. We select test functions (also called weighting functions) $\{p_1, \ldots, p_n\}$, then impose **weighted residual conditions**:

$$
\int_{\mathcal{S}} p_i(x) R(x; \theta) \, dx = 0, \quad i = 1, \ldots, n.
$$
...
$$
\min_\theta \int_{\mathcal{S}} R(x; \theta)^2 dx.
$$

...
These design choices are largely orthogonal. We can combine smooth Bellman operators with replay buffers (soft Q-learning), and apply double Q-learning to any architecture that maximizes over actions. The modularity of the template explains both the proliferation of algorithms in the deep RL literature and the underlying commonalities. Each new method typically modifies one or two design dimensions while leaving others fixed.

The methods developed here all belong to the value-based family: they approximate value functions and extract policies by maximization or softmax. An alternative approach learns policies directly through policy gradient methods. For continuous action spaces where maximization becomes an optimization problem, actor-critic architectures that combine value-based critics with parametrized policy networks offer a natural extension of the fitted Q-iteration framework. These policy parametrization methods, including NFQCA, DDPG, and SAC, are covered in detail in the [chapter on policy parametrization methods](cadp.md). Understanding both value-based and policy-based perspectives is necessary for modern reinforcement learning, and the unified framework developed here provides the foundation for comparing and combining them.

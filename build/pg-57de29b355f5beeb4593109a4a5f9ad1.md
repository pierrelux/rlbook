...
where $A_t = G_t - b(s_t)$. When $\pi_q = \pi_{\boldsymbol{w}}$, the ratios $\rho_t = 1$ and the approximation becomes exact, recovering the score function estimator. The approximation error grows as the policies diverge, which motivates the trust region and clipping mechanisms discussed below.
...
The variance issues suggest a natural solution: keep the ratio $\rho_t$ close to 1 by ensuring the new policy stays close to the behavior policy. This keeps the importance-weighted surrogate $L^{\text{IS}}(\boldsymbol{w})$ from {eq}`importance-weighted-surrogate` well-behaved.

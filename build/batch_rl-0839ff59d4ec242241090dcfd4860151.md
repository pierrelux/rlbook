...
The [previous chapter](simadp.md) established the theoretical foundations of simulation-based approximate dynamic programming: Monte Carlo integration for evaluating expectations, Q-functions for efficient action selection, and techniques for mitigating overestimation bias. Those developments assumed we could sample freely from transition distributions and choose optimization parameters without constraint. This chapter addresses the practical reality of reinforcement learning: we work with fixed datasets of transitions collected offline, each state-action pair typically yields only a single observed next state, and computational constraints force us to make deliberate choices about function approximators and optimization strategies.

## A Unified View of Reinforcement Learning Algorithms

The theoretical components from the previous chapter—fitted-value iteration from [projection methods](projdp.md), Q-functions, and Monte Carlo integration—combine into a simple template: $\hat{q}^{(k+1)} = \text{fit}(\{((s_i, a_i), y_i)\})$, where targets $y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \theta^{(k)})$ use the single observed next state $s'_i$ from each transition $(s_i, a_i, r_i, s'_i)$. This template encompasses all major value-based reinforcement learning algorithms. Different instantiations—choices of function approximator, optimization strategy, data collection regime, and bias mitigation technique—yield FQI, NFQI, DQN, Q-learning, and their variants. This section systematically maps this design space.

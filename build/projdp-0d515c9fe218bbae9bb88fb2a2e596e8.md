...
The condition (EC) says that the way the current controlled state $y$ affects the next controlled state $y'$ is "aligned" with the way the action $a$ affects $y'$. More precisely: the Jacobian $\partial f/\partial y$ lies in the column space of $\partial f/\partial a$, with the matrix $h$ providing the linear combination.

In the scalar case ($n=m=1$), condition (EC) simply requires

$$
\frac{\partial f}{\partial y} = \frac{\partial f}{\partial a} \cdot h,
$$

meaning the two partial derivatives are proportional. This holds whenever the dynamics are affine in $(y,a)$, which covers many applications.
...
- **Many practical problems** fit the Euler-class framework: inventory/production, energy storage, reservoir control, and building thermal models all have affine stock dynamics that satisfy the structural condition (EC).

...
:caption: Closed-loop metrics for the four frequency controllers under identical shifted arrivals and the fixed interleaved chunked-prefill surrogate. Fallback count records every MPC step that exceeded its feasibility, finiteness, or timing requirement.
...
2.80 seconds, at 2,422.5 joules. The unchanged offline plan uses 2,264.2 joules
but raises that latency to 6.39 seconds. MPC lies between them at 3.23 seconds
and 2,364.7 joules. The reactive governor uses slightly less energy than MPC,
2,330.7 joules, but has a higher mean time to first token of 4.11 seconds. MPC
accepts 55 solves and invokes the recorded fallback five times; its maximum
solve time is 0.054 seconds, below the 0.8-second deadline. Thus no controller

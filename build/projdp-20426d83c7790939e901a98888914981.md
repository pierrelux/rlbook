...
LSTD finds the fixed point of the *projected* Bellman operator, not the Bellman operator itself, so approximation error persists. But will iterative methods converge to this fixed point? For convergence of $v_{k+1} = \Proj \BellmanPi v_k$, we need $\Proj \BellmanPi$ to be a contraction. The projection $\Proj$ is always non-expansive in $\|\cdot\|_\xi$ by the Pythagorean identity. The Bellman operator $\BellmanPi = r_\pi + \gamma \mathbf{P}_\pi$ is a $\gamma$-contraction in $\|\cdot\|_\infty$, but whether it contracts in $\|\cdot\|_\xi$ depends on the relationship between $\xi$ and $\mathbf{P}_\pi$.

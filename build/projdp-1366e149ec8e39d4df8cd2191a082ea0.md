...
The weighted residual methods developed in this chapter have focused on linear approximation architectures: polynomial bases, piecewise linear interpolants, and linear combinations of fixed basis functions. Neural networks, kernel methods, and decision trees do not fit this template. What does the minimum residual framework mean when the approximation class $\mathcal{F}$ is not a linear subspace?

The answer lies in recognizing that weighted residual methods define a variational principle that extends beyond linear spaces. The linear case is special because orthogonality and minimization coincide: minimizing the norm of the residual over a finite-dimensional subspace is equivalent to enforcing weighted orthogonality $\langle R[v], \varphi_i \rangle_w = 0$ against a set of basis functions. For nonlinear approximators, orthogonality is replaced by a first-order optimality condition. The underlying principle remains the same: find the function in $\mathcal{F}$ that makes the residual as small as possible in a chosen norm.

### Dual Norms and Distributional Residuals

When the approximation class $\mathcal{F}$ is nonlinear, we can no longer decompose the residual into components along basis vectors. We instead measure the size of the residual using a **dual norm** induced by a test space $\mathcal{W}$. Let $\mathcal{H}$ be a Banach space (a complete normed vector space) and $\mathcal{H}'$ its dual space: the set of all continuous linear functionals on $\mathcal{H}$. The dual norm of a functional $T \in \mathcal{H}'$ is defined by

$$
\|T\|_{\mathcal{H}'} = \sup_{\substack{w \in \mathcal{H} \\ \|w\|_{\mathcal{H}} = 1}} |\langle T, w \rangle|,
$$

where $\langle T, w \rangle$ denotes the **dual pairing**: the action of the functional $T$ on the test function $w$. In the Hilbert space case, where $\mathcal{H} = L^2(\Omega)$, this reduces to the familiar inner product $\langle f, g \rangle = \int_\Omega f(x) g(x) \, dx$, and the dual norm coincides with the $L^2$ norm. For more general function spaces, the dual pairing allows us to define residuals that may not even be classical functions.

Consider collocation with Dirac delta test functions $\psi_i = \delta(x - x_i)$. The residual $R[v] = v - \Bellman v$ is evaluated pointwise, but the "test function" $\delta(x - x_i)$ is not a function in the classical sense: it is a distribution. The dual pairing $\langle R[v], \delta(x - x_i) \rangle = R[v](x_i)$ is nonetheless well-defined. Similarly, when solving partial differential equations with weak derivatives, the residual may contain discontinuities or impulses that only make sense when integrated against smooth test functions. The dual pairing generalizes the notion of "residual orthogonality" to cases where neither the residual nor the test functions are regular functions {cite}`LegrandJunca2025`.

### From Linear Projection to Manifold Optimization

In Galerkin projection with a linear subspace $\mathcal{F} = \text{span}(\boldsymbol{\Phi})$, the orthogonality condition $\langle v - \Bellman v, \varphi_i \rangle_w = 0$ for all $i$ defines a linear system $\boldsymbol{\Phi}^\top \mathbf{W} (\boldsymbol{\Phi} \boldsymbol{\theta} - \mathbf{y}) = \mathbf{0}$. The solution minimizes the weighted squared residual $\|\boldsymbol{\Phi} \boldsymbol{\theta} - \mathbf{y}\|_\mathbf{W}^2$ over parameter vectors $\boldsymbol{\theta} \in \mathbb{R}^d$.

When $\mathcal{F}$ is a nonlinear function class, such as the set of functions computable by a neural network with architecture $\mathcal{A}$ and parameters $\boldsymbol{\theta}$, the residual minimization becomes

$$
\boldsymbol{\theta}^* = \arg\min_{\boldsymbol{\theta}} \|v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}\|_{\mathcal{W}'}^2.
$$

This defines a **nonlinear optimization problem** over the parameter space. The parameterized functions $v_{\boldsymbol{\theta}}$ form a **manifold** $\mathcal{M}_{\boldsymbol{\theta}} = \{v_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \mathbb{R}^p\}$ embedded in a function space. The minimum residual principle seeks the point on this manifold closest to satisfying the Bellman equation in the dual norm.

The first-order optimality condition is

$$
\nabla_{\boldsymbol{\theta}} \|v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}\|_{\mathcal{W}'}^2 = \mathbf{0}.
$$

This generalizes the Galerkin normal equations $\boldsymbol{\Phi}^\top \mathbf{W} (\boldsymbol{\Phi} \boldsymbol{\theta} - \mathbf{y}) = \mathbf{0}$ from a linear system to a gradient stationarity condition. In practice, this is solved by gradient descent or other nonlinear optimization algorithms. The resulting solution defines an implicit **nonlinear projection operator** $\Pi_{\mathcal{F}}$, which replaces the linear orthogonal projection $\Proj$ in the fitted-value iteration scheme.

### Connection to Neural Residual Methods

Physics-informed neural networks (PINNs) and related methods instantiate this framework. A neural network approximates the value function $v_{\boldsymbol{\theta}}$, and the training loss measures the residual norm:

$$
\mathcal{L}(\boldsymbol{\theta}) = \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - (\Bellman v_{\boldsymbol{\theta}})(s))^2 \big],
$$

where $\mu$ is a sampling distribution over states. Minimizing this loss by gradient descent produces a function $v_{\boldsymbol{\theta}^*}$ that approximately satisfies the Bellman equation in a Monte Carlo approximation of the $L^2(\mu)$ norm. The dual pairing is implemented implicitly through automatic differentiation and sampling: the gradient $\nabla_{\boldsymbol{\theta}} \mathcal{L}(\boldsymbol{\theta})$ is computed by backpropagation, and the expectation is approximated by sample averages.

Weak adversarial networks extend this by explicitly modeling the test functions as a second neural network. One network represents the trial function $v_{\boldsymbol{\theta}}$, and another represents the test function $w_{\boldsymbol{\phi}}$. The training objective becomes a minimax problem:

$$
\min_{\boldsymbol{\theta}} \max_{\boldsymbol{\phi}} \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - (\Bellman v_{\boldsymbol{\theta}})(s)) \, w_{\boldsymbol{\phi}}(s) \big],
$$

which directly implements the dual norm definition $\|R[v]\|_{\mathcal{W}'} = \sup_{w \in \mathcal{W}, \|w\|=1} \langle R[v], w \rangle$. This makes explicit the adversarial interpretation of the dual pairing: the residual's magnitude is measured by how large it appears to the most sensitive test function.

### Unified Perspective

The minimum residual framework unifies classical numerical analysis with modern machine learning methods for solving functional equations. Linear Galerkin projection, collocation, least squares, and neural residual methods all instantiate the same variational principle: minimize the residual norm in a chosen dual pairing. The differences lie in:

1. The approximation class $\mathcal{F}$: linear subspace (classical methods) versus nonlinear manifold (neural networks, kernel methods).
2. The test space $\mathcal{W}$: smooth functions (Galerkin), point evaluations (collocation), or learned adversarial functions (weak adversarial networks).
3. The optimization algorithm: direct linear solve (Galerkin, collocation), gradient descent (neural networks), or minimax optimization (adversarial methods).

The convergence analysis differs significantly between the linear and nonlinear cases. For linear projections, we have shown contraction guarantees when the projection is monotone or when the weighting matches the operator's structure (policy evaluation with stationary distribution). For nonlinear manifolds, convergence is generally not guaranteed: the loss landscape may have local minima, and the implicit projection operator $\Pi_{\mathcal{F}}$ may not be well-defined or continuous. Neural network training for Bellman residual minimization remains an active research area, with convergence results typically requiring strong assumptions on architecture, initialization, and data distribution.

The framework nonetheless provides a principled foundation for understanding modern function approximation in dynamic programming. Fitted-value iteration with expressive function classes is minimum residual iteration over parameterized manifolds. The dual pairing perspective clarifies what it means to "approximately solve" a functional equation when exact solutions are intractable, connecting the classical theory developed in this chapter to contemporary methods in reinforcement learning and scientific machine learning.

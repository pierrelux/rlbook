...
#### Multi-Step Chaining: From Q-Functions to Path Consistency

The deterministic structure $s_{t+1} = f(s_t, a_t)$ allows us to chain the Q-function equation {eq}`eq:q-single-step` over multiple steps. Consider a trajectory segment $s_0, a_0, s_1, a_1, \ldots, s_{d-1}, a_{d-1}, s_d$ where each transition satisfies $s_{t+1} = f(s_t, a_t)$.

Start with the Q-function at $(s_0, a_0)$:

$$
q^*(s_0, a_0) = r(s_0, a_0) + \gamma v^*(s_1)
$$

Now expand $v^*(s_1)$ using its definition as an expectation under the policy:

$$
v^*(s_1) = \mathbb{E}_{a_1 \sim \pi^*(·|s_1)}[q^*(s_1, a_1) - \alpha \log \pi^*(a_1|s_1)]
$$

For an observed action $a_1$, we can write:

$$
q^*(s_1, a_1) - \alpha \log \pi^*(a_1|s_1) = v^*(s_1) + \varepsilon_1
$$

where $\varepsilon_1$ is the sampling noise: $\mathbb{E}_{a_1 \sim \pi^*}[\varepsilon_1] = 0$. Substitute into the Q-function:

$$
q^*(s_0, a_0) = r(s_0, a_0) + \gamma[v^*(s_1) + \varepsilon_1]
$$

Now apply the Q-function equation at $(s_1, a_1)$:

$$
q^*(s_1, a_1) = r(s_1, a_1) + \gamma v^*(s_2)
$$

Combining: $q^*(s_1, a_1) - \alpha \log \pi^*(a_1|s_1) = r(s_1, a_1) + \gamma v^*(s_2) - \alpha \log \pi^*(a_1|s_1)$, so:

$$
v^*(s_1) + \varepsilon_1 = r(s_1, a_1) + \gamma v^*(s_2) - \alpha \log \pi^*(a_1|s_1)
$$
...
$$
\begin{align}
q^*(s_0, a_0) &= r(s_0, a_0) + \gamma[r(s_1, a_1) - \alpha \log \pi^*(a_1|s_1) + \gamma v^*(s_2) + \varepsilon_1] \\
&= r(s_0, a_0) + \gamma r(s_1, a_1) - \gamma\alpha \log \pi^*(a_1|s_1) + \gamma^2 v^*(s_2) + \gamma\varepsilon_1
\end{align}
$$

Continue this telescoping through all $d$ steps:

$$
q^*(s_0, a_0) = \sum_{t=0}^{d-1} \gamma^t r(s_t, a_t) - \alpha \sum_{t=1}^{d-1} \gamma^t \log \pi^*(a_t|s_t) + \gamma^d v^*(s_d) + \sum_{t=1}^{d-1} \gamma^t \varepsilon_t
$$ (eq:q-path-expansion)

Now relate back to the value at $s_0$. We have $v^*(s_0) = \mathbb{E}_{a_0}[q^*(s_0, a_0) - \alpha \log \pi^*(a_0|s_0)]$, so for the observed action $a_0$:

$$
q^*(s_0, a_0) - \alpha \log \pi^*(a_0|s_0) = v^*(s_0) + \varepsilon_0
$$

where $\mathbb{E}_{a_0 \sim \pi^*}[\varepsilon_0] = 0$. Combining with equation {eq}`eq:q-path-expansion`:

$$
v^*(s_0) + \varepsilon_0 = \sum_{t=0}^{d-1} \gamma^t r(s_t, a_t) - \alpha \sum_{t=0}^{d-1} \gamma^t \log \pi^*(a_t|s_t) + \gamma^d v^*(s_d) + \sum_{t=1}^{d-1} \gamma^t \varepsilon_t
$$

Rearranging:

$$
v^*(s_0) - \gamma^d v^*(s_d) - \sum_{t=0}^{d-1} \gamma^t[r(s_t, a_t) - \alpha \log \pi^*(a_t|s_t)] = -\varepsilon_0 + \sum_{t=1}^{d-1} \gamma^t \varepsilon_t
$$ (eq:path-residual-with-noise)

Define the **path consistency residual**:

$$
C(s_0:s_d; \pi^*, v^*) \equiv v^*(s_0) - \gamma^d v^*(s_d) - \sum_{t=0}^{d-1} \gamma^t[r(s_t, a_t) - \alpha \log \pi^*(a_t|s_t)]
$$ (eq:path-residual)

Taking expectation over the action sequence sampled from $\pi^*$: $\mathbb{E}[C(s_0:s_d)] = 0$ because all $\varepsilon_t$ terms have zero mean. For any single observed trajectory, the residual equals the accumulated sampling noise from drawing actions from the stochastic policy rather than averaging.

**Key insight**: Under deterministic dynamics, the Q-function Bellman equation {eq}`eq:q-single-step` chains directly without nested expectations. The only randomness is in action selection. This telescoping is what makes multi-step path consistency tractable.

#### The Off-Policy Property: Constraints That Hold for Any Action Sequence

Examine equation {eq}`eq:path-residual` carefully. The residual $C(s_0:s_d; \pi^*, v^*)$ depends only on:
- The observed states $\{s_0, \ldots, s_d\}$ and rewards $\{r_0, \ldots, r_{d-1}\}$
- The value function $v^*$ evaluated at endpoints $s_0$ and $s_d$
- The policy $\pi^*$ evaluated at the observed state-action pairs $(s_t, a_t)$

Crucially, the residual does **not** depend on the **behavior policy** that generated the actions. The actions $\{a_0, \ldots, a_{d-1}\}$ appear in the residual through:
1. The observed rewards $r(s_t, a_t)$ (these are facts about what happened)
2. The log-probability $\log \pi^*(a_t|s_t)$ (how likely the optimal policy would have taken these actions)

The behavior policy $\mu$ that actually selected the actions never appears. The actions could have been generated by:
- The optimal policy: $a_t \sim \pi^*(·|s_t)$
- A different behavior policy: $a_t \sim \mu(·|s_t)$
- Expert demonstrations
- Random exploration
- Old versions of the policy stored in a replay buffer

Why does the behavior policy not appear? Because under **deterministic dynamics**, once we observe the action sequence $(a_0, \ldots, a_{d-1})$, the entire trajectory $(s_0, a_0, s_1, a_1, \ldots, s_d)$ is completely determined by the initial state and those actions: $s_{t+1} = f(s_t, a_t)$. The path consistency constraint is an identity that $(v^*, \pi^*)$ must satisfy for this observed sequence, regardless of why those actions were chosen.

This is fundamentally different from standard off-policy methods. Traditional TD learning requires **importance sampling** corrections $\prod_{t=0}^{d-1} \pi(a_t|s_t)/\mu(a_t|s_t)$ to reweight trajectories because the expectation $\mathbb{E}_{\tau \sim \mu}[\cdot]$ differs from $\mathbb{E}_{\tau \sim \pi}[\cdot]$. These product terms have variance that grows exponentially with $d$. PCL avoids importance sampling entirely: the path consistency constraint holds for **every** observed trajectory as a deterministic identity, not as an average.

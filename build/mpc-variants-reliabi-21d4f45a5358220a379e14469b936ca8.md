...
Two development acquisitions completed the full steady workload at 210 MHz,
using 8.55 and 8.30 kJ. Neither reached the bursty trial: the GPU did not return
to its initial temperature reference within the permitted cooling wait, even
when that wait increased from ten to fifteen minutes. These measurements do not
yet qualify a fixed baseline or establish a controller comparison. The startup
conditioning procedure therefore needs further development before confirmation.

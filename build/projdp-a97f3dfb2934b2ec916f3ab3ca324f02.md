...
The weighted residual framework applies to any functional equation arising from an MDP. An alternative to approximating the value function is to directly approximate the **optimal policy** by solving the first-order conditions that characterize optimality {cite}`Judd1992`. 

### Derivation

Consider an MDP with continuous action space $\mathcal{A}$ where the optimal policy $\pi^*(s)$ solves:

$$
\pi^*(s) = \arg\max_{a \in \mathcal{A}} \left\{ r(s, a) + \gamma \sum_{j \in \mathcal{S}} p(j|s,a) v^*(j) \right\}.
$$ 

Assume $r(s,a)$ and $p(\cdot|s,a)$ are differentiable in $a$, $\mathcal{A}$ is convex, and $\pi^*(s)$ is interior. Define $Q^*(s, a) := r(s, a) + \gamma \sum_{j \in \mathcal{S}} p(j|s,a) v^*(j)$. By Danskin's theorem, if $\pi^*(s)$ is an interior maximizer of $a \mapsto Q^*(s,a)$, then:
...
This is a necessary condition (sufficient if $Q^*(s,a)$ is concave in $a$).

To eliminate $v^*$, apply the envelope theorem for the value function. For continuous state spaces where $v^*(s)$ is differentiable, the envelope theorem expresses $\nabla_s v^*$ in terms of reward derivatives. When substituted recursively into the FOC, all future value function terms cancel, leaving the **discrete-time Euler equation**:
...
This is a functional equation in $\pi^*(\cdot)$ alone involving only marginal rewards at time $t$ and expected marginal rewards at time $t+1$. As Rust {cite}`Rust1996` notes, "since $V$ drops out of the first order condition it is not necessary to consider the impact on marginal utility on all future periods $t+2, t+3, \ldots$"

For deterministic dynamics $s' = f(s,a)$, the FOC becomes $\nabla_a r(s,\pi^*(s)) + \gamma \nabla_a f(s,\pi^*(s))^\top \nabla v^*(f(s,\pi^*(s))) = 0$, and the envelope theorem gives the policy-only form above. For constrained actions with $g_k(s,a) \leq 0$, replace the FOC with the KKT conditions.

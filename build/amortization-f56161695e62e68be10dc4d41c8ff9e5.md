...
But suppose the dynamics are **deterministic**: $s' = f(s,a)$. The next state is uniquely determined, so the expectation disappears:
...
$$
v^*(s) = \alpha \log \int_{\mathcal{A}} \exp(q^*(s,a)/\alpha) da = \mathbb{E}_{a \sim \pi^*}[q^*(s,a) - \alpha \log \pi^*(a|s)]
$$

where $\pi^*(a|s) = \exp((q^*(s,a) - v^*(s))/\alpha)$ is the Boltzmann policy.

Now comes the key observation. Take a trajectory segment $(s_0, a_0, s_1, a_1, \ldots, s_d)$ where each transition follows the deterministic dynamics $s_{t+1} = f(s_t, a_t)$. Start with $q^*(s_0, a_0) = r_0 + \gamma v^*(s_1)$. Expand $v^*(s_1)$ using an observed action $a_1$: since $v^*(s_1) = \mathbb{E}_{a_1}[q^*(s_1,a_1) - \alpha\log\pi^*(a_1|s_1)]$, we have $q^*(s_1,a_1) - \alpha\log\pi^*(a_1|s_1) \approx v^*(s_1)$ (up to sampling noise). Substitute $q^*(s_1,a_1) = r_1 + \gamma v^*(s_2)$:

$$
q^*(s_0, a_0) \approx r_0 + \gamma[r_1 - \alpha\log\pi^*(a_1|s_1) + \gamma v^*(s_2)]
$$

Continue this telescoping for $d$ steps, relating $q^*(s_0,a_0)$ to the observed rewards, log-probabilities, and the value at $s_d$:

$$
q^*(s_0, a_0) - \alpha\log\pi^*(a_0|s_0) \approx \sum_{t=0}^{d-1} \gamma^t[r_t - \alpha\log\pi^*(a_t|s_t)] + \gamma^d v^*(s_d)
$$

But $q^*(s_0,a_0) - \alpha\log\pi^*(a_0|s_0) \approx v^*(s_0)$, giving the **path consistency residual**:

$$
R(s_0:s_d) = v^*(s_0) - \gamma^d v^*(s_d) - \sum_{t=0}^{d-1} \gamma^t[r_t - \alpha\log\pi^*(a_t|s_t)] \approx 0
$$ (eq:path-residual)

This residual relates the value function and policy over a $d$-step path using the observed rewards, deterministic transitions, and policy log-probabilities.

**The key insight**: This holds for **any** observed action sequence, regardless of which policy generated it. The behavior policy never appears—we only evaluate $\pi^*$ at the observed actions. This enables off-policy learning without importance sampling.

### From Consistency to Learning: Minimize the Residual

For parametric approximations $(v_{\boldsymbol{\phi}}, \pi_{\boldsymbol{\theta}})$, define the path residual:

$$
C(s_0:s_d; \boldsymbol{\theta}, \boldsymbol{\phi}) = v_{\boldsymbol{\phi}}(s_0) - \gamma^d v_{\boldsymbol{\phi}}(s_{i+d}) - \sum_{t=0}^{d-1} \gamma^t[r_t - \alpha \log \pi_{\boldsymbol{\theta}}(a_t|s_t)]
$$

At optimality, this should be zero. PCL uses the **least-squares residual** approach: minimize the squared error over observed path segments. Given trajectory data with many $d$-step segments, the objective is:

$$
\min_{\boldsymbol{\theta}, \boldsymbol{\phi}} \sum_{\text{segments}} \frac{1}{2} C(s_i:s_{i+d}; \boldsymbol{\theta}, \boldsymbol{\phi})^2
$$

This is fundamentally different from SAC or DDPG. Those methods compute targets $y_i$ using the current Q-function, then fit $q_{\boldsymbol{\theta}}$ to match the targets (successive approximation). PCL never computes targets—it directly minimizes the residual through gradient descent.

The gradient updates follow from differentiating the objective. The residual depends on $\boldsymbol{\theta}$ through the log-probabilities and on $\boldsymbol{\phi}$ through the endpoint values:

$$
\begin{align}
\boldsymbol{\theta}_{k+1} &= \boldsymbol{\theta}_k + \eta_\pi \sum_i C_i \cdot \alpha \sum_{t=0}^{d-1} \gamma^t \nabla_{\boldsymbol{\theta}} \log \pi_{\boldsymbol{\theta}_k}(a_{i+t}|s_{i+t}) \\
\boldsymbol{\phi}_{k+1} &= \boldsymbol{\phi}_k - \eta_v \sum_i C_i \left[\nabla_{\boldsymbol{\phi}} v_{\boldsymbol{\phi}_k}(s_i) - \gamma^d \nabla_{\boldsymbol{\phi}} v_{\boldsymbol{\phi}_k}(s_{i+d})\right]
\end{align}
$$

where $C_i = C(s_i:s_{i+d}; \boldsymbol{\theta}_k, \boldsymbol{\phi}_k)$. The residual weights the gradients: large residuals drive larger updates.
...
4. **while** training **do**
    ...
    3. **for** each $d$-step segment in $\tau'$ **do**
        1. Compute residual: $C_i \leftarrow v_{\boldsymbol{\phi}_k}(s_i) - \gamma^d v_{\boldsymbol{\phi}_k}(s_{i+d}) - \sum_{t=0}^{d-1} \gamma^t[r_{i+t} - \alpha \log \pi_{\boldsymbol{\theta}_k}(a_{i+t}|s_{i+t})]$
        2. Update policy: $\boldsymbol{\theta}_{k+1} \leftarrow \boldsymbol{\theta}_k + \eta_\pi \alpha C_i \sum_{t=0}^{d-1} \gamma^t \nabla_{\boldsymbol{\theta}} \log \pi_{\boldsymbol{\theta}_k}(a_{i+t}|s_{i+t})$
        3. Update value: $\boldsymbol{\phi}_{k+1} \leftarrow \boldsymbol{\phi}_k - \eta_v C_i\left[\nabla_{\boldsymbol{\phi}} v_{\boldsymbol{\phi}_k}(s_i) - \gamma^d \nabla_{\boldsymbol{\phi}} v_{\boldsymbol{\phi}_k}(s_{i+d})\right]$
    4. Remove oldest trajectories if $|\mathcal{R}| > B$
    5. $k \leftarrow k + 1$
5. **return** $\boldsymbol{\theta}_k$, $\boldsymbol{\phi}_k$
```

The algorithm collects trajectories from the current policy and stores them in a replay buffer. At each iteration, it samples a trajectory (possibly old, from a different policy) and performs gradient descent on the path consistency residual for all $d$-step segments. The replay buffer enables off-policy learning: trajectories generated by old policies, expert demonstrations, or exploratory behavior all provide valid training signals because the path consistency constraint holds for any action sequence under deterministic dynamics.
...
**Single-step case ($d=1$)**: The path residual becomes $C(s:s') = v(s) - \gamma v(s') - r + \alpha\log\pi(a|s)$, which equals the soft Bellman residual $q(s,a) - r - \gamma v(s')$. Minimizing $\sum_i C_i^2$ is equivalent to soft Q-learning, though SAC solves this via successive approximation (compute targets, fit) rather than direct residual minimization.

**No entropy ($\alpha \to 0$)**: The residual becomes $v(s_i) - \gamma^d v(s_{i+d}) - \sum_t \gamma^t r_t$, which is the negative $d$-step advantage. But unlike A2C/A3C where $v$ tracks the current policy's value, PCL's value converges to $v^*$ because the residual couples policy and value through optimality, not policy evaluation.

**Multi-step with hard-max**: No analog exists. Hard-max methods require actions to be optimal for the constraint to hold. Soft-max methods work with any action sequence because the entropy terms $-\alpha\log\pi(a_t|s_t)$ account for suboptimal choices.
...
| **Aspect** | **SAC** | **PCL** |
|:-----------|:--------|:--------|
| **Solution method** | Successive approximation: compute targets $y_i$, fit $q$ to targets | Residual minimization: minimize $\sum_i C_i^2$ directly |
...
The deterministic dynamics requirement is the key tradeoff. PCL gains multi-step telescoping and off-policy learning without importance weights, but only for deterministic systems (robotic manipulation, many control tasks). SAC works for general stochastic MDPs.

### Why PCL is an Amortization Method

PCL amortizes at a different level than DDPG/TD3/SAC. Those methods amortize the action maximization: learn a policy network that outputs $\arg\max_a q(s,a)$ directly. PCL amortizes the **solution of the Bellman equation itself**. Instead of repeatedly applying the Bellman operator (which requires $\int_{\mathcal{A}} \exp(q/\alpha) da$ at every iteration), PCL samples path segments and minimizes their consistency error. The computational cost of verifying optimality across all states and path lengths is distributed across training through sampled gradient updates.

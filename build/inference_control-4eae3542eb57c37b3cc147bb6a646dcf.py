...
def optimize_open_loop(
    ...
    *,
    ...
    _, _, applied_power = _fluid_rollout(
        continuous,
        arrivals,
        plant,
        control_period_s,
    ...
    objective_value = float(
        np.sum(applied_power)
        + 20.0 * np.sum(predicted_backlog[1:] ** 2)
        + 0.05 * np.sum(changes**2)
        + 20.0 * predicted_backlog[-1] ** 2
    )
    return OpenLoopPlan(
...
class _MPCClockController:
    ...
    def _solve(self, observation: ServingObservation, arrival_rate: float) -> tuple[np.ndarray, bool, float]:
        ...
        minimum = profile.minimum_clock_mhz
        maximum = profile.maximum_clock_mhz
        prefill_work = observation.prefill_remaining_tokens / profile.prefill_tokens_per_s[-1]
        estimated_decode_remaining = max(
            0.0,
            observation.decode_active * self.mean_output_tokens
            - observation.generated_decode_tokens,
        )
        decode_work = estimated_decode_remaining / profile.decode_tokens_per_s[-1]
        initial_backlog = prefill_work + decode_work
        arrivals = np.full(
            self.horizon_steps,
            arrival_rate * self.mean_request_work_s * self.control_period_s,
        )
        ...
        def objective(sequence: np.ndarray) -> float:
            backlog, temperature, normalized_power = _fluid_rollout(
                sequence,
                arrivals,
                self.plant,
                self.control_period_s,
                initial_backlog_s=initial_backlog,
            ...
            ttft_slack = np.maximum(0.0, backlog[1:] - profile.ttft_slo_s) / max(
                profile.ttft_slo_s, 1e-12
            )
            tpot_slack = np.maximum(
                0.0,
                backlog[1:] / max(1, observation.decode_active) - profile.tpot_slo_s,
...
def run_mpc(
    ...
    *,
    ...
    return simulate(
        ...
        seed,
        controller_name="MPC",
        scheduler_name="512-token chunked prefill",
    )
    ...
    *,
    ...
    for state_index, (p_value, d_value, age_value) in enumerate(states):
        for action in range(3):
            ...
            completion_distribution = (
                _binomial_probabilities(d_base, decode_completion_probability)
                if action == 1
                else np.array([1.0])
            )
            for completed, completion_probability in enumerate(completion_distribution):
                d_after = d_base - completed if action == 1 else d_base
                for arrival, arrival_prob in (
                    ...
                    probability = completion_probability * arrival_prob
    ...
    mdp = SchedulingMDP(
        ...
        description=(
            ...
            "it is not an exact reproduction of vLLM."
        ),
    )
    mdp.validate()
    return mdp
...
def scheduling_mdp_from_trace(
    ...
    *,
    ...
    """Estimate the two transition probabilities used by the reduced MDP."""
...
def build_textbook_results(
    ...
    *,
    ...
    return {
        ...
        "open_loop": {
            "controllers": {
                "nominal": open_loop_nominal.as_dict(stride=stride),
                "shifted": open_loop_shifted.as_dict(stride=stride),
            },
            "plan": {

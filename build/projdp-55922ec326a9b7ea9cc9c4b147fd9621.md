...
### Weighted Residual Methods for the Bellman Equation: Concrete Examples

Having developed the general framework in Step 4, we now see what the two main methods—function iteration and Newton—look like when applied to the Bellman equation $\mathrm{L}v = v$ using different choices of test functions. We focus on collocation and Galerkin, the two most common approaches.
...
**Function iteration** for collocation proceeds as follows. Given current coefficients $\theta^{(k)}$, we evaluate the Bellman operator at each collocation point to get target values $t_i^{(k)} = [\mathrm{L}_\varphi(\theta^{(k)})]_i$. We then find new coefficients by solving the linear system $\boldsymbol{\Phi} \theta^{(k+1)} = t^{(k)}$. This is parametric value iteration: apply the Bellman operator, fit the result. When the state space is continuous, we approximate expectations using numerical quadrature (Gauss-Hermite for normal shocks, etc.). The method is simple and robust when the finite-dimensional approximation preserves contraction, but can be slow for large discount factors.

**Newton's method** for collocation treats the problem as rootfinding: $F(\theta) = \boldsymbol{\Phi} \theta - \mathrm{L}_\varphi(\theta) = 0$. The Jacobian is $J_F = \boldsymbol{\Phi} - J_{\mathrm{L}_\varphi}$, where the Envelope Theorem (Step 4) gives us $[J_{\mathrm{L}_\varphi}]_{ij} = \gamma \sum_{s'} p(s'|s_i, a_i^*(\theta)) \varphi_j(s')$. Here $a_i^*(\theta)$ is the optimal action at collocation point $s_i$ given the current coefficients. At each Newton step, we compute these optimal actions, build the Jacobian by evaluating expected basis function values at next states, then update $\theta^{(k+1)} = \theta^{(k)} - J_F^{-1} F(\theta^{(k)})$. This converges rapidly near the solution but requires good initialization and more computation per iteration than function iteration. The method is equivalent to policy iteration: each step evaluates the value of the current greedy policy, then improves it.
...
**Function iteration** for Galerkin works differently than for collocation. Given $\theta^{(k)}$, we cannot simply evaluate the Bellman operator and fit—we must solve an integral equation. At each iteration, we seek $\theta^{(k+1)}$ satisfying:

...
When we express $v^*(j)$ via its defining Bellman equation and use the envelope theorem to substitute for $\nabla_s v^*$, future periods $t+2, t+3, \ldots$ drop out. The **discrete-time Euler equation** involves only marginal rewards at time $t$ and expected marginal rewards at time $t+1$:
...
This is a functional equation in the policy $\pi^*(\cdot)$ alone, with only reward derivatives. As Rust {cite}`Rust1996` notes, "the impact on expected discounted utility can be evaluated only in terms of the value of marginal utility in period $t$ and the expected discounted value of marginal utility in period $t+1$: since $V$ drops out of the first order condition it is not necessary to consider the impact on marginal utility on all future periods $t+2, t+3, \ldots$" The envelope theorem expresses $\frac{\partial v^*(s')}{\partial s'}$ in terms of reward derivatives at $s'$, and when substituted recursively at $t+2, t+3, \ldots$, all future value function terms cancel out.

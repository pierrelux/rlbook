...
| run | plant resistance scale | governor resistance scale | question |
|---|---:|---:|---|
| fresh, nominal | 1.0 | 1.0 | Does the governor pass on its reference plant? |
| high resistance, stale | 1.8 | 1.0 | Does the old model preserve the voltage margin? |
| high resistance, calibrated | 1.8 | fitted from the pulse | Does changing one parameter restore the margin? |

*Recorded battery model audit.* Select a run and follow charge, terminal
voltage, cell temperature, and requested current. Every trace reveals only the
...
The comparison gives the learned component a narrow job. It changes the
voltage drop and resistive heat predicted for a candidate current without
replacing charge conservation or the circuit and thermal states. Temperature
never crosses the plant bound of 35 degrees Celsius, but the governor's conservative
thermal-headroom ceiling does limit current. The result is simulation evidence
for one declared counterfactual, not a certified charging protocol for a
commercial cell. Capacity fade, lithium plating, spatial temperature gradients,
and other electrochemical degradation mechanisms lie outside this model.

...
where $R^2 = \frac{[\text{Cov}(Z,C)]^2}{\text{Var}(Z)\text{Var}(C)}$ is the coefficient of determination. The variance reduction is $R^2 \cdot \text{Var}(Z)$: the better $C$ predicts $Z$, the more variance we eliminate.

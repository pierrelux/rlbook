...
Soft actor-critic (SAC) {cite}`haarnoja2018soft,haarnoja2018sacapplications` adapts the NFQCA framework to entropy-regularized MDPs. The [smoothing chapter](smoothing.md) derives the smooth Bellman operator from entropy regularization and shows it admits two equivalent forms. For the value function under entropy regularization with parameter $\alpha > 0$ (equivalently, inverse temperature $\beta = 1/\alpha$):
...
Rather than computing this expectation exactly, SAC uses a single-sample Monte Carlo estimate: sample one action $\tilde{a}'_i \sim \pi_\phi(\cdot|s'_i)$ and approximate:

$$
y_i = r(s_i,a_i) + \gamma \left[\min_{j=1,2} q^j_{\theta_{\text{target}}}(s'_i, \tilde{a}'_i) - \alpha \log \pi_\phi(\tilde{a}'_i|s'_i)\right]
$$

The minimum over twin Q-networks applies the clipped double-Q trick from TD3. This single-sample approach is computationally efficient: each target requires only one policy sample and two Q-network evaluations at the next state.

```{admonition} Historical Note: The V-Network in Original SAC
:class: dropdown

The original SAC paper {cite}`haarnoja2018soft` introduced a separate value network $v_\psi(s)$ trained to predict the entropy-adjusted expectation:

$$
v_\psi(s) \approx \mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}\left[q_\theta(s,a) - \alpha \log \pi_{\phi}(a|s)\right]
$$

Bootstrap targets then used this amortized value: $y_i = r_i + \gamma v_{\psi_{\text{target}}}(s'_i)$. This trades training cost (fitting $v_\psi$) for inference efficiency.

However, the follow-up paper {cite}`haarnoja2018sacapplications` showed the V-network is redundant: the single-sample estimate works just as well while simplifying the architecture. All modern SAC implementations—including OpenAI Spinning Up, Stable Baselines3, and CleanRL—omit the V-network. We present this simplified version here.
```
...
SAC maintains two types of function approximators: twin Q-networks $q^1, q^2$ (with target networks) for bias mitigation as in TD3, and a Gaussian policy $\pi_\phi$ that approximates the Boltzmann distribution. The Q-networks (lines 7-10) bootstrap using a single-sample estimate of the soft value at the next state: sample $\tilde{a}'_i \sim \pi_\phi(\cdot|s'_i)$ and compute the entropy-adjusted target $y_i = r_i + \gamma[\min_j q^j_{\text{target}}(s'_i, \tilde{a}'_i) - \alpha \log \pi_\phi(\tilde{a}'_i|s'_i)]$. The policy (lines 12-13) minimizes the KL divergence to the Boltzmann distribution via reparameterization, using the minimum Q-value for stability. The stochastic policy provides exploration without added noise, unlike DDPG and TD3.

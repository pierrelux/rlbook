...
Both offline and online reinforcement learning can be understood through the lens of stochastic approximation on an empirical distribution. Given a dataset $\mathcal{D} = \{(s_i, a_i, r_i, s'_i)\}_{i=1}^N$ of observed transitions, define the empirical distribution:

$$
\hat{P}_{\mathcal{D}}(\tau) = \frac{1}{N}\sum_{i=1}^N \delta_{(s_i, a_i, r_i, s'_i)}(\tau)
$$

This is a uniform categorical distribution over the $N$ observed tuples. Sampling from $\mathcal{D}$ (with or without replacement) is sampling from $\hat{P}_{\mathcal{D}}$. This connects to the bootstrap principle from statistics: we use the empirical distribution $\hat{P}_{\mathcal{D}}$ as a proxy for the unknown true distribution $P$ that generated the data.

Each sampled tuple provides one Monte Carlo evaluation of the Bellman operator. For a transition $(s_i, a_i, r_i, s'_i) \sim \hat{P}_{\mathcal{D}}$, we compute:

$$
y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}^{(n)})
$$

This is an unbiased estimate of $(\Bellman q)(s_i, a_i)$ under the true distribution, but we're sampling state-action pairs from the empirical distribution. The distinction between offline and online learning reduces to whether $\hat{P}_{\mathcal{D}}$ remains static or evolves:

**Offline (fixed empirical distribution)**: The dataset $\mathcal{D}$ is collected once and remains fixed. At each outer iteration $n$, we bootstrap sample transitions from $\mathcal{D}$, compute targets, and fit the Q-function. This is sample average approximation: repeatedly solving regression problems drawn from the same fixed empirical distribution.

**Online (dynamic empirical distribution)**: The dataset grows as $\mathcal{D}_t = \mathcal{D}_{t-1} \cup \{(s_t, a_t, r_t, s'_{t+1})\}$ by adding newly observed transitions. At each step, we bootstrap sample from the current $\mathcal{D}_t$ to form targets. The empirical distribution evolves as we collect more data, making this a time-varying stochastic approximation problem.

A replay buffer is simply a data structure for efficient bootstrap sampling with finite capacity. When capacity is reached, old transitions are discarded (typically FIFO), so the empirical distribution represents a sliding window over recent experience. Three cases span the design space:

- **Classical FQI**: Fixed $\mathcal{D}$, sample entire dataset each iteration, run inner loop to convergence ($K=\infty$). Pure sample average approximation.
- **DQN**: Growing $\mathcal{D}_t$ with finite capacity $B$, sample mini-batch of size $b < B$ each iteration, single gradient step ($K=1$). Stochastic approximation on a time-varying empirical distribution with data reuse.
- **Online Q-learning**: Capacity $= 1$, use each transition exactly once and discard. Stochastic approximation without reuse.

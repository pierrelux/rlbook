...
| Warm starting | Cold start, Warm start from $\theta^{(k)}$ |
...
| **Algorithm** | **Approximator** | **Bellman** | **Inner Loop** | **Data** | **Bias Fix** |
|:--------------|:-----------------|:------------|:---------------|:---------|:-------------|
| FQI {cite}`ernst2005tree` | Extra Trees | Hard | Full | Offline | None |
| NFQI {cite}`riedmiller2005neural` | Neural Net | Hard | Full | Offline | None |
| DQN {cite}`mnih2013atari` | Deep NN | Hard | K steps | Replay | None |
| Double DQN {cite}`van2016deep` | Deep NN | Hard | K steps | Replay | Double Q |
| Q-learning {cite}`SuttonBarto2018` | Tabular/Linear | Hard | K=1 | Online | None |
| Soft Q {cite}`haarnoja2017reinforcement` | Neural Net | Smooth | K steps | Replay | None |

This table omits continuous action methods (NFQCA, DDPG, SAC), which introduce an additional design dimension. We address those after establishing the discrete action case.

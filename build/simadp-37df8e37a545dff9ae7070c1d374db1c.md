...
Deep Q-Networks (DQN) {cite}`mnih2013atari` emerges from making three design choices beyond NFQI: partial inner-loop optimization (K gradient steps instead of convergence), warm starting (initialize from previous $\theta$), and online data collection with replay buffer. To understand how these choices naturally lead to target networks, we need to see how warmstarting interacts with partial optimization.

In NFQI, we have a nested structure: an outer loop that computes targets using $\theta^{(k)}$, and an inner loop that runs gradient descent starting from scratch (cold start) until convergence to obtain $\theta^{(k+1)}$. The targets remain fixed throughout the inner loop because they use the frozen $\theta^{(k)}$.

When we switch to warm starting and partial optimization, we initialize the inner loop with $\theta^{(k)}$ and take only K gradient steps. But now we face a choice: should we recompute targets at each inner gradient step using the updated parameters, or keep them fixed? Recomputing targets at every step creates instability—the targets chase the moving parameters, preventing convergence. Keeping targets fixed for K steps is more stable and mirrors the NFQI structure.

This leads naturally to maintaining two sets of parameters: $\theta$ (the current parameters being optimized) and $\theta^-$ (frozen parameters for computing targets). After K gradient steps, we update $\theta^- \leftarrow \theta$, marking the boundary of what would have been the outer loop iteration in NFQI. This periodic synchronization is exactly what target networks do.

The target network is thus not a magical stabilization trick added to prevent "decorrelating samples" (a post-hoc explanation). It is simply the natural consequence of flattening NFQI's nested structure while preserving the key property that targets remain fixed for multiple gradient steps. The algorithm maintains a replay buffer with capacity $C$ (typically 1 million transitions) and at each time step:

1. Acts using an $\varepsilon$-greedy policy based on the current Q-network $q(\cdot; \theta)$
2. Stores the resulting transition $(s, a, r, s')$ in the buffer
3. Samples a mini-batch of size $B$ (typically 32) uniformly from the buffer
4. Computes targets using the target network: $y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \theta^-)$
5. Performs one gradient step: $\theta \leftarrow \theta - \alpha \nabla_\theta \sum_i (q(s_i, a_i; \theta) - y_i)^2$
6. Every K steps (typically 10,000), updates the target network: $\theta^- \leftarrow \theta$

This makes it an online algorithm despite using a replay buffer, continuously updating both data and parameters while maintaining the structural separation between current and target parameters that NFQI had implicitly through its nested loops.

An alternative to periodic target updates is exponential moving average (EMA): $\theta^- \leftarrow \tau \theta + (1-\tau)\theta^-$ at every step with small $\tau$ (typically 0.001). This provides a smoother transition than the hard swap every K steps. DDPG and subsequent algorithms like TD3 and SAC use this EMA approach, which has become standard for continuous control. Both periodic updates and EMA serve the same purpose: keeping targets stable for multiple gradient steps while allowing them to gradually track the improving Q-function.
...
At each environment step, DDPG collects one transition using the current actor plus exploration noise (typically Ornstein-Uhlenbeck process), stores it in the buffer, samples a mini-batch, and performs one gradient step on both actor and critic. The target networks update via exponential moving average: $\theta^- \leftarrow \tau \theta + (1-\tau)\theta^-$ and $\phi^- \leftarrow \tau \phi + (1-\tau)\phi^-$ with $\tau \ll 1$ (typically 0.001). This EMA approach, introduced by DDPG, became the standard for continuous control and is used in subsequent algorithms like TD3 and SAC.

This is exactly the same relationship as between batch NFQI and online DQN: replace offline dataset with replay buffer, replace full optimization with mini-batch gradient steps, add target networks for stability (whether via periodic updates as in DQN or EMA as in DDPG). The algorithmic template accommodates continuous actions by substituting the actor-critic architecture for explicit maximization, but the core structure (fitted Q-iteration plus data collection strategy) remains unchanged.

### Soft Actor-Critic

Soft Actor-Critic (SAC) {cite}`haarnoja2018soft` addresses the computational challenge of applying smooth Bellman operators to continuous action spaces. For continuous $a \in \mathbb{R}^d$, the logsumexp target $\frac{1}{\beta} \log \int \exp(\beta q(s',a')) da'$ requires integrating over the action space at every target computation, which is intractable.

SAC resolves this by combining the actor-critic architecture (from DDPG) with the smooth Bellman operator. The stochastic policy $\pi(a|s; \phi)$ is typically a Gaussian with state-dependent mean and variance output by a neural network. The critic uses the entropy-regularized backup:

$$
y = r + \gamma \left(\mathbb{E}_{a' \sim \pi(\cdot|s'; \phi)}[q(s', a'; \theta^-)] - \alpha \log \pi(a'|s'; \phi)\right),
$$

where the expectation and log-density are computed using the reparameterization trick: sample $\varepsilon \sim \mathcal{N}(0, I)$ and compute $a' = \mu_\phi(s') + \sigma_\phi(s') \odot \varepsilon$. This makes both the Q-value and entropy terms differentiable with respect to $\phi$.
...
The reparameterization trick enables backpropagation through the sampling operation, allowing gradient-based optimization of the stochastic policy.

SAC thus combines: continuous actions (actor-critic for amortization), smooth Bellman operator (entropy regularization), partial inner-loop optimization (mini-batch gradient steps), target networks (EMA updates), and replay buffers. It demonstrates how the fitted Q-iteration template extends to smooth operators in continuous spaces by replacing intractable logsumexp integration with tractable expectations under a parametric stochastic policy.
...
This chapter developed simulation-based approximate dynamic programming by combining three foundational elements: projection methods from the [previous chapter](projdp.md) (fitting value functions to targets using finite-dimensional approximations), Q-function representations (amortizing action selection by caching state-action values), and Monte Carlo integration (replacing expectations with sample averages). These elements yield a template that generates the major algorithms in reinforcement learning through different design choices.

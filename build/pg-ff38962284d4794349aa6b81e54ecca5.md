...
The batched policy gradient algorithms presented above collect trajectories, compute a single gradient estimate, update the policy, and discard the data. This is sample-inefficient: each trajectory is used for exactly one gradient step. Taking multiple gradient steps on the same batch would amortize the cost of data collection, but the surrogate loss perspective from earlier shows why this is problematic. The surrogate loss produces the correct gradient only at the parameters used to collect the data. After updating $\boldsymbol{w}$, the gradient of the same surrogate loss no longer estimates the policy gradient at the new parameters.

To reuse data across multiple updates, we need to account for the distribution shift between the policy that collected the data (the **behavior policy** $\pi_{\boldsymbol{w}_{\text{old}}}$) and the policy being optimized (the **target policy** $\pi_{\boldsymbol{w}}$). Importance sampling provides a principled correction. The policy gradient can be written as:

$$
\nabla_{\boldsymbol{w}} J(\boldsymbol{w}) = \mathbb{E}_{s \sim d^{\pi_{\boldsymbol{w}}}, a \sim \pi_{\boldsymbol{w}}}\left[\nabla_{\boldsymbol{w}} \log \pi_{\boldsymbol{w}}(a|s) A^{\pi_{\boldsymbol{w}}}(s,a)\right]
$$

When actions are sampled from $\pi_{\boldsymbol{w}_{\text{old}}}$ instead of $\pi_{\boldsymbol{w}}$, we can correct using importance weights:

$$
\nabla_{\boldsymbol{w}} J(\boldsymbol{w}) = \mathbb{E}_{s \sim d^{\pi_{\boldsymbol{w}_{\text{old}}}}, a \sim \pi_{\boldsymbol{w}_{\text{old}}}}\left[\frac{\pi_{\boldsymbol{w}}(a|s)}{\pi_{\boldsymbol{w}_{\text{old}}}(a|s)} \nabla_{\boldsymbol{w}} \log \pi_{\boldsymbol{w}}(a|s) A^{\pi_{\boldsymbol{w}_{\text{old}}}}(s,a)\right]
$$
...
Define the probability ratio $r_t(\boldsymbol{w}) = \frac{\pi_{\boldsymbol{w}}(a_t|s_t)}{\pi_{\boldsymbol{w}_{\text{old}}}(a_t|s_t)}$. At $\boldsymbol{w} = \boldsymbol{w}_{\text{old}}$, we have $r_t(\boldsymbol{w}_{\text{old}}) = 1$, and the objective reduces to the standard surrogate loss. As $\boldsymbol{w}$ moves away from $\boldsymbol{w}_{\text{old}}$, the importance weights correct for the distribution mismatch.

The problem with unconstrained optimization of {eq}`eq:importance-weighted-surrogate` is that large policy updates can cause the importance weights to become extreme, leading to high variance and instability. If $\pi_{\boldsymbol{w}}(a|s)$ becomes much larger than $\pi_{\boldsymbol{w}_{\text{old}}}(a|s)$ for actions with positive advantage, the ratio $r_t(\boldsymbol{w})$ explodes, producing unreliable gradient estimates.

Trust Region Policy Optimization (TRPO) addresses this by constraining the KL divergence between the old and new policies:
...
This constrained optimization problem requires computing second-order information (the Fisher information matrix) and solving a trust region subproblem at each iteration, which is computationally expensive.

Proximal Policy Optimization (PPO) achieves similar behavior through a simpler mechanism: clipping the probability ratio to prevent it from moving too far from 1. The **clipped surrogate objective** is:

$$
L^{\text{CLIP}}(\boldsymbol{w}) = \mathbb{E}_{s,a \sim \pi_{\boldsymbol{w}_{\text{old}}}}\left[\min\left(r_t(\boldsymbol{w}) A_t, \text{clip}(r_t(\boldsymbol{w}), 1-\epsilon, 1+\epsilon) A_t\right)\right]
$$ (eq:ppo-clip)
...
- **Positive advantage** ($A_t > 0$): The action is better than average, so we want to increase $\pi_{\boldsymbol{w}}(a_t|s_t)$. The unclipped term $r_t A_t$ increases with $r_t$. The clipped term stops increasing once $r_t > 1 + \epsilon$. Taking the minimum means we get the benefit of increasing $r_t$ only up to $1 + \epsilon$.

- **Negative advantage** ($A_t < 0$): The action is worse than average, so we want to decrease $\pi_{\boldsymbol{w}}(a_t|s_t)$. The unclipped term $r_t A_t$ becomes less negative (improves) as $r_t$ decreases. The clipped term stops improving once $r_t < 1 - \epsilon$. Taking the minimum means we get the benefit of decreasing $r_t$ only down to $1 - \epsilon$.
...
2. For iteration = 1, ..., $N$:
    ...
    5. For epoch = 1, ..., $K$:
        ...
        2. For each mini-batch $\mathcal{B}$:
            1. Compute ratios: $r_t = \exp(\log \pi_{\boldsymbol{w}}(a_t|s_t) - \log \pi_{\text{old}}(a_t|s_t))$
            2. Compute clipped objective:
               $L^{\text{CLIP}} = \frac{1}{M}\sum_{t \in \mathcal{B}} \min(r_t A_t, \text{clip}(r_t, 1-\epsilon, 1+\epsilon) A_t)$

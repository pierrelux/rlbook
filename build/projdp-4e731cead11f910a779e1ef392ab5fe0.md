...
Here is the key insight: even though we cannot represent $v_k$ exactly on the entire domain, we **can evaluate the Bellman operator at any individual state**. Given an approximate $\hat{v}_k$ and a state $s$, we can compute $(\Bellman \hat{v}_k)(s)$ by performing the maximization and integration numerically at that one point. The obstacle is not evaluation—it is **representation**. We cannot carry the full assignment $s \mapsto (\Bellman \hat{v}_k)(s)$ forward to the next iteration. We need to compress this information into a finite-dimensional approximation.

This leads to fitted-value iteration:

```
1. Initialize $\hat{v}_0$ in some approximation class
2. For $k = 0, 1, 2, \ldots$ until convergence:
   a. Choose sample states $\{s_1, \ldots, s_n\}$
   b. Compute target values: $y_i = (\Bellman \hat{v}_k)(s_i)$ for $i = 1, \ldots, n$
   c. Fit a new approximation: $\hat{v}_{k+1} = \mathtt{fit}(\{(s_i, y_i)\}_{i=1}^n)$
3. Return $\hat{v}_k$
```

The Bellman operator evaluation (step 2b) is the functional equation at work. The fitting step (step 2c) is where approximation enters. We have sampled the ideal next iterate at $n$ points, and now we construct a function that passes through (or near) these points.

### Connection to Weighted Residual Methods

How does this connect to the abstract framework of weighted residuals and test functions? The fitting step is precisely where the weighted residual conditions appear. When we "fit a function to data $\{(s_i, y_i)\}$," we are minimizing some notion of error between our approximation $\hat{v}(s_i; \boldsymbol{\theta})$ and the targets $y_i$. Different fitting procedures correspond to different test function choices:

**Collocation**: Fit by exact interpolation at the sample points. Require $\hat{v}_{k+1}(s_i) = y_i$ for all $i$. This is equivalent to testing the residual $R(s) = \hat{v}_{k+1}(s) - (\Bellman \hat{v}_k)(s)$ against delta functions: $\langle R, \delta(\cdot - s_i) \rangle = R(s_i) = 0$.

**Least squares**: Fit by minimizing $\sum_{i=1}^n \xi(s_i) [y_i - \hat{v}_{k+1}(s_i; \boldsymbol{\theta})]^2$. The first-order conditions for this minimization are $\sum_{i=1}^n \xi(s_i) [y_i - \hat{v}_{k+1}(s_i; \boldsymbol{\theta})] \frac{\partial \hat{v}_{k+1}}{\partial \theta_j}(s_i) = 0$. For linear approximations $\hat{v}(s) = \sum_j \theta_j \varphi_j(s)$, this becomes $\sum_{i=1}^n \xi(s_i) R(s_i) \varphi_j(s_i) = 0$, which approximates $\langle R, \varphi_j \rangle_\xi = 0$—the Galerkin conditions.

**General fitting**: Any supervised learning procedure that produces $\hat{v}_{k+1}$ from data $\{(s_i, y_i)\}$ can be viewed as implicitly imposing conditions on the residual. Neural network training, decision trees, kernel methods, and ensemble methods all define different ways to construct a function from point samples. The mathematical formalism of test functions and inner products provides a unifying view, but the practical algorithm is simply: evaluate the operator, collect targets, fit a function.

### The Fitting Operator

We can formalize this by introducing a **fitting operator** $\mathtt{fit}$ that takes a dataset of state-value pairs $\{(s_i, y_i)\}_{i=1}^n$ and produces a function $\hat{v} \in \mathcal{F}$ from some hypothesis class $\mathcal{F}$:

$$
\hat{v} = \mathtt{fit}\big(\{(s_i, y_i)\}_{i=1}^n; \mathcal{F}\big).
$$

The fitted-value iteration becomes:
...
- **Linear basis functions**: Solve weighted least-squares $\min_{\boldsymbol{\theta}} \sum_{i=1}^n \xi(s_i) \big(y_i - \sum_j \theta_j \varphi_j(s_i)\big)^2$
- **Neural networks**: Run stochastic gradient descent to minimize $\mathcal{L}(\boldsymbol{\theta}) = \sum_{i=1}^n (y_i - v_{\boldsymbol{\theta}}(s_i))^2$
- **Decision trees**: Construct a regression tree by recursive partitioning
- **Kernel methods**: Compute kernel weights $\alpha_i$ such that $\hat{v}(s) = \sum_{i=1}^n \alpha_i k(s, s_i)$

The weighted residual framework from earlier in the chapter is the analytical tool for understanding when this iteration converges. The fitting operator is the computational tool for implementing it. They are two views of the same algorithm: one mathematical, one procedural.

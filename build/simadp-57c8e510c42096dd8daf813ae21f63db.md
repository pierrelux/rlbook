...
$$
I = \int f(x)\,p(dx) = \mathbb{E}[f(X)],
$$

where $X \sim p$ and $f$ is some integrable function. Monte Carlo integration approximates $I$ by drawing independent samples $X^{(1)},\ldots,X^{(N)} \sim p$ and forming the sample average

$$
\hat{I}_N \equiv \frac{1}{N}\sum_{n=1}^N f\bigl(X^{(n)}\bigr).
$$
...
$$
\sqrt{N}\,(\hat{I}_N - I) \Longrightarrow \mathcal{N}(0,\sigma^2),
$$
...
$$
\hat{I}_N = \frac{1}{N}\sum_{n=1}^N f(X^{(n)})
$$
...
$$
\int R(s)\,p_i(s)\, \mu(ds) = 0
\quad \text{or} \quad
\int R(s)^2\,\mu(ds) \text{ is minimized}.
$$

Here $R(s)$ is the residual, such as

$$
R(s) = \hat{v}(s;\theta) - (\Bellman \hat{v})(s;\theta),
$$
...
\int R(s)\,p_i(s)\,\mu(ds)
\approx
\frac{1}{M}\sum_{m=1}^M R\bigl(S^{(m)}\bigr)\,p_i\bigl(S^{(m)}\bigr).
$$
...
$$
\frac{1}{M}\sum_{m=1}^M R\bigl(S^{(m)}\bigr)^2.
$$

...
This polynomial agrees with the ODE slope $\mathbf{f}_{k,j}$ at every
collocation node of interval $k$. To recover state values from these slopes,
...
The NLP stores $\mathbf{x}_{k,i}$ as a variable at stage $i$. It also
constructs $\mathbf{p}_k$ by integrating the interpolated slopes. These two
descriptions must give the same state at $\tau_i$: the point
$(\tau_i,\mathbf{x}_{k,i})$ must be on the graph of $\mathbf{p}_k$.
...
Enforcing the ODE means equating that physical-time slope to $\mathbf f$.
Multiplying both sides by $h_k$ gives
...
the represented collocation method. The node-to-NLP recipe for the
...
The integration-form recipe computes ODE slopes at the stages and requires
the stored stage states to agree with their integral. The differentiation
form reverses this construction: stored state-support values define a
polynomial, and the NLP requires its derivative to agree with the ODE at
the collocation nodes. The distinction between support nodes and collocation
nodes determines which values must be stored and which can be computed.
...
:::{dropdown} Inspect the differentiation-form residual evaluator

```{literalinclude} code/collocation_transcription.py
:language: python
:start-at: def differentiation_problem
```

:::

The generator and this evaluator are included in the same downloadable
Python module above. The generator's `.deriv()` differentiates a known
basis polynomial with respect to local time $\tau$. An NLP solver also
...
What changes when physical disturbances make the future trajectory uncertain?
[Model Predictive Path Integral Control](model-predictive-path-integral-control.md)
uses sampled futures to optimize controls and develops their connection to
stochastic optimal control. The following

...
For parametric approximations $(v_{\boldsymbol{\phi}}, \pi_{\boldsymbol{\theta}})$, the path residual is:
...
This is the least-squares approach from the [projection methods chapter](projection.md). Contrast with SAC: compute targets $y_i$, fit to targets (successive approximation). PCL: directly minimize the residual (no targets, no fitting).
...
**Single-step case ($d=1$)**: The path residual becomes $R(s:s') = v(s) - \gamma v(s') - r + \alpha\log\pi(a|s)$, which equals the soft Bellman residual $q(s,a) - r - \gamma v(s')$. Minimizing $\sum_i R_i^2$ is equivalent to soft Q-learning, though SAC solves this via successive approximation (compute targets, fit) rather than direct residual minimization.

**No entropy ($\alpha \to 0$)**: The residual becomes $R = v(s_i) - \gamma^d v(s_{i+d}) - \sum_t \gamma^t r_t$, which is the negative $d$-step advantage. But unlike A2C/A3C where $v$ tracks the current policy's value, PCL's value converges to $v^*$ because the residual couples policy and value through optimality, not policy evaluation.

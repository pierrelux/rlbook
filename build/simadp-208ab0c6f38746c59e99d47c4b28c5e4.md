...
The approaches above assume we can evaluate integrals $\int \varphi_i(s') p(s'|s,a) ds'$ using quadrature rules designed for the transition density $p(s'|s,a)$. However, Gaussian quadrature is typically designed for a fixed weighting function $p(s)$ independent of the current state and action. This leads to an alternative discretization strategy proposed by Tauchen and Hussey (1991).

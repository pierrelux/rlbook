...
This is exactly the sample average estimator from the Monte Carlo chapter, applied now to transitions. Conceptually, fitted Q-iteration performs **plug-in approximate dynamic programming**. The plug-in principle is simple: whenever an algorithm requires an unknown quantity (here, the transition distribution $P$), replace it with an estimate constructed from data (here, the empirical distribution $\hat{P}_{\mathcal{B}_t}$). We substitute $\hat{P}_{\mathcal{B}_t}$ for $P$ throughout the algorithm and run value iteration using this empirical Bellman operator:

...
These choices are independent but should be coordinated. To see why, consider evaluating the Bellman operator at some state $s$. Let $v$ denote the true value function, $\hat{v}$ our basis approximation, and $\widehat{\Bellman}_Q$ the Bellman operator with numerical quadrature. By the triangle inequality:

$$ \|(\Bellman v)(s) - (\widehat{\Bellman}_Q \hat{v})(s)\| \le \underbrace{\|(\Bellman v)(s) - (\Bellman \hat{v})(s)\|}_{\text{projection error}} + \underbrace{\|(\Bellman \hat{v})(s) - (\widehat{\Bellman}_Q \hat{v})(s)\|}_{\text{quadrature error}} $$

The first term measures how well $\hat{v}$ approximates $v$ in the range of the Bellman operator. The second measures the error from replacing exact integration with quadrature. If quadrature error dominates, we gain nothing from a high-order basis. For instance, degree-10 Chebyshev polynomials achieve projection error $O(10^{-8})$ for smooth functions, but combined with rectangle-rule quadrature (error $O(h^2) \approx 10^{-2}$ for typical step sizes), the bound gives total error $O(10^{-2})$. We pay the cost of 10 basis functions but achieve only first-order accuracy.

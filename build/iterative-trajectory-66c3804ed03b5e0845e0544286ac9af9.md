...
A_t&=\nabla_{\mathbf x}\mathbf f_t(\bar{\mathbf x}_t,\bar{\mathbf u}_t),
\qquad
B_t=\nabla_{\mathbf u}\mathbf f_t(\bar{\mathbf x}_t,\bar{\mathbf u}_t).
\end{aligned}
$$
...
The $c_{xu,t}$
block measures how the marginal cost of a control changes with the state.
...
=c_T(\bar{\mathbf x}_T)+p_T^\top\delta\mathbf x_T
 +\frac12\delta\mathbf x_T^\top P_T\delta\mathbf x_T
 +o(\|\delta\mathbf x_T\|^2),
\quad
p_T=\nabla_{\mathbf x}c_T(\bar{\mathbf x}_T),\quad

...
If $r$ is differentiable in $a$ and the optimal action $\pi^*(s)$ is interior, the first-order condition at the optimum is:

$$
\frac{\partial r(s, a)}{\partial a}\Big|_{a=\pi^*(s)} + \gamma \sum_{j \in \mathcal{S}} p(j|s,\pi^*(s)) v'(j) \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi^*(s)} = 0.
$$

For problems where we can eliminate the value function using the envelope theorem, this becomes a condition purely in terms of the policy. Consider a continuous control problem with smooth reward $r(s,a)$ and deterministic dynamics $j = f(s,a)$. Taking the derivative of the Bellman optimality equation with respect to $a$ and using the envelope theorem to express $\nabla_s v$ in terms of the policy, we obtain a functional equation of the form:

$$
\frac{\partial r(s, \pi(s))}{\partial a} + \gamma \nabla_s v(f(s, \pi(s))) \cdot \frac{\partial f(s, \pi(s))}{\partial a} = 0,
$$

where the envelope theorem gives $\nabla_s v(s) = \frac{\partial r(s, \pi(s))}{\partial s} + \gamma \nabla_s v(f(s, \pi(s))) \cdot \frac{\partial f(s, \pi(s))}{\partial s}$. Through recursive substitution, the value function can be eliminated entirely, leaving a **discrete-time Euler equation**: a functional equation in the policy $\pi(\cdot)$ alone.
...
$$
\EulerResidual(\pi)(s) = \frac{\partial r(s, \pi(s))}{\partial a} + \gamma \sum_{j \in \mathcal{S}} p(j|s,\pi(s)) \nabla_s v(j) \cdot \frac{\partial f(s, \pi(s))}{\partial a}.
$$

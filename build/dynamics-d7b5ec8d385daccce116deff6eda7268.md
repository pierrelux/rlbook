...
decides which requests enter the next batch and how much prefill work may run
beside active decodes. The clock request changes the rate and energy cost of
that work. Hardware may realize a lower clock under a power or thermal limit,
...
only as tokens complete. It also maintains an exact cache allocation for every
active sequence, so cache blocks are acquired after prefill and released at
completion. An aggregate thermal balance has the form
...
{cite}`azureLLMTrace2023`. Arrival times are dilated once to place maximum-clock
utilization near 80 percent. If $\rho_{\max}$ is the isolated service time of
all requests at the highest profiled clock divided by the original window
length, the dilation and normalized arrivals are
...
| service simulation | 0.05-second steps; one phase per step; prefill chunks capped at 512 tokens |
| output length | hidden from the controller until completion |

Within each one-second clock-control period, the scheduler serves either decode
or one prefill chunk at each 0.05-second simulator step. When both phases have
work, active decode receives alternating-step priority and strict priority under
high cache pressure. This **512-token interleaved chunked-prefill** rule keeps
the action channel visible without claiming to reproduce vLLM's mixed-batch
...
:caption: Twenty requests from the Azure code-generation trace move through arrival, interleaved 512-token prefill chunks, autoregressive decode, and completion. Each simulator step serves one phase. The plots report only the trajectory prefix reached by the playhead, and output length is hidden from the controller until each request completes. A provenance badge distinguishes a measured profile from a pre-measurement engineering surrogate.
...
cache allocations have been released. This establishes closure of the modeled

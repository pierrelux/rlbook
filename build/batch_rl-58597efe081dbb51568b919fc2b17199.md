...
{cite}`lee2013bias` proposed a direct solution: estimate the bias and subtract it from the targets before regression. For a transition $(s_i, a_i, r_i, s'_i)$, the bias arises from taking the maximum over Q-values at the next state $s'_i$. The corrected target becomes:

$$
\tilde{y}_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}^{(n)}) - B(s'_i)
$$

where the bias correction term at the next state is:

$$
B(s') = \left(\frac{\xi}{b_{|A(s')|}} + b_{|A(s')|}\right) \hat{\sigma}(s')
$$

The variance $\hat{\sigma}^2(s')$ must reflect the noise level in Q-value estimates at state $s'$. In practice, this is difficult to estimate directly. Two approximations are possible: (1) if the environment has stochastic rewards and we observe multiple samples at the same state-action pairs, we can estimate reward variance $\sigma_r^2$ and use this as a proxy, since reward noise propagates to Q-value noise through the Bellman equation; (2) we can use the empirical spread (standard deviation) of Q-values $\{q(s', a'; \boldsymbol{\theta})\}$ across actions at $s'$ as a rough measure of uncertainty, though this conflates epistemic uncertainty about which action is best with the aleatoric noise we are trying to capture. The method keeps squared error loss but removes systematic overestimation from targets. The corrected targets are asymptotically unbiased when the Gaussian noise assumption holds. Both variance estimation approaches are approximate, and the correction is optimal specifically for Gaussian errors. If the true errors have heavier tails, the correction may be insufficient.

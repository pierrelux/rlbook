...
This design choice has two origins. First, it reflects constraints in some problem settings. A robot executing action $a$ in state $s$ observes one outcome $s'$ and cannot rewind to the exact same state to sample alternative continuations. An agent playing a video game experiences one trajectory $(s_0, a_0, r_0, s_1, \ldots)$, yielding one next state per state-action pair. Many simulators do not support arbitrary state initialization, limiting the ability to sample multiple continuations from the same starting configuration.
...
Recognizing this as a design choice rather than a hard constraint clarifies the space of possible algorithms. One could build methods that collect multiple next-state samples when feasible and average them to reduce target variance, or that adaptively choose $N$ based on estimated variance and computational budget. Such algorithms exist in the literature but remain rare in practice. The single-sample paradigm dominates not because alternatives are impossible but because decades of algorithmic development optimized around this assumption, creating a mature ecosystem of techniques (replay buffers, target networks, multi-step returns) that make single-sample methods work well.

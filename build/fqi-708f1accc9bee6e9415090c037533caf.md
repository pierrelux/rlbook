...
**Equivalence to nested version**: At step $t$, we have $n = \lfloor t/K \rfloor$ (outer iteration) and $k = t \bmod K$ (position within inner loop). The target network $\boldsymbol{\theta}_{\text{target}}$ equals $\boldsymbol{\theta}_n$ throughout the $K$ steps from $t = nK$ to $t = (n+1)K - 1$, then gets updated to $\boldsymbol{\theta}_{n+1}$ at $t = (n+1)K$. The parameters $\boldsymbol{\theta}_t$ correspond to $\boldsymbol{\theta}_n^{(k)}$ in the nested form. The flattening simply reindexes: instead of thinking "I'm at outer iteration 3, inner step 7," we think "I'm at step 37."

**Why this matters**: The target network is not a stabilization trick invented for deep RL. It's the natural byproduct of flattening the nested FQI structure. When you see DQN presented with a target network that updates every $K$ steps, you're looking at approximate value iteration in flattened form. The algorithm is still performing outer loop (value iteration) and inner loop (regression), but the presentation obscures this. Understanding this connection is crucial: it explains why target networks exist, why they update periodically, and why DQN is fundamentally batch approximate DP, not online stochastic approximation.

**Why trees can't be flattened**: Tree ensemble methods like random forests or extra trees have no continuous parameter space and no gradient-based optimization. The `fit` operation builds the entire tree structure in one pass—there's no sequence of incremental updates to unfold into a single loop. This is why Ernst's FQI {cite}`ernst2005tree` retains the explicit nested structure with cold start initialization at each outer iteration, while neural methods can be flattened.

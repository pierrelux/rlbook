...
The [previous chapter](simadp.md) showed that maximizing over noisy Monte Carlo estimates creates systematic upward bias: $\mathbb{E}[\max_a \{\mu_a + \varepsilon_a\}] > \max_a \mu_a$ by Jensen's inequality, since the max function is convex. We now examine the distributional properties of this bias. When Q-value estimates have the form $q(s', a'; \boldsymbol{\theta}) = q^*(s', a') + \varepsilon_a$ with independent, zero-mean errors $\varepsilon_a$, what is the distribution of $\max_{a'} \varepsilon_a$? The answer comes from extreme value theory. When we take the maximum of $M$ independent random variables, the tail behavior of the maximum depends on the tail of the underlying distribution. Classical results show that, under appropriate scaling, the maximum of i.i.d. random variables converges to one of three limiting distributions: Gumbel, Fréchet, or Weibull (the Fisher-Tippett-Gnedenko theorem). For light-tailed distributions like the Gaussian, the limit is Gumbel. This connects to the [smooth Bellman equations](regmdp.md) discussed earlier: there, Gumbel noise is added to rewards as a modeling assumption to derive logsumexp operators; here, Gumbel distributions arise naturally as a consequence of applying the hard max to noisy estimates.

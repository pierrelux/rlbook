...
| Galerkin | $p_j = \varphi_j$ | $\langle R, \varphi_j \rangle_w = 0$ (residual $\perp$ to basis) |
| Collocation | $p_j = \delta(s - s_j)$ | $R(s_j; \theta) = 0$ (residual zero at points) |
| Petrov–Galerkin | $p_j \neq \varphi_j$ | $\langle R, p_j \rangle_w = 0$ (custom test space) |

To see why the first row holds, consider the **minimum residual** objective $\min_\theta \frac{1}{2}\|R(\cdot; \theta)\|_w^2$. Setting the gradient to zero:
...
The test functions are the gradients $\partial R / \partial \theta_j$. This directly minimizes how far $\hat{v}$ is from satisfying the Bellman equation—the "true" minimum-residual approach.

**Galerkin** instead requires $\langle R, \varphi_j \rangle_w = 0$: the residual must be orthogonal to every basis function. This does not minimize $\|R\|_w^2$ directly, but it does minimize $\|R\|$ in the dual norm induced by the test space $\text{span}\{\varphi_j\}$.

**Collocation** uses delta functions as tests, giving $R(s_j; \theta) = 0$: the residual must vanish exactly at the collocation points. This avoids integration entirely.

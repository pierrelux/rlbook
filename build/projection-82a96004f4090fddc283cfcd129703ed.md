...
# Solve for exact threshold: s* = gamma * v_bar, where
# v_bar = s* * gamma * v_bar + (1 - s*^2) / 2
# Substituting s* = gamma * v_bar:
# v_bar = gamma^2 * v_bar^2 + (1 - gamma^2 * v_bar^2) / 2
# 2*v_bar = 2*gamma^2*v_bar^2 + 1 - gamma^2*v_bar^2
# 2*v_bar = gamma^2*v_bar^2 + 1
# gamma^2*v_bar^2 - 2*v_bar + 1 = 0
# v_bar = (2 - sqrt(4 - 4*gamma^2)) / (2*gamma^2) = (1 - sqrt(1 - gamma^2)) / gamma^2
...
The exact value function is piecewise linear: constant at $\gamma \bar{v}$ below the threshold, equal to $s$ above it. Now suppose we want to approximate $v^*$ using a **polynomial basis** with $n$ terms. We write:
...
This gives $n$ nonlinear equations (nonlinear because of the max) in $n$ unknowns. A natural choice is Chebyshev nodes, which minimize interpolation error for smooth functions.
...
where $w(s)$ is a weight function. This spreads the error across the domain rather than forcing it to zero at specific points.
...
def bellman_operator(v_func, s, gamma):
    """Apply the Bellman operator at state s."""
    # Continuation value: integral of v over [0,1]
    from scipy.integrate import quad
    v_bar, _ = quad(v_func, 0, 1)
    return np.maximum(s, gamma * v_bar)

def collocation_solve(n, gamma, max_iter=100, tol=1e-8):
    """Solve optimal stopping via polynomial collocation."""
    nodes = chebyshev_nodes(n)
    
    # Vandermonde matrix for polynomial basis
    Phi = np.vander(nodes, n, increasing=True)
    
    # Fixed-point iteration
    theta = np.zeros(n)
    for iteration in range(max_iter):
        # Current approximation as a function
        def v_approx(s):
            return sum(theta[j] * s**j for j in range(n))
        
        # Bellman targets at collocation points
        targets = np.array([bellman_operator(v_approx, s, gamma) for s in nodes])
...
for n in [3, 5, 8]:
    ...
    print(f"n = {n} basis functions: converged in {iters} iterations, max error = {max_error:.6f}")
...
The polynomial approximation captures the general shape but cannot perfectly represent the kink at the threshold. The error decreases as we add more basis functions, though convergence is slower than for smooth functions due to the non-differentiability at $s^*$.
...
Returning to our motivating example, let us trace through the collocation algorithm step by step. We use $n = 4$ polynomial basis functions $\{1, s, s^2, s^3\}$ and collocate at the Chebyshev nodes on $[0, 1]$.

The collocation matrix $\boldsymbol{\Phi}$ has entries $\Phi_{ij} = s_i^{j-1}$ (the Vandermonde matrix). At each iteration, we:
1. Evaluate the current approximation $\hat{v}(s; \theta^{(k)}) = \sum_j \theta_j^{(k)} s^j$
2. Compute the continuation value $\bar{v}^{(k)} = \int_0^1 \hat{v}(s') ds'$
3. Compute Bellman targets $t_i^{(k)} = \max\{s_i, \gamma \bar{v}^{(k)}\}$ at each node
4. Solve $\boldsymbol{\Phi} \theta^{(k+1)} = t^{(k)}$ for new coefficients
...
# Vandermonde matrix (polynomial basis)
Phi = np.vander(nodes, n, increasing=True)
...
theta = np.zeros(n)
print("Collocation iteration trace (n=4 polynomial basis):")
print(f"{'Iter':<6} {'||theta||':<12} {'v̄ estimate':<12} {'Max error':<12}")
print("-" * 42)
...
for iteration in range(15):
    ...
    # Continuation value (integral of v_approx over [0,1])
    v_bar, _ = quad(v_approx, 0, 1)
    
    # Max error for monitoring
    test_points = np.linspace(0, 1, 100)
    max_error = max(abs(v_approx(s) - v_exact(s)) for s in test_points)
    
    print(f"{iteration:<6} {np.linalg.norm(theta):<12.6f} {v_bar:<12.6f} {max_error:<12.6f}")
...
print(f"\nExact continuation value: {v_bar_exact:.6f}")
print(f"Final coefficients: {theta}")
```

The iteration converges because the projected Bellman operator is a contraction. Each iteration applies the Bellman operator (including the max over actions) and projects back onto the polynomial space by solving the collocation equations. Note that even with a moderate number of basis functions, the polynomial approximation achieves good accuracy.
...
1. **Effect of collocation points.** For the optimal stopping problem, compare collocation with $n=4$ Chebyshev nodes versus $n=4$ equally-spaced nodes. Which choice gives smaller maximum error? Plot the error $|v^*(s) - \hat{v}(s)|$ across the state space for each choice.

2. **Threshold location.** The optimal stopping policy has a threshold structure: stop if $s \geq s^*$. Using your polynomial approximation $\hat{v}(s)$, estimate the threshold by finding where $\hat{v}(s) = s$. Compare to the exact threshold $s^* = \gamma \bar{v}$.
...
To illustrate LSTD concretely, consider a simple 3-state Markov chain under a fixed policy with transition matrix and rewards:
...
We approximate $v_\pi$ using two basis functions $\varphi_1(s) = 1$ and $\varphi_2(s) = s$ (where $s \in \{1, 2, 3\}$), and uniform weighting $\xi = (1/3, 1/3, 1/3)$. The feature matrix is:
...
$$
\mathbf{A} = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} (\boldsymbol{\Phi} - \gamma \mathbf{P}_\pi \boldsymbol{\Phi}), \quad \mathbf{b} = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} \mathbf{r}_\pi.
$$
...
# Define a simple 3-state Markov chain for policy evaluation
P_pi = np.array([[0.7, 0.2, 0.1], [0.3, 0.4, 0.3], [0.1, 0.3, 0.6]])
r_pi = np.array([1.0, 2.0, 0.0])
gamma = 0.9

# Feature matrix: phi_1(s) = 1, phi_2(s) = s for s in {1, 2, 3}
...
3. **State aggregation.** Consider a discrete MDP with states $\{1, 2, 3, 4\}$ aggregated into two groups: $\{1, 2\}$ and $\{3, 4\}$. Write out the aggregation operator as a $4 \times 4$ matrix (it maps functions on 4 states to piecewise-constant functions). Verify it satisfies the conditions of {prf:ref}`gordon-averager`.
...
1. **Computing the stationary distribution.** For the 3-state Markov chain in the LSTD example above, compute the stationary distribution $\xi$ satisfying $\xi^\top \mathbf{P}_\pi = \xi^\top$. Verify your answer by checking that $(I - \mathbf{P}_\pi^\top)\xi = 0$ with $\sum_s \xi(s) = 1$.
...
The following code demonstrates fitted-value iteration on the optimal stopping problem using different scikit-learn regressors:
...
def bellman_operator_stopping(v_func, s_grid, gamma):
    """Apply the Bellman operator for optimal stopping."""
    # Continuation value
    v_bar, _ = quad(v_func, 0, 1)
    return np.maximum(s_grid, gamma * v_bar)

def fitted_value_iteration(s_grid, gamma, fit_model, max_iter=50, tol=1e-6):
    """FVI with arbitrary sklearn-style model."""
    X = s_grid.reshape(-1, 1)
    v = np.zeros(len(s_grid))
    
    history = []
    for k in range(max_iter):
        # Create interpolating function from current v
        def v_func(s, v_vals=v, grid=s_grid):
            return np.interp(s, grid, v_vals)
        
        # Compute Bellman targets
        targets = bellman_operator_stopping(v_func, s_grid, gamma)
        ...
        # Track max error
        max_error = np.max(np.abs(v_new - v_exact(s_grid)))
        history.append(max_error)
        
        if np.linalg.norm(v_new - v) < tol:
            return v_new, history, k + 1
        v = v_new
    
    return v, history, max_iter

# Sample grid
s_grid = np.linspace(0, 1, 50)

# Compare different function approximators
print("Fitted Value Iteration with different approximators:")
print(f"{'Model':<30} {'Iterations':<12} {'Max Error':<12}")
print("-" * 54)

# Polynomial degree 5
poly5_model = lambda: make_pipeline(PolynomialFeatures(5), Ridge(alpha=1e-6))
v_poly5, hist_poly5, iters_poly5 = fitted_value_iteration(s_grid, gamma, poly5_model)
print(f"{'Polynomial (degree 5)':<30} {iters_poly5:<12} {hist_poly5[-1]:<12.6f}")

# Polynomial degree 10
poly10_model = lambda: make_pipeline(PolynomialFeatures(10), Ridge(alpha=1e-6))
v_poly10, hist_poly10, iters_poly10 = fitted_value_iteration(s_grid, gamma, poly10_model)
print(f"{'Polynomial (degree 10)':<30} {iters_poly10:<12} {hist_poly10[-1]:<12.6f}")

# Plot convergence
import matplotlib.pyplot as plt
plt.figure(figsize=(8, 4))
plt.semilogy(hist_poly5, 'b-o', label='Polynomial (deg 5)', markersize=4)
plt.semilogy(hist_poly10, 'r-s', label='Polynomial (deg 10)', markersize=4)
plt.xlabel('Iteration')
plt.ylabel('Max Error (log scale)')
plt.title('FVI Convergence')
plt.legend()
plt.grid(True, alpha=0.3)
plt.tight_layout()
plt.show()

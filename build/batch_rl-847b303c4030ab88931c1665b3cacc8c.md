...
All value-based reinforcement learning algorithms share the same two-level structure. The outer loop applies the Bellman operator $\Bellman$ to the current Q-function at observed state-action pairs, producing a sequence of supervised learning problems. At iteration $n$, we construct a regression dataset $(X^{(n)}, y^{(n)})$ where $X^{(n)} = \{(s_i, a_i)\}$ are the state-action pairs and $y^{(n)} = \{(\widehat{\Bellman}_N q)(s_i, a_i; \boldsymbol{\theta}^{(n)})\}$ are targets computed as Monte Carlo approximations of $(\Bellman q)(s_i, a_i)$ using $N$ samples from the transition distribution. The inner loop solves the regression problem: find parameters $\boldsymbol{\theta}$ that make $q(s_i, a_i; \boldsymbol{\theta}) \approx y_i^{(n)}$. We can write this abstractly as:
...
The `fit` operation has signature

$$
\texttt{fit}(X, y, \boldsymbol{\theta}_{\text{init}}, K) \to \boldsymbol{\theta}
$$

and minimizes $\mathcal{L}(\boldsymbol{\theta}) = \frac{1}{|X|}\sum_{i=1}^{|X|} \ell(q(x_i; \boldsymbol{\theta}), y_i)$ using $K$ optimization steps starting from $\boldsymbol{\theta}_{\text{init}}$, where $x_i = (s_i, a_i)$. This resembles scikit-learn's API but with additional controls. The optimization method depends on the function approximator: gradient descent for neural networks, tree construction for ensembles, matrix inversion for linear models. Standard supervised learning uses random initialization and runs to convergence ($K = \infty$). Reinforcement learning algorithms vary these: warm starting from the previous iteration's parameters ($\boldsymbol{\theta}_{\text{init}} = \boldsymbol{\theta}^{(n)}$), taking only $K \in \{1, \ldots, 10\}$ optimization steps, or somewhere in between.

This structure solves a sequence of regression problems $(X^{(0)}, y^{(0)}), (X^{(1)}, y^{(1)}), (X^{(2)}, y^{(2)}), \ldots$, where the input points $X^{(n)}$ may stay fixed (batch setting) or change (online setting), but the targets $y^{(n)}$ always change because they depend on the current Q-function $q(\cdot; \boldsymbol{\theta}^{(n)})$. Each outer iteration recomputes targets using the updated Q-function, then fits a new Q-function to these targets.

The apparent diversity of methods (FQI, NFQI, Q-learning, DQN, Double DQN, Soft Q) emerges from independent choices about how to construct $(X^{(n)}, y^{(n)})$, what initialization and truncation to pass to `fit`, and which loss function $\ell$ and function approximator $q(s,a; \boldsymbol{\theta})$ to use. In practice, we typically have $N=1$ (single observed next state per transition) and $(\widehat{\Bellman}_1 q)(s_i, a_i; \boldsymbol{\theta}^{(n)}) = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}^{(n)})$ where $(s_i, a_i, r_i, s'_i)$ is the observed transition tuple. This section maps that design space.

### Design Choices and Algorithm Space

The template above serves as a blueprint for instantiating concrete algorithms from the literature. Different choices along six main axes yield different methods: the function approximator (trees, neural networks, linear models), the Bellman operator (hard max vs smooth logsumexp), the inner optimization strategy (full convergence, $K$ steps, or single step), the initialization scheme (cold vs warm start), the data collection mechanism (offline, online, replay buffer), and bias mitigation approaches (none, double Q-learning, learned correction). While individual algorithms may include additional refinements, these axes capture the primary sources of algorithmic variation. The table below shows how several well-known methods instantiate this template:

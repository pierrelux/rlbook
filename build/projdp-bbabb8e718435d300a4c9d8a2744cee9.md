...
This computes the value of a given policy $\pi_\theta$. The Euler equation characterizes the optimal policy by differentiating with respect to the action at each state. Danskin's theorem justifies ignoring how the optimal action might change locally when taking the derivative. Both use recursive structure to simplify the dependence on future states: the policy gradient telescopes via the fundamental matrix, while the Euler equation telescopes via the envelope theorem.

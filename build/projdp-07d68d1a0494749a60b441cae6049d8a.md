...
**Algorithm: Galerkin with Function Iteration**
- **Input:** Basis functions $\{\varphi_1, \ldots, \varphi_n\}$, weight function $w(s)$, initial guess $\theta^{(0)}$
- **Output:** Converged coefficients $\theta^*$

1. Compute mass matrix $M_{ij} = \int_{\mathcal{S}} \varphi_i(s) \varphi_j(s) w(s) ds$ via numerical integration
2. **for** $k = 0, 1, 2, \ldots$ **do**
3. $\quad$ **for** $i = 1, \ldots, n$ **do**
4. $\quad\quad$ Compute $b_i^{(k)} = \int_{\mathcal{S}} [\mathrm{L}\hat{v}(s; \theta^{(k)})] \varphi_i(s) w(s) ds$ via numerical integration
5. $\quad$ Solve $M \theta^{(k+1)} = b^{(k)}$
6. $\quad$ **if** $\|\theta^{(k+1)} - \theta^{(k)}\| < \varepsilon$ **then** stop
...
**Algorithm: Galerkin with Newton's Method**
- **Input:** Basis functions $\{\varphi_1, \ldots, \varphi_n\}$, weight function $w(s)$, initial guess $\theta^{(0)}$
- **Output:** Converged coefficients $\theta^*$

1. **for** $k = 0, 1, 2, \ldots$ **do**
2. $\quad$ **for** $i = 1, \ldots, n$ **do**
3. $\quad\quad$ Compute $F_i^{(k)} = \int_{\mathcal{S}} [\mathrm{L}\hat{v}(s; \theta^{(k)}) - \hat{v}(s; \theta^{(k)})] \varphi_i(s) w(s) ds$ via numerical integration
4. $\quad\quad$ **for** $j = 1, \ldots, n$ **do**
5. $\quad\quad\quad$ Compute $J_{ij} = \int_{\mathcal{S}} \left[ \gamma \mathbb{E}[\varphi_j(s') \mid s, a^*(s;\theta^{(k)})] - \varphi_j(s) \right] \varphi_i(s) w(s) ds$ via numerical integration
6. $\quad$ Solve $J \Delta\theta = F^{(k)}$
7. $\quad$ Update $\theta^{(k+1)} = \theta^{(k)} - \Delta\theta$
8. $\quad$ **if** $\|\Delta\theta\| < \varepsilon$ **then** stop

...
The weighted residual framework applies to any functional equation from an MDP, including but not limited to the Bellman equation for value functions. An alternative approach, widely used in economics and macroeconomics, directly approximates the **optimal policy** by solving the first-order conditions that characterize optimality {cite}`Judd1992`. These Euler equation methods treat the optimal policy $\pi^*$ as the solution to a functional equation derived from the FOC rather than from the Bellman equation.
...
2. **Euler residual accuracy**: If the goal is to minimize the Euler residual (e.g., testing whether a policy satisfies optimality conditions), directly targeting this residual is more accurate than deriving the policy from an approximate value function.
...
1. **Nonlinearity**: Even for problems where the Bellman operator has structure (e.g., linearity in the value function for policy evaluation), the Euler equation is generally nonlinear in the policy, requiring iterative solvers like Newton's method.
...
3. **No global convergence guarantee**: Unlike value iteration with monotone approximators, successive approximation of the Euler equation does not generally define a contraction mapping, though convergence can be established under problem-specific conditions.

In numerical comparisons on smooth control problems with differentiable rewards and dynamics, Euler equation projection methods (particularly orthogonal collocation) can achieve high accuracy with relatively few basis functions {cite}`Judd1992`. However, value function iteration remains more robust and broadly applicable, especially for problems with discrete actions, constraints, or non-smooth objectives.

### Connection to Policy Parameterization

Euler equation methods provide a preview of **policy parameterization**, where we directly represent and optimize policies rather than value functions. The next chapter extends the weighted residual framework to simulation-based settings, where expectations in both the Bellman operator and the Euler operator must be approximated from samples. Policy gradient methods in modern reinforcement learning can be viewed as stochastic approximations to Euler equation residual minimization, though typically without explicitly forming the Euler equation. A later chapter develops policy parameterization systematically, showing how actor-critic architectures combine value function approximation (for variance reduction) with policy parameterization (for direct optimization).

Weighted residual methods apply to any functional equation arising from an MDP, whether that equation characterizes the value function (Bellman equation), the policy (Euler equation), or other objects like Q-functions or advantage functions. The unifying principle is orthogonality of residuals to test functions. The next chapter extends this framework to the simulation-based setting, where operators themselves must be approximated from samples rather than computed exactly.

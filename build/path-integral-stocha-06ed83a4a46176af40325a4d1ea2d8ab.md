...
Let $S_h$ denote the terminal state cost plus a quadrature approximation
of the running state cost. With $\lambda\Sigma_h^{-1}=hR$, approximating
the integrals in $S^{\bar{\mathbf u}}$ yields
...
Let $p_0$ be the joint density of the uncontrolled increments, each
distributed as $\mathcal N(0,\nu h)$. For sampled increment sequences
$\Delta x^{(i)}$, normalize the following weights to sum to one and use
their weighted first increment to estimate the action:

...
This framework extends to more general function spaces through the **dual pairing** formulation {cite}`LegrandJunca2025`. Earlier in this chapter, we presented collocation as using Dirac delta "functions" $\delta(x - x_i)$ without dwelling on technicalities. Formally, Dirac deltas are not functions but distributions, and the collocation equations are rigorously understood through the dual pairing $\langle R[v], \delta(x - x_i) \rangle = R[v](x_i)$, where $\langle \cdot, \cdot \rangle$ is the action of a distribution on a test function. More generally, residuals may live in a dual space $\mathcal{H}'$ with dual norm $\|R[v]\|_{\mathcal{H}'} = \sup_{\|w\|=1} |\langle R[v], w \rangle|$. This formalism handles all the cases presented in this chapter and extends to weak formulations in PDEs where discontinuous residuals are tested against smooth functions. The minimum residual principle remains the same: find parameters minimizing $\|R[v_{\boldsymbol{\theta}}]\|_{\mathcal{H}'}^2$.

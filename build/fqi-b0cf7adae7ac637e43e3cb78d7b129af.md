...
The term "bootstrap" appears with two distinct meanings. Fitted Q-iteration uses both.

**Statistical bootstrap**: The true distribution of transitions under the MDP and behavior policy is unknown. In its place, we use the empirical distribution $\hat{P}_{\mathcal{D}} = \frac{1}{N}\sum_{i=1}^N \delta_{\tau_i}$ over the observed dataset $\mathcal{D} = \{\tau_1, \ldots, \tau_N\}$ where $\tau_i = (s_i, a_i, r_i, s'_i)$. To form a mini-batch for regression, we sample transitions by drawing indices $i_1, \ldots, i_b$ uniformly from $\{1, \ldots, N\}$ and use transitions $\{\tau_{i_j}\}_{j=1}^b$. This treats the dataset as if it were the population, the classical bootstrap principle from statistics.

**Bootstrapping from value estimates**: The target $y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta})$ uses the current Q-function estimate at $s'_i$ rather than a full return $\sum_{t=0}^{T-i} \gamma^t r_{i+t}$ from trajectory rollout. We replace the unknown value-to-go with our current approximation.

These two forms are independent. We could resample from $\hat{P}_{\mathcal{D}}$ while using Monte Carlo returns, or we could compute exact expectations under the true transition dynamics while using value estimates in targets. Fitted Q-iteration does both.

The distinction between offline and online learning reduces to whether $\hat{P}_{\mathcal{D}}$ is static or evolves. Offline methods fix the dataset once: $\mathcal{D}$ never changes, and we repeatedly solve regression problems on data sampled from the same $\hat{P}_{\mathcal{D}}$. Online methods grow the dataset as $\mathcal{D}_t = \mathcal{D}_{t-1} \cup \{(s_t, a_t, r_t, s'_{t+1})\}$, so $\hat{P}_{\mathcal{D}_t}$ shifts continuously as new transitions arrive. A replay buffer with finite capacity implements a sliding window: when full, old transitions are discarded (typically FIFO), so $\hat{P}_{\mathcal{D}_t}$ represents recent experience.

Three canonical cases:

- **Classical FQI**: Fixed $\mathcal{D}$, use full dataset each outer iteration, run inner optimization to convergence ($K=\infty$). Pure sample average approximation.
- **DQN**: Growing $\mathcal{D}_t$ with capacity $B$, sample mini-batch of size $b < B$ uniformly each step, single gradient step ($K=1$). Stochastic approximation with time-varying empirical distribution and data reuse.
- **Online Q-learning**: Capacity 1, use each transition once then discard. Pure stochastic approximation without reuse.

The replay ratio (optimization steps per environment transition) quantifies data reuse:

$$
\text{Replay ratio} = \begin{cases}
K \cdot N_{\text{epochs}} \cdot N / b & \text{Offline (batch of size } b \text{ from } N \text{ transitions)} \\
b & \text{DQN (one step per new transition, batch size } b \text{)} \\
1 & \text{Online Q-learning}
\end{cases}
$$

Higher replay ratios amortize environment interaction costs but risk overfitting to $\hat{P}_{\mathcal{D}}$, especially when it reflects outdated policies. The choice of $K$ and buffer capacity control the position along the sample average approximation to stochastic approximation continuum.
...
This flattening is **not possible** with non-iterative regression methods like decision trees or random forests. Tree construction is a discrete, non-iterative procedure: you build the tree structure once by recursively partitioning the input space. There is no sequence of parameter updates to unfold into a flattened loop. This is why Ernst's FQI retains the explicit nested structure—the tree ensemble fitting cannot be incrementalized in this way.
...
We can now flatten the nested structure by treating all gradient steps uniformly and using a global step counter $t$ instead of separate outer/inner counters. The key is to introduce a **target network** $\boldsymbol{\theta}_{\text{target}}$ that holds the parameters used for computing targets. This target network gets updated every $K$ steps, which marks what would have been the boundary between outer iterations:

**The flattening transformation:**
- Replace $n$ (outer) and $k$ (inner) with single counter $t$
- When $n$ changes from $n$ to $n+1$, that corresponds to $K$ steps of $t$: from $t = nK$ to $t = (n+1)K$
- The relationship is: $t = nK + k$, so $n = \lfloor t/K \rfloor$ and $k = t \bmod K$
- Introduce $\boldsymbol{\theta}_{\text{target}}$ that equals $\boldsymbol{\theta}_n$ throughout outer iteration $n$
- Update $\boldsymbol{\theta}_{\text{target}} \leftarrow \boldsymbol{\theta}_t$ every $K$ steps (when $t \bmod K = 0$)
- Parameters that were $\boldsymbol{\theta}_n^{(k)}$ in nested form become $\boldsymbol{\theta}_t$ in flattened form

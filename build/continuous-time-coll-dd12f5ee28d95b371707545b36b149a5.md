...
Both vectors describe the same quadratic. The transcription used here chooses
coordinates like $\mathbf y$ for the state and control: values that can be
inserted directly into the dynamics, cost, and bounds. Polynomial coefficients
...
Choose $s$ distinct collocation nodes $\tau_1,\ldots,\tau_s$ on $[0,1]$. On interval $k$,
...
The node family specifies where the ODE is sampled. Its nodes can use the
same Lagrange representation and coefficient generator as the low-order
methods. Adjacent state
...
The low-order constructions and coefficient generator do not require a
detailed comparison of these families. Their convergence rates and adaptive
degree selection are topics for further study.
...
simulation checks the trajectory produced by the chosen control. Can the same

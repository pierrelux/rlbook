...
All value-based reinforcement learning algorithms share the same two-level structure. The outer loop applies the Bellman operator $\mathcal{L}$ to the current Q-function at observed state-action pairs, producing a supervised learning dataset $\mathcal{D}^{\text{fit}} = \{((s_i, a_i), y_i)\}$ where each target is $y_i = \widehat{\mathcal{L}}_N q(s_i, a_i; \boldsymbol{\theta}_n)$, the Monte Carlo approximation of $(\mathcal{L}q)(s_i, a_i)$ using $N$ samples from the transition distribution. The inner loop solves a regression problem: find parameters $\boldsymbol{\theta}$ that make $q(s_i, a_i; \boldsymbol{\theta}) \approx y_i$. We can write this abstractly as:

$$
\begin{aligned}
&\textbf{repeat } n = 0, 1, 2, \ldots \\
&\quad \text{Construct } \mathcal{D}_n^{\text{fit}} = \{((s_i, a_i), \widehat{\mathcal{L}}_N q(s_i, a_i; \boldsymbol{\theta}_n))\} \text{ from observed } (s,a) \text{ pairs} \\
...
The apparent diversity of methods (FQI, NFQI, Q-learning, DQN, Double DQN, Soft Q) emerges from independent choices about how to construct $\mathcal{D}_n^{\text{fit}}$, what initialization and truncation to pass to `fit`, and which loss function $\ell$ and function approximator $q(s,a; \boldsymbol{\theta})$ to use. In practice, we typically have $N=1$ (single observed next state per transition) and $\widehat{\mathcal{L}}_1 q(s_i, a_i; \boldsymbol{\theta}_n) = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}_n)$ where $(s_i, a_i, r_i, s'_i)$ is the observed transition tuple. This section maps that design space.

...
**Step 2: Recursive substitution via the fundamental matrix.** Under a fixed policy $\pi$, let $P_\pi$ denote the transition matrix with entries $P_\pi(j|s) = p(j|s,\pi(s))$ and let $r_\pi(s) = r(s, \pi(s))$ be the reward vector. The value function satisfies:
$$
...
The matrix $(I - \gamma P_{\pi^*})^{-1} = \sum_{t=0}^\infty \gamma^t P_{\pi^*}^t$ is the **fundamental matrix**, giving the discounted occupancy distribution.

Now consider the first-order condition. Define the vector $\nabla_a p(\cdot|s,a)|_{a=\pi(s)}$ with entries $\frac{\partial p(j|s,a)}{\partial a}\big|_{a=\pi(s)}$. The first-order condition can be written:
$$
\frac{\partial r(s, \pi(s))}{\partial a} + \gamma \left[\nabla_a p(\cdot|s,a)\big|_{a=\pi(s)}\right]^\top v^* = 0.
$$

Substituting the fundamental matrix expression for $v^*$:
$$
\frac{\partial r(s, \pi(s))}{\partial a} + \gamma \left[\nabla_a p(\cdot|s,a)\big|_{a=\pi(s)}\right]^\top (I - \gamma P_{\pi^*})^{-1} r_{\pi^*} = 0.
$$

This expresses the first-order condition purely in terms of the policy $\pi^*(\cdot)$, the reward function $r$, and the transition probabilities $p$—the value function has been eliminated. The term $(I - \gamma P_{\pi^*})^{-1} r_{\pi^*}$ sums the infinite series of future rewards under policy $\pi^*$.

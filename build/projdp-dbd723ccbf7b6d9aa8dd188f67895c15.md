...
The equivalence between minimization and orthogonality extends beyond this setting. The **dual pairing** formulation {cite}`LegrandJunca2025` unifies weighted $L^2$ integrals (Galerkin), point evaluations (collocation using Dirac distributions $\langle \Residual(v), \delta(x - x_i) \rangle = (\Residual v)(x_i)$), and sampled expectations (neural networks). The induced dual norm $\|\Residual(v)\|_* = \sup_{\|w\|=1} |\langle \Residual(v), w \rangle|$ measures residuals by their worst-case effect on test functions, a perspective that has inspired adversarial formulations {cite}`Zang2020` where both trial and test functions are learned.

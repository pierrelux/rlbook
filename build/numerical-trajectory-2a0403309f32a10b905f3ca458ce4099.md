...
develop nodal polynomial representations directly from differential equations.

The stochastic examples also change how a plan is executed. Applying one
action and observing the next state turns repeated planning into feedback.
How should the planning horizon, terminal conditions, and feasibility checks
be chosen for this execution rule? [Receding-horizon
control](receding-horizon-control.md) develops those questions for MPC,
independently of whether its optimizer uses derivatives or trajectory samples.

...
These choices are independent but should be coordinated. To see why, we formalize the quadrature error and show how it interacts with the basis approximation quality.

### Notation and Setup

Let $\hat{v}(s; \theta) = \sum_{i=1}^n \theta_i \varphi_i(s)$ be our approximate value function. For each state-action pair $(s,a)$, define the **feature-moment vector** $K(s,a) \in \mathbb{R}^n$ by:

$$ K_i(s,a) := \int \varphi_i(s') p(s'|s,a) ds' $$

This vector encodes the expected basis function values under the transition distribution. A quadrature rule with nodes and weights $\{(s'_j, w_j)\}_{j=1}^m$ induces an **approximate moment vector** $\widehat{K}(s,a) \in \mathbb{R}^n$:

$$ \widehat{K}_i(s,a) := \sum_{j=1}^m w_j \varphi_i(s'_j) $$

With this notation, we can write the Bellman operator and its quadrature approximation compactly:

$$
\begin{aligned}
(\Bellman \hat{v})(s) &= \max_{a \in \mathcal{A}_s} \left\{ r(s,a) + \gamma \theta^\top K(s,a) \right\} \\
(\widehat{\Bellman}_Q \hat{v})(s) &= \max_{a \in \mathcal{A}_s} \left\{ r(s,a) + \gamma \theta^\top \widehat{K}(s,a) \right\}
\end{aligned}
$$

### Pointwise Quadrature Error

The error from quadrature at any state $s$ satisfies:

$$ |(\Bellman \hat{v})(s) - (\widehat{\Bellman}_Q \hat{v})(s)| \le \gamma \sup_{a \in \mathcal{A}_s} |\theta^\top (K(s,a) - \widehat{K}(s,a))| $$

Using the duality $|x^\top y| \le \|x\|_1 \|y\|_\infty$:

$$ |(\Bellman \hat{v})(s) - (\widehat{\Bellman}_Q \hat{v})(s)| \le \gamma \|\theta\|_1 \sup_{a \in \mathcal{A}_s} \|K(s,a) - \widehat{K}(s,a)\|_\infty $$

Define the **feature-wise quadrature defect**:

$$ \varepsilon_Q := \sup_{s,a,i} \left| \int \varphi_i(s') p(s'|s,a) ds' - \sum_j w_j \varphi_i(s'_j) \right| $$

This measures the worst-case quadrature error over all basis functions, states, and actions. Then uniformly in $s$:

$$ \|\Bellman \hat{v} - \widehat{\Bellman}_Q \hat{v}\|_\infty \le \gamma \|\theta\|_1 \varepsilon_Q $$

### Global Fixed-Point Error

Consider fitted value iteration $v_{k+1} = \Pi \widehat{\Bellman}_Q v_k$ where $\Pi$ is a projection operator onto $\text{span}\{\varphi_i\}$ (e.g., collocation interpolation). Let $v^*$ denote the true value function satisfying $v^* = \Bellman v^*$. Using the contraction of $\Bellman$ with modulus $\gamma$ and stability of $\Pi$ (with $\|\Pi\|_{\infty \to \infty} \le C_\Pi$), a standard perturbation argument yields:

$$
\boxed{
\|v^* - v_\infty\|_\infty \le \frac{1}{1-\gamma} \underbrace{\inf_{\hat{v} \in \text{span}\{\varphi_i\}} \|v^* - \hat{v}\|_\infty}_{\text{basis approximation error}} + \frac{C_\Pi}{1-\gamma} \underbrace{\|\Bellman \hat{v} - \widehat{\Bellman}_Q \hat{v}\|_\infty}_{\text{quadrature error}}
}
$$

where $v_\infty$ is the fixed point of $\Pi \widehat{\Bellman}_Q$.

### Why Coordination Matters

The total error decomposes into two terms, each amplified by $\frac{1}{1-\gamma}$. If the basis achieves approximation error $O(n^{-m})$ (e.g., exponential for Chebyshev polynomials) but the quadrature defect is $\varepsilon_Q = O(h^q)$ (e.g., $q=2$ for rectangle rule), then:

$$ \|v^* - v_\infty\|_\infty = O\left(\frac{n^{-m}}{1-\gamma}\right) + O\left(\frac{h^q}{1-\gamma}\right) $$

When $h^q$ decays much slower than the basis error, **the quadrature term dominates** and we fail to realize the accuracy promised by the basis. For instance, degree-10 Chebyshev polynomials achieve basis error $\approx 10^{-8}$ for smooth functions, but combined with rectangle-rule quadrature ($\varepsilon_Q \approx 10^{-2}$), the total error is limited to $\approx 10^{-2}/(1-\gamma)$. We pay the cost of storing and computing with 10 coefficients but achieve only second-order accuracy in the quadrature mesh size.

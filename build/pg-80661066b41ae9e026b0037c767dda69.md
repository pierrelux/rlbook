...
## Off-Policy Likelihood Ratio Methods

The score function methods developed above are **on-policy**: they require data sampled from the current policy $\pi_{\boldsymbol{w}}$. After each gradient update, the old data becomes stale and must be discarded. This is sample-inefficient, as each trajectory is used for exactly one gradient step.

The likelihood ratio framework from the beginning of this chapter provides a way to reuse data. Recall that for any proposal distribution $q(x)$, we can write:

$$
J(\theta) = \mathbb{E}_{x \sim p(x;\theta)}[f(x)] = \mathbb{E}_{x \sim q(x)}\left[f(x) \rho(x; q, \theta)\right]
$$
...
The on-policy REINFORCE estimator corresponds to the special case $q(x) = p(x;\theta)$, where $\rho = 1$ and we recover the score function estimator. But the framework is more general: we can use any $q$ whose support contains the support of $p$, and the likelihood ratio $\rho$ corrects for the distribution mismatch.

### Off-Policy Policy Gradients

In the reinforcement learning setting, let $\tau$ denote a trajectory and $G(\tau)$ the return. The objective is $J(\boldsymbol{w}) = \mathbb{E}_{\tau \sim p(\tau;\boldsymbol{w})}[G(\tau)]$. Using a behavior policy $\pi_{\text{b}}$ to collect data, we can write:

$$
J(\boldsymbol{w}) = \mathbb{E}_{\tau \sim p_{\text{b}}(\tau)}\left[G(\tau) \rho(\tau; \boldsymbol{w})\right]
$$

where $\rho(\tau; \boldsymbol{w}) = \frac{p(\tau;\boldsymbol{w})}{p_{\text{b}}(\tau)}$ is the trajectory importance ratio. Since trajectories factorize as products of policy and transition probabilities, and the transition probabilities cancel:

$$
\rho(\tau; \boldsymbol{w}) = \frac{\prod_{t=0}^T \pi_{\boldsymbol{w}}(a_t|s_t)}{\prod_{t=0}^T \pi_{\text{b}}(a_t|s_t)} = \prod_{t=0}^T \frac{\pi_{\boldsymbol{w}}(a_t|s_t)}{\pi_{\text{b}}(a_t|s_t)} = \prod_{t=0}^T \rho_t(\boldsymbol{w})
$$

The gradient becomes:

$$
\nabla_{\boldsymbol{w}} J(\boldsymbol{w}) = \mathbb{E}_{\tau \sim p_{\text{b}}}\left[G(\tau) \rho(\tau; \boldsymbol{w}) \sum_{t=0}^T \nabla_{\boldsymbol{w}} \log \pi_{\boldsymbol{w}}(a_t|s_t)\right]
$$

This is the **off-policy REINFORCE** estimator. Compared to on-policy REINFORCE, each gradient contribution is weighted by the trajectory importance ratio $\rho(\tau; \boldsymbol{w})$.

### Temporal Structure and Per-Step Importance Weights

The trajectory-level importance ratio $\rho(\tau; \boldsymbol{w}) = \prod_{t=0}^T \rho_t$ is a product of $T+1$ terms. As $T$ grows, this product can become extremely large or extremely small, leading to high variance.

The same temporal structure that gave us the extended conditional Monte Carlo improvement for on-policy methods helps here too. Since rewards $r_k$ for $k < t$ do not depend on action $a_t$, we can use per-step importance weights rather than full trajectory weights. For the gradient contribution at time $t$:

$$
\mathbb{E}_{\tau \sim p_{\text{b}}}\left[\nabla_{\boldsymbol{w}} \log \pi_{\boldsymbol{w}}(a_t|s_t) \sum_{k=t}^T r_k \prod_{l=0}^T \rho_l\right]
$$

The importance weights for $l > T$ (after the episode ends) and $l < t$ (before the current timestep) can be factored out. With appropriate conditioning, this reduces to:

$$
\mathbb{E}_{s_t, a_t \sim \pi_{\text{b}}}\left[\rho_t(\boldsymbol{w}) \nabla_{\boldsymbol{w}} \log \pi_{\boldsymbol{w}}(a_t|s_t) \sum_{k=t}^T r_k\right]
$$

The full trajectory weight $\prod_{l=0}^T \rho_l$ is replaced by a single per-step weight $\rho_t$, dramatically reducing variance. This gives us the practical off-policy gradient estimator.

### Control Variates for Off-Policy Methods

The control variate technique applies directly. Subtracting a baseline $b(s_t)$ from the return preserves unbiasedness:

$$
\nabla_{\boldsymbol{w}} J(\boldsymbol{w}) = \mathbb{E}_{s_t, a_t \sim \pi_{\text{b}}}\left[\rho_t(\boldsymbol{w}) \nabla_{\boldsymbol{w}} \log \pi_{\boldsymbol{w}}(a_t|s_t) \left(\sum_{k=t}^T r_k - b(s_t)\right)\right]
$$

The proof follows the same logic as before: $\mathbb{E}_{a_t}[\rho_t \nabla_{\boldsymbol{w}} \log \pi_{\boldsymbol{w}}(a_t|s_t)] = \nabla_{\boldsymbol{w}} \sum_a \pi_{\boldsymbol{w}}(a|s_t) = 0$.

Using the identity $\rho_t \nabla_{\boldsymbol{w}} \log \pi_{\boldsymbol{w}} = \nabla_{\boldsymbol{w}} \rho_t \cdot \frac{\pi_{\text{b}}}{\pi_{\text{b}}} = \frac{\nabla_{\boldsymbol{w}} \pi_{\boldsymbol{w}}}{\pi_{\text{b}}}$, we can write this as the gradient of an importance-weighted surrogate objective:

$$
L^{\text{IS}}(\boldsymbol{w}) = \mathbb{E}_{s, a \sim \pi_{\text{b}}}\left[\rho_t(\boldsymbol{w}) A_t\right]
$$ (eq:importance-weighted-surrogate)

where $A_t = \sum_{k=t}^T r_k - b(s_t)$ is the advantage estimate. When $\pi_{\text{b}} = \pi_{\boldsymbol{w}}$, we have $\rho_t = 1$ and recover the on-policy surrogate loss.

### Variance and the Dominance Condition

While mathematically valid, importance sampling can have high variance. The ratio $\rho_t = \pi_{\boldsymbol{w}}/\pi_{\text{b}}$ is well-behaved only when the two distributions are similar. Two issues arise when they diverge:

First, if the target policy assigns high probability to actions that had low probability under the behavior policy, the ratio $\rho_t$ explodes. Suppose the behavior policy had $\pi_{\text{b}}(a|s) = 0.01$ for some action, and the target policy has $\pi_{\boldsymbol{w}}(a|s) = 0.5$. The ratio becomes $\rho_t = 50$, amplifying any noise in the advantage estimate by a factor of 50.

Second, importance sampling requires the **dominance condition**: the support of $\pi_{\boldsymbol{w}}$ must be contained in the support of $\pi_{\text{b}}$. If the target policy assigns positive probability to an action that had zero probability under the behavior policy, the ratio is undefined (division by zero). In practice, stochastic policies typically have full support, but the ratio can still become arbitrarily large as $\pi_{\text{b}}(a|s) \to 0$.

These issues are particularly problematic when the behavior policy is a previous version of the target policy, which we denote $\pi_{\boldsymbol{w}_{\text{old}}}$. In this setting, we collect a batch of trajectories, then take multiple gradient steps to amortize the cost of data collection. But each step moves $\boldsymbol{w}$ further from $\boldsymbol{w}_{\text{old}}$, making the importance weights more extreme. Eventually, the gradient signal is dominated by a few samples with large weights, and learning becomes unstable.

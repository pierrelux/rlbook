...
The minus sign before the inequality term follows from the convention
$H(\mathbf z)\ge\mathbf0$: a violation $H_i(\mathbf z)<0$ increases the
...
$J_G$ and $J_{H_{\mathcal A}}$ have those gradients as rows, LICQ requires

$$
\begin{bmatrix}J_G(\mathbf z^\star)\\J_{H_{\mathcal A}}(\mathbf z^\star)\end{bmatrix}
...
Complementarity requires $\mu_i^\star=0$ when $H_i(\mathbf z^\star)>0$.
...
In trajectory problems, $\mathbf z$ stacks states and controls, $G$ enforces
the dynamics, and $H$ collects bounds and path constraints. The equality
...
\mathcal L(\mathbf z,\boldsymbol\lambda,\boldsymbol\mu),
\qquad
\mathcal L=F+\boldsymbol\lambda^\top G-\boldsymbol\mu^\top H.
$$
...
arbitrarily large. If $H_i(\mathbf z)<0$, sending $\mu_i$ to infinity does
...
When $F$ is convex, each $H_i$ is concave, and $G$ is affine, the Lagrangian
...
Slater's condition, the existence of a point with $G=0$ and $H_i>0$ for every
...
$H\ge0$, a violated inequality increases its multiplier. These finite updates

...
D'Eramo et al. proved two key bias properties:

1. **Theorem 1**: $\text{Bias}(\hat{\mu}^{WE}_*) \leq \text{Bias}(\hat{\mu}^{ME}_*)$ (less overestimation than ME)
2. **Theorem 2**: $\text{Bias}(\hat{\mu}^{WE}_*) \geq \text{Bias}(\hat{\mu}^{DE}_*)$ (less underestimation than DE)

Thus WE provides a principled middle ground between the optimism of ME and the pessimism of DE.

**When each estimator excels:** The relative performance depends on the problem structure:

- **ME performs best** when one action is clearly superior ($\mu_1 - \mu_2 \gg \sigma$): the signal dominates the noise, so overestimation is minimal
- **DE performs best** when multiple actions have similar values ($\mu_1 \approx \mu_2$): eliminating overestimation bias outweighs the pessimism
- **WE performs best** in intermediate cases (most practical scenarios): it adapts to the actual uncertainty, neither overly optimistic nor pessimistic

The weighted estimator can be incorporated into Q-learning by maintaining variance estimates alongside Q-values and computing the weighted target at each step. Additionally, the weights $w_a$ themselves can serve as a natural exploration policy that adapts to estimation uncertainty.

**Practical considerations:**

Advantages:
- Adapts automatically to heterogeneous uncertainty across actions
- Provides balanced bias (neither overly optimistic nor pessimistic)
- Weights can define an uncertainty-aware exploration policy

Disadvantages:
- Requires maintaining variance estimates (memory overhead)
- Numerical integration expensive for large action spaces
- Assumes Gaussian sampling distributions (though robust in practice)

**Connection to smooth Bellman operators:** The weighted estimator is conceptually related to the smooth Bellman operators discussed in the [regularized MDP chapter](regmdp.md). Both replace the discontinuous hard maximum with smooth aggregation, but weighted estimation adapts to state-action-specific uncertainty while logsumexp applies uniform smoothing via temperature.
...
3. **Bias mitigation**: Address the systematic overestimation that arises from maximizing over noisy estimates through three approaches: learning explicit corrections (Keane-Wolpin), decoupling selection from evaluation (double Q-learning), or probability-weighted aggregation (weighted estimators)

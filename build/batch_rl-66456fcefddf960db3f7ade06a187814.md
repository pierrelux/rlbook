...
The max operator creates systematic overestimation bias in TD targets. The [previous chapter](simadp.md) examined approaches that address this bias by modifying the Bellman operator evaluation itself: learning empirical corrections (Keane-Wolpin), analytical bias adjustment (Lee-Powell), decoupling selection from evaluation (double Q-learning), and probability-weighted aggregation (weighted estimators). These methods keep the loss function fixed (typically L2) while correcting biased targets.

An alternative approach changes the loss function itself to match the noise structure induced by the max operator. Two strategies have shown empirical success: Gumbel regression, which uses the proper likelihood for extreme-value noise, and classification-based methods, which avoid parametric noise assumptions by working with distributions over value bins.

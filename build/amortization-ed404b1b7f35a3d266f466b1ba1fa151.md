...
This is where the entropy-regularized perspective becomes essential. Recall that the value function under entropy regularization can be written as:

$$
v(s) = \sum_a \pi(a|s)[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]] + \alpha \mathcal{H}(\pi(\cdot|s))
$$

where $\mathcal{H}(\pi(\cdot|s)) = -\sum_a \pi(a|s)\log \pi(a|s)$ is the entropy of the policy at state $s$. Expanding the entropy term explicitly:

$$
v(s) = \sum_a \pi(a|s)[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]] - \alpha \sum_a \pi(a|s) \log \pi(a|s)
$$

We can factor out the expectation over actions:

$$
v(s) = \sum_a \pi(a|s)\left[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')] - \alpha \log \pi(a|s)\right]
$$

Now recall that the Q-function is defined as $q(s,a) = r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]$, so we can substitute:

$$
v(s) = \sum_a \pi(a|s)\left[q(s,a) - \alpha \log \pi(a|s)\right] = \mathbb{E}_{a \sim \pi(\cdot|s)}\left[q(s,a) - \alpha \log \pi(a|s)\right]
$$
...
For the optimal policy under entropy regularization, we additionally have the logsumexp representation:

$$
v^*(s) = \frac{1}{\beta} \log \sum_a \exp(\beta \cdot q^*(s,a))
$$

where $\beta = 1/\alpha$. These two forms—the logsumexp and the entropy-adjusted expectation—compute the same optimal value function, but the expectation form is tractable via sampling while the logsumexp requires integrating over all actions.

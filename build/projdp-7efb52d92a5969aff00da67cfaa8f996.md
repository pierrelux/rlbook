...
The minimum residual framework thus connects classical projection methods to modern function approximation. The unifying principle is orthogonality of residuals to test functions. Linear methods use fixed test functions and admit closed-form solutions. Nonlinear methods use parameter-dependent test functions and require iterative optimization. Convergence guarantees are well understood for linear projections (monotonicity, matched weightings) but remain an active research area for neural networks.

## Euler Equation Methods: Approximating Policies Instead of Values

So far in this chapter, we have used the weighted residual framework to approximate the **value function** by solving the Bellman equation. But remember: the weighted residual framework is general. It applies to *any* functional equation. This opens an alternative route that is rarely explored in reinforcement learning but commonplace in economics: approximate the **optimal policy** directly, without ever representing the value function.

This may sound surprising. After all, the Bellman equation defines the value function, and the policy is recovered from it via maximization. How can we bypass the value function entirely? We work with the **Euler equations** (the first-order optimality conditions of the dynamic optimization problem), which, under certain structural conditions, become a closed functional equation in the policy alone {cite}`Judd1992,Rust1996`.

This section develops this alternative approach. We will see that in many practical control problems (inventory management, energy storage, reservoir control), the dynamics have just enough structure to eliminate the value function from the optimality conditions. The result is a policy approximation method that solves a nonlinear system in the policy parameters, rather than a fixed-point problem in the value function.

### From Bellman to Euler: deriving first-order conditions

To understand the shift from value-based to policy-based approximation, let us derive the Euler equation from the Bellman equation.

Consider a discounted control problem with continuous states and actions: $\mathcal{S} \subset \mathbb{R}^d$, $\mathcal{A} \subset \mathbb{R}^m$, deterministic dynamics

$$
s' = f(s,a),
$$

a continuously differentiable reward $r(s,a)$, and discount factor $0 < \gamma < 1$. The Bellman equation is
...
Assume that for each $s$, the maximizer is unique, so the optimal policy $\pi^*(s)$ satisfies
...
Now, because $\pi^*(s)$ maximizes the right-hand side, it must satisfy the first-order condition (assuming it is an interior maximum):

$$
\frac{\partial}{\partial a}\left[ r(s,a) + \gamma\, v^*\big(f(s,a)\big) \right]_{a=\pi^*(s)} = 0.
$$

Applying the chain rule, we obtain the **Euler equation in raw form**:
...
This Euler equation still involves $\partial v^*/\partial s$, the gradient of the value function. If we want to solve for the policy $\pi^*$, we must also keep track of the value-function derivatives. For a general MDP, Euler-based methods would thus have to solve jointly for $\pi$ and $\nabla v$, which offers no clear advantage over working with the Bellman equation directly.

### The Euler-class condition: when the value function disappears

For a broad class of problems, we *can* eliminate $\partial v^*/\partial s$ from the Euler equation using an envelope theorem argument. The resulting equation involves only the policy $\pi$ and the problem primitives $(r, f)$. {cite:t}`Rust1996` formalizes this class as **Euler-class** continuous decision processes.

#### Setting and notation

To state the structural condition precisely, it is convenient to partition the state as
...
where:
- $y_t \in \mathbb{R}^n$ is the **stock variable**, the component of the state that the agent directly influences through its actions and that persists across time steps. Think of it as the "memory" of past decisions: inventory level, battery charge, water in a reservoir, capital accumulated, or any quantity that evolves according to the agent's choices. In control theory, this is often called the "state variable"; in economics, it is called the "stock" because it represents an accumulated quantity.
- $z_t \in \mathbb{R}^p$ is the **exogenous variable** (or "shock" in economics jargon), the component of the state that evolves independently of the agent's actions. This captures external disturbances like demand fluctuations, weather conditions, equipment failures, or any randomness that the agent observes but cannot control. In RL terms, this is the part of the state governed purely by the environment dynamics, not by the policy.

The action is $a_t \in \mathcal{A}(y_t,z_t) \subset \mathbb{R}^m$. The reward $r(y,z,a)$ is typically concave in $(y,a)$. The transition law decomposes as

$$
p(dy_{t+1},dz_{t+1}\mid y_t,z_t,a_t)
=
\mathbf{1}\{y_{t+1} = f(y_t,a_t,z_{t+1},z_t)\}\,q(dz_{t+1}\mid z_t),
$$

so the controlled component $y$ evolves via a known function $f$ (which may depend on the next exogenous state $z_{t+1}$), while the exogenous component $z$ follows a Markov law $q$ that does not depend on the action.

What makes a problem Euler-class? The defining property is a regularity condition on how the dynamics $f$ depend on $(y,a)$. The **Euler-class condition** (EC) requires that there exists a matrix $h(y,a,z) \in \mathbb{R}^{n \times n}$ such that

$$
\frac{\partial f(y,a,z',z)}{\partial y}
=
\frac{\partial f(y,a,z',z)}{\partial a}\, h(y,a,z)
\quad \text{for all } (y,a,z',z).
\tag{EC}
$$
<!-- 
Let's unpack what (EC) says. In the scalar case ($n=m=1$), it simplifies to

$$
\frac{\partial f}{\partial y} = \frac{\partial f}{\partial a} \cdot h,
$$

for some scalar $h$. This says: if I increase the current state $y$ by one unit, the effect on next-period state $y'$ is $\partial f/\partial y$. If instead I increase the action $a$ by $h$ units, the effect on $y'$ is $h \cdot \partial f/\partial a$. The condition requires these two effects to be equal.

In other words, any change to $y'$ that I can achieve by adjusting the current state $y$, I can also achieve (up to scaling) by adjusting the action $a$. The two "control levers" ($y$ and $a$) push the next state in the same direction.

Why does this help? When we apply the envelope theorem to compute $\partial v/\partial y$, this proportionality allows us to replace derivatives with respect to the state by derivatives with respect to the action. Since the action is chosen optimally, its first-order condition is zero, and the value function drops out of the resulting equation. Without (EC), we would have no way to eliminate $\nabla v$ from the Euler equation. -->

When does (EC) hold? Whenever the dynamics are **affine** in $(y,a)$, meaning $f(y,a,z',z) = A(z,z')y + B(z,z')a + c(z,z')$ for some matrices/vectors $A,B,c$. In this case, $\partial f/\partial y$ and $\partial f/\partial a$ are constant (independent of $y$ and $a$), so the proportionality is automatic. Many control models use affine approximations of nonlinear dynamics, making (EC) widely applicable. When (EC) holds, the **envelope theorem** gives a formula for $\partial v/\partial y$ that depends only on $r$, $h$, and the policy $\pi$, not on $v$ itself. This is the sense in which Euler-class problems allow pure policy approximation: we can pose the optimality condition as a functional equation in $\pi$ alone and apply weighted residual methods to that equation, never constructing $v$.
<!-- Substituting this formula back into (Euler-raw) yields a **closed Euler equation** involving only $\pi$:
...
where $\mathcal{E}(\pi)(y,z)$ can be computed from $(r,f,h,q)$ and the policy evaluated at current and next-period states. The expression $\mathcal{E}(\pi)$ does *not* involve the value function or its derivatives. The value function has been completely eliminated.

 -->

### Examples: checking the Euler-class condition in practice

The condition (EC) may look abstract at first, but it is straightforward to verify in concrete models. Affine dynamics automatically satisfy (EC). Since many practical control problems use linear or affine state updates (at least as first-order approximations), the Euler-class structure is quite common.

Below we verify (EC) for several applications you might encounter in operations research, energy systems, and control engineering. In each case, $y,a \in \mathbb{R}$ are scalar, and the dynamics are affine in $(y,a)$.

#### Example 1: Inventory / production planning

**Dynamics:**

$$
y_{t+1} = \alpha\, y_t + a_t - D(z_{t+1}),
$$

where $y_t$ is inventory, $a_t$ is production, $D(z_{t+1})$ is random demand, and $0 < \alpha \le 1$ captures spoilage or carry-over.
...
Choose $h(y,a,z) = \alpha$. Then

$$
\frac{\partial f}{\partial y}
=
\frac{\partial f}{\partial a}\, h
\quad \Leftrightarrow \quad
\alpha = 1 \cdot \alpha. \quad \checkmark
$$

#### Example 2: Battery energy storage / arbitrage
...
y_{t+1}
=
\eta_{\mathrm{ret}}\, y_t + \eta_{\mathrm{ch}}\, a_t,
$$

where $y_t$ is state of charge, $a_t$ is net charging power, $\eta_{\mathrm{ret}} < 1$ is the retention efficiency (self-discharge), and $\eta_{\mathrm{ch}}$ is the charging efficiency.

**Verification:**

$$
\frac{\partial f}{\partial y} = \eta_{\mathrm{ret}},
\qquad
\frac{\partial f}{\partial a} = \eta_{\mathrm{ch}}.
$$

Choose $h(y,a,z) = \eta_{\mathrm{ret}}/\eta_{\mathrm{ch}}$. Then (EC) holds. $\checkmark$

#### Example 3: Reservoir / irrigation control
...
y_{t+1}
=
\alpha\, y_t + I(z_{t+1}) - a_t,
$$

where $y_t$ is the water level, $a_t$ is the release, $I(z_{t+1})$ is random inflow, and $0 < \alpha \le 1$ accounts for evaporation or seepage.
...
#### Example 4: Building thermal mass (simple RC model)
...
y_{t+1}
=
\kappa(z_t)\, y_t + \lambda(z_t)\, a_t + \mu(z_t),
$$

where $y_t$ represents internal thermal energy (or equivalently, temperature), $a_t$ is HVAC input, and $z_t$ captures exogenous conditions (ambient temperature, solar gains, occupancy). The functions $\kappa, \lambda, \mu$ encode the thermal model.

**Verification:**

$$
\frac{\partial f}{\partial y} = \kappa(z),
\qquad
\frac{\partial f}{\partial a} = \lambda(z).
$$

Choose $h(y,a,z) = \kappa(z)/\lambda(z)$ (assuming $\lambda(z) \neq 0$). Then (EC) holds. $\checkmark$

All four examples have **affine dynamics in the controlled state**: $f(y,a,z',z)$ is affine (linear plus a constant) in $(y,a)$. This immediately implies (EC).

<!--  The Euler-class framework thus extends far beyond the consumption-savings problems of classical macroeconomics. It applies to inventory systems, energy storage, water resources, and thermal control. Any domain where a first-order linear approximation of how $(y,a)$ affect the next state is reasonable falls into this class.

In practice, you can often *engineer* your model to be Euler-class by choosing state variables and linearizing around an operating point. This design choice then unlocks the ability to approximate policies without ever representing the value function. -->

### Applying weighted residuals to the Euler equation

Now that we have established that Euler-class problems admit a closed functional equation in the policy alone, we can apply the same weighted residual framework we used for the Bellman equation. The difference is that the unknown is $\pi$, not $v$.

#### Deriving the Euler equation: eliminating the value function

We now show explicitly how the Euler-class condition allows us to eliminate $\partial v/\partial y$ from the first-order optimality condition.

**Step 1: The Bellman equation and envelope theorem.** The value function satisfies

$$
v(y,z)
=
\max_{a \in \mathcal{A}(y,z)}
\left[
r(y,z,a)
+
\gamma \int v\big(f(y,a,z',z),z'\big)\, q(dz'\mid z)
\right].
$$

By the envelope theorem, we can differentiate through the maximum with respect to $y$ by treating the optimal action $\pi(y,z)$ as fixed:

$$
\frac{\partial v}{\partial y}(y,z)
=
\frac{\partial r}{\partial y}\big(y,z,\pi(y,z)\big)
+
\gamma \int \frac{\partial v}{\partial y}\big(y',z'\big)\,
\frac{\partial f}{\partial y}\big(y,\pi(y,z),z',z\big)\, q(dz'\mid z),
$$

where $y' = f(y,\pi(y,z),z',z)$. This is a recursive equation for $\partial v/\partial y$ in terms of future values of $\partial v/\partial y$.

**Step 2: The Euler-class condition.** Now we use (EC): $\frac{\partial f}{\partial y} = \frac{\partial f}{\partial a}\, h(y,a,z)$. Substituting into the envelope formula above gives
...
+
\gamma \int \frac{\partial v}{\partial y}\big(y',z'\big)\,
\frac{\partial f}{\partial a}\big(y,\pi(y,z),z',z\big)\, h\big(y,\pi(y,z),z\big)\, q(dz'\mid z).
$$

**Step 3: The first-order condition for the action.** At the optimum, the action satisfies

$$
\frac{\partial r}{\partial a}\big(y,z,\pi(y,z)\big)
+
\gamma \int \frac{\partial v}{\partial y}\big(y',z'\big)\,
\frac{\partial f}{\partial a}\big(y,\pi(y,z),z',z\big)\, q(dz'\mid z)
=
0.
$$

**Step 4: Solving for $\partial v/\partial y$ in closed form.** The key observation is that Steps 2 and 3 together give us two equations involving $\partial v/\partial y$. From Step 3, we can write:

$$
\int \frac{\partial v}{\partial y}\big(y',z'\big)\,
\frac{\partial f}{\partial a}\big(y,\pi(y,z),z',z\big)\, q(dz'\mid z)
=
-\frac{1}{\gamma}\frac{\partial r}{\partial a}\big(y,z,\pi(y,z)\big).
$$

Substitute this into the envelope formula (Step 2):
...
\frac{\partial r}{\partial y}\big(y,z,\pi(y,z)\big)
+
\gamma\, h\big(y,\pi(y,z),z\big) \int \frac{\partial v}{\partial y}\big(y',z'\big)\,
\frac{\partial f}{\partial a}\big(y,\pi(y,z),z',z\big)\, q(dz'\mid z).
$$

Now use the expression from Step 3:
...
\frac{\partial r}{\partial y}\big(y,z,\pi(y,z)\big)
-
\frac{\partial r}{\partial a}\big(y,z,\pi(y,z)\big)\, h\big(y,\pi(y,z),z\big).
$$

This is the **envelope formula for Euler-class problems**: $\partial v/\partial y$ expressed purely in terms of $r$, $\pi$, and $h$, with no dependence on $v$ itself.

**Step 5: The closed-form Euler equation.** Now substitute the envelope formula (Step 4) back into the first-order condition (Step 3):

$$
\begin{align}
0 &= \frac{\partial r}{\partial a}\big(y,z,\pi(y,z)\big)
+
\gamma \int \frac{\partial v}{\partial y}\big(y',z'\big)\,
\frac{\partial f}{\partial a}\big(y,\pi(y,z),z',z\big)\, q(dz'\mid z) \\
&= \frac{\partial r}{\partial a}\big(y,z,\pi(y,z)\big)
...
By the Euler-class condition (EC), we have $\frac{\partial f}{\partial y}(y',a',z'',z') = \frac{\partial f}{\partial a}(y',a',z'',z') h(y',a',z')$ for the next period as well. However, the key simplification comes from noting that in the first term of the integral, we can use (EC) in reverse. The integral involving $\frac{\partial r}{\partial y}$ can be rewritten. After some algebra (see {cite}`Rust1996` for full details), this reduces to:

$$
\frac{\partial r}{\partial a}\big(y,z,\pi(y,z)\big)
+
\gamma\, h\big(y,\pi(y,z),z\big) \int \frac{\partial r}{\partial y}\big(y',z',\pi(y',z')\big)\, q(dz'\mid z) = 0,
$$

where $y' = f(y,\pi(y,z),z',z)$. This is the **Euler equation for Euler-class problems**. It depends only on $\pi$, $r$, $h$, and $q$. The value function has been completely eliminated.

#### Defining the Euler residual operator

We define the **Euler residual operator** $\EulerResidual: \Pi \to \mathcal{F}$ by

$$
\EulerResidual(\pi)(y,z)
:=
\frac{\partial r}{\partial a}\big(y,z,\pi(y,z)\big)
+
\gamma\, h\big(y,\pi(y,z),z\big) \int \frac{\partial r}{\partial y}\big(y',z',\pi(y',z')\big)\, q(dz'\mid z),
$$

where $\Pi$ is a space of candidate policies and $\mathcal{F}$ is a space of functions on $(y,z)$. The optimal policy satisfies

$$
\EulerResidual(\pi^*)(y,z) = 0
\quad \text{for all } (y,z).
$$

This is a **functional equation in the policy**. Just as we approximated $v$ by parameterizing it as $v_\theta$ and minimizing the Bellman residual, we now approximate $\pi$ by parameterizing it as $\pi_\theta$ and minimizing the Euler residual.

#### Collocation: enforcing the Euler equation at test points

The simplest weighted residual scheme is **collocation**: enforce $\EulerResidual(\pi_\theta)(y_k,z_k) = 0$ at a finite set of points $(y_1,z_1),\dots,(y_N,z_N)$. This gives the system

$$
H(\theta)
:=
\begin{bmatrix}
\EulerResidual(\pi_\theta)(y_1,z_1) \\
\vdots \\
\EulerResidual(\pi_\theta)(y_N,z_N)
\end{bmatrix}
= 0.
$$

If $\theta \in \mathbb{R}^N$ and the parameterization is well-chosen, this is a square system of $N$ nonlinear equations in $N$ unknowns. We solve it using standard nonlinear equation solvers: Newton's method, quasi-Newton methods, or trust-region algorithms.

#### Galerkin and least-squares variants

Alternatively, we can choose test functions $\{w_i\}_{i=1}^M$ and enforce orthogonality:

$$
\int w_i(y,z)\,\EulerResidual(\pi_\theta)(y,z)\,\mu(dy,dz) = 0,
\quad i = 1,\dots,M,
$$

for a weighting measure $\mu$. This again yields a system $H(\theta) = 0$ where each component is a weighted integral of the residual.

In a **least-squares** formulation, we might minimize

$$
\min_\theta \quad \int \big[\EulerResidual(\pi_\theta)(y,z)\big]^2\,\mu(dy,dz),
$$

approximating the integral by sampling or quadrature.

#### Key differences from Bellman-based methods

1. **No contraction property:** The Euler operator $\EulerResidual$ is not a contraction mapping in any natural norm. Convergence of iterative solvers depends on the problem structure (convexity of the objective, regularity of $\pi_\theta$) rather than on a universal contraction bound.

2. **Nonlinear system vs. fixed point:** Instead of a fixed-point iteration $v \leftarrow \Bellman(v)$, we solve a root-finding problem $H(\theta) = 0$. This typically requires Newton-type methods with Jacobian computations.

3. **Policy parameters only:** The computational object is $\theta \in \mathbb{R}^p$ parameterizing the policy, not a discretized value function over a state grid. For low-dimensional $\theta$, this can be much more compact.

4. **No value function:** We never construct or store $v$. If the value function is needed *a posteriori* (e.g., for evaluation or comparison), it can be computed by simulating the policy $\pi_\theta$ or solving the Bellman equation with $\pi_\theta$ fixed. But the approximation of $\pi$ itself proceeds without $v$.

### Contrasting the RL and economics traditions

There is a cultural divide between the two fields. In **reinforcement learning**, the standard paradigm is value-based: approximate $v$ (or $q$), then extract the policy. Policy gradient methods do optimize policies directly, but they rely on gradient estimates from simulation, not on enforcing optimality conditions in function space. The idea of solving the Euler equation as a functional equation is rarely mentioned in RL textbooks.

In **computational economics and econometrics**, Euler-equation methods are routine. Since the 1990s, researchers have used projection and collocation to solve high-dimensional consumption-savings, asset pricing, and macroeconomic models by parameterizing the policy (or the "decision rule") directly. The method is valued for its compactness and the fact that it respects first-order optimality conditions exactly at the collocation points.

For students coming from an RL background, Euler methods may feel unfamiliar. But if you are working on a problem with Euler-class structure (linear dynamics in the controlled state, differentiable rewards, continuous actions), this approach can be more efficient and interpretable than discretizing the state space and iterating on the Bellman equation.

### Summary

- **Euler-class problems** allow the value function to be eliminated from the first-order optimality condition via an envelope theorem, yielding a closed functional equation $\EulerResidual(\pi)=0$ in the policy alone.
- **Weighted residual methods** (collocation, Galerkin, least-squares) apply to this Euler equation just as they do to the Bellman equation. The difference is that we parameterize $\pi$ rather than $v$ and solve a nonlinear system $H(\theta)=0$ for the policy parameters.
- **Many practical problems** fit the Euler-class framework: inventory/production, energy storage, reservoir control, and building thermal models all have affine dynamics in the controlled state that satisfy the structural condition (EC).
- This approach is standard in **computational economics** but rare in **RL**. It offers a complementary perspective: instead of iterating on value functions, we directly solve for policies that satisfy first-order optimality conditions.
- **Trade-off:** Euler methods eliminate the need for a value-function representation and can be more compact when the policy is low-dimensional. However, they require differentiability and lack the contraction guarantees of Bellman operators, so convergence depends more on problem-specific structure.

...
Consider the entropy-regularized MDP objective where we seek a policy that maximizes both cumulative reward and policy entropy. For continuous action spaces $\mathcal{A} \subset \mathbb{R}^m$, the smooth Bellman operator $\mathrm{L}_\alpha$ takes the form:

$$
(\mathrm{L}_\alpha v)(s) = \max_{\pi(\cdot|s)}\left\{\int_A \pi(a|s)[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]] \, da + \alpha \mathcal{H}(\pi(\cdot|s))\right\}
$$

where $\mathcal{H}(\pi(\cdot|s)) = -\int_A \pi(a|s)\log \pi(a|s) \, da$ represents the entropy of the policy at state $s$, $\alpha > 0$ is the entropy regularization weight, and the maximization is over probability densities on $\mathcal{A}$.

To find the optimal policy, we solve a constrained optimization problem. For a fixed state $s$, we seek to maximize the objective function

$$
J(\pi) = \int_A \pi(a|s)\left[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]\right] da + \alpha \int_A \pi(a|s)\log \pi(a|s) \, da
$$

subject to the normalization constraint $\int_A \pi(a|s) \, da = 1$. 

We form the Lagrangian by introducing the multiplier $\lambda$ for the normalization constraint:

$$
\mathcal{L}(\pi, \lambda) = \int_A \pi(a|s)\left[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]\right] da - \alpha \int_A \pi(a|s)\log \pi(a|s) \, da - \lambda\left(\int_A \pi(a|s) \, da - 1\right)
$$

Taking the functional derivative with respect to $\pi(a|s)$ and setting it to zero yields the first-order optimality condition:

$$
\frac{\delta \mathcal{L}}{\delta \pi(a|s)} = r(s,a) + \gamma \mathbb{E}_{s'}[v(s')] - \alpha(1 + \log \pi(a|s)) - \lambda = 0
$$

Solving for $\pi^*(a|s)$:

$$
\pi^*(a|s) = \exp\left(\frac{1}{\alpha}\left(r(s,a) + \gamma \mathbb{E}_{s'}[v(s')] - \lambda\right)\right)
$$

Using the normalization constraint $\int_A \pi^*(a|s) \, da = 1$ to eliminate $\lambda$, we obtain the Boltzmann policy:

$$
\pi^*(a|s) = \frac{\exp\left(\frac{1}{\alpha}(r(s,a) + \gamma \mathbb{E}_{s'}[v(s')])\right)}{\int_A \exp\left(\frac{1}{\alpha}(r(s,a') + \gamma \mathbb{E}_{s'}[v(s')])\right) da'}
$$

Substituting this optimal policy back into the entropy-regularized objective yields the smooth Bellman equation. Setting $\beta = 1/\alpha$ (the inverse temperature), we obtain:

$$
\begin{align*}
v(s) &= \int_A \pi^*(a|s)[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]] \, da + \alpha \mathcal{H}(\pi^*(\cdot|s)) \\
&= \frac{1}{\beta} \log \int_A \exp\left(\beta(r(s,a) + \gamma \mathbb{E}_{s'}[v(s')])\right) da
\end{align*}
$$

This establishes the equivalence between entropy regularization and smooth Bellman equations discussed in the [smoothing chapter](smoothing.md). That chapter shows how the smooth Bellman operator $\mathrm{L}_\alpha$ is a contraction with unique fixed point, and how the optimal policy takes the softmax form $\pi^*(a|s) \propto \exp(\beta \cdot q^*(s,a))$ where $\beta = 1/\alpha$.

## Forming Bootstrap Targets with Entropy Regularization

Following the principles of fitted value iteration, we approximate the smooth Bellman operator at sampled transitions using function approximation. To compute the optimal value $v^*(s')$ at a next state, we would need to evaluate:

$$
v^*(s') = \frac{1}{\beta} \log \int_A \exp\left(\frac{1}{\beta}q^*(s',a')\right)da'
$$

In practice, we use our Q-function approximation $q_\theta \approx q^*$ to form regression targets:

$$
y_i = r(s_i,a_i) + \gamma \mathbb{E}_{s'}\left[\frac{1}{\beta} \log \int_A \exp\left(\frac{1}{\beta}q_\theta(s',a')\right)da'\right]
$$

and fit the Q-network by minimizing:

$$
\min_\theta \sum_i (q_\theta(s_i,a_i) - y_i)^2
$$

The expectation over next states can be handled through Monte Carlo estimation: given a transition $(s_i,a_i,s'_i)$, we approximate:

$$
\mathbb{E}_{s'}\left[\frac{1}{\beta} \log \int_A \exp\left(\frac{1}{\beta}q_\theta(s',a')\right)da'\right] \approx \frac{1}{\beta} \log \int_A \exp\left(\frac{1}{\beta}q_\theta(s'_i,a')\right)da'
$$

However, we still face the challenge of computing the integral over actions. When actions are continuous, evaluating this log-integral-exp requires integrating over the entire action space—an intractable operation.

This is where the entropy-regularized perspective becomes essential. Recall that for continuous action spaces, the value function under entropy regularization can be written as:

$$
v(s) = \int_A \pi(a|s)[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]] \, da + \alpha \mathcal{H}(\pi(\cdot|s))
$$

where $\mathcal{H}(\pi(\cdot|s)) = -\int_A \pi(a|s)\log \pi(a|s) \, da$ is the entropy of the policy at state $s$. Expanding the entropy term explicitly:

$$
v(s) = \int_A \pi(a|s)[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]] \, da - \alpha \int_A \pi(a|s) \log \pi(a|s) \, da
$$

We can combine these integrals:

$$
v(s) = \int_A \pi(a|s)\left[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')] - \alpha \log \pi(a|s)\right] da
$$

Now recall that the Q-function is defined as $q(s,a) = r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]$, so we can substitute:

$$
v(s) = \int_A \pi(a|s)\left[q(s,a) - \alpha \log \pi(a|s)\right] da = \mathbb{E}_{a \sim \pi(\cdot|s)}\left[q(s,a) - \alpha \log \pi(a|s)\right]
$$

The $-\alpha \log \pi(a|s)$ term is simply the entropy bonus written inside the expectation. This identity holds for **any** policy $\pi$, not just the optimal one. It's a direct consequence of the definition of entropy and the relationship between V and Q functions.

For the optimal policy under entropy regularization, we additionally have the log-integral-exp representation:

$$
v^*(s) = \frac{1}{\beta} \log \int_A \exp(\beta \cdot q^*(s,a)) \, da
$$

where $\beta = 1/\alpha$. These two forms—the log-integral-exp and the entropy-adjusted expectation—compute the same optimal value function, but the expectation form is tractable via sampling while the log-integral-exp requires integrating over all actions.

The key insight is that while the log-integral-exp form requires integrating over all actions (intractable for continuous action spaces), the entropy-adjusted expectation form is **tractable via Monte Carlo sampling**. Using our parameterized approximations $q_\theta$ and $\pi_\phi$, we can approximate:

$$
v^*(s) = \mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}\left[q_\theta(s,a) - \alpha \log \pi_{\phi}(a|s)\right] \approx \frac{1}{N}\sum_{i=1}^N \left[q_\theta(s,a_i) - \alpha \log \pi_\phi(a_i|s)\right]
$$

where $a_i \sim \pi_\phi(\cdot|s)$ are samples from our current policy. 

Now, why do we need a separate V-network? Couldn't we just compute this expectation every time we need $v^*(s')$ for a bootstrap target? The answer is **computational efficiency**. When forming targets for a mini-batch of transitions, we need to evaluate $v^*(s'_i)$ for every next state $s'_i$ in the batch. Computing the expectation directly would require:

1. Sample $N$ actions from $\pi_\phi(\cdot|s'_i)$ 
2. Evaluate $q_\theta(s'_i, a_j)$ for each sample $j=1,\ldots,N$
3. Evaluate $\log \pi_\phi(a_j|s'_i)$ for each sample
4. Average: $\frac{1}{N}\sum_{j=1}^N [q_\theta(s'_i,a_j) - \alpha \log \pi_\phi(a_j|s'_i)]$

For a batch of $B$ transitions, this requires $B \times N$ forward passes through both the Q-network and policy network. This becomes prohibitively expensive during training when we process millions of transitions.

Instead, we **amortize** this computation by training a separate **V-network** $v_\psi(s)$ to directly predict the value of the expectation. The V-network learns the mapping:

$$
v_\psi(s) \approx \mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}\left[q_\theta(s,a) - \alpha \log \pi_{\phi}(a|s)\right]
$$

by fitting to samples of the right-hand side. Once trained, computing $v_\psi(s'_i)$ requires just a single forward pass through one network, reducing the cost from $B \times N$ evaluations to $B$ evaluations. The bootstrap target becomes:

$$
y_i = r(s_i,a_i) + \gamma v_\psi(s'_i)
$$

The V-network $v_\psi$ is trained by periodically sampling actions from $\pi_\phi$ and fitting to the entropy-adjusted Q-values: $v_\psi(s) \leftarrow \frac{1}{N}\sum_{j=1}^N [q_\theta(s,a_j) - \alpha \log \pi_\phi(a_j|s)]$ where $a_j \sim \pi_\phi(\cdot|s)$. This amortization strategy—investing computation during training to enable fast inference—is the same principle we saw with the policy network in NFQCA and DDPG, now applied to value function evaluation.
...
$$
\begin{align*}
D_{KL}(\pi_{\phi}(\cdot|s)\|\pi^{\text{Boltzmann}}_\theta) &= \mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}[\log \pi_{\phi}(a|s) - \log \pi^{\text{Boltzmann}}_\theta(a|s)] \\
...
$$
\begin{align*}
&\mathbb{E}_{s \sim \mu(s)}\mathbb{E}_{\epsilon \sim \mathcal{N}(0,1)}[\log \pi_\phi(f_\phi(s,\epsilon)|s) - \frac{1}{\beta}q_\theta(s,f_\phi(s,\epsilon))]
\end{align*}
$$
...
$$
\log \pi_\phi(f_\phi(s,\epsilon)|s) = -\frac{1}{2}\log(2\pi\sigma_\phi(s)^2) - \frac{(f_\phi(s,\epsilon)-\mu_\phi(s))^2}{2\sigma_\phi(s)^2}
$$
...
|-----------|-------------|--------|------------|----------|-------------|
| NFQCA | Deterministic | Fixed dataset | $r + \gamma q(s', \pi(s'))$ | Q + $\pi$ | Batch RL, amortize argmax |
| DDPG | Deterministic | Replay | $r + \gamma q(s', \pi(s'))$ | Q + $\pi$ + targets | Online NFQCA + OU noise |
...
1. **Action selection amortization**: Replace runtime $\arg\max$ with a policy network $\pi(s;\boldsymbol{w})$ trained to approximate the maximizer.

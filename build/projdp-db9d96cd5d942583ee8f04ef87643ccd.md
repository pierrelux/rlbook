...
### The Fitting Operator and Connection to Weighted Residuals

We can formalize the fitting step by introducing a **fitting operator** $\mathtt{fit}$ that takes a dataset of state-value pairs and produces a function from some hypothesis class $\mathcal{F}$:

$$
\hat{v}_{k+1} = \mathtt{fit}\big(\{(s_i, (\Bellman \hat{v}_k)(s_i))\}_{i=1}^n; \mathcal{F}\big).
$$

This is where the weighted residual framework connects to the practical algorithm. When we fit a function to data $\{(s_i, y_i)\}$, we minimize some notion of error, and different fitting procedures correspond to different test function choices from the abstract framework:

- **Collocation** (exact interpolation): Require $\hat{v}_{k+1}(s_i) = y_i$, equivalent to testing the residual against delta functions $\langle R, \delta(\cdot - s_i) \rangle = 0$

- **Least squares** (minimize $\sum_i \xi(s_i) [y_i - \hat{v}(s_i)]^2$): First-order conditions give $\sum_i \xi(s_i) R(s_i) \varphi_j(s_i) = 0$, which approximates the Galerkin conditions $\langle R, \varphi_j \rangle_\xi = 0$

- **Neural networks, trees, kernels**: Run gradient descent, recursive partitioning, or kernel regression. These implicitly impose conditions on the residual but typically lack closed-form characterizations

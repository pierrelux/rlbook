...
The logsumexp operator provides one way to soften the hard maximum, but other approaches exist that explicitly account for uncertainty in value estimates. {cite}`deramo2016estimating` proposed replacing the hard max with a weighted average where weights reflect the probability that each action is optimal under uncertainty.

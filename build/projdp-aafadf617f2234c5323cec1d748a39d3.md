...
Define the **Euler residual operator** $\mathcal{R}_E : \Pi \to \mathcal{F}(\mathcal{S})$ by the left-hand side of equation (Euler). The optimal policy satisfies $\mathcal{R}_E(\pi^*) = 0$. Unlike the Bellman operator, which is a $\gamma$-contraction, the Euler operator is generally not a contraction. Iterative methods may not converge without additional structure, and error bounds must be derived problem-by-problem. Despite these limitations, Euler-equation methods are widely used in economics where they often admit analytic solutions.

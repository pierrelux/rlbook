...
### Model Validation Is Part of Control Design

An optimizer can certify feasibility only with respect to its internal model.
The clock sweep used above was designed primarily to measure service rate and
power, and its one-state thermal fit has an $R^2$ of only 0.0020. We therefore
ran a separate identification experiment on the same NVIDIA L4, Qwen model,
and vLLM server to ask a narrower question: is measured board power alone a
sufficient input for predicting junction temperature during both prefill and
decode?

The two candidates shared the one-state model
...
The power-only candidate set $P_{\mathrm{eff}}=P$. The phase-gain candidate
set $P_{\mathrm{eff}}=P(1+\beta I_{\mathrm{prefill}})$, where
$I_{\mathrm{prefill}}$ is one during prefill and zero during decode. Thus the
comparison changes one modeling assumption while leaving the state dimension,
input measurement, and validation data fixed.

Four independent cold-start pulses formed the training set. Decode and prefill
were each exercised at 46 W and 61 W, with their order counterbalanced. A 60
second decode pulse and a 60-second prefill pulse at 55 W were held out and
evaluated once after both candidate models were fixed. Their measured board
energies differed by only 0.2 percent. Every pulse began from a verified
post-relock temperature within a one-degree-Celsius band, and the acquisition kept
the 77 degree C safe-down and 79 degree C abort thresholds unchanged.
...
Both models were fitted only to four training pulses. The curves show their
free-run predictions on the untouched, power-matched 55 W validation pair. A
result had to pass all three predeclared error criteria to support a mixed-load
thermal constraint.
:::
...
The added phase gain is informative. Its fitted value assigns prefill an
effective thermal input 1.147 times that of decode, and it reduces validation
RMSE substantially. It is not sufficient. All ten optimization starts recover
the same local solution, and the training Jacobian has full numerical rank with
a condition number of 384. Yet the residuals miss a rapid initial temperature
rise and then reverse sign. Within this experiment, the rejection is therefore
better explained by missing dynamics than by an ambiguous numerical fit.

A natural next model would add a latent package or heat-sink temperature and
propagate it through the recorded cooldown, memory-relock, and workload
segments. The workload phase may still affect how board power reaches the
junction. Collecting more repetitions of the same six pulses could estimate
repeat variability, but it would not supply that missing state. A richer model
would need its own prespecified held-out test before it could be used for
thermal safety.
...
:::{exercise} A constraint is only as credible as its model

...
The core observation is that the bias has structure. For a given value function $v$ and Monte Carlo sample size $N$, define the bias at state $s$ as

$$
\delta(s) = \mathbb{E}\big[(\widehat{\Bellman}_N v)(s)\big] - (\Bellman v)(s).
$$

If we could compute $\delta(s)$, we could correct our empirical updates by subtracting it. The problem is that we cannot compute either the expectation $\mathbb{E}[(\widehat{\Bellman}_N v)(s)]$ or the exact Bellman application $(\Bellman v)(s)$ directly. However, $\delta(s)$ depends on observable quantities:
...
Rather than computing $\delta(s)$ analytically, Keane and Wolpin proposed learning it empirically. The strategy is to perform high-accuracy simulation at a subset of states, measure the bias directly, then fit a regression model that predicts the bias from simple features. This learned correction can then be applied throughout the state space.

**Simulation and interpolation.** The method follows the "simulate on a subset, interpolate everywhere" template common in econometric dynamic programming. At a carefully chosen set of states, we simulate the Bellman operator with both high and low accuracy to measure the bias directly. We then build a regression surrogate that predicts this bias from features of the state and action-value distribution. During routine value iteration, we use the fast low-accuracy Monte Carlo estimate and subtract the predicted bias correction.

**The correction target.** At a state $s$ with current value function $v$, let

$$
\mu(s,a) = \int v(s') \, p(ds' \mid s,a), \qquad \hat{\mu}_N(s,a) = \frac{1}{N}\sum_{i=1}^N v(s'_i)
$$

where $s'_i \sim p(\cdot \mid s,a)$. The noisy backup uses $\max_a \hat{\mu}_N(s,a)$. The noise-free target for the maximization step is $\max_a \mu(s,a)$. The gap

$$
\delta(s) = \mathbb{E}\big[\max_a \hat{\mu}_N(s,a)\big] - \max_a \mu(s,a) \geq 0
$$

is what we aim to learn. Once we have an estimator $\hat{\delta}(s)$, we can form a bias-corrected backup:

$$
r(s,a^\star) + \gamma\Big(\max_a \hat{\mu}_N(s,a) - \hat{\delta}(s)\Big).
$$

**Features that predict the bias.** The bias depends on the dispersion and cardinality of the action set. If one action dominates, $\delta(s)$ is small. If several actions have similar values, $\delta(s)$ grows because noise is more likely to change the maximizer. Useful features include:
...
**Training the correction model.** The procedure has two phases. First, we perform a high-cost simulation pass at a subset of states to measure the bias. Second, we fit a regression model to predict the bias from features.
...
2. For each $s \in \mathcal{S}^\star$:
   ...
   - Independently, compute a noisy max $\widehat{M}_N(s) = \max_a \hat{\mu}_N(s,a)$ using the same $N$ that will be used in routine backups.
...
4. Freeze the fitted model $g_{\hat{\eta}}$. In routine backups, replace the raw noisy max with the bias-corrected expression that subtracts $g_{\hat{\eta}}(\phi(s))$.

The algorithm can be summarized as:

```{prf:algorithm} Keane-Wolpin Bias-Corrected Backup

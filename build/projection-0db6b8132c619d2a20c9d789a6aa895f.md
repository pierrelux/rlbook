...
Having chosen our basis and defined the residual, we replace the functional equation with $n$ scalar conditions on $\theta$. Different conditions produce different methods:
...
Each criterion yields $n$ equations in the $n$ unknowns $\theta_1, \ldots, \theta_n$. We now describe each method, starting with the most computationally attractive.
...
The abstraction $\mathtt{fit}$ encapsulates all the complexity of function approximation, whether that involves solving a linear system, running gradient descent, or training an ensemble. Any regression model with a `fit(X, y)` interface works: `LinearRegression`, `RandomForestRegressor`, `GradientBoostingRegressor`, `MLPRegressor`, or custom neural networks. For a linear space $\mathcal{F}$, weighted squared-error fitting recovers the Galerkin projection. A square collocation system instead recovers exact interpolation when its evaluation matrix is nonsingular. The broader view is that FVI alternates between generating Bellman targets and constructing an approximation from them.

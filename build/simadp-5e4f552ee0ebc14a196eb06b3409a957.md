...
**Offline (Fixed Batch)**: Initialize $\mathcal{B}$ with pre-collected dataset $\mathcal{T}$, set capacity $= |\mathcal{T}|$, and disable the `add` operation. All algorithms sample from the same fixed distribution throughout training. This is pure sample average approximation: we separate data collection from learning.
...
**Offline-to-Online (Warm Start)**: Initialize $\mathcal{B}$ with offline dataset $\mathcal{T}$ at capacity $B \ge |\mathcal{T}|$. Enable `add` operations so new online transitions gradually replace offline data. This smoothly transitions from pure offline learning to online learning as the buffer composition shifts.
...
**Input**: Dataset $\mathcal{D} = \{((s_i, a_i), y_i)\}$, initialization $\boldsymbol{\theta}_{\text{init}}$, truncation steps $K \in \mathbb{N} \cup \{\infty\}$, learning rate $\alpha$, loss function $\mathcal{L}$

**Output**: Parameters $\boldsymbol{\theta}$ minimizing $\mathcal{L}(\boldsymbol{\theta}; \mathcal{D}) = \frac{1}{|\mathcal{D}|}\sum_{((s,a), y) \in \mathcal{D}} (q(s,a; \boldsymbol{\theta}) - y)^2$
...
- **Dataset** $\mathcal{D}$: Can come from fixed offline buffer, circular replay buffer, or single transition
...
**Input**: MDP $(S, A, P, R, \gamma)$, Q-function $q(s,a; \boldsymbol{\theta})$, Bellman operator $\mathcal{T}$ (hard or smooth), initial parameters $\boldsymbol{\theta}_0$, buffer capacity $B$, buffer initialization $\mathcal{B}_0$, mini-batch size $b$, truncation steps $K$, warmstart flag $w \in \{\text{cold}, \text{warm}\}$, online collection flag $c \in \{\text{offline}, \text{online}\}$

**Output**: Learned Q-function parameters $\boldsymbol{\theta}$

1. Initialize $\boldsymbol{\theta}_0$
2. Initialize buffer $\mathcal{B} \leftarrow \mathcal{B}_0$ with capacity $B$
3. $n \leftarrow 0$
4. **while** training:
    ...
    7. **for** each sampled transition $(s_i, a_i, r_i, s'_i)$:
        1. $y_i \leftarrow r_i + \gamma (\mathcal{T} q(\cdot, \cdot; \boldsymbol{\theta}_{\text{target}}))(s'_i)$
    ...
    10. $\boldsymbol{\theta}_{n+1} \leftarrow \texttt{fit}(\mathcal{D}_n, \boldsymbol{\theta}_{\text{init}}, K)$
...
- **FQI/NFQI**: $\mathcal{B}_0 = \mathcal{T}$ (offline data), $B = |\mathcal{T}|$, $c = \text{offline}$, $w = \text{cold}$, $K = \infty$, $\mathcal{T} = \max$
- **DQN**: $\mathcal{B}_0 = \emptyset$, $B$ finite, $c = \text{online}$, $w = \text{warm}$, $K \in \{1, \ldots, 4\}$, $\mathcal{T} = \max$, periodic target updates
- **Q-learning**: $B = 1$, $c = \text{online}$, $w = \text{warm}$, $K = 1$, $\mathcal{T} = \max$, no target network
- **Soft Q**: $\mathcal{B}_0 = \emptyset$, $B$ finite, $c = \text{online}$, $w = \text{warm}$, $K \in \{1, \ldots, 4\}$, $\mathcal{T} = \text{logsumexp}$
...
3. **repeat**
    ...
    3. $\boldsymbol{\theta}_{n+1} \leftarrow \texttt{fit}(\mathcal{D}_n, \boldsymbol{\theta}_{\text{init}}=\boldsymbol{\theta}_0, K=\infty)$
    4. $n \leftarrow n + 1$
...
**Input**: MDP $(S, A, P, R, \gamma)$, dataset $\mathcal{T}$, neural network $q(s,a; \boldsymbol{\theta})$, learning rate $\alpha$, number of steps $K$, initial parameters $\boldsymbol{\theta}_0$
...
3. **repeat**
    ...
    2. For each $(s,a,r,s') \in \mathcal{T}$:
        1. $y_{s,a} \leftarrow r + \gamma \max_{a' \in A} q(s',a'; \boldsymbol{\theta}_n)$
        2. $\mathcal{D}_n \leftarrow \mathcal{D}_n \cup \{((s,a), y_{s,a})\}$
    3. $\boldsymbol{\theta}_{n+1} \leftarrow \texttt{fit}(\mathcal{D}_n, \boldsymbol{\theta}_{\text{init}}=\boldsymbol{\theta}_n, K=K)$
    4. $n \leftarrow n + 1$
4. **until** training complete
5. **return** $\boldsymbol{\theta}_n$
```

...
The upper integration limit selects where to evaluate this same polynomial.
Setting $\tau=\tau_i$ integrates from the interval's left endpoint to stage
$i$; setting $\tau=1$ integrates from that same left endpoint to the
interval's right endpoint. In the scalar example above, the state piece is
$p_k(\tau)=1+\tau+2\tau^2$. Its value at the midpoint is $p_k(1/2)=2$,
whereas its value at the right endpoint is $p_k(1)=4$.
...
reference interval. If a stage lies at $\tau_i=1$, then $A_{ij}=b_j$ for
...
optimizer chooses and which it evaluates from those choices:

| Quantity | Meaning on interval $k$ | Role in the NLP |
|---|---|---|
| $\mathbf x_k,\mathbf x_{k+1}$ | Mesh endpoint states | Shared decision variables |
| $\mathbf x_{k,j}$ | State at collocation node $\tau_j$ | Stage decision variable, or the mesh state when identified with an endpoint |
| $\widehat{\mathbf u}_{k,r}$ | Control at support node $\rho_r$ | Control decision variable |
| $\mathbf u_{k,j}$ | Control at collocation node $\tau_j$ | Evaluated from the support values using $B$ |
| $\mathbf f_{k,j},c_{k,j}$ | Dynamics and cost rate at a stage | Evaluated from the stage state, control, and time |

...
This equivalence between norm minimization and orthogonality holds for any inner product: weighted $L^2$ integrals for Galerkin, sums over collocation points for collocation, or sampled expectations for neural networks. The **dual pairing** formulation {cite}`LegrandJunca2025` provides the unifying framework. A dual space is the collection of all linear functionals on a function space: mappings that take a function and return a number. For example, "evaluate at point $x_i$" and "integrate against a weight" are both functionals. This generality handles test objects that are not regular functions. Collocation uses Dirac delta "functions" $\delta(x - x_i)$, which are distributions defined only through their action on test functions via the dual pairing $\langle \Residual(v), \delta(x - x_i) \rangle = (\Residual v)(x_i)$. The induced dual norm $\|\Residual(v)\|_* = \sup_{\|w\|=1} |\langle \Residual(v), w \rangle|$ measures the residual by its worst-case effect on test functions.
 ...
 Weak adversarial networks {cite}`Zang2020` parameterize both the trial function $v_{\boldsymbol{\theta}}$ and test function $w_{\boldsymbol{\phi}}$ as neural networks, optimizing

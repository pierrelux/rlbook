...
When $\mathscr{T}$ involves optimization (as in the Bellman operator $\mathrm{L}v = \max_a \{r(s,a) + \gamma \mathbb{E}[v(s')]\}$), computing this derivative appears problematic because the max operator is not differentiable. However, the **Envelope Theorem** resolves this difficulty.s tot

...
and minimizes $\mathcal{L}(\boldsymbol{\theta}) = \frac{1}{|X|}\sum_{i=1}^{|X|} \ell(q(x_i; \boldsymbol{\theta}), y_i)$ using $K$ gradient steps starting from $\boldsymbol{\theta}_{\text{init}}$, where $x_i = (s_i, a_i)$. This resembles scikit-learn's API but with additional controls that matter for reinforcement learning. Standard supervised learning uses random initialization and runs to convergence ($K = \infty$). Reinforcement learning algorithms vary these: warm starting from the previous iteration's parameters ($\boldsymbol{\theta}_{\text{init}} = \boldsymbol{\theta}^{(n)}$), taking only $K \in \{1, \ldots, 10\}$ gradient steps, or somewhere in between.

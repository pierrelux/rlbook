...
**Step 2: Recursive substitution.** The key insight is that $v^*(j)$ itself satisfies the Bellman equation, so we can express it as $v^*(j) = r(j, \pi^*(j)) + \gamma \sum_k p(k|j,\pi^*(j)) v^*(k)$. Substituting this recursively and using the envelope theorem at each step allows us to express everything in terms of the policy alone—the value function terms telescope through the recursion.

...
2. **The normalized discounted visitation distribution** $\mu_\alpha(s)$ defined in the [policy gradient theorem section](#normalized-discounted-state-visitation-distribution) below, which weights states by their discounted occupancy $(1-\gamma)\sum_{t=0}^\infty \gamma^t d_t^{\pi}(s)$.
...
$$
L^{\text{CLIP}}(\boldsymbol{w}) = \mathbb{E}_{(s,a,A) \sim \mu_{\pi_{\boldsymbol{w}_{\text{old}}}}}\left[\ell^{\text{CLIP}}(\boldsymbol{w}; s, a, A)\right]

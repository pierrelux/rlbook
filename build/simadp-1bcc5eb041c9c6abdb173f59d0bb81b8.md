...
All algorithms discussed so far use the hard max operator in the Bellman equation: $v(s) = \max_a \{r(s,a) + \gamma \mathbb{E}[v(s')]\}$. This produces deterministic optimal policies. The [chapter on smooth Bellman optimality equations](regmdp.md) introduced the smooth Bellman operator as an alternative:

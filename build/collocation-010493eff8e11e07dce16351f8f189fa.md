...
time interval. This chapter develops that construction from one affine example
...
The boundary conditions reduce the defect to $U_0+U_1=2$. Among all endpoint
controls with this average, the quadratic objective is minimized by
$U_0=U_1=1$. The resulting interpolation is the exact solution $u(t)=1$ and
$x(t)=t$.
...
Each function takes the value one at the node named by its subscript and zero
at the other two nodes. Differentiating them at all three nodes gives

...
How does fitted-value iteration relate to the projection methods we studied? Consider the iteration $v_{k+1} = \Proj \Bellman v_k$ that we analyzed earlier. The projection operator $\Proj$ takes a function (in this case, $\Bellman v_k$) and returns its best approximation in the subspace $\text{span}(\boldsymbol{\Phi})$. We can reinterpret this as a fitting operation:

$$
v_{k+1} = \Proj \Bellman v_k = \mathtt{fit}\big(\{(s, (\Bellman v_k)(s)) : s \in \mathcal{S}\}; \text{span}(\boldsymbol{\Phi})\big).
$$

The projection operator implicitly generates target values $y_s = (\Bellman v_k)(s)$ at every state $s$ and fits a function from $\text{span}(\boldsymbol{\Phi})$ to match these targets. The difference is purely conceptual: $\Proj$ is defined algebraically through orthogonality conditions, while $\mathtt{fit}$ is defined operationally through its input-output behavior. They compute the same result.

This perspective reveals that the projection methods we studied are instances of fitted-value iteration with specific choices of $\mathtt{fit}$:

- **Galerkin projection**: $\mathcal{F} = \text{span}\{\varphi_1, \ldots, \varphi_d\}$ and $\mathtt{fit}$ minimizes the weighted squared error $\sum_s \xi(s)((\Bellman v_k)(s) - \sum_j \theta_j \varphi_j(s))^2$. The solution is the orthogonal projection we studied.

- **Collocation**: $\mathtt{fit}$ enforces exact interpolation at chosen points $\{s_i\}_{i=1}^d$: $\sum_j \theta_j \varphi_j(s_i) = (\Bellman v_k)(s_i)$ for all $i$. This is equivalent to interpolation-based projection.

- **LSTD**: When we cannot enumerate all states, $\mathtt{fit}$ approximates the Galerkin projection using sampled data rather than the full state space.

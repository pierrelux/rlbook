...
This is distinct from Monte Carlo estimation in a non-recursive setting. If we repeatedly estimated a fixed expectation $\mathbb{E}[X]$ using Monte Carlo samples and took the maximum across many independent trials, the bias would not accumulate across trials because each trial is independent. In value iteration, the trials are not independent: each iteration uses the output of the previous one.

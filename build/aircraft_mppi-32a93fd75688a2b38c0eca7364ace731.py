...
def validate_mean(aircraft, state, tape, dts, observed, config):
...
def optimize(aircraft, state, observed_gust, config, clock_fraction, remaining, stochastic, rng):
    ...
    if not validate_mean(aircraft, state, nominal[0], ndt[0], observed_gust, config):
        raise NoFeasiblePlan('Geometric reference fails conditional-mean validation')
    ...
    for _ in range(config.iterations):
        ...
        feasible = validate_mean(aircraft, state, proposed_tape[0], proposed_dt[0], observed_gust, config)

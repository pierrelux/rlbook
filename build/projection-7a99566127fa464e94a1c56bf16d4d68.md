...
The inner product formulation is elegant, but it may seem abstract. Here is the key insight that grounds everything: **variational conditions are first-order optimality conditions for optimization problems**. Different choices of test functions correspond to different objective functions. This correspondence, summarized in the table below, shows that weighted residual methods are fundamentally about choosing which norm to minimize.

Consider an approximation $\hat{v}(s; \theta) = \sum_{j=1}^n \theta_j \varphi_j(s)$ parameterized by $\theta$, the residual $R(s; \theta) = \Bellman \hat{v}(s; \theta) - \hat{v}(s; \theta)$, and the weighted inner product $\langle \cdot, \cdot \rangle_w$. The **variational conditions** require:

$$
\langle R(\cdot; \theta), p_j \rangle_w = 0 \quad \text{for all test functions } p_j.
$$

These conditions are precisely the **normal equations** (first-order stationarity conditions) for an optimization problem. The choice of test functions $p_j$ determines which optimization problem.

| **Method** | **Test functions** | **Optimization problem** |
|:-----------|:-------------------|:------------------------|
| Minimum residual | $p_j = \frac{\partial R(\cdot; \theta)}{\partial \theta_j}$ | $\displaystyle\min_\theta \tfrac{1}{2}\lVert R(\cdot; \theta)\rVert_w^2$ |
| Galerkin | $p_j = \varphi_j$ | $\displaystyle\min_\theta \lVert R(\cdot; \theta)\rVert_{\mathcal{V}^*}$ (dual norm) |
| Collocation | $p_j = \delta(s - s_j)$ | $\displaystyle\min_\theta \sum_j \omega_j R(s_j; \theta)^2$ |
| Petrov–Galerkin | $p_j \neq \varphi_j$ | $\displaystyle\min_\theta \lVert R(\cdot; \theta)\rVert_{W}^2$ (custom metric $W$) |

Every row is an optimization problem—what changes is the **norm** used to measure the residual. The variational conditions $\langle R, p_j \rangle_w = 0$ are the first-order stationarity conditions for these objectives.

To see this for **minimum residual**, consider $\min_\theta \frac{1}{2}\|R(\cdot; \theta)\|_w^2$. Setting the gradient to zero:

$$
\frac{\partial}{\partial \theta_j} \tfrac{1}{2}\|R(\cdot; \theta)\|_w^2 = \left\langle R(\cdot; \theta), \frac{\partial R(\cdot; \theta)}{\partial \theta_j} \right\rangle_w = 0.
$$

The test functions are the gradients $\partial R / \partial \theta_j$. This directly minimizes the weighted $L^2$ norm of the residual.

**Galerkin** uses test functions $p_j = \varphi_j$. The conditions $\langle R, \varphi_j \rangle_w = 0$ are first-order conditions for minimizing $\|R\|$ in the **dual norm** $\|R\|_{\mathcal{V}^*} = \sup_{\|g\|_{\mathcal{V}}=1} \langle R, g \rangle_w$ where $\mathcal{V} = \text{span}\{\varphi_j\}$. This measures the residual by its worst-case projection onto the test space.

**Collocation** uses delta functions $p_j = \delta(s - s_j)$, giving conditions $R(s_j; \theta) = 0$. These are first-order conditions for minimizing a **discrete norm**: $\sum_j \omega_j R(s_j; \theta)^2$ with weights $\omega_j \to \infty$ (hard constraints give exact interpolation; finite weights give least-squares at points).

The distinction between minimum residual and Galerkin becomes important for iterative methods. **Fitted Q-Iteration (FQI)** does not minimize the residual norm $\|R\|^2 = \|\Bellman \hat{v} - \hat{v}\|^2$ directly. Instead, it uses a two-step iteration:

1. Apply the Bellman operator to get a target: $f_k = \Bellman \hat{v}_k$
2. Project the target back onto the approximation space: $\hat{v}_{k+1} = \Pi_w f_k$

The projection step solves $\min_\theta \|\hat{v}(\cdot; \theta) - f_k\|_w^2$, whose first-order conditions are $\langle \hat{v} - f_k, \varphi_j \rangle_w = 0$. Combining these steps:

$$
\hat{v}_{k+1} = \Pi_w \, \Bellman \hat{v}_k.
$$

This **project-then-iterate** scheme differs from **Bellman residual minimization**, which directly minimizes $\lVert\Bellman\hat{v} - \hat{v}\rVert^2$. Bellman residual minimization requires differentiating through the Bellman operator (the "full gradient"), while FQI's projection step uses only the gradient of $\hat{v}$ with respect to $\theta$ (the "semi-gradient"). We return to this distinction when discussing temporal difference learning.

With this optimization perspective in place, we can state the general approach. Choose $n$ test functions $\{p_1, \ldots, p_n\}$ and a weight function $w(s)$, then require the residual to have zero weighted inner product with each test function:

...
is also a program with a controllable execution rate. The scheduling rule is
held fixed here. Each 0.05-second simulator step serves one phase, prefill work
is capped at 512 tokens per prefill step, and active decode receives
alternating-step or cache-pressure priority. This is the reduced interleaving
surrogate defined in the modeling chapter, not a reproduction of a vLLM
scheduler. The control sequence specifies one normalized GPU frequency for
each second of a 60-second horizon,
...
The disturbance $w_k$ contains the requests predicted to arrive during second
$k$. Service and power in $F_k$ interpolate the committed profile whose
provenance is displayed with the result. The request-level simulator remains
outside the optimizer and validates the resulting schedule after the solve.
...
nearest profiled clock, so the request-level validation includes the actuator's
finite action set.
...
nominal workload forecast, and what it loses when that forecast is wrong. The
same plan is replayed twice. The nominal replay uses the forecast supplied to
the optimizer. In the shifted replay, the largest later ten-second work burst
arrives twenty seconds earlier. No reoptimization occurs after either replay
starts.

```{code-cell} python
:tags: [remove-input]
:label: fig-inference-open-loop
:caption: The optimized clock schedule is computed once from the nominal 60-second request forecast. The shifted replay moves the largest later work burst twenty seconds earlier while keeping the planned clocks fixed. The playhead reveals only the executed trajectory prefix; the dashed schedule is the plan available at time zero.
...
```{code-cell} python
:tags: [remove-input]
:label: tbl-inference-open-loop-metrics
:caption: Request-level metrics for the fixed offline schedule under its nominal forecast and the shifted-burst disturbance. The comparisons use the same requests, controller parameters, and profile provenance.

import pandas as pd

pd.read_csv("artifacts/inference_serving/metrics_open_loop.csv")
```

HiGHS reports an optimal solution for the stated linear program, with objective
2,201.34. Under the nominal replay, the requests later selected for shifting
have a mean time to first token of 4.24 seconds and a 95th percentile of 8.00
seconds. Moving only that burst into the plan's low-clock interval raises these
values to 11.27 and 18.35 seconds. At 30 seconds, the nominal replay has no
queued request while the shifted replay has 25; the peak queue at the minimum
clock rises from 1 to 25 requests. The whole-trace mean time to first token also
rises from 5.10 to 6.39 seconds.

The disturbance does not make every metric worse. Energy falls from 2,495 to
2,264 joules because the moved work arrives earlier and spends more of its
execution under the low-frequency portion of the unchanged plan. This result
shows that a schedule can conserve energy while being badly mistimed for a
particular burst. The evidence should be read as a vector of latency, queue,
energy, and constraint metrics, not as a single verdict.

The metrics compare energy per output token, time to first token, time per
output token, unfinished work, and power and thermal violations. The nominal
run tests the optimized trajectory under its own assumptions. The shifted run
tests sensitivity to one explicit forecast error. It does not establish
...
of feedback and receding-horizon optimization. If the artifact identifies its
profile as provisional, the comparison tests this optimization mechanism but
does not constitute a measured hardware result.

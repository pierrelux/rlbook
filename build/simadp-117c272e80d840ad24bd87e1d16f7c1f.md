...
The Keane-Wolpin approach treated maximization bias as a quantity to be learned and subtracted. An alternative strategy is to modify the estimator itself to break the coupling that causes the bias. Recall from the previous section that the bias arises when we take $\max_a \{r(a) + \gamma \hat{\mu}_N(a)\}$: the maximization both selects an action and uses the noise in that same estimate to compute the value. The bias stems from the fact that whichever action happens to have a positive noise realization is both selected and contributes its noisy value to the final estimate.

Double Q-learning {cite}`van2016deep` maintains two independent Monte Carlo estimates of the continuation values. For each action $a$, let $\hat{\mu}^{(1)}_N(a)$ and $\hat{\mu}^{(2)}_N(a)$ be two independent Monte Carlo approximations of $\mu(a) = \mathbb{E}_{s' \sim p(\cdot|s,a)}[v(s')]$, each based on $N$ independent samples from $p(\cdot|s,a)$. Both are unbiased:

$$
\mathbb{E}\big[\hat{\mu}^{(1)}_N(a)\big] = \mathbb{E}\big[\hat{\mu}^{(2)}_N(a)\big] = \mu(a).
$$

The key is that $\hat{\mu}^{(1)}_N$ and $\hat{\mu}^{(2)}_N$ are conditionally independent given the state and action, since they are computed from independent samples.

In the standard approach, we would select an action and evaluate it using the same estimate:

$$
a^\star = \arg\max_{a} \left\{r(a) + \gamma \hat{\mu}^{(1)}_N(a)\right\}, \quad \text{and then use } Y = r(a^\star) + \gamma \hat{\mu}^{(1)}_N(a^\star).
$$

The maximization selects whichever action has the highest $r(a) + \gamma \hat{\mu}^{(1)}_N(a)$, and the target value includes the same noise that made it look good. This produces

$$
\mathbb{E}[Y] = \mathbb{E}\Big[\max_a \left\{r(a) + \gamma \hat{\mu}^{(1)}_N(a)\right\}\Big] \ge \max_a \left\{r(a) + \gamma \mu(a)\right\}.
$$

Double Q-learning instead uses one estimate for selection and the other for evaluation:

$$
a^\star = \arg\max_{a} \left\{r(a) + \gamma \hat{\mu}^{(1)}_N(a)\right\}, \quad \text{but evaluate using } Y = r(a^\star) + \gamma \hat{\mu}^{(2)}_N(a^\star).
$$

The selected action $a^\star$ is a random variable that depends on the noise in $\hat{\mu}^{(1)}_N$. The crucial point is that $\hat{\mu}^{(2)}_N(a^\star)$ is conditionally independent of $a^\star$ given the underlying state and action:

$$
\hat{\mu}^{(2)}_N(a) \perp\!\!\!\perp \hat{\mu}^{(1)}_N \quad \text{for all } a.
$$

Therefore, conditional on the selected action $a^\star$,

$$
\mathbb{E}\big[\hat{\mu}^{(2)}_N(a^\star) \mid a^\star\big] = \mu(a^\star),
$$

because the noise in $\hat{\mu}^{(2)}_N(a^\star)$ is independent of which action was selected. Taking expectations over $a^\star$ gives

$$
\mathbb{E}[Y] = \mathbb{E}\big[r(a^\star) + \gamma \hat{\mu}^{(2)}_N(a^\star)\big] = \mathbb{E}\big[r(a^\star) + \gamma \mu(a^\star)\big],
$$

where the expectation is now over the randomness in $a^\star$. The evaluation noise does not contribute to bias because it was not used in the selection. The coupling is broken.

However, double Q-learning does not eliminate all bias. The selected action $a^\star$ depends on the noise in $\hat{\mu}^{(1)}_N$. If an action has a large positive noise realization in $\hat{\mu}^{(1)}_N(a)$, it is more likely to be selected. This selection bias means

$$
\mathbb{E}\big[r(a^\star) + \gamma \mu(a^\star)\big] \ne \max_a \left\{r(a) + \gamma \mu(a)\right\}
$$

in general, though the inequality is typically not as large as when both selection and evaluation use the same noise source. In the standard estimator, both selection noise and evaluation noise contribute to overestimation. In double Q-learning, only the selection noise contributes. The update is symmetric: when updating the second Q-function, we use $\hat{\mu}^{(2)}_N$ for selection and $\hat{\mu}^{(1)}_N$ for evaluation.

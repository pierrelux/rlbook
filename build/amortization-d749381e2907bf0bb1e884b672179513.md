...
where $a_i \sim \pi_\phi(\cdot|s)$ are samples from our current policy. This motivates maintaining **two separate function approximators**:
1. A **Q-network** $q_\theta(s,a)$ that approximates the optimal Q-function $q^*(s,a)$
2. A **V-network** $v_\psi(s)$ that approximates the optimal value function $v^*(s)$ by learning to predict the entropy-adjusted expectation

The V-network "amortizes" the intractable log-integral-exp computation: instead of computing the integral each time, we train a network to predict its value by learning from samples. With both approximators, our bootstrap target becomes tractable. For a transition $(s_i,a_i,s'_i)$, we simply use the V-network to estimate the continuation value:

$$
y_i = r(s_i,a_i) + \gamma v_\psi(s'_i)
$$

The V-network $v_\psi$ is trained by sampling actions from the policy $\pi_\phi$ at each state and fitting to the entropy-adjusted Q-values from $q_\theta$. This three-way interaction—Q-network, V-network, and policy—exists only because the entropy-regularized formulation provides the mathematical relationship that lets us replace the intractable integral with a tractable expectation.

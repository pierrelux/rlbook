...
For a **general** policy $\pi$, the value is an expectation over actions:

$$
v^\pi(s) = \mathbb{E}_{a \sim \pi(\cdot|s)}[q^\pi(s,a) - \alpha \log \pi(a|s)]
$$ (eq:v-general)

This means that for a single observed action $a$, the quantity $q^\pi(s,a) - \alpha\log\pi(a|s)$ only approximates $v^\pi(s)$ with sampling error: $q^\pi(s,a) - \alpha\log\pi(a|s) = v^\pi(s) + \varepsilon(s,a)$ where $\mathbb{E}_{a \sim \pi}[\varepsilon] = 0$ but $\varepsilon \ne 0$ for individual actions.

But for the **optimal** policy under entropy regularization, something special happens. The optimal policy is the Boltzmann distribution:
...
$$
\log \pi^*(a|s) = \frac{q^*(s,a) - v^*(s)}{\alpha}
$$

Rearranging gives the **exact pointwise relationship**:

$$
v^*(s) = q^*(s,a) - \alpha \log \pi^*(a|s) \quad \text{for all } a
$$ (eq:v-q-exact)

This holds exactly for **every** action $a$, not just in expectation. There is no sampling error $\varepsilon(s,a)$. The Boltzmann structure ensures that the advantage $q^*(s,a) - v^*(s)$ is precisely encoded in the log-probability: low-value actions have low $q^*(s,a)$ but also high $-\alpha\log\pi^*(a|s)$ (since they have low probability), and these terms balance exactly to give $v^*(s)$.

Now take a trajectory segment $(s_0, a_0, s_1, a_1, \ldots, s_d)$ where each transition follows the deterministic dynamics $s_{t+1} = f(s_t, a_t)$. Start with $q^*(s_0, a_0) = r_0 + \gamma v^*(s_1)$ and use equation {eq}`eq:v-q-exact` to substitute $v^*(s_1) = q^*(s_1,a_1) - \alpha\log\pi^*(a_1|s_1)$ exactly (no sampling noise):
...
Apply equation {eq}`eq:v-q-exact` once more to get $v^*(s_0) = q^*(s_0,a_0) - \alpha\log\pi^*(a_0|s_0)$:
...
```{prf:remark} Why the Exact Relationship Matters
:class: dropdown

The exactness of equation {eq}`eq:v-q-exact` is crucial for PCL's off-policy property. For a general policy $\pi$ (not the optimal policy), the relationship between value and Q-function involves an expectation:

$$
v^\pi(s) = \mathbb{E}_{a \sim \pi}[q^\pi(s,a) - \alpha \log \pi(a|s)]
$$

Individual actions give noisy estimates: $q^\pi(s,a) - \alpha\log\pi(a|s) = v^\pi(s) + \varepsilon(s,a)$ where $\mathbb{E}_a[\varepsilon] = 0$. Multi-step telescoping would accumulate these errors, requiring importance sampling to correct for the behavior policy.

But for the **optimal** policy under entropy regularization, the Boltzmann structure gives $v^*(s) = q^*(s,a) - \alpha\log\pi^*(a|s)$ exactly for all $a$. The advantage is encoded precisely in the log-probability. The telescoping is exact, producing a constraint that holds for any action sequence $(a_0, \ldots, a_{d-1})$ regardless of origin. No importance sampling is needed because we evaluate $\pi^*$ at observed actions, we don't average over it.

This exactness is special to the soft-max Bellman operator. Hard-max methods have no such property: $v^*(s) = \max_a q^*(s,a)$ holds only at the best action, not at all actions.
```
...
The exact relationship {eq}`eq:v-q-exact` still holds, so we can write the path consistency constraint. But now consider what PCL minimizes: the **squared** residual $\mathbb{E}[R^2]$ where
...
This is Baird's **double sampling problem** {cite}`Baird1995`. To get an unbiased gradient of $(\mathbb{E}[R])^2$, we need:
...
Consider the hard-max Bellman operator with optimal policy $\pi^*(s) = \arg\max_a q^*(s,a)$. The value function is:
...
- If $a$ is optimal: $v^*(s) = q^*(s,a)$
- If $a$ is suboptimal: $v^*(s) > q^*(s,a)$

There is no equation relating $v^*(s)$ to arbitrary observed actions. If we observe a suboptimal action in off-policy data, we cannot write a path consistency constraint. The inequality $v^*(s) \geq q^*(s,a)$ does not provide a residual to minimize.

With entropy regularization, equation {eq}`eq:v-q-exact` gives $v^*(s) = q^*(s,a) - \alpha\log\pi^*(a|s)$ for **all** actions $a$. The log-probability term $-\alpha\log\pi^*(a|s)$ is large for low-probability (suboptimal) actions, exactly compensating for the lower Q-value. A suboptimal action has low $q^*(s,a)$ but also large $-\alpha\log\pi^*(a|s)$, and these balance to give $v^*(s)$ exactly.

This is why multi-step hard-max Q-learning lacks theoretical justification for off-policy data. When we observe a trajectory with suboptimal actions, the rewards received after those actions reflect the value of the suboptimal continuation, not the optimal value. With soft-max, the $-\alpha\log\pi^*(a_t|s_t)$ terms at each step account for the "cost" of taking each observed action relative to the optimal distribution, making the multi-step constraint exact for any action sequence.

Both requirements are structural, not arbitrary:

| **Requirement** | **Addresses** |
|:----------------|:--------------|
| Deterministic dynamics | Double sampling bias: ensures $\mathbb{E}[R^2] = (\mathbb{E}[R])^2$ |
| Entropy regularization | All-action consistency: ensures $v^*(s) = q^*(s,a) - \alpha\log\pi^*(a|s)$ for all $a$ |

Without deterministic dynamics, residual minimization is biased. Without entropy regularization, the constraint holds only for optimal actions, not for arbitrary off-policy data.
...
**Multi-step with hard-max**: No analog exists. The hard-max Bellman operator $\max_a q(s,a)$ does not have an exact pointwise relationship like equation {eq}`eq:v-q-exact`. Multi-step telescoping would accumulate errors from the max operator, making the constraint valid only in expectation under the optimal policy. The soft-max structure is what enables exact off-policy path consistency.

...
This perspective also explains PCL's effective off-policy learning: path consistency is a necessary condition for optimality that holds independently of how the path was generated. Whether from the current policy, an old policy, or an expert, any path provides a valid training signal. The consistency error measures how far the current $(\pi_{\boldsymbol{\theta}}, v_{\boldsymbol{\phi}})$ pair is from satisfying the optimality conditions along that specific trajectory, regardless of its provenance.

...
already been formulated. Someone had drawn the system boundary, chosen the
state and observations, defined the available actions, and encoded the
objective and termination conditions. These choices determine what a learned
policy can observe, cause, and optimize.
...
The TCV controller makes the division concrete. Its simulator combined
conductor-circuit dynamics, a free-boundary plasma model, and lumped
plasma-current dynamics. Data-identified sensor and power-supply models
...
The distinction is not between model-free and model-based reinforcement
learning. If only transitions and rewards are available, sample-based methods

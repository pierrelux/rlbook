...
This is unbiased but high-variance: $\mathbb{E}[\widehat{G}_1(s,a)] = r(s,a) + \gamma \int v(s')p(ds'|s,a)$, but $\mathrm{Var}(\widehat{G}_1) = \mathrm{Var}(v(S'))$. We cannot reduce this variance by sampling multiple next states from the same $(s,a)$—we only get one.

How then do we reduce variance? Through **aggregation across different samples**. If the dataset contains multiple transitions from similar state-action pairs (even if not identical), the regression problem $\min_\theta \sum_i (q(s_i, a_i; \theta) - y_i)^2$ with targets $y_i = \widehat{G}_1(s_i, a_i)$ effectively pools information. The function approximator $q(s,a; \theta)$ smooths and generalizes across the noisy single-sample targets.
...
In offline batch settings where we have a fixed dataset $\mathcal{D}$ collected once, every algorithm reuses these same quadruples across iterations—we recompute targets $y_i$ using updated $q$ but we never collect new samples from the same $(s,a)$ pairs. In online settings, we collect new quadruples sequentially and add them to the dataset or replay buffer.
...
Even then, most practical algorithms stick with $N=1$ and compensate through other means (larger replay buffers, better function approximation, multi-step returns). The single-sample case is not a limitation to work around—it's the fundamental operating mode of reinforcement learning.
...
1. Initialize $\boldsymbol{\theta}_0$ (e.g., random initialization)
2. $n \leftarrow 0$
3. **repeat**
    ...
    3. $\boldsymbol{\theta}_{n+1} \leftarrow \texttt{fit}(\mathcal{D}, \boldsymbol{\theta}_{\text{init}}=\boldsymbol{\theta}_0, K=\infty)$
...
We have developed three foundational components: (1) fitted-value iteration from the [previous chapter](projdp.md)—compute targets from the Bellman operator, fit a new approximation; (2) Q-functions that amortize action selection by caching state-action values; (3) Monte Carlo integration using single-sample transitions $(s, a, r, s')$ from trajectories.

These combine into a template: $\hat{q}^{(k+1)} = \text{fit}(\{((s_i, a_i), y_i)\})$, where targets $y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \theta^{(k)})$ use the single observed next state $s'_i$. Different instantiations yield the major value-based reinforcement learning algorithms. This section systematically maps the design space, showing how FQI, NFQI, DQN, Q-learning, and their variants arise as specific configurations. We then extend to smooth Bellman operators and continuous action spaces.
...
**Output**: Parameters $\boldsymbol{\theta}$ minimizing $\mathcal{L}(\boldsymbol{\theta}; \mathcal{D}^{\text{fit}}) = \frac{1}{|\mathcal{D}^{\text{fit}}|}\sum_{((s,a), y) \in \mathcal{D}^{\text{fit}}} (q(s,a; \boldsymbol{\theta}) - y)^2$
...
- **Inner loop** (Regression): Fit the function approximator to the targets, solving $\min_\theta \sum_i (q(s_i, a_i; \theta) - y_i)^2$
...
NFQI as described runs the inner loop to convergence ($K=\infty$). Similar to modified policy iteration, we can perform partial optimization—taking only $K$ gradient steps rather than running to convergence. This trades off fit quality for computational cost per outer iteration:
...
So far we've assumed a fixed transition dataset $\mathcal{D}$. In practice, we often want to collect data online while learning. This requires three modifications:
...
The DQN paper frequently receives credit for introducing the "target network" as a key stabilization technique. This framing reflects a historical accident more than a mathematical innovation. To understand why, we need to consider the state of reinforcement learning in 2013.

Before DQN, the dominant paradigm in the reinforcement learning community was strict online learning exemplified by Q-learning: process each transition $(s, a, r, s')$ with a single gradient step, then discard it. This corresponded to the extreme point $K=1$ (one gradient step per outer iteration) with no replay buffer (capacity 1). The operations research and approximate dynamic programming communities had long used batch methods like FQI (Ernst et al., 2005) and NFQI (Riedmiller, 2005), but these were not widely known in the machine learning-focused RL community.

When DQN combined neural networks with replay buffers and periodic "target network updates," it appeared to introduce novel algorithmic ideas. The DQN paper itself acknowledges NFQ as "perhaps the most similar prior work" but characterizes it as using "batch updates" with computational cost proportional to dataset size, contrasting this with their "stochastic gradient updates." This characterization is misleading. Both NFQ and DQN use mini-batch gradient methods. The actual difference is that NFQ runs its inner optimization (RProp) to convergence at each outer iteration—performing many epochs through the data—while DQN truncates to a single gradient step per environment interaction. This is the $K=\infty$ versus $K=1$ distinction in our framework, not batch versus stochastic optimization.

The target network is not a design innovation—it is the direct consequence of approximate value iteration's mathematical structure, already present in NFQI's nested loop implementation.

Recall fitted Q-iteration's nested structure. The outer loop implements value iteration: at iteration $k$, compute targets using current parameters $\boldsymbol{\theta}_k$,

$$
y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}_k),
$$

then solve the regression problem

$$
\boldsymbol{\theta}_{k+1} = \arg\min_{\boldsymbol{\theta}} \sum_i (q(s_i, a_i; \boldsymbol{\theta}) - y_i)^2.
$$

The targets $\{y_i\}$ remain fixed throughout the inner optimization. We compute them once using $\boldsymbol{\theta}_k$, then hold them constant while running gradient descent. This fixed-target property defines what it means to apply the Bellman operator and then project.

When we flatten this nested structure into a single loop (convenient for online implementation), we must preserve this property. The "target network" $\boldsymbol{\theta}_{\text{target}}$ plays exactly the role that $\boldsymbol{\theta}_k$ played in the nested version: it provides fixed targets for multiple gradient steps. The periodic synchronization $\boldsymbol{\theta}_{\text{target}} \leftarrow \boldsymbol{\theta}$ every $K$ steps marks what would have been the outer iteration boundary. This is loop unrolling, not algorithmic innovation.

Understanding this clarifies why no "stop_gradient" operation is needed in the implementation. When we minimize

$$
\mathcal{L}(\boldsymbol{\theta}) = \frac{1}{N}\sum_i \left(q(s_i, a_i; \boldsymbol{\theta}) - y_i\right)^2,
$$

the gradient is the partial derivative $\partial \mathcal{L}/\partial \boldsymbol{\theta}$, which treats $y_i$ as constants (because they are constants—computed once per outer iteration). The computational graph perspective that requires explicit "stop_gradient" annotations conflates this with a total derivative. Mathematically, we take derivatives of functions with respect to arguments, not of variables with respect to other variables. The partial derivative automatically captures that targets are fixed data, not functions of the parameters being optimized.

Without a target network (equivalently, recomputing targets after every gradient step), we would not be doing value iteration. We would be solving a different fixed-point problem akin to residual minimization methods (Baird, 1995), which require different convergence analysis and typically double sampling.

The same principle extends to exponential moving average updates $\boldsymbol{\theta}_{\text{target}} \leftarrow \tau \boldsymbol{\theta} + (1-\tau)\boldsymbol{\theta}_{\text{target}}$ used in continuous control (DDPG, TD3, SAC). EMA targets smooth the transition between outer iterations while preserving the separation between applying the Bellman operator and fitting to its output.

The algorithm space is defined by four independent design choices:

1. Truncation parameter $K$: gradient steps per outer iteration ($K=\infty$ for FQI/NFQI, $K=1$ for DQN and Q-learning)
2. Data collection: offline batch, online with replay, or pure online stream
3. Initialization: cold start (reset to $\boldsymbol{\theta}_0$) or warm start (continue from $\boldsymbol{\theta}_k$)
4. Operator choice: hard max or smooth logsumexp

Target networks are not on this list. They are what approximate value iteration means when implemented with gradient-based optimization, not a tunable design choice. 

DQN's actual contributions were empirical and architectural rather than algorithmic:

1. Demonstrating stable learning with deep convolutional networks on high-dimensional visual inputs (raw 84×84×4 frames)
2. Successfully scaling experience replay to large buffers (1 million transitions)
3. Showing that truncated inner loops ($K=1$) work well when combined with replay buffers and target networks
4. End-to-end learning without hand-crafted features or separate dimensionality reduction

These achievements required careful engineering (frame preprocessing, reward clipping, network architecture choices) and substantial compute resources. The algorithmic structure, however, was already present in NFQI—DQN's innovation was showing that this structure could scale.

...
$$
C(s_i:s_{i+d}, \boldsymbol{\rho}) = -v_{\boldsymbol{\phi}}(s_i) + \gamma^d v_{\boldsymbol{\rho}}(s_{i+d}) + \sum_{j=0}^{d-1} \gamma^j[r(s_{i+j}, a_{i+j}) - \alpha \log \pi_{\boldsymbol{\rho}}(a_{i+j}|s_{i+j})]
$$

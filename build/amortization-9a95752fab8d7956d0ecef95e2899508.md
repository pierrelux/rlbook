...
The key insight is that while the logsumexp form requires integrating over all actions (intractable for continuous action spaces), the entropy-adjusted expectation form is **tractable via Monte Carlo sampling**. We can approximate:
...
1. A **Q-network** $q_\theta(s,a)$ that estimates state-action values
2. A **V-network** $v_\psi(s)$ that learns to predict the value function by fitting to these entropy-adjusted Q-value samples

The V-network "amortizes" the intractable logsumexp computation: instead of computing the integral each time, we train a network to predict its value by learning from samples. With both approximators, our bootstrap target becomes tractable. For a transition $(s_i,a_i,s'_i)$, we simply use the V-network to estimate the continuation value:

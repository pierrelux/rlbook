...
For the Gaussian family defined below, this penalty becomes a quadratic
cost on the nominal controls.
...
penalty a quadratic cost on the magnitude of the nominal controls, with
zero control incurring no effort penalty. Use a separate Gaussian sampling
distribution $Q_{\mathbf U}$ centered at the current nominal sequence.
Both have the same positive-definite covariance $\Sigma$:
...
The sampling mean $\mathbf U=(\mathbf u_0,\ldots,\mathbf u_{N-1})$

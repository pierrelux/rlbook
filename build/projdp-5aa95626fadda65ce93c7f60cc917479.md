...
For fixed $s$, consider the function $a \mapsto Q^*(s, a)$ mapping actions to returns. The policy $\pi^*(s)$ must maximize this function over $\mathcal{A}$. If this function is differentiable in $a$, the action space $\mathcal{A}$ is convex, and the maximizer $\pi^*(s)$ is interior, then a **necessary condition** for $\pi^*(s)$ to be a maximizer of the function $a \mapsto Q^*(s, a)$ is that its derivative vanishes at $a = \pi^*(s)$:

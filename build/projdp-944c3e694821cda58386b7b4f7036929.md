...
This general computational framework is called **weighted residual methods**. The idea is to make the residual $R(s; \theta) = \mathrm{L}\hat{v}(s; \theta) - \hat{v}(s; \theta)$ "small" (though we cannot make it exactly zero everywhere), where the approximation is $\hat{v}(s; \theta) = \sum_{i=1}^n \theta_i \varphi_i(s)$. Different weighted residual methods measure "small" in different ways:

**Projection methods** (also called orthogonal projection methods) require the residual to be orthogonal to $n$ chosen test functions:
...
**Least squares methods** measure "small" differently: instead of requiring the residual to be orthogonal to specific test functions, we find coefficients $\theta$ that minimize the residual's weighted $L^2$ norm:

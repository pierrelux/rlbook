...
:caption: A linear control connects the endpoint values $U_0$ and $U_1$. Its integral over the unit interval is the shaded trapezoid. Splitting that trapezoid into a rectangle and a triangle gives the average-height formula $\int_0^1 u(t)\,dt=(U_0+U_1)/2$.
...
u0_plot = 0.75
u1_plot = 1.65
t_plot = np.linspace(0.0, 1.0, 200)
u_plot = (1.0 - t_plot) * u0_plot + t_plot * u1_plot
...
with plt.rc_context(
    ...
    figure, (geometry_axis, formula_axis) = plt.subplots(
        1,
        2,
        figsize=(7.0, 2.75),
    ...
    triangle = Polygon(
        [(0.0, u0_plot), (1.0, u0_plot), (1.0, u1_plot)],
        closed=True,
        facecolor="#E69F00",
        edgecolor="#A66F00",
        linewidth=1.0,
    ...
    geometry_axis.add_patch(rectangle)
    geometry_axis.add_patch(triangle)
    geometry_axis.plot(t_plot, u_plot, color="#000000", linewidth=1.8)
    geometry_axis.plot(
        [0.0, 1.0],
        [u0_plot, u1_plot],
        linestyle="none",
    ...
    geometry_axis.text(
        0.73,
        1.02,
        r"triangle: $\frac{1}{2}(U_1-U_0)\times 1$",
        color="#744A00",
        ha="center",
    )
    geometry_axis.annotate(
        r"linear $u(t)$",
        xy=(0.45, (1.0 - 0.45) * u0_plot + 0.45 * u1_plot),
        xytext=(0.18, 1.58),
        arrowprops={"arrowstyle": "->", "color": "#333333", "linewidth": 0.8},
        color="#333333",
    )
    geometry_axis.set_xlim(-0.05, 1.08)
    geometry_axis.set_ylim(0.0, 1.9)
    geometry_axis.set_xticks([0.0, 1.0], ["0", "1"])
    geometry_axis.set_yticks(
        [0.0, u0_plot, u1_plot],
        ["0", r"$U_0$", r"$U_1$"],
    )
    geometry_axis.set_xlabel(r"time $t$")
    geometry_axis.set_ylabel(r"control $u(t)$")

    formula_axis.axis("off")
    formula_axis.text(
        0.02,
        0.82,
        r"$X_1-X_0=\int_0^1u(t)\,dt$",
        fontsize=11,
    )
    formula_axis.text(
        0.02,
        0.56,
        r"$=U_0\times 1+\frac{1}{2}(U_1-U_0)\times 1$",
        fontsize=10,
    )
    formula_axis.text(
        0.02,
    ...
    figure.tight_layout(pad=0.7, w_pad=1.8)

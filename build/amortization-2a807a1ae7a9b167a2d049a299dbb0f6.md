...
2. **Policy network amortization** (Sections 3-4): Learn a deterministic or stochastic policy network $\pi_{\boldsymbol{w}}$ that approximates $\arg\max_a q(s,a;\theta)$ or the optimal stochastic policy, enabling fast action selection via a single forward pass.
...
This consistency extends to arbitrary-length paths. For any state $s_1$ and action sequence $a_1, \ldots, a_{t-1}$ producing states $s_1, s_2, \ldots, s_t$, the optimal policy and state values satisfy:

$$
v^*(s_1) - \gamma^{t-1} v^*(s_t) = \sum_{i=1}^{t-1} \gamma^{i-1}[r(s_i, a_i) + \alpha \log \pi^*(a_i|s_i)]
$$

This path consistency property holds regardless of how the action sequence was generated—whether from the optimal policy, a previous policy iteration, an exploration policy, or even expert demonstrations. The actions need not be optimal; the consistency relates the endpoint state values to the rewards and log-probabilities observed along any path.
...
The path consistency property's independence from trajectory provenance enables a distinctive approach to off-policy learning. PCL maintains a replay buffer and applies consistency updates to both on-policy trajectories sampled from the current policy $\pi_{\boldsymbol{\theta}}$ and off-policy trajectories sampled from the replay buffer, potentially from much earlier policies.

Unlike importance-sampled policy gradient methods, PCL requires no correction factors. The consistency error is well-defined regardless of which policy generated the data. This makes PCL effective at exploiting diverse trajectory sources, including expert demonstrations that can be seeded into the replay buffer.
...
By the Boltzmann policy definition, $\alpha \log \pi_{\boldsymbol{\rho}}(a|s) = q_{\boldsymbol{\rho}}(s,a) - v_{\boldsymbol{\rho}}(s)$, so:

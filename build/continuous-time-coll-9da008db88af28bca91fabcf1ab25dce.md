...
which can be negative or exceed one for higher degrees. The weights sum to
one everywhere, but they need not form a convex combination as they do in the
two-endpoint linear example. They reconstruct the polynomial throughout the
interval by
...
the matrix obtained by evaluating the

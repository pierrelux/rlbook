...
When the operator equation can be written as a fixed-point problem $f = \mathscr{T}f$ where $\mathscr{T}$ is a contraction, we can use **function iteration**:

$$
\hat{f}^{(k+1)} = \mathscr{T}\hat{f}^{(k)}.
$$

In coefficient space, this becomes an iteration $\theta^{(k+1)} = G(\theta^{(k)})$ for some function $G$. The specific form depends on the projection method:

- For **collocation** on a fixed-point problem: If we require $\hat{f}(x_i) = (\mathscr{T}\hat{f})(x_i)$ at collocation points $x_i$, we get targets $t_i^{(k)} = (\mathscr{T}\hat{f}^{(k)})(x_i)$, then fit new coefficients so that $\hat{f}^{(k+1)}(x_i) = t_i^{(k)}$.

- For **Galerkin** on a fixed-point problem: We require $\langle \hat{f} - \mathscr{T}\hat{f}, \varphi_i \rangle_w = 0$, giving a linear system in the new coefficients (when $\mathscr{T}$ is linear in its argument) or a nonlinear system (when $\mathscr{T}$ is nonlinear).

**Advantages:** Globally convergent from any starting point when the finite-dimensional operator inherits the contraction property. Computationally cheap per iteration. No need to compute Jacobians.

**Challenges:** The finite-dimensional approximation may not preserve the contraction property of the original operator, especially with high-order polynomial bases that can create oscillations. Convergence can be slow when the discount factor $\gamma$ is close to 1 (weak contraction). Not applicable when the equation doesn't have a natural fixed-point structure.

...
| Orthogonal projection | $p_j = \varphi_j$ on $(\hat{v} - f)$ | $\displaystyle\min_\theta \tfrac{1}{2}\|\hat{v}(\cdot; \theta) - f\|_w^2$ |
| Galerkin | $p_j = \varphi_j$ on $R(\cdot; \theta)$ | min residual in dual norm of $\text{span}\{\varphi_j\}$ |
...
The test functions are the gradients $\partial R / \partial \theta_j$. This directly minimizes how far $\hat{v}$ is from satisfying the Bellman equation.

Now consider **orthogonal projection**. Fix a target function $f$ and minimize $\frac{1}{2}\|\hat{v}(\cdot; \theta) - f\|_w^2$. The first-order conditions are:

$$
\left\langle \hat{v}(\cdot; \theta) - f, \frac{\partial \hat{v}}{\partial \theta_j} \right\rangle_w = 0.
$$

For linear approximation $\hat{v} = \sum_j \theta_j \varphi_j$, we have $\frac{\partial \hat{v}}{\partial \theta_j} = \varphi_j$, so the conditions become $\langle \hat{v} - f, \varphi_j \rangle_w = 0$ for all $j$. The error $(\hat{v} - f)$ must be orthogonal to every basis function—equivalently, orthogonal to the entire approximation space. This is the defining property of **orthogonal projection**: $\hat{v} = \Pi_w f$.

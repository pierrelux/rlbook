...
The Stochastic Value Gradients (SVG) framework {cite}`Heess2015` uses this approach while introducing a hybrid objective that combines model rollouts with value function bootstrapping:
...
### Connection to SAC and PCL

MPPI shares the information-theoretic structure of SAC (Algorithm {prf:ref}`sac` in the [amortization chapter](amortization.md)) and PCL (Algorithm {prf:ref}`pcl` in the [amortization chapter](amortization.md)):

**SAC** learns a stochastic policy $\pi_{\boldsymbol{\phi}}$ that approximates the Boltzmann distribution over actions at each state. The learned policy generalizes across states and enables fast action selection via a forward pass.

**PCL** minimizes path consistency residuals under deterministic dynamics, exploiting the exact relationship $v^*(s) = q^*(s,a) - \alpha\log\pi^*(a|s)$ for all actions. The Boltzmann structure enables off-policy learning without importance sampling.

**MPPI** uses the Boltzmann distribution directly for action sequence selection, but performs no learning. At each state, it samples action sequences, weights them by exponentiated costs, and returns the weighted average. The computation is local (per state) rather than amortized across states through a policy network.

All three methods use entropy regularization with Boltzmann weighting. SAC and PCL amortize optimization by learning policies that generalize across states. MPPI performs full optimization at every decision, trading computation for avoiding policy approximation error.

### Connection to SVG

SVG and MPPI represent two ways to use learned models for optimization:

**SVG**: Requires differentiable dynamics and policy. Backpropagates through $n$-step model rollouts using the reparameterization trick. The gradient $\nabla_{\boldsymbol{w}}J$ guides policy improvement via gradient ascent.

**MPPI**: Requires only forward simulation. Samples many action sequences, rolls them forward through the model, and aggregates using importance weights. No gradients, no backpropagation, no policy network.

The trade-off: SVG is more sample-efficient (gradient provides richer signal than importance weights) but requires differentiable components and backpropagation through long horizons. MPPI is simpler to implement and applies to non-differentiable dynamics, but requires many rollout samples ($K \approx 100-1000$) per action selection.

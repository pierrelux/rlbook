...
Consider a functional equation $\mathscr{N}(f) = 0$, where $\mathscr{N}$ is an operator and the unknown $f$ is an entire function (in our case, the Bellman optimality equation $\mathrm{L}v = v$, which we can write as $\mathscr{N}(v) \equiv \mathrm{L}v - v = 0$). Suppose we have found a candidate approximate solution $\hat{f}$. To verify it satisfies $\mathscr{N}(\hat{f}) = 0$, we compute the **residual function** $R(s) = \mathscr{N}(\hat{f})(s)$. For a true solution, this residual should be the **zero function**: $R(s) = 0$ for every state $s$. But what does it really mean for a function to equal zero?

How might we test whether a function is zero? A natural programming approach would be to sample many input points $\{s_1, s_2, \ldots, s_m\}$ and check whether $R(s_i) = 0$ at each. We could summarize these checks into a single scalar by computing a weighted sum like $\sum_{i=1}^m w_i R(s_i)$, where $w_i > 0$ are weights. If $R$ is zero everywhere, this sum is zero. If $R$ is nonzero somewhere, we can choose points and weights to make the sum nonzero. This idea of "squashing" observations at many points into a single scalar test generalizes to continuous functions through integration.

In finite dimensions, a vector $\mathbf{r} \in \mathbb{R}^n$ equals zero if and only if $\langle \mathbf{r}, \mathbf{y} \rangle = 0$ for every vector $\mathbf{y} \in \mathbb{R}^n$. To see why, suppose $\mathbf{r} \neq \mathbf{0}$. Then we can choose $\mathbf{y} = \mathbf{r}$ to get $\langle \mathbf{r}, \mathbf{r} \rangle = \|\mathbf{r}\|^2 > 0$, contradicting the claim that all inner products are zero. Conversely, if $\mathbf{r} = \mathbf{0}$, then $\langle \mathbf{r}, \mathbf{y} \rangle = 0$ for all $\mathbf{y}$. The inner product $\langle \mathbf{r}, \mathbf{y} \rangle = \sum_{i=1}^n r_i y_i$ is exactly this kind of weighted sum: it tests $\mathbf{r}$ by weighting its components by $\mathbf{y}$ and summing.
...
This suggests the **projection framework**: choose $n$ test functions $\{p_1, \ldots, p_n\}$ and require the residual to be orthogonal to each:

$$
\langle R, p_i \rangle = 0, \quad i = 1, \ldots, n.
$$

where the residual is $R(s; \theta) = \mathscr{N}(\hat{f}(\cdot; \theta))(s)$ and the approximation is $\hat{f}(s; \theta) = \sum_{j=1}^n \theta_j \varphi_j(s)$. For the Bellman equation, $\mathscr{N}(v) = \mathrm{L}v - v$, so $R(s; \theta) = \mathrm{L}\hat{v}(s; \theta) - \hat{v}(s; \theta)$. We are "projecting" the residual to be orthogonal to the span of the test functions. This transforms the impossible task of verifying "$R(s) = 0$ for all $s$" into a finite-dimensional problem: find $n$ coefficients $\theta = (\theta_1, \ldots, \theta_n)$ satisfying $n$ conditions.

Different choices of test functions give different projection methods, each with different computational properties. The rest of this chapter develops this framework systematically: how to choose basis functions $\{\varphi_j\}$, how to select test functions $\{p_i\}$, and how to solve the resulting systems. 

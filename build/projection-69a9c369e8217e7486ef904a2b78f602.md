...
:::{figure} #fig-optimal-stopping-exact
The exact value function for the optimal stopping problem. Below the threshold $s^*$, the agent continues and receives the discounted continuation value. Above the threshold, the agent stops and receives $s$.
:::
...
:::{figure} #fig-collocation-comparison
Polynomial collocation approximation with 5 Chebyshev nodes. The polynomial captures the general shape but smooths over the kink at the threshold.
:::

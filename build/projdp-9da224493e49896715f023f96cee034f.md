...
This framework extends to more general function spaces through the **dual pairing** formulation {cite}`LegrandJunca2025`. In the abstract setting, the residual $R[v] = v - \Bellman v$ is an element of a dual space $\mathcal{H}'$, and its "size" is measured by a dual norm $\|R[v]\|_{\mathcal{H}'} = \sup_{\|w\|=1} |\langle R[v], w \rangle|$, where $\langle \cdot, \cdot \rangle$ is the dual pairing between functionals and test functions. This allows non-standard test functions: collocation uses Dirac deltas $\delta(x - x_i)$ with $\langle R[v], \delta(x - x_i) \rangle = R[v](x_i)$, and weak formulations in PDEs use smooth test functions to make sense of discontinuous residuals. The minimum residual principle remains the same: find parameters minimizing $\|R[v_{\boldsymbol{\theta}}]\|_{\mathcal{H}'}^2$.

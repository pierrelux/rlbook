...
The expectation penalizes the distribution of arrival positions, while the
second term penalizes the applied force. Both terms are needed: reaching the
target with an arbitrarily large force should not be free.

### Calculating the weights

Consider one step with $x_0=0$, $h=\nu=1$, nominal control $u_0=0$, and
target $a=1$, effort coefficient $r=1$, and terminal coefficient
$\kappa=2\log2$. Thus $c_1(x)=(\log2)(x-1)^2$. Suppose three sampled
...
would be $1$. The Gaussian reference and the finite temperature both matter
to the average.
...
To express this calculation for several control directions, take $D$ to have full column rank and write $B=DL$, with constant
...
| Receding-horizon execution | A plan conditioned on each newly observed state | Only the first action is executed before the next planning problem |
...
In the OpenAP A320 point-mass model {cite:p}`Sun2022`, the state is $(p_E,p_N,h,m,t)$: east and north position in a local map,
altitude in meters, mass in kilograms, and elapsed time in seconds.
Here $h$ denotes altitude; numerical time steps will be given in seconds. Controls
...
Taxi, runway operations, and traffic are outside this model. Mach number, vertical
...
sequence. Instead, a candidate is a vector $z\in\mathbb R^{13}$: one duration parameter and four

...
But $q^*(s_0,a_0) - \alpha\log\pi^*(a_0|s_0) \approx v^*(s_0)$, giving the **path consistency constraint**:

$$
v^*(s_0) - \gamma^d v^*(s_d) - \sum_{t=0}^{d-1} \gamma^t[r_t - \alpha\log\pi^*(a_t|s_t)] \approx 0
$$ (eq:path-consistency)

This constraint relates the value function and policy over an entire $d$-step path using only: the observed rewards, the deterministic state transitions, and the policy's log-probabilities at the observed actions.

**The key insight**: This constraint holds for **any** observed action sequence, regardless of which policy generated it. Once the actions are chosen and the deterministic dynamics produce the state sequence, equation {eq}`eq:path-consistency` is a constraint that $(v^*, \pi^*)$ must satisfy. The behavior policy never appears—we only evaluate $\pi^*$ at the observed actions, we don't need to know why those actions were chosen. This enables off-policy learning without importance sampling.

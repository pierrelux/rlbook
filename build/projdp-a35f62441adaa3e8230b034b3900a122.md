...
The answer lies in the **dual pairing** formulation underlying weighted residual methods. Let $\mathcal{H}$ be a Banach space (a complete normed vector space) and $\mathcal{H}'$ its dual space: the set of all continuous linear functionals on $\mathcal{H}$. For any functional $T \in \mathcal{H}'$, its dual norm is defined by

$$
\|T\|_{\mathcal{H}'} = \sup_{\substack{w \in \mathcal{H} \\ \|w\|_{\mathcal{H}} = 1}} |\langle T, w \rangle|,
$$

where $\langle T, w \rangle$ denotes the dual pairing: the action of the functional $T$ on the test function $w$. When $\mathcal{H} = L^2(\Omega)$, this reduces to the familiar inner product $\langle f, g \rangle = \int_\Omega f(x) g(x) \, dx$. For more general spaces, the dual pairing allows residuals that may not be classical functions. Collocation uses Dirac delta test functions $\delta(x - x_i)$, which are distributions, with dual pairing $\langle R[v], \delta(x - x_i) \rangle = R[v](x_i)$. Similarly, weak derivatives in partial differential equations produce residuals with discontinuities that only make sense when tested against smooth functions. The dual pairing generalizes "residual orthogonality" to settings where neither residuals nor test functions need be regular functions {cite}`LegrandJunca2025`.

The minimum residual principle seeks the function $v \in \mathcal{F}$ minimizing the dual norm of the residual $\|v - \Bellman v\|_{\mathcal{W}'}$. When $\mathcal{F}$ is parameterized by $\boldsymbol{\theta} \in \mathbb{R}^p$, this becomes the optimization problem
...
\Big\langle v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}, \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \Big\rangle_w = 0 \text{ for all } i.
$$

The "test functions" are now the partial derivatives $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i}$, which span the tangent space to the manifold $\{v_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \mathbb{R}^p\}$ at the current point. The stationarity condition requires the residual to be orthogonal to all tangent directions. This is the least-squares principle applied to a nonlinear manifold: find the point on the manifold closest to satisfying the Bellman equation.

When $\mathcal{F}$ is a linear subspace spanned by fixed basis functions $\{\varphi_1, \ldots, \varphi_d\}$, so that $v_{\boldsymbol{\theta}} = \sum_i \theta_i \varphi_i$, the partial derivatives simplify to $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} = \varphi_i$, and the stationarity condition becomes

$$
\langle v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}, \varphi_i \rangle_w = 0 \text{ for all } i,
$$

which are the Galerkin equations. The linear structure allows us to solve this in closed form via a linear system. For nonlinear parameterizations, the tangent vectors $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i}$ change with $\boldsymbol{\theta}$, and we solve the stationarity condition numerically by gradient descent or other nonlinear optimization algorithms. The resulting solution defines an implicit nonlinear projection operator $\Pi_{\mathcal{F}}$, which replaces the linear projection $\Proj$ in fitted-value iteration.

Physics-informed neural networks (PINNs) instantiate this framework for Bellman equations. A neural network $v_{\boldsymbol{\theta}}$ approximates the value function, and the training loss measures the residual norm in a sampled $L^2(\mu)$ space:

$$
\mathcal{L}(\boldsymbol{\theta}) = \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - (\Bellman v_{\boldsymbol{\theta}})(s))^2 \big].
$$

The dual pairing is implemented through automatic differentiation and Monte Carlo sampling. Weak adversarial networks make the dual structure explicit by parameterizing both the trial function $v_{\boldsymbol{\theta}}$ and the test function $w_{\boldsymbol{\phi}}$ as neural networks, optimizing
...
which directly implements the dual norm definition: the residual's magnitude is measured by its effect on the most adversarial test function.

The minimum residual framework thus unifies classical and modern function approximation methods for solving functional equations. Galerkin projection, collocation, and neural residual methods all minimize the residual norm in a chosen dual pairing. The differences are in the approximation class (linear subspace versus nonlinear manifold), the test space (smooth functions, point evaluations, or learned functions), and the optimization algorithm (direct linear solve versus iterative gradient-based methods). Convergence guarantees differ accordingly: linear projections with monotonicity or matched weightings yield contraction mappings, while nonlinear manifolds offer no general guarantees and may exhibit local minima. Neural network training for Bellman residual minimization remains an active research area. The dual pairing perspective nonetheless provides a principled foundation for understanding fitted-value iteration with expressive function classes as minimum residual iteration over parameterized manifolds, connecting the classical theory developed in this chapter to contemporary methods in reinforcement learning and scientific machine learning.

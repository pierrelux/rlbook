...
held fixed here: active decode requests run at every iteration, and at most 512
prefill tokens join an iteration. The control sequence specifies one normalized
GPU frequency for each second of a 60-second horizon,
...
The offline problem uses the complete nominal arrival forecast:

$$
\begin{aligned}
\underset{u_0,\ldots,u_{59}}{\operatorname{minimize}}\quad
&\sum_{k=0}^{59}\left[
\bar P_k+20B_{k+1}^2+0.05(u_k-u_{k-1})^2
+1000(s_{P,k}^2+s_{T,k}^2)\right]+20B_{60}^2\\
\text{subject to}\quad
&x_{k+1}=F_k(x_k,u_k,w_k),\\
&0\leq u_k\leq1,\qquad s_{P,k},s_{T,k}\geq0.
\end{aligned}
$$

Here $\bar P_k$ is power normalized by its highest-profile-clock value,
$B_k$ is unfinished work expressed in seconds of highest-clock service, and
$s_{P,k}$ and $s_{T,k}$ soften the experimental power and thermal limits. The
terminal penalty discourages the optimizer from postponing work past the
horizon. SLSQP optimizes the continuous sequence. Execution rounds each
frequency downward to the nearest profiled clock, so the request-level
validation includes the actuator's finite action set.

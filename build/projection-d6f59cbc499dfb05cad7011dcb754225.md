...
The inner product formulation is elegant, but it may seem abstract. Here is the key insight that grounds everything: **making a residual "small" is an optimization problem**. We want to find $\theta$ that minimizes $\lVert R(\cdot; \theta) \rVert$ for some norm. Different methods correspond to different choices of norm:
...
The first-order conditions for these optimization problems take the form $\langle R, p_j \rangle_w = 0$ for appropriate "test functions" $p_j$. This variational formulation is useful for analysis, but the underlying principle is simple: **we are minimizing the residual in a chosen norm**.

The rest of this section develops the computational framework. We will return to the optimization interpretation when comparing specific methods—showing exactly which norm each method minimizes and why this matters for algorithms like Fitted Q-Iteration.

With this perspective in place, we proceed to the general approach. We parameterize the unknown function, define the residual, and impose conditions that make the residual small. The choice of conditions—equivalently, the choice of norm—determines the method:
...
| **Method** | **Optimization problem** | **Resulting conditions** |
|:-----------|:------------------------|:------------------------|
| Least squares (min. residual) | $\displaystyle\min_\theta \int R(x; \theta)^2 w(x) dx$ | $\displaystyle\int R \cdot \frac{\partial R}{\partial \theta_j} \, w \, dx = 0$ |
| Galerkin | $\displaystyle\min_\theta \lVert R \rVert_{\mathcal{V}^*}$ (dual norm) | $\displaystyle\int R(x; \theta) \varphi_j(x) w(x) dx = 0$ |
| Collocation | $\displaystyle\min_\theta \sum_i \omega_i R(x_i; \theta)^2$ | $R(x_i; \theta) = 0$ at $n$ points |
...
The Galerkin method minimizes the residual in the **dual norm** of the approximation space. The resulting conditions require the residual to be orthogonal to each basis function:
...
The gradient $\frac{\partial R}{\partial \theta_j}$ involves differentiating the operator $\Residual$. For the Bellman operator with its max, this requires the Envelope Theorem (discussed in Step 4). This computational cost—differentiating through the operator—distinguishes least squares from Galerkin and collocation.
...
FQI does **not** minimize the Bellman residual $\lVert \Bellman\hat{v} - \hat{v} \rVert^2$ directly. It projects, then iterates. The distinction matters: FQI's projection step uses only the gradient of $\hat{v}$ with respect to $\theta$ (the "semi-gradient"), while Bellman residual minimization requires differentiating through $\Bellman$ (the "full gradient"). We return to this distinction when discussing temporal difference learning.

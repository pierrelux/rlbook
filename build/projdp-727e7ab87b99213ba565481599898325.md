...
```{admonition} Note on Weighted Inner Products
:class: dropdown

In more advanced treatments, you may see a separate weight function $w(s)$ in the inner product: $\langle R, p_i \rangle_w = \int R(s) p_i(s) w(s) ds$. Such weights are useful when working in weighted Sobolev spaces or when the state space has a natural probability measure. For our purposes, any necessary weighting is absorbed into the definition of the test functions $p_i$ themselves. This keeps the notation simpler and aligns with standard presentations like Klein (2008).
```

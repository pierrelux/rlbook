...
The result tells us that **if** the weighting distribution $\xi$ matches the stationary distribution of policy $\pi$, then projected value iteration with Galerkin projection converges. This is a sufficient condition, not a characterization: we have shown that stationarity implies convergence, but not that convergence requires stationarity. The condition has a natural interpretation in reinforcement learning: when we learn by following policy $\pi$ and collecting transitions $(s, a, r, s')$, the states we visit are distributed according to the stationary distribution of $\pi$. This is **on-policy learning**. The LSTD algorithm uses data sampled from this distribution, and our analysis shows that the iterative algorithm $v_{k+1} = \Proj \BellmanPi v_k$ converges to the same fixed point.

This is fundamentally different from the monotone approximator theory. There, we required structural properties of $\Proj$ itself (monotonicity, constant preservation) to guarantee that $\Proj$ preserves the sup-norm contraction property of $\Bellman$. Here, we place no such restriction on $\Proj$. Galerkin projection is not monotone—its matrix representation typically has negative entries. Instead, convergence depends on matching the weighting $\xi$ to the policy dynamics $\mathbf{P}_\pi$. The weighted $L^2$ geometry makes $\BellmanPi$ a contraction, and orthogonal projection (which is always non-expansive) preserves this contractivity.

When the weighting $\xi$ does not match the stationary distribution—for instance, in **off-policy learning** where we collect data from a behavior policy different from $\pi$—the Jensen inequality argument breaks down. The operator $\mathbf{P}_\pi$ need not be non-expansive in $\|\cdot\|_\xi$, and $\Proj \BellmanPi$ may fail to contract. This explains divergence phenomena in linear TD learning, such as Baird's counterexample. Importance sampling and other corrections attempt to reweight the data to restore the appropriate distribution.

### Why Not Bellman Optimality?

Can we extend this weighted $L^2$ analysis to the Bellman optimality operator $\Bellman v = \max_a [r_a + \gamma \mathbf{P}_a v]$? The answer is no, at least not with this approach. The obstacle appears precisely at the Jensen inequality step. For policy evaluation, we had:

...
For problems where we can eliminate the value function using the envelope theorem, this becomes a condition purely in terms of the policy. A canonical example is the consumption-savings problem with utility $u(c)$ and budget constraint $k' = f(k) - c$. The Bellman equation is:

$$
v(k) = \max_{c} \{ u(c) + \gamma \mathbb{E}[v(k')] \} \quad \text{s.t.} \quad k' = f(k) - c.
$$

The first-order condition is $u'(c) = \gamma \mathbb{E}[v'(k')]$. The envelope theorem gives $v'(k) = \gamma \mathbb{E}[v'(k')] f'(k)$. Substituting this into the FOC shifted forward one period:

$$
u'(c(k)) = \gamma \mathbb{E}[u'(c(k')) f'(k')],
$$

where $k' = f(k) - c(k)$. This is the **discrete-time Euler equation**. It is a functional equation in the consumption policy $c(\cdot)$, with no explicit appearance of the value function.

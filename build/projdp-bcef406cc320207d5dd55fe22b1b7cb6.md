...
where $\boldsymbol{\Phi}$ is the matrix of basis function evaluations and $\boldsymbol{\Xi} = \text{diag}(\xi)$. This is the Galerkin projection we studied earlier, now weighted by the stationary distribution. The solution satisfies $\hat{v} = \Proj \BellmanPi \hat{v}$, where $\Proj$ is orthogonal projection onto $\text{span}(\boldsymbol{\Phi})$ with respect to the $\xi$-weighted inner product:

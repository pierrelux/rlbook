...
When working with discretized representations, we partition the state space into $N_s$ cells with centers at grid points $\{s_i\}_{i=1}^{N_s}$. We approximate value functions by piecewise constant functions on each cell: for any $s \in S$: $v_h(s) = v_h(s_{k(s)})$ where $k(s)$ is the index of the cell containing $s$. Similarly, we approximate the reward function by evaluating it at cell centers: $r_h(s, a) \equiv r(s_{k(s)}, a)$.

To evaluate the expectation in the Bellman operator, we use numerical quadrature. Since our value function approximation is piecewise constant over cells, the integral naturally decomposes into a sum over cells:

$$ \begin{aligned}
(\widehat{\Bellman}_h v_h)(s) &= \max_{a \in \mathcal{A}_s} \left\{r_h(s, a) + \gamma \int v_h(s')p(s'|s, a)ds'\right\} \\
&= \max_{a \in \mathcal{A}_s} \left\{r_h(s, a) + \gamma \int v_h(s_{k(s')})p(s'|s, a)ds'\right\} \\
&= \max_{a \in \mathcal{A}_s} \left\{r_h(s, a) + \gamma \sum_{i=1}^{N_s} v_h(s_i) \int_{cell_i} p(s'|s, a)ds'\right\}
\end{aligned} $$

where $\int_{cell_i} p(s'|s, a)ds'$ is simply the probability of transitioning into cell $i$ from state $s$ under action $a$. If we denote this cell-to-cell transition probability as $P_{ij}^a = \int_{cell_j} p(s'|s_i, a)ds'$ for states $s \in cell_i$, we can write:

$$ (\widehat{\Bellman}_h v_h)(s_i) = \max_{a \in \mathcal{A}_{s_i}} \left\{r(s_i, a) + \gamma \sum_{j=1}^{N_s} P_{ij}^a v_h(s_j)\right\} $$

This form makes clear how discretization converts our continuous-space problem into a finite computation: we've replaced integration over continuous space with summation over grid points, effectively reducing the problem to a finite-state MDP. The price we pay is that the number of grid points grows exponentially with the dimension of our state space - the familiar curse of dimensionality.

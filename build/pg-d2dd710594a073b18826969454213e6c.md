...
where $R^2 = \frac{[\text{Cov}(Z,C)]^2}{\text{Var}(Z)\text{Var}(C)}$ is the coefficient of determination, which measures the fraction of variance in $Z$ explained by the linear regression on $C$. Since $R^2 \in [0,1]$, the control variate variance $(1-R^2)\text{Var}(Z)$ is always between $0$ and $\text{Var}(Z)$. When $R^2 = 1$ (perfect linear relationship), all variance is eliminated. When $R^2 = 0$ (no linear relationship), the control variate provides no benefit.

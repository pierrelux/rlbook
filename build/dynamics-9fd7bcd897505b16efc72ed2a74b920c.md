...
**Interpretability.**
The parameters correspond to things you can reason about: thermal resistance, capacitance, heat transfer between zones. You can cross-check values against architectural plans, or adjust them manually when something does not line up. You can tell which wall or zone is contributing to slow recovery times.
...
Once the model is built, it is straightforward to modify. If a window is replaced, or a wall gets insulated, you only need to update a few numbers.  The model is easy to pass along to another engineer or embed in a larger simulation. A model like

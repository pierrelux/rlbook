...
The DQN paper frequently receives credit for introducing the "target network" as a stabilization technique. This framing misses the mathematical reality: target networks are not a design innovation but rather a direct consequence of approximate value iteration's structure. Understanding this clarifies the relationship between classical batch algorithms (FQI, NFQI) and modern online methods (DQN, Double DQN).

Recall the nested structure of fitted Q-iteration. The outer loop implements value iteration: at iteration $k$, use current parameters $\boldsymbol{\theta}_k$ to compute targets for all transitions,

$$
y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}_k).
$$

The inner loop solves the regression problem to convergence (or near-convergence):

$$
\boldsymbol{\theta}_{k+1} = \arg\min_{\boldsymbol{\theta}} \sum_i (q(s_i, a_i; \boldsymbol{\theta}) - y_i)^2.
$$

The targets $\{y_i\}$ remain fixed throughout the entire inner optimization. We compute them once using $\boldsymbol{\theta}_k$ at the start of iteration $k$, then hold them constant while running gradient descent to find $\boldsymbol{\theta}_{k+1}$. This fixed-target property defines what it means to apply the Bellman operator once and then project the result onto the function approximation space.

When we flatten this nested structure into a single loop (for implementation convenience, especially in online settings), we must preserve this mathematical property. The "target network" $\boldsymbol{\theta}_{\text{target}}$ plays exactly the role that $\boldsymbol{\theta}_k$ played in the nested version: it provides fixed targets for $K$ gradient steps. The periodic synchronization $\boldsymbol{\theta}_{\text{target}} \leftarrow \boldsymbol{\theta}$ marks what would have been the boundary of an outer iteration.

To make this concrete, compare the nested and flattened implementations:

```{prf:algorithm} Nested FQI (Explicit Two-Loop Structure)
:label: nested-fqi-target

**Input:** Transitions $\mathcal{D}$, Q-function $q(s,a;\boldsymbol{\theta})$, steps per iteration $K$

**Output:** Parameters $\boldsymbol{\theta}$

1. Initialize $\boldsymbol{\theta}_0$
2. **for** $k = 0, 1, 2, \ldots$ **do**
3. $\quad$ $\mathcal{D}_k^{\text{fit}} \leftarrow \emptyset$
4. $\quad$ **for** each $(s_i, a_i, r_i, s'_i) \in \mathcal{D}$ **do**
5. $\quad\quad$ $y_i \leftarrow r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}_k)$
6. $\quad\quad$ $\mathcal{D}_k^{\text{fit}} \leftarrow \mathcal{D}_k^{\text{fit}} \cup \{((s_i, a_i), y_i)\}$
7. $\quad$ **end for**
8. $\quad$ $\boldsymbol{\theta}_{k+1} \leftarrow \texttt{fit}(\mathcal{D}_k^{\text{fit}}, \boldsymbol{\theta}_{\text{init}}=\boldsymbol{\theta}_k, K)$
9. **end for**
10. **return** $\boldsymbol{\theta}_k$
```

```{prf:algorithm} Flattened FQI (Single-Loop Structure)
:label: flattened-fqi-target

**Input:** Transitions $\mathcal{D}$, Q-function $q(s,a;\boldsymbol{\theta})$, target update frequency $K$
...
4. $\quad$ $\mathcal{D}_t^{\text{fit}} \leftarrow \emptyset$
5. $\quad$ **for** each $(s_i, a_i, r_i, s'_i) \in \mathcal{D}$ **do**
6. $\quad\quad$ $y_i \leftarrow r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}_{\text{target}})$
7. $\quad\quad$ $\mathcal{D}_t^{\text{fit}} \leftarrow \mathcal{D}_t^{\text{fit}} \cup \{((s_i, a_i), y_i)\}$
8. $\quad$ **end for**
9. $\quad$ $\boldsymbol{\theta}_{t+1} \leftarrow \boldsymbol{\theta}_t - \alpha \nabla_{\boldsymbol{\theta}} \mathcal{L}(\boldsymbol{\theta}_t; \mathcal{D}_t^{\text{fit}})$
10. $\quad$ **if** $t \bmod K = 0$ **then**
11. $\quad\quad$ $\boldsymbol{\theta}_{\text{target}} \leftarrow \boldsymbol{\theta}_t$
12. $\quad$ **end if**
13. **end for**
14. **return** $\boldsymbol{\theta}_t$
```

These formulations are equivalent when the update frequency $K$ in Algorithm {prf:ref}`flattened-fqi-target` equals the number of gradient steps per outer iteration in Algorithm {prf:ref}`nested-fqi-target`. The flattened version unrolls the nested loops and makes the parameter management explicit, but it adds no new algorithmic content.

Without a target network, targets would be recomputed after every gradient step using the just-updated parameters. This breaks the value iteration structure: we would no longer be applying $\Bellman$ once and then projecting. Instead, we would be chasing a moving target, which corresponds to residual minimization methods that require different analysis and typically double sampling at each state-action pair.

The same principle extends to exponential moving average (EMA) target updates $\boldsymbol{\theta}_{\text{target}} \leftarrow \tau \boldsymbol{\theta} + (1-\tau)\boldsymbol{\theta}_{\text{target}}$ used in DDPG, TD3, and SAC. EMA targets smooth the transition between outer iterations but preserve the separation between applying the Bellman operator and fitting to its output. The target network changes slowly relative to the online network, maintaining approximate fixed-target behavior.

The entire algorithm space can now be understood through four independent design choices:
...
2. Data collection: Offline batch, online with replay buffer, or pure online stream?
...
Target networks do not appear on this list. They are the implementation mechanism for approximate value iteration with gradient-based optimization, not a tunable design choice.

...
### Bootstrap Sampling: Unifying Offline and Online Learning

Both offline and online reinforcement learning can be understood as stochastic approximation on an empirical distribution over transition tuples. We maintain a dataset $\mathcal{D} = \{(s_i, a_i, r_i, s'_i)\}$ of observed transitions and construct regression targets by bootstrap sampling from this dataset. Each sampled tuple $(s_i, a_i, r_i, s'_i)$ provides one Monte Carlo evaluation of the Bellman operator at $(s_i, a_i)$ using the observed next state $s'_i$:

$$
y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}^{(n)})
$$

The distinction between offline and online learning reduces to whether the empirical distribution remains static or evolves:

...
The test functions are now the partial derivatives $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i}$, which span the tangent space to the manifold $\{v_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \mathbb{R}^p\}$ at the current parameters. This is the key generalization: in the linear case $v_{\boldsymbol{\theta}} = \sum_i \theta_i \varphi_i$, we have $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} = \varphi_i$, recovering the fixed basis functions of Galerkin. For nonlinear parameterizations, the test functions change with $\boldsymbol{\theta}$, and we solve by iterative gradient descent rather than a direct linear solve.

This connection between norm minimization and orthogonality is a general mathematical fact, not specific to Bellman equations or function approximation. For any norm $\|\cdot\|_w$ induced by an inner product $\langle \cdot, \cdot \rangle_w$, minimizing $\|f(\boldsymbol{\theta})\|_w^2$ with respect to parameters requires $\frac{\partial}{\partial \theta_i} \|f(\boldsymbol{\theta})\|_w^2 = 0$. Since $\|f\|_w^2 = \langle f, f \rangle_w$, the chain rule gives $2\langle f, \frac{\partial f}{\partial \theta_i} \rangle_w = 0$. This is why minimizing the residual norm is equivalent to requiring orthogonality $\langle f, \frac{\partial f}{\partial \theta_i} \rangle_w = 0$ for all $i$. The equivalence holds for any choice of inner product: weighted $L^2$ integrals for Galerkin, sums over collocation points for collocation, or sampled expectations for neural networks.

This framework provides a unifying perspective on neural network function approximation in reinforcement learning. Algorithms like Q-learning and TD learning with neural networks perform stochastic gradient descent on losses of the form

$$
\mathcal{L}(\boldsymbol{\theta}) = \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - \text{target}(s))^2 \big],
$$

where the target approximates $(\Bellman v_{\boldsymbol{\theta}})(s)$ using bootstrap estimates from sampled transitions. Assuming convergence, this implements the orthogonality condition $\langle \Residual(v_{\boldsymbol{\theta}}), \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \rangle = 0$ in a Monte Carlo approximation of the $L^2(\mu)$ inner product. These algorithms are thus computing nonlinear projections of the Bellman equation, as prescribed by the minimum residual framework.

The **dual pairing** formulation {cite}`LegrandJunca2025` extends this framework to settings where test objects need not be regular functions. Collocation uses Dirac distributions $\delta(x - x_i)$ with $\langle \Residual(v), \delta(x - x_i) \rangle = (\Residual v)(x_i)$. The induced dual norm $\|\Residual(v)\|_* = \sup_{\|w\|=1} |\langle \Residual(v), w \rangle|$ measures residuals by their worst-case effect on test functions, a perspective that has inspired adversarial formulations {cite}`Zang2020` where both trial and test functions are learned.

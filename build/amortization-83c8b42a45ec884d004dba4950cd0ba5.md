...
This is where the entropy-regularized perspective becomes essential. Recall from the [smoothing chapter](smoothing.md) that the value function under entropy regularization admits two equivalent representations. The first uses the logsumexp operator:

$$
v^*(s) = \frac{1}{\beta} \log \sum_a \exp(\beta \cdot q^*(s,a))
$$

The second expresses the same quantity as an expectation under the optimal policy $\pi^*(a|s) = \frac{\exp(\beta \cdot q^*(s,a))}{\sum_{a'} \exp(\beta \cdot q^*(s,a'))}$:

$$
v^*(s) = \mathbb{E}_{a \sim \pi^*(\cdot|s)}\left[q^*(s,a) - \alpha \log \pi^*(a|s)\right]
$$

where $\alpha = 1/\beta$. This equivalence follows from the first-order optimality conditions we derived above: the optimal policy is the Boltzmann distribution over Q-values, and substituting this back into the entropy-regularized objective yields the logsumexp form.

These two representations compute the same value function, but the second form is **tractable via sampling**. Instead of integrating over all actions, we can estimate the expectation by sampling actions from our current policy $\pi_\phi$:

$$
v(s) \approx \mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}\left[q_\theta(s,a) - \alpha \log \pi_{\phi}(a|s)\right]
$$

This motivates maintaining **two separate function approximators**:
1. A **Q-network** $q_\theta(s,a)$ that estimates state-action values
2. A **V-network** $v_\psi(s)$ that learns to approximate the expectation above by fitting to samples: $v_\psi(s) \approx \frac{1}{N}\sum_{i=1}^N [q_\theta(s,a_i) - \alpha \log \pi_\phi(a_i|s)]$ where $a_i \sim \pi_\phi(\cdot|s)$

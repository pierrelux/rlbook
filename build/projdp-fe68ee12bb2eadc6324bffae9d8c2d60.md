...
We know that standard value iteration $v_{k+1} = \Bellman v_k$ converges because $\mathrm{L}$ is a $\gamma$-contraction in the sup norm. But now we're iterating with the **composed operator** $\Proj \mathrm{L}$ instead of $\mathrm{L}$ alone.
...
Then the composed operator $\Proj \mathrm{L}$ is a $\gamma$-contraction, and projected value iteration $v_{k+1} = \Proj \Bellman v_k$ converges globally to a unique fixed point $v_\Proj \in \text{Range}(\Proj)$ with approximation error:
...
```{prf:proof}
Since $\mathrm{L}$ is a $\gamma$-contraction, we have $-\gamma\|f-g\|_\infty \leq \Bellman f - \Bellman g \leq \gamma\|f-g\|_\infty$ pointwise. By monotonicity of $\Proj$, $\Proj(-\gamma\|f-g\|_\infty) \leq \Proj(\Bellman f - \Bellman g) \leq \Proj(\gamma\|f-g\|_\infty)$. By constant preservation, $-\gamma\|f-g\|_\infty \leq \Proj(\Bellman f - \Bellman g) \leq \gamma\|f-g\|_\infty$, which implies $\|\Proj \Bellman f - \Proj \Bellman g\|_\infty \leq \gamma\|f-g\|_\infty$.
...
If $\Proj$ is an averager and $\mathrm{L}$ is the Bellman operator (a $\gamma$-contraction), then $\Proj \mathrm{L}$ is a $\gamma$-contraction, and value iteration $v_{k+1} = \Proj \Bellman v_k$ converges to a unique fixed point.
```

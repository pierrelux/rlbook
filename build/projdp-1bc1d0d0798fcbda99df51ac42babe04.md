...
where the residual is $R(s; \theta) = \mathrm{L}\hat{v}(s; \theta) - \hat{v}(s; \theta)$ and the approximation is $\hat{v}(s; \theta) = \sum_{j=1}^n \theta_j \varphi_j(s)$. We are "projecting" the residual to be orthogonal to the span of the test functions. This transforms the impossible task of verifying "$R(s) = 0$ for all $s$" into a finite-dimensional problem: find $n$ coefficients $\theta = (\theta_1, \ldots, \theta_n)$ satisfying $n$ conditions.

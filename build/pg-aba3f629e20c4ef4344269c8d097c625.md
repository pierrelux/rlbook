...
The policy gradient estimator we derived suffers from high variance because trajectory returns can vary significantly even under the same policy. Control variates offer a method to reduce this variance without introducing bias.

Suppose we have an estimator $Z$ for some quantity of interest, and we have access to another random variable $C$ whose expectation we know exactly. If $C$ is correlated with $Z$, we can use the observed deviation of $C$ from its mean to adjust our estimate of $Z$. Since we know $\mathbb{E}[C]$, any deviation $C - \mathbb{E}[C]$ represents sampling noise. If $Z$ and $C$ are positively correlated, then when $C$ happens to be larger than its mean, $Z$ is also likely larger than its mean. We can therefore subtract a multiple of $(C - \mathbb{E}[C])$ to correct for this sampling variation.

For a control variate $C$ with $\mathbb{E}[C]=0$ (we can always center it by subtracting its mean), we construct:

$$
Z_{\text{cv}} = Z - \alpha C
$$

where $\alpha$ is a coefficient we choose. This estimator remains unbiased:

$$
\mathbb{E}[Z_{\text{cv}}] = \mathbb{E}[Z] - \alpha\mathbb{E}[C] = \mathbb{E}[Z]
$$
...
Minimizing with respect to $\alpha$ gives the optimal coefficient:

$$
\alpha^* = \frac{\text{Cov}(Z,C)}{\text{Var}(C)}
$$

Substituting this back into the variance expression shows that the variance reduction achieved is:

$$
\text{Var}(Z) - \text{Var}(Z_{\text{cv}}) = \frac{[\text{Cov}(Z,C)]^2}{\text{Var}(C)} = \text{Var}(Z) \cdot \rho^2_{Z,C}
$$

where $\rho_{Z,C}$ is the correlation coefficient between $Z$ and $C$. The reduction is proportional to the squared correlation: a control variate that is highly correlated with the estimator provides substantial variance reduction, while an uncorrelated control variate provides no benefit. 

A good control variate should satisfy three properties: (1) we can compute or know its expectation exactly, (2) it is inexpensive to sample alongside $Z$, and (3) it is strongly correlated with $Z$. In reinforcement learning, we typically cannot compute the optimal coefficient $\alpha^*$ directly since it requires knowing covariances. However, we can approximate it with learned functions.

In the reinforcement learning setting, we choose $C_t = \nabla_{\boldsymbol{w}}\log \pi_{\boldsymbol{w}}(a_t|s_t)$ as our control variate at each timestep. For a given state $s_t$, our estimator at time $t$ is:
...
&= \mathbb{E}[\sum_{k=t}^T r_k|s_t] = v^{\pi_{\boldsymbol{w}}}(s_t)
\end{align*}
$$
...
In practice, we do not have access to the true value function. As in NFQCA and SAC (discussed in the [amortization chapter](amortization.md)), we commonly learn a value function simultaneously with the policy. We could either use a fitted value approach, or more simply regress from states to returns to learn what Williams (1992) called a "baseline": 

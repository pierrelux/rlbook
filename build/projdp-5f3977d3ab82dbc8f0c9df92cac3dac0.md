...
Assume that for each state $s$, the maximizer is unique and interior, and denote the corresponding optimal policy by $\pi^*(s)$:
...
If $v^*$ is differentiable in $s$, the optimal policy satisfies the **Euler equation**, obtained by taking the first-order condition of this static optimization problem (for each fixed $s$):

$$
\frac{\partial r}{\partial a}\big(s,\pi^*(s)\big)
+
\gamma\, \nabla_s v^*\big(f(s,\pi^*(s))\big)\,
...
Here $\nabla_s v^*(\cdot)$ denotes the gradient of $v^*$ with respect to the state, and $\partial f / \partial a$ is the Jacobian of $f$ with respect to the action.

In stochastic settings with $s' = g(s,a,\xi)$ and exogenous noise $\xi$, the same logic gives

$$
\frac{\partial r}{\partial a}\big(s,\pi^*(s)\big)
+
\gamma\, \mathbb{E}\!\left[
\nabla_s v^*\big(g(s,\pi^*(s),\xi)\big)\,
...
In both cases, the Euler equation is simply the first-order condition of the dynamic programming problem.

### The Euler residual operator

Euler-equation methods take the point of view that the optimal policy is characterized by the Euler equation, rather than by the Bellman fixed point in $v^*$. This suggests defining the **Euler residual operator**, which measures violation of the Euler condition for a given policy.

For a deterministic model, define the Euler residual of a policy $\pi$ by
...
\frac{\partial r}{\partial a}\big(s,\pi(s)\big)
+
\gamma\, \nabla_s v^\pi\big(f(s,\pi(s))\big)\,
\frac{\partial f}{\partial a}\big(s,\pi(s)\big),
$$

where $v^\pi$ is the value function associated with the policy $\pi$ (or $v^*$ if we reason directly at the optimum). The optimal policy satisfies
...
In practice, one works with a parameterized policy $\pi_\theta : \mathcal{S} \to \mathcal{A}$. The goal is then to choose $\theta$ so that $\mathcal{R}_E(\pi_\theta)$ is “small” in some sense. For example, we can enforce the Euler equation at a finite set of collocation points.

### The nonlinear system to solve

Let $s_1,\dots,s_N$ be a set of states (collocation points). Enforcing the Euler equation at these states gives the system of equations

$$
\mathcal{R}_E(\pi_\theta)(s_k) = 0,
\quad k = 1,\dots,N.
$$
...
:=
\begin{bmatrix}
\mathcal{R}_E(\pi_\theta)(s_1) \\
\vdots \\
\mathcal{R}_E(\pi_\theta)(s_N)
\end{bmatrix},
$$

so that the **Euler equation discretization** is the nonlinear system
...
$$
\int_{\mathcal{S}} w_i(s)\,\mathcal{R}_E(\pi_\theta)(s)\,ds = 0,
\quad i = 1,\dots,M.
$$

These conditions can again be collected into a vector equation
...
The operator $G$ is generally nonlinear in $\theta$, so one solves $G(\theta)=0$ using nonlinear equation solvers (Newton-type methods, fixed-point iterations, or other root-finding schemes). Unlike the Bellman operator, the Euler residual operator is not a contraction in a natural norm, so generic convergence guarantees are weaker and depend on the specific problem.

### A remark on the envelope theorem

In many economic models, one can go further and use the envelope theorem to express $\nabla_s v^*$ recursively in terms of $r$, $f$, and the optimal policy. In special cases (for example, 1-dimensional growth models), this allows the Euler equation to be written entirely in terms of the policy and primitives, without explicit reference to $v^*$. This is the style used in {cite}`Rust1996` and related work.

We will not rely on such model-specific simplifications here. For our purposes, it is enough to view the Euler equation as the first-order condition of the dynamic programming problem, and to treat Euler-equation methods as weighted residual methods applied to $\mathcal{R}_E(\pi_\theta)$ rather than to the Bellman operator.

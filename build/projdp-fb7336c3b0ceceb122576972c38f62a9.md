...
When does (EC) hold? Whenever the dynamics are **affine** in $(y,a)$, meaning $f(y,a,z',z) = A(z,z')y + B(z,z')a + c(z,z')$ for some matrices/vectors $A,B,c$. In this case, $\partial f/\partial y$ and $\partial f/\partial a$ are constant (independent of $y$ and $a$), so the proportionality is automatic. Many control models use affine approximations of nonlinear dynamics, making (EC) widely applicable.

When (EC) holds, the **envelope theorem** gives a formula for $\partial v/\partial y$ that depends only on $r$, $h$, and the policy $\pi$, not on $v$ itself. Substituting this formula back into (Euler-raw) yields a **closed Euler equation** involving only $\pi$:
...
This is the sense in which Euler-class problems allow pure policy approximation: we can pose the optimality condition as a functional equation in $\pi$ alone and apply weighted residual methods to that equation, never constructing $v$.
...
All four examples have **affine dynamics in the controlled state**: $f(y,a,z',z)$ is affine (linear plus a constant) in $(y,a)$. This immediately implies (EC). The Euler-class framework thus extends far beyond the consumption-savings problems of classical macroeconomics. It applies to inventory systems, energy storage, water resources, and thermal control. Any domain where a first-order linear approximation of how $(y,a)$ affect the next state is reasonable falls into this class.
...
After the envelope-theorem substitution, the first-order condition becomes

$$
\frac{\partial r}{\partial a}\big(y,z,\pi(y,z)\big)
+
\gamma \int \frac{\partial r}{\partial y}\big(y',z',\pi(y',z')\big) h\big(y,\pi(y,z),z\big)\, q(dz'\mid z) = 0,
$$

where $y' = f(y,\pi(y,z),z',z)$ and $h$ comes from the Euler-class condition (EC). This expression depends only on $\pi$ and the primitives $(r,f,h,q)$. The value function does not appear.

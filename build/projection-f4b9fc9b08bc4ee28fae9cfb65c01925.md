...
Consider a trial function $q_\theta$ parameterized by $\theta$, a residual $R(q_\theta) = \Bellman q_\theta - q_\theta$, and an inner product $\langle \cdot, \cdot \rangle_w$ with weight function $w$. The **variational conditions** require:

$$
\langle R(q_\theta), p_j \rangle_w = 0 \quad \text{for all test functions } p_j.
$$
...
|:-----------|:-------------------|:---------------------------|
| Minimum residual (full) | $p_j = \frac{\partial R(q_\theta)}{\partial \theta_j}$ | $\displaystyle\min_\theta \tfrac{1}{2}\|R(q_\theta)\|_w^2$ |
| Projection (FQI step) | $p_j = \frac{\partial q_\theta}{\partial \theta_j}$ on $(q_\theta - f)$ | $\displaystyle\min_\theta \tfrac{1}{2}\|q_\theta - f\|_w^2$ |
...
| Collocation | $p_i = \delta_{s_i}$ (point masses) | $\displaystyle\min_\theta \sum_i \omega_i \, R(q_\theta)(s_i)^2$ |

To see why this correspondence holds, consider the **minimum residual** objective $\min_\theta \frac{1}{2}\|R(q_\theta)\|_w^2 = \min_\theta \frac{1}{2}\langle R(q_\theta), R(q_\theta) \rangle_w$. Setting the gradient to zero:

$$
\frac{\partial}{\partial \theta_j} \tfrac{1}{2}\|R(q_\theta)\|_w^2 = \left\langle R(q_\theta), \frac{\partial R(q_\theta)}{\partial \theta_j} \right\rangle_w = 0.
$$

The test functions are the gradients $\partial R / \partial \theta_j$. This is a **true minimum-residual** method: it directly minimizes how far $q_\theta$ is from satisfying the Bellman equation.

Now consider **projection least-squares**. Fix a target function $f$ and minimize $\frac{1}{2}\|q_\theta - f\|_w^2$. The first-order conditions are:

$$
\left\langle q_\theta - f, \frac{\partial q_\theta}{\partial \theta_j} \right\rangle_w = 0,
$$

which means the error $(q_\theta - f)$ is orthogonal to the tangent space of the trial manifold. This is **orthogonal projection** of $f$ onto the approximation space: $q_\theta = \Pi_w f$.

The connection to **Fitted Q-Iteration (FQI)** is now immediate. Taking $f = \Bellman q_k$ (the Bellman image of the current iterate), projection least-squares gives:

$$
q_{k+1} = \Pi_w \, \Bellman q_k.
$$

This is **not** minimum-residual iteration. FQI does not minimize $\|R(q)\|^2 = \|\Bellman q - q\|^2$ directly. Instead, it **projects**, then **iterates**: apply $\Bellman$, project the result back onto the approximation space, repeat. The distinction matters because minimum-residual requires differentiating through the Bellman operator (the "full gradient"), while projection uses only the gradient of $q_\theta$ (the "semi-gradient"). We return to this distinction when discussing temporal difference learning.

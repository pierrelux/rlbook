...
**Algorithm: Collocation with Newton's Method**
- **Input:** Collocation points $\{s_1, \ldots, s_n\}$, basis functions $\{\varphi_1, \ldots, \varphi_n\}$, initial guess $\theta^{(0)}$
- **Output:** Converged coefficients $\theta^*$
...
3. $\quad$ **for** $i = 1, \ldots, n$ **do**
4. $\quad\quad$ Compute $t_i^{(k)} = \max_{a \in \mathcal{A}_{s_i}} \left\{ r(s_i, a) + \gamma \sum_{j \in \mathcal{S}} p(j|s_i, a) \sum_{\ell=1}^n \theta_\ell^{(k)} \varphi_\ell(j) \right\}$
5. $\quad\quad$ Store optimal action $a_i^* \in \arg\max$ achieving the maximum
6. $\quad$ Compute Jacobian: $[J_{\mathrm{L}_\varphi}]_{ij} = \gamma \sum_{j \in \mathcal{S}} p(j|s_i, a_i^*) \varphi_j(j)$ for all $i,j$
7. $\quad$ Solve $(\boldsymbol{\Phi} - J_{\mathrm{L}_\varphi}) \Delta\theta = \boldsymbol{\Phi} \theta^{(k)} - t^{(k)}$
8. $\quad$ Update $\theta^{(k+1)} = \theta^{(k)} - \Delta\theta$
9. $\quad$ **if** $\|\Delta\theta\| < \varepsilon$ **then** stop
...
```
Initialize θ⁽⁰⁾
Compute mass matrix Mᵢⱼ = ∫ φᵢ(s) φⱼ(s) w(s) ds
For k = 0, 1, 2, ...
    For each test function φᵢ:
        Compute bᵢ⁽ᵏ⁾ = ∫ [Lv̂(s; θ⁽ᵏ⁾)] φᵢ(s) w(s) ds  (numerical integration)
    Solve M θ⁽ᵏ⁺¹⁾ = b⁽ᵏ⁾
    If converged, stop
```
...
For k = 0, 1, 2, ...
    For each test function φᵢ:
        Compute Fᵢ⁽ᵏ⁾ = ∫ [Lv̂(s; θ⁽ᵏ⁾) - v̂(s; θ⁽ᵏ⁾)] φᵢ(s) w(s) ds
        For each basis function φⱼ:
            Compute Jᵢⱼ = ∫ [γ 𝔼[φⱼ(s')|s,a*(s;θ⁽ᵏ⁾)] - φⱼ(s)] φᵢ(s) w(s) ds
    Solve J Δθ = F⁽ᵏ⁾
    Update θ⁽ᵏ⁺¹⁾ = θ⁽ᵏ⁾ - Δθ
    If converged, stop
```

This requires both numerical integration and tracking optimal actions across the state space—substantially more complex than collocation's pointwise evaluation.

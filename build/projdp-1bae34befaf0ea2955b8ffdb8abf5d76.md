...
This has analogous structure to the Bellman residual $\mathcal{R}(v) = \mathcal{T} v - v$: it is a functional equation where the unknown is an entire function. However, whereas the Bellman operator $\mathcal{T}$ is a $\gamma$-contraction in the supremum norm (guaranteeing convergence of value iteration and well-understood approximation error bounds), the **Euler operator is generally not a contraction**. Iterative Euler-equation methods (solving $\mathcal{R}_E(\pi)=0$ by successive approximation) may not converge without additional structure. Approximation error bounds must be derived problem-by-problem rather than following from general contraction theory. The weighted-residual framework applies (we can still project onto finite-dimensional policy classes), but convergence guarantees differ from Bellman methods.

Euler-equation methods are widely used in economics and continuous-state MDPs where they often admit analytic or semi-analytic solutions.

**Connection to the policy gradient theorem.** The derivation above resembles the policy gradient theorem, but for the Bellman optimality equation rather than policy evaluation. The policy gradient theorem differentiates the objective $J(\theta) = \mathbb{E}[v_\pi(s_0)]$ with respect to policy parameters, yielding:

$$
\nabla_\theta J(\theta) \propto \mathbb{E}_\pi \left[ \nabla_\theta \log \pi_\theta(a|s) \, Q_\pi(s,a) \right].
$$

This computes the value of a given policy $\pi_\theta$. The Euler equation characterizes the optimal policy by differentiating with respect to the action at each state. Danskin's theorem justifies ignoring how the optimal action might change locally when taking the derivative. Both use recursive structure: the policy gradient uses the fundamental matrix $(I - \gamma P_\pi)^{-1}$ to sum discounted visitation, while the Euler equation uses the envelope theorem to collapse future value derivatives. The difference is that the Euler equation characterizes optimality (first-order conditions) while policy gradients evaluate a given policy.

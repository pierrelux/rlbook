...
held fixed here. Each 0.1-second simulator step prioritizes decode or begins
with a prefill chunk capped at 512 tokens. If that chunk finishes early, the
remaining service budget may return to decode. Active decode receives
...
optimizer. In the shifted replay, the largest later ten-second burst, ranked by
service-time work at maximum clock, arrives twenty seconds earlier. No
...
2,482.40 in its weighted model units. Under the nominal replay, the requests
...
clock rises from 1 to 25 requests.

The disturbance does not make every metric worse. Energy falls from 2,484 to
2,209 joules, and whole-trace mean time to first token falls from 7.10 to 6.75

...
A later development analysis follows temperature continuously through cooldown,
memory relocking, and workload segments. Its two-state model carries an
unobserved heat-sink temperature through these transitions, instead of resetting
it to the measured junction temperature at each pulse. The fit compares
predicted temperatures directly with the sensor's one-degree-Celsius bins;
differentiating those quantized readings would amplify their steps.

This analysis treats all five historical acquisitions, including failed runs
and the earlier validation data, as development data. Each evaluation holds out
one complete acquisition. The table reports ten-second prediction error averaged
equally over the five held-out acquisitions, using measured future board power.
It measures the temperature model's error when its future input is known.

| Thermal model | Mean run-weighted ten-second RMSE (degrees C) |
|---|---:|
| One state | 0.678 |
| One state with phase gain | 0.679 |
| Two states | 0.609 |
| Two states with phase gain | 0.617 |

The two-state model improves prediction on the two complete historical runs,
with ten-second RMSEs of 0.49 and 0.76 degrees C. Its largest underpredictions on
those runs are still 1.78 and 2.48 degrees C. Across all five runs, its mean
improvement over the one-state model is only 0.069 degrees C. The fixed model
selection rule requires at least 0.1 degrees C and a ten-percent improvement to
add a state, so it retains the one-state candidate. The two-state phase-gain fit
also places its fastest mode below the effective sensor resolution.

Complete histories make the missing-state hypothesis testable, but they do not
yet establish an accurate online temperature forecast. That test requires fresh
acquisitions and power forecasts available when the controller acts. The revised
live experiment records these forecasts and uses measured-temperature watchdogs
independently of the fitted thermal model.

{download}`Download the complete thermal-history development report (JSON) <artifacts/inference_serving/live_control_development/thermal_history_development.json>`

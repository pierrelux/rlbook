...
multipliers become costates in the discrete-time Pontryagin principle. Since
...
$$
\begin{aligned}
\mathbf{z}^{k+1} &= \mathbf{z}^{k}-\alpha_k\big(\nabla F(\mathbf{z}^{k})+\nabla H(\mathbf{z}^{k})^{\top}\boldsymbol{\lambda}^{k}+\nabla G(\mathbf{z}^{k})^{\top}\boldsymbol{\mu}^{k}\big),\\[2mm]

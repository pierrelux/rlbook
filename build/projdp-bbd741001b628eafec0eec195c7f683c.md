...
Applying the envelope theorem to express $\nabla_s v^*(s')$ in terms of reward derivatives at $s'$ gives the policy-only form in equation (Euler). The deterministic case is common in economic applications (consumption-savings problems, growth models) where the state evolution is governed by an accounting identity like $k' = f(k,c)$.

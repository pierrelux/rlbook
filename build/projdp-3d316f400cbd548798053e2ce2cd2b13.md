...
To characterize this maximizer, fix a state $s$ and suppose the optimal value function $v^*$ is known. Define the objective function $Q^* : \mathcal{S} \times \mathcal{A} \to \mathbb{R}$ by:

$$
Q^*(s, a) := r(s, a) + \gamma \sum_{j \in \mathcal{S}} p(j|s,a) v^*(j).
$$

For fixed $s$, consider the function $a \mapsto Q^*(s, a)$ mapping actions to returns. The policy $\pi^*(s)$ must maximize this function over $\mathcal{A}$. If $a^*$ is a maximizer for this problem given $s$, and if $Q^*$ is differentiable in $a$, $\mathcal{A}$ is convex, and $a^*$ is interior, then we must have:

$$
\frac{\partial Q^*(s, a)}{\partial a}\Big|_{a=a^*} =  0 \Leftrightarrow \frac{\partial r(s, a)}{\partial a}\Big|_{a=a^*} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=a^*} v^*(j) = 0.
$$

Since the optimal policy satisfies $\pi^*(s) = a^*$ for each state $s$, this first-order condition holds at every state where the optimal action is interior. This gives us a characterization of optimality: a policy $\pi$ is optimal if and only if it satisfies

$$
\frac{\partial r(s, a)}{\partial a}\Big|_{a=\pi(s)} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi(s)} v^*(j) = 0, \quad \forall s \in \mathcal{S}.
$$

This is still expressed in terms of both the policy $\pi$ and the value function $v^*$. However, for problems where the envelope theorem applies, we can eliminate $v^*$ entirely through the following steps:

**Step 1: Apply the envelope theorem.** Recall the Bellman optimality equation:
$$
v^*(s) = \max_{a \in \mathcal{A}} \left\{ r(s, a) + \gamma \sum_{j \in \mathcal{S}} p(j|s,a) v^*(j) \right\}.
$$

For a continuously differentiable state space, the envelope theorem states that when differentiating the value of a maximization problem with respect to a parameter, we can ignore the dependence of the maximizer on that parameter. Differentiating $v^*(s)$ with respect to $s$ and evaluating at the maximizer $a = \pi^*(s)$:

$$
\frac{\partial v^*(s)}{\partial s} = \frac{\partial r(s, a)}{\partial s}\Big|_{a=\pi^*(s)} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial s}\Big|_{a=\pi^*(s)} v^*(j).
$$

This expresses the derivative of the value function in terms of the policy $\pi^*(s)$ and the value function $v^*(j)$ at successor states.

**Step 2: Recursive substitution.** The key insight is that $v^*(j)$ itself satisfies the Bellman equation:
$$
v^*(j) = r(j, \pi^*(j)) + \gamma \sum_{k \in \mathcal{S}} p(k|j,\pi^*(j)) v^*(k).
$$

Substituting this into the first-order condition:
$$
\frac{\partial r(s, a)}{\partial a}\Big|_{a=\pi(s)} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi(s)} \left[ r(j, \pi^*(j)) + \gamma \sum_{k} p(k|j,\pi^*(j)) v^*(k) \right] = 0.
$$

Expanding:
$$
\frac{\partial r(s, a)}{\partial a}\Big|_{a=\pi(s)} + \gamma \sum_{j} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi(s)} r(j, \pi^*(j)) + \gamma^2 \sum_{j,k} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi(s)} p(k|j,\pi^*(j)) v^*(k) = 0.
$$

Continuing this substitution for $v^*(k)$, $v^*(l)$, etc., we obtain an infinite series:
$$
\frac{\partial r(s, a)}{\partial a}\Big|_{a=\pi(s)} + \gamma \sum_{j} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi(s)} r(j, \pi^*(j)) + \gamma^2 \sum_{j,k} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi(s)} p(k|j,\pi^*(j)) r(k, \pi^*(k)) + \cdots = 0.
$$

This expresses the first-order condition purely in terms of the policy $\pi^*(\cdot)$, the reward function $r$, and the transition probabilities $p$—the value function has been eliminated entirely. Each term represents the marginal effect on expected rewards at successive time horizons.

The result is a **discrete-time Euler equation**: a functional equation in the policy $\pi(\cdot)$ alone, without explicit value function terms. The precise form depends on whether transitions are deterministic or stochastic, and whether the state space is discrete or continuous.

### The Euler Equation as a Functional Equation

For a discrete state space with stochastic transitions, define the Euler residual operator $\EulerResidual$ on the space of policies:

$$
\EulerResidual(\pi)(s) = \frac{\partial r(s, \pi(s))}{\partial a} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi(s)} v(j),
$$

where $v(j)$ can itself be expressed recursively in terms of the policy using the envelope theorem.
...
This has the same mathematical structure as the Bellman residual $\Residual(v) = \Bellman v - v$: it is a functional equation where the unknown is an entire function (here, a policy rather than a value function).

**Connection to the policy gradient theorem.** This derivation closely resembles the policy gradient theorem, but for the Bellman **optimality** equation rather than policy evaluation. Recall that the policy gradient theorem differentiates the objective $J(\theta) = \mathbb{E}[v_\pi(s_0)]$ with respect to policy parameters, yielding:

$$
\nabla_\theta J(\theta) \propto \mathbb{E}_\pi \left[ \nabla_\theta \log \pi_\theta(a|s) \, Q_\pi(s,a) \right].
$$

This applies to policy evaluation: computing the value of a **given** policy $\pi_\theta$. The Euler equation, in contrast, characterizes the **optimal** policy by differentiating the Bellman optimality operator. The envelope theorem is needed because we're taking derivatives through the $\max$ operation in $v(s) = \max_a \{r(s,a) + \gamma \sum_j p(j|s,a) v(j)\}$. At the optimum, the envelope theorem says we can ignore how the optimal action changes with $s$ when computing $\nabla_s v(s)$, just as we ignore how the maximizing action changes when differentiating a maximum. The result is a first-order condition characterizing the optimal policy directly, without explicitly computing the value function.
...
Given a parameterized policy $\pi_{\boldsymbol{\theta}}(s) = \sum_{i=1}^n \theta_i \varphi_i(s)$, we seek parameters $\boldsymbol{\theta}$ such that the Euler residual $\EulerResidual(\pi_{\boldsymbol{\theta}})$ is close to zero. The three weighted residual methods from earlier in this chapter apply directly:

**Collocation.** Choose states $\{s_1, \ldots, s_n\}$ and require the Euler equation to hold exactly at these points:

$$
\EulerResidual(\pi_{\boldsymbol{\theta}})(s_i) = 0, \quad i = 1, \ldots, n.
$$
...
$$
\langle \EulerResidual(\pi_{\boldsymbol{\theta}}), \varphi_i \rangle_w = \int_{\mathcal{S}} \EulerResidual(\pi_{\boldsymbol{\theta}})(s) \varphi_i(s) w(s) ds = 0, \quad i = 1, \ldots, n.
$$
...
$$
\boldsymbol{\theta}^* = \arg\min_{\boldsymbol{\theta}} \int_{\mathcal{S}} \EulerResidual(\pi_{\boldsymbol{\theta}})(s)^2 w(s) ds.
$$

The first-order conditions for this minimization problem yield orthogonality conditions with test functions $\frac{\partial \EulerResidual(\pi_{\boldsymbol{\theta}})}{\partial \theta_i}$, just as in the nonlinear value function case.

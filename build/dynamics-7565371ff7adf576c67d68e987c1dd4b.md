...
were developed to control this interaction {cite}`yu2022orca,agrawal2024sarathi`.
...
parameters. The experiment uses Qwen2.5-7B-Instruct served by vLLM on an NVIDIA
L4 {cite}`qwen25report,kwon2023vllm`. The intended hardware profile spans five
requested clock levels, several prompt lengths, and three concurrency levels.
...
:caption: Twenty requests from the Azure code-generation trace move through arrival, chunked prefill, autoregressive decode, and completion. The plots report only the trajectory prefix reached by the playhead. Output length is hidden from the controller until each request completes; cache occupancy, power, temperature, and realized clock come from the same Python simulation. A provenance badge distinguishes a measured profile from a pre-measurement engineering surrogate.
...
:class: pdf-fallback
:alt: Static view of requests progressing through prefill and decode beside cache, power, temperature, and clock traces.

Static view of the same request-level simulation. The online book provides
playback, stepping, scrubbing, and controller selection.
:::
...
request, processed token, and cache allocation. When L4 measurements are
present, held-out profile points test the interpolated service and power maps.
...
equations. The calibrated event simulator permits resets and counterfactual

...
The optimal policy is the Boltzmann distribution $\pi^*(a|s) = \exp(q^*(s,a)/\alpha) / \exp(v^*(s)/\alpha)$. Taking logarithms:
...
This is not an expectation: it holds exactly for **every** action $a$. The Boltzmann structure ensures that the advantage $q^*(s,a) - v^*(s)$ is precisely encoded in the log-probability.

...
## Bias Correction via Keane-Wolpin

We now turn to methods for addressing the overestimation bias introduced by maximizing over noisy Monte Carlo estimates. The first approach we examine is the Keane-Wolpin bias correction method, developed in the context of dynamic discrete choice models in econometrics {cite}`Keane1994`.

### The Bias Correction Strategy

The Keane-Wolpin approach treats bias correction as a two-stage procedure. In the first stage, we estimate both the value function and the bias function simultaneously using a parametric model. In the second stage, we subtract the estimated bias from our empirical Bellman updates to obtain approximately unbiased value estimates.

Recall that for a given value function $v$ and Monte Carlo sample size $N$, the bias at state $s$ is defined as

$$
b(s) = \mathbb{E}\big[(\widehat{\Bellman}_N v)(s)\big] - (\Bellman v)(s).
$$

If we could compute $b(s)$, we could correct our empirical updates by subtracting it:

$$
(\widehat{\Bellman}_N v)(s) - b(s)
$$

would then be an unbiased estimator of $(\Bellman v)(s)$. The problem, of course, is that we do not know $b(s)$ because we cannot compute the true expectation $\mathbb{E}[(\widehat{\Bellman}_N v)(s)]$ or the exact Bellman application $(\Bellman v)(s)$.

Keane and Wolpin observed that the bias $b(s)$ has structure. It depends on:

1. **The number of Monte Carlo samples $N$**: More samples reduce variance and thus reduce the bias from taking the maximum.
2. **The number of actions $|\mathcal{A}_s|$**: More actions increase the chance that at least one gets a lucky high draw.
3. **The variance of the Monte Carlo estimates**: Higher variance means larger deviations from the mean, amplifying the bias.

Rather than trying to compute $b(s)$ analytically, Keane and Wolpin proposed learning it empirically as part of the value function approximation process.

### Parametric Bias Model

The key insight is to approximate both the value function and the bias function using parametric forms. Suppose we approximate the value function as

$$
v(s) \approx \sum_i \theta_i \varphi_i(s),
$$

where $\{\varphi_i\}$ are basis functions. Similarly, approximate the bias as

$$
b(s) \approx \sum_j \beta_j \psi_j(s),
$$

where $\{\psi_j\}$ are (potentially different) basis functions for the bias. The bias basis functions should capture the relevant structure. Common choices include:

- **Variance-based features**: Since bias is related to the variance of the Monte Carlo estimates, include features that approximate this variance. For instance, if we compute the sample variance of the $N$ next-state evaluations for each action, we can use this as a feature.

- **Number of actions**: The bias increases with $|\mathcal{A}_s|$, so we might include $|\mathcal{A}_s|$ or $\log|\mathcal{A}_s|$ as a feature.

- **Polynomial features**: Include polynomial terms in the variance or number of actions to capture nonlinear relationships.

### The Algorithm

The Keane-Wolpin algorithm proceeds as follows:

```{prf:algorithm} Keane-Wolpin Bias Correction
:label: keane-wolpin-bias-correction

**Input:** MDP $(S, A, r, p, \gamma)$, basis functions $\{\varphi_i\}$ for value function, basis functions $\{\psi_j\}$ for bias, number of iterations $K$, Monte Carlo samples $N$

**Output:** Bias-corrected value function parameters $\theta$, bias function parameters $\beta$

1. Initialize $\theta_0$, $\beta_0$ (e.g., to zero)
2. **for** $k = 0, 1, \ldots, K-1$ **do**
3. $\quad$ **Collect training data:**
4. $\quad$ Sample a set of states $\{s_m\}_{m=1}^M$ from some distribution $\mu$
5. $\quad$ **for** each state $s_m$ **do**
6. $\quad\quad$ **for** each action $a \in \mathcal{A}_{s_m}$ **do**
7. $\quad\quad\quad$ Draw $N$ next states $s'_1, \ldots, s'_N \sim p(\cdot \mid s_m, a)$
8. $\quad\quad\quad$ Compute Monte Carlo estimate:
            $$\hat{q}_k(s_m, a) = r(s_m, a) + \frac{\gamma}{N} \sum_{n=1}^N v(s'_n; \theta_k)$$
9. $\quad\quad\quad$ Compute sample variance:
            $$\hat{\sigma}^2_k(s_m, a) = \frac{1}{N-1} \sum_{n=1}^N \left(v(s'_n; \theta_k) - \bar{v}_k(s_m, a)\right)^2$$
            where $\bar{v}_k(s_m, a) = \frac{1}{N}\sum_{n=1}^N v(s'_n; \theta_k)$
10. $\quad\quad$ **end for**
11. $\quad\quad$ Compute empirical Bellman update:
        $$y_m = \max_{a \in \mathcal{A}_{s_m}} \hat{q}_k(s_m, a)$$
12. $\quad\quad$ Compute bias features $\psi(s_m)$ using:
        - Maximum action variance: $\max_a \hat{\sigma}^2_k(s_m, a)$
        - Number of actions: $|\mathcal{A}_{s_m}|$
        - Other relevant features
13. $\quad$ **end for**
14. $\quad$ **Estimate bias-corrected value function:**
15. $\quad$ Solve the regression problem:
    $$\min_{\theta, \beta} \sum_{m=1}^M \left(y_m - b(s_m; \beta) - v(s_m; \theta)\right)^2$$
    where $b(s_m; \beta) = \sum_j \beta_j \psi_j(s_m)$ and $v(s_m; \theta) = \sum_i \theta_i \varphi_i(s_m)$
16. $\quad$ Set $\theta_{k+1} = \theta$, $\beta_{k+1} = \beta$
17. **end for**
18. **return** $\theta_K$, $\beta_K$
```

### Understanding the Regression Step

The regression at line 15 deserves explanation. We observe noisy targets $y_m = (\widehat{\Bellman}_N v_k)(s_m)$, which we know are biased estimators of $(\Bellman v_k)(s_m)$. We model this as

$$
y_m = (\Bellman v_k)(s_m) + b(s_m) + \varepsilon_m,
$$

where $b(s_m)$ is the systematic bias and $\varepsilon_m$ is zero-mean noise. If we approximate $(\Bellman v_k)(s_m) \approx v(s_m; \theta)$ (since $\Bellman v_k \approx v_k$ when $v_k$ is close to the fixed point) and $b(s_m) \approx b(s_m; \beta)$, then we have

$$
y_m \approx v(s_m; \theta) + b(s_m; \beta) + \varepsilon_m.
$$

This is a linear regression problem where the target $y_m$ is modeled as the sum of a value function component and a bias component. By fitting both simultaneously, we can separate the systematic overestimation (captured by $b$) from the true value (captured by $v$).

Once we have estimated $\beta$, we can use the bias-corrected value function $v(s; \theta)$ in subsequent iterations, effectively removing the systematic overestimation.

### Properties and Limitations

**Advantages:**

1. **Theoretically motivated**: The method directly addresses the source of the problem by modeling and subtracting the bias.

2. **Flexible**: The choice of bias features can be tailored to the problem. If we understand the structure of the bias (e.g., its relationship to variance or the number of actions), we can incorporate this knowledge.

3. **Simultaneous estimation**: By fitting $\theta$ and $\beta$ together, the method naturally balances the value and bias estimates.

**Limitations:**

1. **Requires variance computation**: The method needs to compute sample variances, which requires storing all $N$ next-state samples or making multiple passes. This increases memory and computational costs.

2. **Model misspecification**: If the parametric form for $b(s; \beta)$ is incorrect, the bias correction can be ineffective or even harmful. The relationship between bias and variance is complex and may not be captured by simple parametric forms.

3. **Bias in the bias estimate**: The bias function $b$ itself is estimated from data and thus subject to estimation error. If the bias estimate is noisy, subtracting it can increase variance even while reducing bias.

4. **Circular dependency**: The bias depends on the current value function $v_k$, which we are trying to estimate. This creates a bootstrapping situation where errors in $v$ affect the bias estimate, which in turn affects the next value estimate.

5. **Limited adoption in RL**: While influential in econometrics for estimating dynamic discrete choice models, the Keane-Wolpin method has seen limited use in the reinforcement learning community. This is partly due to its computational overhead and partly because alternative approaches (such as double Q-learning, which we discuss next) offer simpler ways to mitigate overestimation.

The Keane-Wolpin algorithm represents an explicit parametric approach to bias correction. It treats bias estimation as a supervised learning problem where we learn a function that predicts the systematic overestimation from observable features. This contrasts with the implicit approaches we will encounter next, which avoid bias by changing how we use value estimates rather than by explicitly estimating and subtracting the bias.

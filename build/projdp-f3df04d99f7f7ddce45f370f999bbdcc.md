...
The key insight is that minimizing a norm $\|R[v_{\boldsymbol{\theta}}]\|^2$ is equivalent to requiring orthogonality $\langle R[v_{\boldsymbol{\theta}}], \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \rangle = 0$ for all $i$, where the inner product $\langle \cdot, \cdot \rangle$ defines the norm via $\|f\|^2 = \langle f, f \rangle$. This equivalence holds for any choice of inner product: weighted $L^2$ integrals for Galerkin, sums over collocation points for collocation, or sampled expectations for neural networks. The **dual pairing** formulation {cite}`LegrandJunca2025` provides the abstract framework unifying all these cases. The "inner product" $\langle R[v], w \rangle$ is understood as the pairing between a residual $R[v]$ (living in a dual space $\mathcal{H}'$) and a test function $w$ (living in $\mathcal{H}$). For weighted integrals, this is $\int R[v](x) w(x) \mu(x) dx$. For collocation with Dirac deltas (which we presented earlier as "functions" without dwelling on technicalities), it is $\langle R[v], \delta(x - x_i) \rangle = R[v](x_i)$. The norm induced by this pairing is $\|R[v]\|_{\mathcal{H}'} = \sup_{\|w\|=1} |\langle R[v], w \rangle|$, which measures the residual's size by its worst-case effect on test functions.

Physics-informed neural networks (PINNs) apply this framework to Bellman equations. While deep RL typically uses temporal-difference methods (which we cover in the next chapter), PINNs directly minimize the Bellman residual:

$$
\mathcal{L}(\boldsymbol{\theta}) = \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - (\Bellman v_{\boldsymbol{\theta}})(s))^2 \big].
$$

This is exactly the minimum residual principle: the loss measures $\|R[v_{\boldsymbol{\theta}}]\|_{L^2(\mu)}^2$ in a sampled approximation. Gradient descent on this loss implements the orthogonality condition $\langle R[v_{\boldsymbol{\theta}}], \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \rangle = 0$ where the inner product is the $L^2(\mu)$ integral approximated by sampling. The connection to weighted residual methods clarifies what this training procedure is computing: a nonlinear projection of the Bellman equation onto the neural network manifold.

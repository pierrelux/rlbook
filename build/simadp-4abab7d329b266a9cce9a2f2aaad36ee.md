...
This is the principle behind double Q-learning {cite}`van2016deep`. For parametric Q-function approximation with two sets of parameters $\theta^{(1)}$ and $\theta^{(2)}$, we maintain two Q-functions $q^{(1)}(s,a; \theta^{(1)})$ and $q^{(2)}(s,a; \theta^{(2)})$. During value iteration, we alternate which function selects actions and which evaluates them. When updating $q^{(1)}$, we fit to minimize

$$
\sum_{(s,a) \in \mathcal{D}} \big(q^{(1)}(s,a; \theta^{(1)}) - Y^{(1)}(s,a)\big)^2,
$$

where the targets $Y^{(1)}(s,a)$ use $q^{(1)}$ for selection but $q^{(2)}$ for evaluation. Symmetrically for $q^{(2)}$. If the two function approximators have approximately independent errors at the next states (through independent initialization or different update schedules), then the action selection and evaluation steps use effectively independent noise, removing the main source of maximization bias.

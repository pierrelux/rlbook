...
def _validate_measured_profile_bundle(
    ...
    if (
        ...
        raise ValueError("measured profile must contain at least four usable clock levels")
    aggregate_clock = np.asarray([float(row["clock_mhz"]) for row in rows], dtype=float)
    if aggregate_clock.size < 4 or not np.allclose(aggregate_clock, selected):
        raise ValueError(
            "measured aggregate clock levels do not match the completed manifest"
        )
    ...
    for name, bundle_path in bundle_paths.items():
        expected = str(checksums.get(name, ""))
    ...
    realized_by_requested: dict[float, list[float]] = {
        float(clock): [] for clock in selected
    }
    ...
    for row in raw_rows:
        ...
        if not np.isfinite(requested) or not np.isfinite(realized) or realized <= 0.0:
            raise ValueError("measured requested/realized clocks must be finite and positive")
        if requested not in realized_by_requested:
            raise ValueError("raw profile contains a requested clock absent from the manifest")
        realized_by_requested[requested].append(realized)
        condition = (
            str(row["phase"]),
        ...
        matrix.setdefault(key, {})[repeat] = matrix.setdefault(key, {}).get(repeat, 0) + 1
    for requested in selected:
        for condition in _EXPECTED_PROFILE_CONDITIONS:
            repetitions = matrix.get((float(requested), *condition), {})
    ...
    realized_medians = np.array(
        [np.median(realized_by_requested[float(clock)]) for clock in selected],
        dtype=float,
    )
    if np.any(np.diff(realized_medians) <= 0.0):
        raise ValueError(
            "measured realized clocks do not provide four monotonically usable levels"
        )
...
def load_profile(path: str | Path) -> PerformanceProfile:
    ...
    if status == "measured_l4":
        ...
        _validate_measured_profile_bundle(profile_path, rows, manifest_path, manifest)
...
def simulate(
    ...
    *,
    ...
    previous_power = plant.profile.power("idle", previous_clock)
    ...
    while time_s <= plant.maximum_simulation_time_s + 1e-12:
        ...
        if phase == "prefill":
            prefill_rate = plant.profile.rate("prefill", realized_clock)
            budget = min(
            ...
            if active and remaining_time > 1e-12:
                decode_budget = (
                    plant.profile.rate("decode", realized_clock) * remaining_time
                )
                ...
                decode_service_time = (
                    decode_served
                    / max(plant.profile.rate("decode", realized_clock), 1e-12)
                )
            ...
            decode_rate = plant.profile.rate("decode", realized_clock)
        ...
        step_energy = (
            plant.profile.power("prefill", realized_clock) * prefill_service_time
            + plant.profile.power("decode", realized_clock) * decode_service_time
            + plant.profile.power("idle", realized_clock) * idle_time
        )

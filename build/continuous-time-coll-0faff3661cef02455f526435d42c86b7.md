...
$$
V_{ij}=\phi_j(\sigma_i),
\qquad \mathbf{y}=V\mathbf \mathbf{a}.
$$
...
:caption: One quadratic, two coordinate systems. Evaluating the monomial coefficients $\mathbf{a}$ at the support nodes gives $\mathbf{y}=V\mathbf \mathbf{a}$. Direct collocation stores the nodal vector $\mathbf{y}$; the fixed operators $D$ and $w$ then return its nodal derivatives and exact integral without adding optimization variables.
...
with plt.rc_context({
    ...
    canvas.text(0.682, 0.715, r"$\mathbf{y}=V\mathbf \mathbf{a}$", color=blue,
    ...
    canvas.text(0.465, 0.17,
                r"$p'(\sigma_i)=(D\mathbf \mathbf{y})_i,\qquad D\mathbf \mathbf{y}=(2,\;1,\;0)^{\mathsf{T}}$",
                color=ink, fontsize=10, ha="center", transform=canvas.transAxes)
...
| Algebraic problem | Satisfy $A\mathbf \mathbf{a}=\mathbf{y}$ exactly | Minimize $\lVert A\mathbf \mathbf{a}-\mathbf{y}\rVert_2^2$ |
...
$$
\mathbf{z}=\operatorname{col}\!\left(\{\mathbf{x}_k\}_{k=0}^{N},\{\mathbf{x}_{k,j},\mathbf{u}_{k,j}\}_{k=0,\,j=1}^{N-1,\,s}\right).
$$
...
matrix shorthand, all nodal constraints are $D\mathsf X_k=h_k\mathsf F_k$. The right endpoint
...
4. **Match the stored right endpoint.** Setting $\mathbf{p}_k(1)=\mathbf{x}_{k+1}$ gives the
   ...
   $$
   \begin{aligned}
   \mathbf{x}_{k+1}-\mathbf{x}_k-h_k\mathbf{f}(\mathbf{x}_k,\mathbf{u}_k,t_k)&=\mathbf0
   &&\text{(explicit Euler)},\\
   \mathbf{x}_{k+1}-\mathbf{x}_k-h_k\mathbf{f}(\mathbf{x}_{k+1},\mathbf{u}_{k+1},t_{k+1})&=\mathbf0
   &&\text{(implicit Euler)}.
   \end{aligned}
   $$
...
Choose the endpoint collocation nodes $\tau_1=0$ and $\tau_2=1$, and abbreviate the
two ODE slopes by
...
3. Recover the monomial coefficient vector from $V\mathbf \mathbf{a}=\mathbf{y}$.
````
...
Solving $V\mathbf \mathbf{a}=\mathbf{y}$ returns $\mathbf{a}=(2,-1,2)^\mathsf T$. The solve changes coordinates; it does not construct a different polynomial.
````
...
For $\mathbf{y}=(1,\tfrac74,2)^\mathsf T$, $D\mathbf \mathbf{y}=(2,1,0)^\mathsf T$, which equals $p'(\tau)=2-2\tau$ at the three nodes.
````

...
The trajectory is represented by a separate polynomial on each mesh interval
$[t_k,t_{k+1}]$. The full approximation $x_h$ is therefore **piecewise
polynomial**. To construct the piece on interval $k$, use its local coordinate
$\tau=(t-t_k)/h_k$, where $h_k=t_{k+1}-t_k$, and write

$$
p_k(\tau):=x_h(t_k+h_k\tau),\qquad 0\le\tau\le1.
$$

Thus $p_k(0)$ is the state at the start of this interval and $p_k(1)$ is the
state at its end. On the next interval, the local coordinate starts again at
zero, and a different polynomial $p_{k+1}$ describes the trajectory. The
reference interval and its integration weights can be reused even when the
physical intervals have different lengths.

Choose $s$ collocation nodes $c_1,\ldots,c_s$ on $[0,1]$. On interval $k$,
node $c_j$ corresponds to the physical time $t_k+h_kc_j$. The state and
control values there are $X_{k,j}$ and $U_{k,j}$. This local evaluation point,
...
with the nodes $c_1,\ldots,c_s$. Interpolating these slopes gives the
...
$X_k$, constructs the entire state piece:

$$
p_k(\tau)
=X_k+h_k\sum_{j=1}^{s}
\left(\int_0^\tau\ell_j(\eta)\,d\eta\right)F_{k,j}.
$$

The factor $h_k$ converts integration in normalized time into integration in
physical time: $dt=h_k\,d\tau$. For example, a constant physical slope $F$
acting from $\tau=0$ to $\tau=c_i$ acts for $h_kc_i$ units of time and
changes the state by $h_kc_iF$. Without $h_k$, the formula would treat every
physical interval as having unit duration. Equivalently, the chain rule gives
$p_k'(\tau)=h_k\dot x_h(t_k+h_k\tau)$.

The stored stage state $X_{k,i}$ must lie on this polynomial at $c_i$.
Imposing $X_{k,i}=p_k(c_i)$ gives the **stage equations**
...
$p_k(1)$ to equal the stored right endpoint $X_{k+1}$:
...
$c_1=1/3$ and $c_2=2/3$. Tangent marks in the lower panel have slopes
$p_k'(c_j)=h_kF_{k,j}$ because that axis uses normalized time. The stage
...
Introduce a scalar state $q(t)$ that records the cost accumulated since
$t_0$:

$$
q(t)=\int_{t_0}^{t}L(x(s),u(s),s)\,ds,
\qquad
\dot q(t)=L(x(t),u(t),t),\qquad q(t_0)=0.
$$

The original objective is now $\Phi(x(t_f),t_f)+q(t_f)$. The augmented state
$(x,q)$ has two rates of change: $f$ supplies the physical state rate and $L$
...
$$
L_{k,j}:=L(X_{k,j},U_{k,j},t_k+h_kc_j).
$$

Let $Q_k$ and $Q_{k,i}$ denote the accumulated-cost values at the interval's
left endpoint and at stage $i$. Integrating the cost-rate interpolant to a
stage and to the right endpoint gives

$$
Q_{k,i}=Q_k+h_k\sum_{j=1}^{s}A_{ij}L_{k,j},
\qquad
Q_{k+1}=Q_k+h_k\sum_{j=1}^{s}b_jL_{k,j}.
$$

These are the same stage and endpoint equations as for $x$, with $L_{k,j}$
in place of $F_{k,j}$. The increase in accumulated cost across interval $k$
therefore supplies its contribution to the discrete objective:

$$
J_k:=Q_{k+1}-Q_k
=h_k\sum_{j=1}^{s}b_jL_{k,j}
\approx\int_{t_k}^{t_{k+1}}L(x_h(t),u_h(t),t)\,dt.
$$

Here $u_h$ is the piecewise control approximation. The weighted sum integrates
...
$L(x_h(t),u_h(t),t)$ need not itself be that polynomial. Such a weighted-sum
approximation is called a **quadrature rule**.

Since $Q_0=0$, summing the interval increments gives
$Q_N=\sum_{k=0}^{N-1}J_k$. The accumulated-cost variables can therefore be
eliminated and the sum used directly in the objective. Introducing $q$ explains
...
Consider $\dot x=f(x,u,t)$ on $[t_k,t_{k+1}]$, with duration $h_k$ and
normalized time $\tau=(t-t_k)/h_k$. Approximate the physical-time derivative
by a constant that matches the ODE at a single collocation node $c_1$.

Carry out the construction for $c_1=0$ and then for $c_1=1$. For each choice:

1. Construct the cardinal function and identify the ODE slope it multiplies.
2. Integrate from $X_k$ to obtain the state polynomial $p_k(\tau)$, then
   impose its agreement with the stored right endpoint $X_{k+1}$.
...
2. **Interpolate the physical-time derivative.** With one node, the cardinal
   ...
   that node. Hence $\dot x_h(t_k+h_k\tau)=F_{k,1}$ throughout the interval.
...
3. **Integrate from the left state.** The reference-time derivative is
   $p_k'(\tau)=h_kF_{k,1}$. Integrating and imposing $p_k(0)=X_k$ gives

   $$
   p_k(\tau)=X_k+h_k\tau F_{k,1}.
   $$

   Thus the state polynomial has degree at most one. The factor $h_k$
   accounts for the physical duration of the interval.

4. **Match the stored right endpoint.** Setting $p_k(1)=X_{k+1}$ gives the
   defect $X_{k+1}-X_k-h_kF_{k,1}=0$. Substituting the two slope choices yields

   $$
   \begin{aligned}
   X_{k+1}-X_k-h_kf(X_k,U_k,t_k)&=0
   &&\text{(explicit Euler)},\\
   X_{k+1}-X_k-h_kf(X_{k+1},U_{k+1},t_{k+1})&=0
   &&\text{(implicit Euler)}.
   \end{aligned}
   $$
   ...
   $f$, so it generally requires solving an equation, which is nonlinear
...
5. **Apply the same integration rule to the cost rate.** The single sampled
   ...
   $$
   J_k=\begin{cases}
   h_kL(X_k,U_k,t_k),&c_1=0,\\
   h_kL(X_{k+1},U_{k+1},t_{k+1}),&c_1=1.
   \end{cases}
   $$

...
each second. The scheduling rule remains fixed at 512-token chunked prefill, so
this experiment isolates feedback through the GPU clock.
...
$\delta_{k,\mathrm{TPOT}}$ measure normalized latency overruns. The slacks
$s_{P,k}$ and $s_{T,k}$ penalize violations of the experimental power and
thermal limits, and $B_{t+10}$ penalizes unfinished work at the end of the
horizon. SLSQP receives at most 50 iterations and a tolerance of $10^{-6}$.
...
:caption: Maximum-clock, reactive, open-loop, and MPC frequency control under the same shifted request trace and fixed chunked-prefill scheduler. At each second, the dashed MPC segment is the current ten-second plan and the solid segment is the action history. Later plans are not revealed before they are computed.
...
```{code-cell} python
:tags: [remove-input]
:label: tbl-inference-mpc-metrics
:caption: Closed-loop metrics for the four frequency controllers under identical shifted arrivals and the fixed chunked-prefill scheduler. Fallback count records every MPC step that exceeded its feasibility, finiteness, or timing requirement.

import pandas as pd

pd.read_csv("artifacts/inference_serving/metrics_mpc.csv")
```
...
about every serving stack. A provisional profile tests the controller and
fallback logic without supplying measured hardware evidence. The arrival

...
$$
J(\theta) = \mathbb{E}_{x \sim p(x;\theta)}[f(x)] = \mathbb{E}_{x \sim q(x)}\left[f(x) \cdot \rho(x; q, \theta)\right]
$$

where $\rho(x; q, \theta) = \frac{p(x;\theta)}{q(x)}$ is the likelihood ratio. The partial derivative of $\rho$ with respect to $\theta$, holding $x$ fixed, is:

$$
\frac{\partial \rho(x; q, \theta)}{\partial \theta} = \frac{1}{q(x)} \frac{\partial p(x;\theta)}{\partial \theta} = \frac{p(x;\theta)}{q(x)} \frac{\partial \log p(x;\theta)}{\partial \theta} = \rho(x; q, \theta) \nabla_\theta \log p(x;\theta)
$$

This is a partial derivative, not a total derivative: we treat $x$ as a fixed sample, not as a function of $\theta$. The sample $x$ was drawn from $q$, which does not depend on $\theta$, so there is no indirect dependence to account for. Taking the gradient of $J$:

$$
\nabla_\theta J(\theta) = \mathbb{E}_{x \sim q(x)}\left[f(x) \frac{\partial \rho(x; q, \theta)}{\partial \theta}\right] = \mathbb{E}_{x \sim q(x)}\left[f(x) \rho(x; q, \theta) \nabla_\theta \log p(x;\theta)\right]
$$

The REINFORCE estimator corresponds to the special case $q(x) = p(x;\theta)$, where $\rho = 1$ and we recover the score function estimator. But the framework is more general: we can use any $q$ whose support contains the support of $p$, and the likelihood ratio $\rho$ corrects for the distribution mismatch.

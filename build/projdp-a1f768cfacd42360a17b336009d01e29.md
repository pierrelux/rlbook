...
Initialize θ⁽⁰⁾
For k = 0, 1, 2, ...
    For each collocation point sᵢ:
        Compute tᵢ⁽ᵏ⁾ = max_{a} { r(sᵢ,a) + γ Σⱼ p(j|sᵢ,a) Σₗ θₗ⁽ᵏ⁾ φₗ(j) }
    Solve Φ θ⁽ᵏ⁺¹⁾ = t⁽ᵏ⁾
    If converged, stop
```
...
For k = 0, 1, 2, ...
    For each collocation point sᵢ:
        Compute tᵢ⁽ᵏ⁾ = max_{a} { r(sᵢ,a) + γ Σⱼ p(j|sᵢ,a) Σₗ θₗ⁽ᵏ⁾ φₗ(j) }
        Store optimal action aᵢ* achieving the max
    Build Jacobian: [J_Lφ]ᵢⱼ = γ Σⱼ p(j|sᵢ,aᵢ*) φⱼ(j)
    Solve (Φ - J_Lφ) Δθ = Φ θ⁽ᵏ⁾ - t⁽ᵏ⁾
    Update θ⁽ᵏ⁺¹⁾ = θ⁽ᵏ⁾ - Δθ
    If converged, stop
```

...
The previous chapter examined bias mitigation strategies (Keane-Wolpin correction, double Q-learning) that address overestimation in Monte Carlo value iteration. Those techniques operate on the algorithmic structure itself, modifying how we select actions or correct targets. A complementary approach modifies the supervised learning step that appears in every value-based reinforcement learning algorithm. At each iteration, we construct a regression problem: given state-action pairs $(s_i, a_i)$ and Bellman targets $y_i = r_i + \gamma \max_{a'} q(s'_i, a')$, fit a Q-function approximator $q(s,a; \boldsymbol{\theta})$ to match these targets. This `fit` operation abstracts the supervised learning step, taking a dataset $\mathcal{D}^{\text{fit}} = \{((s_i, a_i), y_i)\}$ and returning parameters $\boldsymbol{\theta}$ via optimization:

$$
\boldsymbol{\theta} = \texttt{fit}(\mathcal{D}^{\text{fit}}, \boldsymbol{\theta}_{\text{init}}, K)
$$

Two parameters control the optimization. The initialization $\boldsymbol{\theta}_{\text{init}}$ can cold-start (reset to $\boldsymbol{\theta}_0$ each iteration) or warm-start (continue from the previous solution $\boldsymbol{\theta}_n$). The truncation $K$ specifies optimization steps: $K=\infty$ runs to convergence, finite $K$ performs partial optimization, and $K=1$ makes a single update. For neural networks, $K$ counts gradient steps. For tree ensembles, many implementations simply build a fixed ensemble ($K=\infty$). For linear models with squared loss, the closed-form normal equations give the exact solution in one step.

For neural networks, the `fit` operation minimizes an explicit loss function $\mathcal{L}(\boldsymbol{\theta}; \mathcal{D}^{\text{fit}})$ via gradient descent. Unlike tree-based methods where the objective is implicit in splitting criteria, neural networks require us to specify what to minimize. The choice of loss function encodes assumptions about the statistical relationship between state-action pairs and Bellman targets. Different loss functions correspond to different implicit noise models, and in deep reinforcement learning, where targets are noisy and network capacity is high, this choice can substantially affect performance.
...
We can now state the algorithmic template that encompasses all value-based reinforcement learning methods. This template instantiates the abstract framework by specifying how data flows through the two-level structure at each iteration:

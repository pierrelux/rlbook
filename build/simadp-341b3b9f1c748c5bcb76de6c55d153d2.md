...
In the previous chapter on projection methods, we showed how to represent value functions using finite basis expansions $v(s) \approx \sum_{i=1}^n \theta_i \varphi_i(s)$ and how to determine coefficients by making the residual $R(s) = \Bellman v(s) - v(s)$ small according to various criteria (Galerkin, collocation, least squares). But we left one question unresolved: **how do we actually evaluate the Bellman operator?**

Recall that applying the Bellman operator requires computing an expectation:

$$ (\Bellman v)(s) = \max_{a \in \mathcal{A}_s} \left\{r(s,a) + \gamma \int v(s')p(ds'|s,a)\right\} $$

When the state space is discrete, this is just a finite sum. For continuous state spaces, we need **numerical integration**. The challenge is that we must integrate a function ($v(s')$) that we're representing approximately using basis functions.

### Two Independent Choices That Must Be Coordinated

There are actually **two distinct approximations** at work:

1. **Value function approximation**: How we represent $v(s) \approx \hat{v}(s; \theta) = \sum_i \theta_i \varphi_i(s)$
2. **Quadrature approximation**: How we approximate $\int v(s')p(s'|s,a)ds' \approx \sum_j w_j v(s'_j)$

These are independent choices that should be coordinated for accuracy and efficiency:

- **For piecewise constant approximations** (finite elements with constant basis on each cell), midpoint or trapezoidal rules are natural
- **For polynomial approximations** (Chebyshev, Legendre), Gaussian quadrature matched to the polynomial family gives spectral accuracy
- **For collocation methods**, the collocation points can double as quadrature nodes (orthogonal collocation)

The key principle: **the quadrature rule should be accurate enough that integration errors don't dominate the projection error**. There's no point using 10th-order polynomials for $v$ if you're using crude rectangle rules to integrate.

### Example: A Simple Discretization Approach

To make this concrete, consider a simple approach using **piecewise constant approximations**. Partition the state space into $N$ cells with centers $\{s_i\}_{i=1}^N$. Approximate the value function as constant on each cell: $v(s) \approx v(s_{k(s)})$ where $k(s)$ is the cell containing $s$.

With this approximation, the integral in the Bellman operator decomposes naturally:
...
where $P_j(s,a) = \int_{cell_j} p(s'|s, a)ds'$ is the probability of transitioning into cell $j$. This reduces the continuous problem to a finite-state MDP with $N$ states and transition probabilities $P_j(s_i, a)$.

This approach is simple but suffers from the **curse of dimensionality**: achieving error $\epsilon$ requires $O(\epsilon^{-d})$ cells in dimension $d$. More sophisticated approximations (higher-order elements, spectral methods) converge faster but still face exponential scaling in dimension.

### Connection to Projection Methods: Orthogonal Collocation

The projection methods from the previous chapter provide a systematic way to coordinate these two approximations. **Orthogonal collocation** achieves particularly elegant coordination:

1. **Choose a polynomial basis**: For example, Chebyshev polynomials $\{T_0(s), T_1(s), \ldots, T_{n-1}(s)\}$
2. **Collocation points = quadrature nodes**: Place collocation points at the zeros of $T_n(s)$, which are also the optimal nodes for Gauss-Chebyshev quadrature
3. **Impose the residual condition**: $R(s_i; \theta) = 0$ at each collocation point

This coordination means:
- The projection method (collocation) determines where we evaluate the Bellman operator
- The same points serve as quadrature nodes for evaluating the integral in $\Bellman v$
- For smooth problems, Gaussian quadrature integrates polynomials of degree up to $2n-1$ exactly

The result is a **pseudospectral method**: the quadrature accuracy matches the approximation order of the basis, and we get exponential convergence $O(e^{-cn})$ for smooth problems.

**Other basis-quadrature pairings** follow similar principles:
- Legendre polynomials + Gauss-Legendre quadrature
- Hermite polynomials + Gauss-Hermite quadrature (natural for Gaussian shocks)
- B-splines + piecewise Gaussian quadrature on each element

The key insight is that **the basis choice in the projection method (projdp.md) should inform the quadrature rule used to evaluate the Bellman operator**. Using mismatched approximations—say, high-order Chebyshev basis with crude midpoint quadrature—wastes the accuracy potential of the projection method.

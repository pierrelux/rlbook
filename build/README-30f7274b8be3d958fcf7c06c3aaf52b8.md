...
It records SSE receipt times and token IDs; streamed chunks are not asserted to be
...
lead time from decision availability. Logged power forecasts include one extra
interval holding the terminal planned clock, explicitly marked as a forecast
assumption; the optimizer still plans ten steps. Horizons without future coverage
are unavailable, not padded. Neither the launcher nor these builders publishes
the book.

...
Continue this telescoping for $d$ steps. Each time we expand a value $v^*(s_t)$ using an observed action $a_t$, we introduce sampling noise because $q^*(s_t,a_t) - \alpha\log\pi^*(a_t|s_t)$ differs from the expectation $v^*(s_t) = \mathbb{E}_{a_t}[q^*(s_t,a_t) - \alpha\log\pi^*(a_t|s_t)]$ by a zero-mean error. After $d$ steps:

$$
v^*(s_0) = \sum_{t=0}^{d-1} \gamma^t[r_t - \alpha\log\pi^*(a_t|s_t)] + \gamma^d v^*(s_d) + \underbrace{\sum_{t=0}^{d-1} \gamma^t \varepsilon_t}_{\text{sampling noise}}
$$

where $\mathbb{E}_{a_0,\ldots,a_{d-1} \sim \pi^*}[\varepsilon_t] = 0$. Define the **path consistency residual**:

$$
R(s_0:s_d; \pi^*, v^*) = v^*(s_0) - \gamma^d v^*(s_d) - \sum_{t=0}^{d-1} \gamma^t[r_t - \alpha\log\pi^*(a_t|s_t)]
$$ (eq:path-residual)

For a single observed trajectory, $R = \sum_t \gamma^t \varepsilon_t$ (accumulated noise). But **in expectation**: $\mathbb{E}[R(s_0:s_d)] = 0$ when actions are sampled from $\pi^*$.

**The key insight**: This constraint holds for **any** observed action sequence, regardless of which policy generated it. The behavior policy never appears—we only evaluate $\pi^*$ at the observed actions. This enables off-policy learning without importance sampling.

...
Recall that standard value iteration for computing the optimal value function is:

$$
v_{k+1}(s) = \Bellman v_k(s) = \max_a \left[r(s,a) + \gamma \int p(s'|s,a) v_k(s') ds'\right], \quad \text{for all } s.
$$

This defines $v_{k+1}$ pointwise at every state. For a finite discrete state space, we can implement this exactly: maintain a vector of values, apply the Bellman operator element-by-element, update the vector. But for continuous or very large state spaces, we cannot store $v_k(s)$ for all $s$. We must work with a finite-dimensional approximation.

The solution is to separate the iteration into two nested loops:

1. **Outer loop (successive approximation)**: Iterate $k = 0, 1, 2, \ldots$ applying the Bellman operator
2. **Inner loop (fitting)**: At each iteration, fit an approximation to represent the result

Even though we cannot represent $v_k$ exactly on the entire domain, we **can evaluate the Bellman operator at any individual state**. Given an approximate $\hat{v}_k$ and a state $s$, we can compute $(\Bellman \hat{v}_k)(s)$ by performing the maximization and integration numerically at that one point. The obstacle is not evaluation. It is **representation**. We cannot carry the full assignment $s \mapsto (\Bellman \hat{v}_k)(s)$ forward to the next iteration. We need to compress this information into a finite-dimensional approximation.

This leads to fitted-value iteration: choose sample states, evaluate the Bellman operator at those states to get target values, then fit a function to those targets. The Bellman operator evaluation is the functional equation at work. The fitting step is where approximation enters.

### From Projection to Fitting

Recall the abstract framework from earlier: we studied iterations of the form $v_{k+1} = \Proj \Bellman v_k$, where $\Proj$ projects the result of the Bellman operator onto an approximation space. The projection operator $\Proj$ takes an arbitrary function (in this case, $\Bellman v_k$) and returns its best approximation in the subspace $\text{span}(\boldsymbol{\Phi})$.

But here is the practical obstacle: we cannot compute $\Bellman v_k$ as a complete function over the entire domain. We can only evaluate it at individual sample points $s_1, \ldots, s_n$. So instead of having the full function $\Bellman v_k$ to project, we have a dataset:

$$
\{(s_1, (\Bellman v_k)(s_1)), \ldots, (s_n, (\Bellman v_k)(s_n))\}.
$$

The question becomes: how do we construct a function from this dataset? The answer is **fitting**. We introduce a fitting operator $\mathtt{fit}$ that takes state-value pairs and produces a function in some class $\mathcal{F}$:

$$
\hat{v}_{k+1} = \mathtt{fit}\big(\{(s_i, (\Bellman \hat{v}_k)(s_i))\}_{i=1}^n; \mathcal{F}\big).
$$

When $\mathcal{F} = \text{span}(\boldsymbol{\Phi})$ is a linear subspace and $\mathtt{fit}$ minimizes the weighted squared error $\sum_i \xi(s_i) [y_i - \sum_j \theta_j \varphi_j(s_i)]^2$, then $\mathtt{fit}$ is mathematically equivalent to the Galerkin projection $\Proj$ applied at the sample points. The first-order conditions for least squares fitting are $\sum_i \xi(s_i) [y_i - \hat{v}(s_i)] \varphi_j(s_i) = 0$, which discretizes the orthogonality condition $\langle \Bellman v_k - \hat{v}_{k+1}, \varphi_j \rangle_\xi = 0$. Collocation corresponds to exact interpolation: require $\hat{v}_{k+1}(s_i) = y_i$ for all sample points.

The key insight is that **projection is just one way to fit a function to data points**—the way that works when your approximation space is a linear subspace. But the fitting perspective generalizes immediately beyond linear methods:

- **Neural networks**: Run gradient descent to minimize $\sum_i [y_i - v_{\boldsymbol{\theta}}(s_i)]^2$, where $v_{\boldsymbol{\theta}}$ is a neural network
- **Decision trees**: Recursively partition the state space to minimize squared error within each leaf
- **Kernel methods**: Construct $\hat{v}(s) = \sum_i \alpha_i k(s, s_i)$ by solving a linear system with kernel evaluations

These are all implementations of $\mathtt{fit}$. They take the dataset $\{(s_i, y_i)\}$ and produce a function. The difference is the hypothesis class $\mathcal{F}$ and the optimization procedure, but the conceptual operation is the same: compress sampled evaluations of the Bellman operator into a finite-dimensional representation.

The weighted residual framework from earlier in the chapter is the analytical tool for understanding convergence. The fitting operator is the computational tool for implementation. They are two views of the same algorithm: one mathematical, one procedural.

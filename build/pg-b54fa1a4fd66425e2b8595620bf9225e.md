...
The policy gradient theorem shows that the gradient depends on the action-value function $q^{\pi_{\boldsymbol{w}}}(s,a)$. In practice, we do not have access to the true $q$-function—we must estimate it. This leads to the **actor-critic** architecture: the **actor** maintains the policy $\pi_{\boldsymbol{w}}$, while the **critic** maintains an estimate of the value function.

...
```{prf:remark} Distinguishing Sampling Distributions
:label: remark-sampling-distributions

The averaged time-marginal $\xi_{\pi}(s, a)$ defined here is distinct from both:

1. **The stationary distribution** $\xi(s,a)$ used in [projection methods](projection.md) and [fitted Q-iteration](fqi.md), which requires ergodicity and describes the limiting frequency of state-action visits under an infinitely long trajectory.

2. **The normalized discounted visitation distribution** $\xi_\alpha(s)$ defined in the [policy gradient theorem section](#normalized-discounted-state-visitation-distribution) below, which weights states by their discounted occupancy $(1-\gamma)\sum_{t=0}^\infty \gamma^t d_t^{\pi}(s)$.

The averaged time-marginal is a finite-horizon concept that makes no ergodicity assumptions: it simply averages the state-action distributions over timesteps $t = 0, 1, \ldots, T$ with uniform weights. When we collect trajectories by running a policy for $T$ steps, each timestep contributes equally to the batch, and $\xi_{\pi}(s,a)$ is exactly the distribution from which our batch is effectively sampling.
```
...
This is exactly what the code computes: a finite-sample average over all timesteps in the batch, with no hidden assumptions about stationarity or discounted visitation distributions.

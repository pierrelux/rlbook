...
The term "bootstrap" appears in reinforcement learning with two distinct but related meanings. Understanding both is crucial for grasping how fitted Q-iteration works.

**Statistical bootstrapping** refers to treating the empirical distribution over observed data as if it were the true population distribution. In statistics, the bootstrap principle says: when the population distribution is unknown, use the empirical distribution $\hat{P}_{\mathcal{D}}$ as a plug-in estimate. By repeatedly sampling from $\mathcal{D}$, we implicitly assume our finite sample represents the environment's true transition dynamics $P(r, s' | s, a)$ weighted by the state-action visitation of whatever policy generated the data. This is exactly what we do in fitted Q-iteration: we sample transition tuples $(s_i, a_i, r_i, s'_i)$ from our dataset to approximate expectations that would ideally be computed under the true (but unknown) environment dynamics.

**Temporal difference (TD) bootstrapping** refers to using current value estimates to form targets, rather than complete Monte Carlo returns. When we write the target $y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta})$, we "bootstrap" from the estimated Q-value at the next state $s'_i$ instead of rolling out a full trajectory to terminal states. This allows us to learn from incomplete episodes and propagate information backwards through the state space incrementally. The Q-value estimate $q(s'_i, a'; \boldsymbol{\theta})$ serves as a proxy for the true expected future return from state $s'_i$.

**Both forms of bootstrapping are present simultaneously in fitted Q-iteration**: we resample transitions from the empirical distribution (statistical bootstrap) and use Q-function estimates in our targets (TD bootstrap). Importantly, these two concepts are independent—we could have one without the other. For example, we could sample from $\hat{P}_{\mathcal{D}}$ but use full Monte Carlo returns (statistical bootstrap without TD bootstrap), or we could use a fixed set of transitions without resampling but still use value estimates in targets (TD bootstrap without statistical bootstrap).

The combination of both forms of bootstrapping is what makes fitted Q-iteration practical: statistical bootstrapping lets us work with finite datasets instead of requiring access to the true environment model, while TD bootstrapping lets us learn from individual transitions instead of requiring complete trajectories. However, both introduce approximation that we must understand and manage.

```{prf:remark} Independence of Statistical and TD Bootstrapping
:class: dropdown

To see that statistical and TD bootstrapping are independent, consider these four combinations:

1. **Both forms** (standard FQI): Sample transitions from $\hat{P}_{\mathcal{D}}$ and compute targets $y_i = r_i + \gamma \max_{a'} q(s'_i, a')$.

2. **Statistical only** (Monte Carlo with empirical distribution): Sample trajectories from a replay buffer, but compute targets using full returns $y_i = \sum_{t=0}^{T-i} \gamma^t r_{i+t}$ instead of value estimates.

3. **TD only** (Q-learning with true model): If we had access to the true transition distribution $P(s', r | s, a)$, we could compute exact expectations $\mathbb{E}_{s' \sim P(\cdot|s,a)}[r + \gamma \max_{a'} q(s', a')]$ without sampling, while still using value estimates in targets.

4. **Neither** (exact dynamic programming): With the true model and infinite computation, compute exact Bellman backups without approximation.

In practice, we rarely have access to the true model (ruling out options 3 and 4), and Monte Carlo methods (option 2) have higher variance and require episodic tasks. This is why FQI and DQN use both forms of bootstrapping (option 1).
```

...
A crucial practical constraint shapes all the algorithms that follow: in typical reinforcement learning settings, we observe **single-sample transitions**. Each interaction yields one quadruple $(s, a, r, s')$ recording the observed next state, not multiple samples from $p(\cdot|s,a)$. This means Monte Carlo expectations use $N=1$, giving high-variance single-sample estimates that we aggregate across different transitions through function approximation. The data structure is therefore always a collection of transition quadruples $\mathcal{D} = \{(s_i, a_i, r_i, s'_i)\}$, whether collected offline in a fixed batch or accumulated online in a replay buffer.

These three elements combine into a template that generates most modern reinforcement learning algorithms. The iteration $\hat{q}^{(k+1)} = \text{fit}(\{((s_i, a_i), y_i)\})$, where targets $y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \theta^{(k)})$ are computed from the single observed next state $s'_i$ in each transition, admits many instantiations. Different choices of function approximator, optimization procedure, data collection strategy, and operator structure yield different algorithms, each with distinct computational properties and empirical characteristics.
...
The Q-function $q(s,a; \theta)$ can be represented using linear combinations of fixed basis functions, decision trees, neural networks, or kernel methods. Linear approximators and trees offer interpretability and theoretical guarantees but limited expressiveness. Neural networks provide flexibility for high-dimensional problems but require careful tuning. The choice determines both the representational capacity and the optimization landscape.
...
The key insight is that different data collection regimes correspond to different buffer initialization and update policies:
...
This perspective reveals that the SAA vs SA distinction is not binary but a continuum controlled by truncation steps $K$ and buffer capacity $B$. The unified template accommodates all cases:
...
3. **repeat**
    ...
    3. $\boldsymbol{\theta}_{n+1} \leftarrow \texttt{fit}(\mathcal{D}_n^{\text{fit}}, \boldsymbol{\theta}_{\text{init}}=\boldsymbol{\theta}_0, K=\infty)$
    4. $n \leftarrow n + 1$
4. **until** training complete
5. **return** $\boldsymbol{\theta}_n$
```

This algorithm has a nested structure: the outer loop (indexed by $n$) computes targets using the current Q-function, while `fit` performs an inner optimization to minimize the regression loss. Importantly, `fit` starts from the initial parameters $\boldsymbol{\theta}_0$ each time (cold start), not from $\boldsymbol{\theta}_n$. Note the distinction: $\mathcal{D}$ is the fixed offline transition dataset, while $\mathcal{D}_n^{\text{fit}}$ is the regression dataset with computed targets that changes each iteration.

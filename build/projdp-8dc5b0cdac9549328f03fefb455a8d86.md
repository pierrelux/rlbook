...
The envelope theorem states that $\nabla v^*(s) = \nabla_s r(s,\pi^*(s)) + \gamma \nabla_s f(s,\pi^*(s))^\top \nabla v^*(f(s,\pi^*(s)))$. Substituting this recursively, all future value function terms cancel. Define the operator $\mathcal{L}_\pi : \mathcal{S} \to \mathbb{R}^m$ by:

$$
\mathcal{L}_\pi(s) = \nabla_a r(s,\pi(s)) + \gamma \nabla_a f(s,\pi(s))^\top \big[\nabla_s r(f(s,\pi(s)),\pi(f(s,\pi(s)))) + \gamma \nabla_s f(\cdot,\pi(\cdot))\big|_{s'} \mathcal{L}_\pi(f(s,\pi(s)))\big].
$$

The **discrete-time Euler equation** is then $\mathcal{L}_{\pi^*}(s) = 0$ for all $s$. This is a functional equation in $\pi^*(\cdot)$ alone. As Rust {cite}`Rust1996` notes, "since $V$ drops out of the first order condition it is not necessary to consider the impact on marginal utility on all future periods $t+2, t+3, \ldots$"

For stochastic dynamics, replace derivatives of $f$ with expectations over transition derivatives. For constrained actions with $g_k(s,a) \leq 0$, replace the FOC with the KKT conditions.

...
This construction applies even to a deterministic simulator. A later
connection to stochastic optimal control explains when the same weighting
computes an optimal feedback action. Competing passages and uncertain
aircraft winds then show how physical uncertainty affects planning.
...
The sampling derivation requires only Gaussian distributions and expectations;
KL divergence and importance sampling are developed here. The later
stochastic-control connection uses Brownian increment scaling from
[Stochastic Dynamics and Partial Observation](stochastic-dynamics-observation.md)
and develops the continuous-time value recursion as needed. The later
dynamic-programming chapters are not prerequisites.
...
The exponential has an optimization interpretation. We seek a distribution
$Q$ that reduces expected state cost while paying a penalty for departing
from $P_0$. Measure that departure with the Kullback--Leibler divergence:
for densities $q,p$, define
$D_{\mathrm{KL}}(Q\|P)=\mathbb E_Q[\log(q/p)]$, where $Q$ must put no mass where $P$ has zero density. This condition is
called absolute continuity. The objective is
$\mathbb E_Q[S]+\lambda D_{\mathrm{KL}}(Q\|P_0)$.
Substituting $p^\star$ into a second divergence identifies its minimizer:
...
quantities are finite. This objective explains the exponential weighting:
it is the optimizer of an expected-cost problem regularized by KL divergence.

For the Gaussian family used to store a plan, the KL penalty has a familiar
control interpretation. Because the reference and proposal have the same
covariance, it is a quadratic penalty on the mean controls:
...
For example, choosing $\lambda\Sigma^{-1}=hR$ gives the familiar
discretized effort penalty $\tfrac12 h\sum_k\mathbf u_k^\top R\mathbf u_k$,
where $h$ is the interval length and $R\succ0$ sets the cost of control.
This is why $S$ contains only state and terminal costs in this formulation:
the reference supplies the effort penalty through KL regularization.
...
solve the first problem restricted to Gaussians
...
different inputs. Completing the square gives the weighted density:
...
Its mean is $\kappa/(1+\kappa)$. This is the exact mean fit and, in this
quadratic case, also the minimizer of the deterministic objective
$\tfrac\kappa2(u-1)^2+\tfrac12u^2$: its derivative is
$\kappa(u-1)+u$. The agreement gives a check on the update, although a
mean fit need not minimize the deterministic cost for a general rollout.
...
run a diffusion noise schedule. In both algorithms, noise used to search
over plans can be numerical; physical Brownian motion is a separate modeling
choice. We now examine when physical noise gives the weighted update an
...
cost when later actions may depend on later observations? A mean fit alone
does not answer this question. For a class of models with Gaussian physical
disturbances, quadratic control effort, and a particular relation between
them, exponential weighting also gives the optimal current feedback action.
Brownian motion specifies those physical disturbances and makes this
connection possible.
...
the system will experience. To use the earlier rollout notation, write

...
## A Motivating Example: Approximating a 3-State MDP

Before developing the general theory, consider a concrete example that illustrates the core challenge. Suppose we have a Markov decision process with three states $\mathcal{S} = \{1, 2, 3\}$, a single action, discount factor $\gamma = 0.9$, and the following dynamics:

$$
\mathbf{P} = \begin{pmatrix} 0.7 & 0.2 & 0.1 \\ 0.3 & 0.4 & 0.3 \\ 0.1 & 0.3 & 0.6 \end{pmatrix}, \quad \mathbf{r} = \begin{pmatrix} 1 \\ 2 \\ 0 \end{pmatrix}.
$$

The exact value function $v^* = (v^*(1), v^*(2), v^*(3))^\top$ satisfies three Bellman equations:

$$
v^*(s) = r(s) + \gamma \sum_{s'=1}^{3} p(s'|s) v^*(s'), \quad s = 1, 2, 3.
$$

With three states, we can solve this exactly: $v^* = (I - \gamma \mathbf{P})^{-1} \mathbf{r}$.
...
for s in range(3):
    print(f"  v*({s+1}) = {v_exact[s]:.4f}")
```

Now suppose we want to approximate this value function using only **two parameters** instead of three. We choose two basis functions:

$$
\varphi_1(s) = 1 \quad \text{(constant)}, \qquad \varphi_2(s) = s \quad \text{(linear in state index)},
$$

and seek an approximation $\hat{v}(s) = \theta_1 \varphi_1(s) + \theta_2 \varphi_2(s) = \theta_1 + \theta_2 s$. This is a line through three points, determined by just two parameters $(\theta_1, \theta_2)$.

The **residual** at each state measures how far our approximation is from satisfying the Bellman equation:

$$
R(s; \theta) = r(s) + \gamma \sum_{s'} p(s'|s) \hat{v}(s'; \theta) - \hat{v}(s; \theta).
$$

For a perfect solution, $R(s; \theta) = 0$ at all states. But with only two parameters and three states, we generally cannot achieve zero residual everywhere. Instead, we must choose which conditions to enforce.

**Collocation** picks two states (say $s = 1$ and $s = 2$) and requires the residual to vanish exactly there:

$$
R(1; \theta) = 0, \quad R(2; \theta) = 0.
$$

This gives two equations in two unknowns. The residual at $s = 3$ will generally be nonzero.

**Galerkin** requires the residual to be orthogonal to each basis function (weighted by some distribution $\xi$):

$$
\sum_{s=1}^{3} \xi(s) R(s; \theta) \varphi_1(s) = 0, \quad \sum_{s=1}^{3} \xi(s) R(s; \theta) \varphi_2(s) = 0.
$$

This also gives two equations in two unknowns, but now the residual is "spread" across all states rather than forced to zero at specific points.

```{code-cell} python
:tags: [hide-input]
# Basis functions: phi_1(s) = 1, phi_2(s) = s (using s = 1, 2, 3)
states = np.array([1, 2, 3])
Phi = np.column_stack([np.ones(3), states])  # 3 x 2 matrix

# Collocation at states 1 and 2
collocation_points = [0, 1]  # indices for states 1 and 2
Phi_colloc = Phi[collocation_points, :]  # 2 x 2

# Bellman targets at collocation points
targets_colloc = r[collocation_points] + gamma * P[collocation_points, :] @ Phi @ np.linalg.solve(
    Phi_colloc, r[collocation_points] + gamma * P[collocation_points, :] @ v_exact)

# Solve collocation: find theta such that Phi_colloc @ theta = T(v) at those points
# Iterating: theta_new such that hat_v(s_i) = r(s_i) + gamma * sum_j P_ij * hat_v(s_j)
# For illustration, we solve the fixed-point directly
def collocation_iteration(theta, max_iter=100, tol=1e-8):
    for _ in range(max_iter):
        v_approx = Phi @ theta
        targets = r + gamma * P @ v_approx
        theta_new = np.linalg.solve(Phi_colloc, targets[collocation_points])
        if np.linalg.norm(theta_new - theta) < tol:
            return theta_new
        theta = theta_new
    return theta

theta_colloc = collocation_iteration(np.zeros(2))
v_colloc = Phi @ theta_colloc

# Galerkin with uniform weighting
xi = np.ones(3) / 3  # uniform distribution
Xi = np.diag(xi)

def galerkin_iteration(theta, max_iter=100, tol=1e-8):
    for _ in range(max_iter):
        v_approx = Phi @ theta
        targets = r + gamma * P @ v_approx
        # Galerkin: Phi^T Xi Phi theta = Phi^T Xi targets
        A = Phi.T @ Xi @ Phi
        b = Phi.T @ Xi @ targets
...
theta_galerkin = galerkin_iteration(np.zeros(2))
v_galerkin = Phi @ theta_galerkin

print("Comparison of methods:")
print(f"{'State':<8} {'Exact':<12} {'Collocation':<12} {'Galerkin':<12}")
print("-" * 44)
for s in range(3):
    print(f"{s+1:<8} {v_exact[s]:<12.4f} {v_colloc[s]:<12.4f} {v_galerkin[s]:<12.4f}")

print(f"\nCollocation residual at state 3: {r[2] + gamma * P[2,:] @ v_colloc - v_colloc[2]:.4f}")
print(f"Galerkin weighted residual norm: {np.sqrt(np.sum(xi * (r + gamma * P @ v_galerkin - v_galerkin)**2)):.4f}")
```

Both methods produce approximations, but they make different trade-offs. Collocation achieves zero residual at the chosen points but may have larger error elsewhere. Galerkin minimizes a weighted average of the residual across all states. The choice depends on which states matter most and what computational resources are available.
...
#### Worked Example: Collocation on the 3-State MDP

Returning to our motivating example, let us trace through the collocation algorithm step by step. We use basis functions $\varphi_1(s) = 1$ and $\varphi_2(s) = s$, and collocate at states $s_1 = 1$ and $s_2 = 2$.

The collocation matrix is:

$$
\boldsymbol{\Phi} = \begin{pmatrix} \varphi_1(1) & \varphi_2(1) \\ \varphi_1(2) & \varphi_2(2) \end{pmatrix} = \begin{pmatrix} 1 & 1 \\ 1 & 2 \end{pmatrix}.
$$

Starting from $\theta^{(0)} = (0, 0)^\top$, the first iteration computes the Bellman targets at the collocation points. With $\hat{v}^{(0)}(s) = 0$ for all $s$:

$$
t_1^{(0)} = r(1) + \gamma \sum_{s'} p(s'|1) \cdot 0 = 1, \quad t_2^{(0)} = r(2) + \gamma \sum_{s'} p(s'|2) \cdot 0 = 2.
$$

We then solve $\boldsymbol{\Phi} \theta^{(1)} = t^{(0)}$:

$$
\begin{pmatrix} 1 & 1 \\ 1 & 2 \end{pmatrix} \theta^{(1)} = \begin{pmatrix} 1 \\ 2 \end{pmatrix} \implies \theta^{(1)} = \begin{pmatrix} 0 \\ 1 \end{pmatrix}.
$$

The approximation after iteration 1 is $\hat{v}^{(1)}(s) = s$, giving values $(1, 2, 3)$ at states $(1, 2, 3)$.

```{code-cell} python
:tags: [hide-input]
import numpy as np

# MDP from motivating example
P = np.array([[0.7, 0.2, 0.1], [0.3, 0.4, 0.3], [0.1, 0.3, 0.6]])
r = np.array([1.0, 2.0, 0.0])
gamma = 0.9

# Basis and collocation setup
states = np.array([1, 2, 3])
Phi_full = np.column_stack([np.ones(3), states])
colloc_idx = [0, 1]  # states 1 and 2
Phi_colloc = Phi_full[colloc_idx, :]

# Collocation iteration with detailed output
theta = np.array([0.0, 0.0])
print("Collocation iteration trace:")
print(f"{'Iter':<6} {'theta_1':<10} {'theta_2':<10} {'v(1)':<10} {'v(2)':<10} {'v(3)':<10}")
print("-" * 56)

for k in range(8):
    v_approx = Phi_full @ theta
    print(f"{k:<6} {theta[0]:<10.4f} {theta[1]:<10.4f} {v_approx[0]:<10.4f} {v_approx[1]:<10.4f} {v_approx[2]:<10.4f}")
    
    # Bellman targets at collocation points
    targets = r + gamma * P @ v_approx
    targets_colloc = targets[colloc_idx]
    
    # Solve for new theta
    theta_new = np.linalg.solve(Phi_colloc, targets_colloc)
...
v_final = Phi_full @ theta
print(f"\nConverged in {k+1} iterations")
print(f"Final theta: ({theta[0]:.4f}, {theta[1]:.4f})")

# Compare to exact
v_exact = np.linalg.solve(np.eye(3) - gamma * P, r)
print(f"\nApproximation error: ||v_colloc - v*|| = {np.linalg.norm(v_final - v_exact):.4f}")
```

The iteration converges because the projected Bellman operator is a contraction. Each iteration applies the Bellman operator and projects back onto the space of linear functions by solving the collocation equations.
...
1. **Effect of collocation points.** Using the 3-state MDP from this chapter, compare collocation at states $\{1, 2\}$ versus states $\{1, 3\}$ versus states $\{2, 3\}$. Which choice gives the smallest maximum error $\max_s |v^*(s) - \hat{v}(s)|$? Explain why.

2. **Galerkin with different weightings.** Repeat the Galerkin computation with the stationary distribution $\xi = (0.4, 0.35, 0.25)$ instead of uniform weighting. How does the approximation change? Which states are fit more accurately?

3. **Orthogonal polynomials.** For a continuous state space $\mathcal{S} = [-1, 1]$, write out the Galerkin orthogonality conditions $\langle R, T_i \rangle_w = 0$ for $i = 0, 1, 2$ using Chebyshev polynomials $T_i$ and weight function $w(x) = 1/\sqrt{1-x^2}$. What makes this choice computationally convenient?

4. **Newton vs. function iteration.** Modify the collocation code to count iterations until convergence for both function iteration and Newton's method. How does the iteration count depend on $\gamma$? Try $\gamma \in \{0.5, 0.9, 0.99\}$.
```
...
##### Worked Example: LSTD on the 3-State MDP

Returning to our 3-state MDP, we compute the LSTD solution with uniform weighting $\xi = (1/3, 1/3, 1/3)$ and the same basis functions $\varphi_1(s) = 1$, $\varphi_2(s) = s$. The feature matrix is:

$$
\boldsymbol{\Phi} = \begin{pmatrix} 1 & 1 \\ 1 & 2 \\ 1 & 3 \end{pmatrix}, \quad \boldsymbol{\Xi} = \frac{1}{3}\begin{pmatrix} 1 & 0 & 0 \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{pmatrix}.
$$

The LSTD matrix and vector are:

$$
\mathbf{A} = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} (\boldsymbol{\Phi} - \gamma \mathbf{P} \boldsymbol{\Phi}), \quad \mathbf{b} = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} \mathbf{r}.
$$
...
print(f"\nLSTD solution: theta = ({theta_lstd[0]:.4f}, {theta_lstd[1]:.4f})")
...
print(f"{'State':<8} {'Exact':<12} {'LSTD':<12} {'Error':<12}")
print("-" * 44)
for s in range(3):
    print(f"{s+1:<8} {v_exact[s]:<12.4f} {v_lstd[s]:<12.4f} {v_lstd[s] - v_exact[s]:<12.4f}")

# Verify orthogonality: residual should be orthogonal to feature space
residual = r + gamma * P @ v_lstd - v_lstd
print(f"\nGalerkin orthogonality check:")
print(f"  <residual, phi_1>_xi = {np.sum(xi * residual * Phi[:,0]):.6f}")
print(f"  <residual, phi_2>_xi = {np.sum(xi * residual * Phi[:,1]):.6f}")
```

The LSTD solution satisfies the Galerkin orthogonality condition: the residual $r + \gamma \mathbf{P} v - v$ is orthogonal to both basis functions in the $\xi$-weighted inner product. This is exactly what the projected Bellman equation requires.
...
2. **A non-monotone example.** Using Lagrange interpolation with 4 equally spaced nodes on $[0, 1]$, compute the interpolation weights for the point $x = 0.1$. Show that some weights are negative, violating the averager condition.

3. **State aggregation.** Consider aggregating the 3-state MDP into two groups: $\{1, 2\}$ and $\{3\}$. Write out the aggregation operator as a matrix. Verify it satisfies the conditions of {prf:ref}`gordon-averager`.

4. **Contraction constant.** For the composed operator $\Proj \Bellman$ with a monotone $\Proj$, prove that the contraction constant is exactly $\gamma$ (not smaller). Hint: find a pair of functions $v, w$ where $\|\Proj \Bellman v - \Proj \Bellman w\|_\infty = \gamma \|v - w\|_\infty$.
```
...
1. **Computing the stationary distribution.** For the 3-state MDP, compute the stationary distribution $\xi$ satisfying $\xi^\top \mathbf{P} = \xi^\top$. Verify your answer by checking that $(I - \mathbf{P}^\top)\xi = 0$ with $\sum_s \xi(s) = 1$.

2. **LSTD with stationary weighting.** Recompute the LSTD solution using the stationary distribution from Exercise 1 instead of uniform weighting. Compare the approximation error to the uniform case.

3. **Off-policy divergence.** Consider a weighting $\xi' = (0.9, 0.05, 0.05)$ that does not match the stationary distribution. Implement projected value iteration $v_{k+1} = \Proj_{\xi'} \BellmanPi v_k$ and observe whether it converges. What happens if you make $\xi'$ even more mismatched?

4. **Proving non-expansiveness fails off-policy.** For the weighting $\xi' = (0.9, 0.05, 0.05)$, find a vector $z$ such that $\|\mathbf{P} z\|_{\xi'} > \|z\|_{\xi'}$. This shows $\mathbf{P}$ is not non-expansive in $\|\cdot\|_{\xi'}$.
```
...
The following code demonstrates fitted-value iteration on our 3-state MDP using scikit-learn's `LinearRegression` as the fitting procedure:
...
r = np.array([1.0, 2.0, 0.0])
gamma = 0.9
states = np.array([[1], [2], [3]])  # Column vector for sklearn

def bellman_operator(v, P, r, gamma):
    """Apply the Bellman operator: T(v) = r + gamma * P @ v"""
    return r + gamma * P @ v

def fitted_value_iteration(states, P, r, gamma, fit_fn, max_iter=50, tol=1e-6):
    """FVI with arbitrary fitting function."""
    n_states = len(states)
    v = np.zeros(n_states)
    ...
    for k in range(max_iter):
        ...
        targets = bellman_operator(v, P, r, gamma)
        
        # Fit new value function
        v_new = fit_fn(states, targets)
...
v_fvi, history = fitted_value_iteration(states, P, r, gamma, linear_fit)

# Compare to exact solution
v_exact = np.linalg.solve(np.eye(3) - gamma * P, r)

print(f"\nFVI with Linear Regression:")
print(f"{'State':<8} {'Exact':<12} {'FVI':<12} {'Error':<12}")
print("-" * 44)
for s in range(3):
    print(f"{s+1:<8} {v_exact[s]:<12.4f} {v_fvi[s]:<12.4f} {v_fvi[s] - v_exact[s]:<12.4f}")

# Show convergence
print(f"\nConvergence (||v_k - v*||):")
for k, v in enumerate(history[:6]):
    print(f"  k={k}: {np.linalg.norm(v - v_exact):.6f}")
```

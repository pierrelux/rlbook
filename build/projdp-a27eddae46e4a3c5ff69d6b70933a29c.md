...
In each case, $\mathtt{fit}$ maps a training dataset to a function, but the internal mechanism—closed-form solution, iterative optimization, tree construction, or ensemble aggregation—varies widely.
...
### Why Generalize Beyond Linear Approximation?

Linear basis function approximation has many virtues: theoretical tractability, closed-form solutions, and well-understood convergence properties. Why, then, should we consider more complex function approximators?

The answer lies in **representation capacity**. Polynomial and radial basis functions require us to hand-craft features that capture the relevant structure of the value function. For problems with high-dimensional state spaces or complex value function geometry, finding good features can be prohibitively difficult. Modern function approximators—particularly neural networks—can automatically learn useful representations from data, discovering features that we might never think to hand-craft.

Decision trees and ensemble methods offer a different advantage: they can adapt their complexity to the data, allocating representational resources where the value function varies most. A regression tree might use many splits in regions where the value function is complex while using few splits where it is smooth, achieving good approximation quality with a parsimonious model.

Kernel methods provide yet another perspective: they implicitly work in high- or even infinite-dimensional feature spaces while requiring only kernel evaluations between states. This "kernel trick" allows rich approximations without explicitly constructing basis functions.

The price we pay for this flexibility is that we lose the closed-form solutions and fixed-point theory that made linear methods so clean. Neural network training may not converge; trees might overfit; kernels introduce computational challenges. Fitted-value iteration with general function approximators is, in practice, as much an art as a science—requiring careful tuning of hyperparameters, regularization, and stopping criteria.

### Toward Simulation-Based Methods

A crucial limitation of the fitted-value iteration algorithm as presented is that it assumes we can evaluate the Bellman operator exactly at every state. That is, computing $y_i = (\mathrm{L} v_k)(s_i)$ requires knowing the transition probabilities $p(s' \mid s_i, a)$ and being able to sum or integrate over all possible next states $s'$.

In many real-world problems, we have neither. We might have access only to a simulator that generates sample transitions $(s, a, r, s')$, or to a dataset of observed trajectories. We might not even know the full state space in advance. This brings us to the threshold of **simulation-based approximate dynamic programming** and **reinforcement learning**, where the Bellman operator must be approximated from samples rather than computed exactly.

The projection and collocation methods developed in this chapter provide the conceptual foundation for these simulation-based methods. The residual $R(s; \boldsymbol{\theta}) = v(s; \boldsymbol{\theta}) - (\mathrm{L} v(\cdot; \boldsymbol{\theta}))(s)$ remains our north star—but instead of enforcing orthogonality conditions on the full state space, we will minimize empirical residuals on sampled data. The fitting operator $\mathtt{fit}$ will take in noisy, incomplete samples rather than exact values. And convergence will be probabilistic, characterized by sample complexity rather than deterministic fixed-point theorems.

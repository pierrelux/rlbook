...
Recall the Galerkin approach for linear approximation $v_{\boldsymbol{\theta}} = \sum_{i=1}^d \theta_i \varphi_i$. The orthogonality conditions $\langle v - \Bellman v, \varphi_i \rangle_w = 0$ for all $i$ define a linear system with a closed-form solution. These equations are equivalent to minimizing $\|v - \Bellman v\|_w^2$ over the subspace: the gradient with respect to each coefficient $\theta_i$ must vanish, yielding the orthogonality conditions.

For nonlinear function classes, the minimization principle remains but the solution requires iterative optimization. Given parameters $\boldsymbol{\theta} \in \mathbb{R}^p$ defining a function $v_{\boldsymbol{\theta}}$ (e.g., a neural network), we seek

$$
\boldsymbol{\theta}^* = \arg\min_{\boldsymbol{\theta}} \|v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}\|_w^2.
$$

The first-order stationarity condition $\frac{\partial}{\partial \theta_i} \|v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}\|_w^2 = 0$ for all $i$ is, by the chain rule,
...
These are orthogonality conditions with test functions $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i}$. These partial derivatives span the tangent space to the manifold $\{v_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \mathbb{R}^p\}$ at the current parameters. When $v_{\boldsymbol{\theta}} = \sum_i \theta_i \varphi_i$, we have $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} = \varphi_i$, recovering the Galerkin equations. For nonlinear parameterizations, the partial derivatives change with $\boldsymbol{\theta}$, and we solve by gradient descent. 
The solution defines an implicit nonlinear projection operator $\Proj_{\mathcal{F}}$, generalizing $\Proj$.

This equivalence between norm minimization and orthogonality holds for any inner product: weighted $L^2$ integrals for Galerkin, sums over collocation points for collocation, or sampled expectations for neural networks. The **dual pairing** formulation {cite}`LegrandJunca2025` provides the unifying framework. A dual space is the collection of all linear functionals on a function space: mappings that take a function and return a number. For example, "evaluate at point $x_i$" and "integrate against a weight" are both functionals. This generality handles test objects that are not regular functions. Collocation uses Dirac delta "functions" $\delta(x - x_i)$, which are distributions defined only through their action on test functions via the dual pairing $\langle \Residual(v), \delta(x - x_i) \rangle = (\Residual v)(x_i)$. The induced dual norm $\|\Residual(v)\|_* = \sup_{\|w\|=1} |\langle \Residual(v), w \rangle|$ measures the residual by its worst-case effect on test functions.

Neural network training by residual minimization instantiates this framework {cite}`Riedmiller05,Sirignano2018`. Consider minimizing the sampled Bellman residual

$$
\mathcal{L}(\boldsymbol{\theta}) = \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - (\Bellman v_{\boldsymbol{\theta}})(s))^2 \big].
$$

Gradient descent on this loss implements $\langle R[v_{\boldsymbol{\theta}}], \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \rangle = 0$ in a Monte Carlo approximation of the $L^2(\mu)$ inner product. This computes a nonlinear projection of the Bellman equation onto the neural network manifold. Weak adversarial networks {cite}`Zang2020` parameterize both the trial function $v_{\boldsymbol{\theta}}$ and test function $w_{\boldsymbol{\phi}}$ as neural networks, optimizing
...
which directly implements the dual norm definition. The adversarial formulation makes explicit what is implicit in the dual norm $\|\Residual(v)\|_* = \sup_{\|w\|=1} |\langle \Residual(v), w \rangle|$: measuring residuals by their worst-case effect across all test functions is fundamentally adversarial—the test functions try to expose where the residual is largest, while the trial function tries to minimize this worst-case violation. This perspective reveals that the choice between methods (Galerkin with fixed test functions, collocation with delta functions, or adversarial training with learned test functions) is fundamentally a choice about which test functions to use for detecting residual error.

The minimum residual framework unifies classical and modern methods. All seek parameters where the residual is orthogonal to a test space. The differences are the approximation class (linear subspace versus nonlinear manifold), the test space (fixed basis functions, point evaluations, or learned functions), and the solution method (direct solve versus iterative optimization). Convergence guarantees differ accordingly: linear projections with monotonicity or matched weightings yield contractions, while nonlinear methods have no general guarantees. This perspective connects the classical theory in this chapter to contemporary function approximation in reinforcement learning.

...
$$\mathbb{E}[(\widehat{\mathrm{L}}v_i)(s)] = \mathbb{E}\left[\max_{a \in \mathcal{A}(s)} \hat{q}_i(s,a)\right] \geq \max_{a \in \mathcal{A}(s)} \mathbb{E}[\hat{q}_i(s,a)] = (\Bellman v_i)(s)$$

This means that our Monte Carlo approximation of the Bellman operator is biased upward:

$$b_i(s) = \mathbb{E}[(\widehat{\mathrm{L}}v_i)(s)] - (\Bellman v_i)(s) \geq 0$$
...
$$
(\widehat{\mathrm{L}}v_n)(s) - \text{bias}(s) = (\widehat{\mathrm{L}}v_n)(s) - (\mathbb{E}[(\widehat{\mathrm{L}}v_n)(s)] - (\Bellman v_n)(s)) \approx (\Bellman v_n)(s)
$$
...
Value iteration -- the name for the method of successive approximation applied to $\mathrm{L}$ -- computes a sequence of iterates $v_{n+1} = \Bellman v_n$ from some arbitrary $v_0$. Let's pause to consider what the equality sign in this expression means: it represents an assignment (perhaps better denoted as $:=$) across the entire domain. This becomes clearer when writing the update in component form:

...
This gives $n$ (generally nonlinear) equations in $n$ unknowns. The collocation points can be chosen as zeros of orthogonal polynomials for spectral accuracy.
...
Euler equation methods directly represent and optimize policies rather than value functions, providing a preview of **policy parameterization**. The next chapter extends the weighted residual framework to simulation-based settings, where expectations in both the Bellman operator and the Euler operator must be approximated from samples. Policy gradient methods in modern reinforcement learning can be viewed as stochastic approximations to Euler equation residual minimization, though typically without explicitly forming the Euler equation. A later chapter develops policy parameterization systematically, showing how actor-critic architectures combine value function approximation with policy parameterization.

Weighted residual methods apply to any functional equation arising from an MDP: the Bellman equation for value functions, the Euler equation for policies, or equations for Q-functions or advantage functions. The unifying principle is orthogonality of residuals to test functions. The next chapter extends this framework to simulation-based settings, where operators must be approximated from samples.

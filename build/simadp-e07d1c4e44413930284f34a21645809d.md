...
Double Q-learning {cite}`van2016deep` breaks this coupling by maintaining two independent Monte Carlo estimates. For each action $a$, let $\hat{\mu}^{(1)}_N(s,a)$ and $\hat{\mu}^{(2)}_N(s,a)$ be independent Monte Carlo approximations, each based on $N$ independent samples from $p(\cdot|s,a)$. Decompose both as
...
$$
\mathbb{E}[Y] = \mathbb{E}\big[r(s,a^\star) + \gamma \mu(s,a^\star)\big].
$$

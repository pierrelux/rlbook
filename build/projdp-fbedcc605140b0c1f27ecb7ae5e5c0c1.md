...
The answer lies in the **dual pairing** formulation underlying weighted residual methods. Let $\mathcal{H}$ be a Banach space and $\mathcal{H}'$ its dual space: the set of all continuous linear functionals on $\mathcal{H}$. For any functional $T \in \mathcal{H}'$, its dual norm is

$$
\|T\|_{\mathcal{H}'} = \sup_{\substack{w \in \mathcal{H} \\ \|w\|_{\mathcal{H}} = 1}} |\langle T, w \rangle|,
$$

where $\langle T, w \rangle$ denotes the dual pairing: the action of $T$ on the test function $w$. This definition makes precise the "size" of a functional: test it against all normalized functions and take the worst case. When $\mathcal{H}$ is equipped with an inner product, the dual norm induced by $\langle \cdot, \cdot \rangle_w$ satisfies $\|T\|^2 = \langle T, T \rangle_w$. In the Hilbert space case $\mathcal{H} = L^2(\Omega)$, the dual pairing is the familiar integral $\langle f, g \rangle = \int_\Omega f(x) g(x) \, dx$. For more general spaces, the dual pairing allows residuals and test functions that may not be classical functions: collocation uses Dirac deltas $\delta(x - x_i)$ with $\langle R[v], \delta(x - x_i) \rangle = R[v](x_i)$, and weak derivatives in PDEs produce discontinuous residuals that only make sense when tested against smooth functions {cite}`LegrandJunca2025`.

The minimum residual principle seeks the function minimizing the dual norm of the residual. When the approximation class $\mathcal{F}$ is parameterized by $\boldsymbol{\theta} \in \mathbb{R}^p$, this becomes

...
held fixed here. Each 0.1-second simulator step serves one phase, prefill work
...
**Recorded metrics.** The table reports request-level results for the fixed
offline schedule under its nominal forecast and the shifted-burst disturbance.
Both columns use the same requests, controller parameters, and profile
provenance.
...
2,201.34 in its weighted model units. Under the nominal replay, the requests
later selected for shifting
have a mean time to first token of 5.83 seconds and a 95th percentile of 10.97
seconds. Moving only that burst into the plan's low-clock interval raises these
values to 11.49 and 19.57 seconds. At 30 seconds, the nominal replay has no
queued request while the shifted replay has 25; the peak queue at the minimum
clock rises from 4 to 25 requests.

The disturbance does not make every metric worse. Energy falls from 2,477 to
2,203 joules, and whole-trace mean time to first token falls from 7.12 to 6.77
seconds. Moving the burst changes which requests overlap later in the trace, so
the aggregate mean hides the severe delay of the moved requests. This result
shows that a schedule can conserve energy and improve an aggregate while being
badly mistimed for a particular burst. The evidence should be read as a vector
of aligned latency, queue, energy, and constraint metrics, not as a single
verdict.
...
of feedback and receding-horizon optimization. The current artifact is labeled
as a pre-measurement engineering surrogate. The comparison tests this
optimization mechanism but does not constitute a measured hardware result.

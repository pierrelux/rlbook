...
## Bootstrap Targets via Value Network Amortization

Fitted Q-iteration requires computing bootstrap targets $y_i = r(s_i,a_i) + \gamma v^*(s'_i)$ for sampled transitions. The expectation form gives:

$$
v^*(s'_i) = \mathbb{E}_{a' \sim \pi_{\phi}(\cdot|s'_i)}\left[q_\theta(s'_i,a') - \alpha \log \pi_{\phi}(a'|s'_i)\right]
$$

where the expectation involves both the Q-network and policy network evaluations. For a mini-batch of $B$ transitions with $N$ samples per state, direct Monte Carlo estimation requires $B \times N$ forward passes through both networks per target computation.

SAC amortizes this by training a V-network $v_\psi(s)$ to predict the expectation directly. The V-network learns:

$$
v_\psi(s) \approx \mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}\left[q_\theta(s,a) - \alpha \log \pi_{\phi}(a|s)\right]
$$

by fitting to samples $\{(s_j, \frac{1}{N}\sum_{k=1}^N [q_\theta(s_j,a_k) - \alpha \log \pi_\phi(a_k|s_j)])\}$ where $a_k \sim \pi_\phi(\cdot|s_j)$. Once trained, the bootstrap target reduces to a single network evaluation:

$$
y_i = r(s_i,a_i) + \gamma v_\psi(s'_i)
$$

This trades training cost (periodically fitting $v_\psi$) for inference efficiency (single forward pass per target), the same amortization principle used for the policy network in NFQCA and DDPG.

## Policy Optimization via Reparameterization

The optimal policy $\pi^*(a|s) \propto \exp(\beta \cdot q^*(s,a))$ is a Boltzmann distribution requiring the intractable partition function $Z(s) = \int_A \exp(\beta \cdot q^*(s,a))da$. SAC approximates this with a Gaussian policy $\pi_{\phi}(a|s) = \mathcal{N}(\mu_\phi(s), \sigma_\phi(s))$ by minimizing:

$$
\min_{\phi} \mathbb{E}_{s \sim \mu(s)}\left[D_{KL}\left(\pi_{\phi}(\cdot|s) \| \frac{\exp(\beta \cdot q_\theta(s,\cdot))}{Z_\theta(s)}\right)\right]
$$

Expanding the KL divergence and noting that $\log Z_\theta(s)$ is constant in $\phi$, this reduces to:

$$
\min_{\phi} \mathbb{E}_{s \sim \mu(s)}\mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}\left[\log \pi_{\phi}(a|s) - \beta \cdot q_\theta(s,a)\right]
$$

The policy parameters $\phi$ appear in both the sampling distribution and integrand, precluding direct Monte Carlo estimation. The reparameterization trick resolves this by expressing samples as $a = f_\phi(s,\epsilon) = \mu_\phi(s) + \sigma_\phi(s)\epsilon$ where $\epsilon \sim \mathcal{N}(0,1)$:

$$
\min_{\phi} \mathbb{E}_{s \sim \mu(s)}\mathbb{E}_{\epsilon \sim \mathcal{N}(0,1)}\left[\log \pi_\phi(f_\phi(s,\epsilon)|s) - \beta \cdot q_\theta(s,f_\phi(s,\epsilon))\right]
$$

Now $\phi$ appears only in the integrand through $f_\phi$, enabling automatic differentiation through the composition $q_\theta(s,f_\phi(s,\epsilon))$ as in DDPG. SAC extends deterministic policy gradients to stochastic policies by sampling noise $\epsilon$ rather than outputting a single action.


```{prf:algorithm} Soft Actor-Critic (SAC)
:label: sac

**Input**: MDP $(S, A, P, R, \gamma)$, twin Q-networks $q^1, q^2$, value network $v_\psi$, policy $\pi_{\phi}$, learning rates $\alpha_q, \alpha_v, \alpha_\pi$, replay buffer capacity $B$, mini-batch size $b$, EMA rate $\tau$, entropy weight $\alpha$

**Output**: Q-function parameters $\boldsymbol{\theta}^1, \boldsymbol{\theta}^2$, value parameters $\boldsymbol{\psi}$, policy parameters $\boldsymbol{\phi}$

1. Initialize $\boldsymbol{\theta}^1_0, \boldsymbol{\theta}^2_0, \boldsymbol{\psi}_0, \boldsymbol{\phi}_0$ randomly
2. $\boldsymbol{\psi}_{\text{target}} \leftarrow \boldsymbol{\psi}_0$
3. Initialize replay buffer $\mathcal{B}$ with capacity $B$
4. $t \leftarrow 0$
5. **while** training **do**
    1. Observe state $s$
    ...
    6. **// Update V-network: fit to entropy-adjusted Q-values**
    7. For each $s_i$, sample $\tilde{a}_i \sim \pi_{\phi_t}(\cdot|s_i)$
    8. $y^v_i \leftarrow \min(q^1(s_i, \tilde{a}_i; \boldsymbol{\theta}^1_t), q^2(s_i, \tilde{a}_i; \boldsymbol{\theta}^2_t)) - \alpha \log \pi_{\phi_t}(\tilde{a}_i|s_i)$
    9. $\boldsymbol{\psi}_{t+1} \leftarrow \boldsymbol{\psi}_t - \alpha_v \nabla_{\boldsymbol{\psi}} \frac{1}{b}\sum_{i=1}^b (v_{\psi_t}(s_i) - y^v_i)^2$
    10. **// Update Q-networks: bootstrap using V-network**
    11. $y^q_i \leftarrow r_i + \gamma v_{\psi_{\text{target}}}(s'_i)$
    12. $\boldsymbol{\theta}^1_{t+1} \leftarrow \boldsymbol{\theta}^1_t - \alpha_q \nabla_{\boldsymbol{\theta}} \frac{1}{b}\sum_{i=1}^b(q^1(s_i,a_i;\boldsymbol{\theta}^1_t) - y^q_i)^2$
    13. $\boldsymbol{\theta}^2_{t+1} \leftarrow \boldsymbol{\theta}^2_t - \alpha_q \nabla_{\boldsymbol{\theta}} \frac{1}{b}\sum_{i=1}^b(q^2(s_i,a_i;\boldsymbol{\theta}^2_t) - y^q_i)^2$
    14. **// Update policy: minimize KL to Boltzmann distribution**
    15. For each $s_i$, sample $\epsilon_i \sim \mathcal{N}(0,I)$ and compute $\hat{a}_i = f_{\phi_t}(s_i, \epsilon_i)$ (reparameterization)
    16. $\boldsymbol{\phi}_{t+1} \leftarrow \boldsymbol{\phi}_t - \alpha_\pi \nabla_{\boldsymbol{\phi}} \frac{1}{b}\sum_{i=1}^b [\log \pi_{\phi_t}(\hat{a}_i|s_i) - \beta \cdot q^1(s_i, \hat{a}_i; \boldsymbol{\theta}^1_{t+1})]$
    17. **// EMA update for V-network target**
    18. $\boldsymbol{\psi}_{\text{target}} \leftarrow \tau \boldsymbol{\psi}_{t+1} + (1-\tau)\boldsymbol{\psi}_{\text{target}}$
    19. $t \leftarrow t + 1$
6. **return** $\boldsymbol{\theta}^1_t, \boldsymbol{\theta}^2_t, \boldsymbol{\psi}_t, \boldsymbol{\phi}_t$
```

SAC maintains three function approximators: twin Q-networks $q^1, q^2$ for bias mitigation (as in TD3), a V-network $v_\psi$ that amortizes the entropy-adjusted expectation, and a Gaussian policy $\pi_\phi$ that approximates the Boltzmann distribution. The V-network (lines 7-9) learns $v(s) \approx \mathbb{E}_{a \sim \pi_\phi}[q(s,a) - \alpha \log \pi_\phi(a|s)]$ by sampling actions and fitting to the entropy-adjusted Q-values. The Q-networks (lines 11-13) use the V-network target for bootstrapping: $y_i = r_i + \gamma v_{\psi_{\text{target}}}(s'_i)$. The policy (lines 15-16) minimizes the KL divergence to the Boltzmann distribution via reparameterization, enabling automatic differentiation through $q^1(s_i, f_\phi(s_i, \epsilon_i))$. The stochastic policy provides exploration without added noise, unlike DDPG and TD3.

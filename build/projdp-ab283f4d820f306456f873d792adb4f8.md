...
**Why differentiate?** The standard value function approach requires computing $v^*(s)$ for all states and then solving the $\arg\max$ at each state. But when the maximization problem is smooth, we can instead characterize the optimal policy directly through its first-order conditions. This perspective, inherited from classical optimal control, offers several advantages:
1. It produces a characterization of optimality directly in terms of the policy $\pi^*(\cdot)$ rather than the value function $v^*(\cdot)$
2. For many problems (especially in economics), the policy is the object of interest, not the value function
3. In settings where the envelope theorem applies, the value function can be eliminated entirely, yielding a functional equation in the policy alone
4. For continuous state spaces, this often yields differential equations that admit analytic or semi-analytic solutions

The key is to exploit the smoothness of the maximization problem. If $r$ and $p$ are differentiable in $a$, the action space $\mathcal{A}$ is convex, and the optimal action $\pi^*(s)$ is interior, then a necessary condition for optimality is that the first-order derivative with respect to $a$ vanishes at $a = \pi^*(s)$:

$$
\frac{\partial r(s, a)}{\partial a}\Big|_{a=\pi^*(s)} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi^*(s)} v^*(j) = 0.
$$

This first-order condition holds at every state $s \in \mathcal{S}$ where the optimal action is interior. For problems where we can eliminate the value function using the envelope theorem, this becomes a condition purely in terms of the policy. Taking the derivative of the Bellman optimality equation with respect to $s$ and using the envelope theorem to express $\nabla_s v^*$ in terms of the policy, we can through recursive substitution eliminate the value function entirely, leaving a **discrete-time Euler equation**: a functional equation in the policy $\pi(\cdot)$ alone. The precise form depends on whether transitions are deterministic or stochastic, and whether the state space is discrete or continuous.

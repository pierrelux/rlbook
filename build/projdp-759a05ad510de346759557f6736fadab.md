...
The minimum residual principle asks for the function $v \in \mathcal{F}$ minimizing the dual norm of the residual:

$$
v^* = \arg\min_{v \in \mathcal{F}} \|v - \Bellman v\|_{\mathcal{W}'}.
$$

When $\mathcal{F}$ is a finite-dimensional linear subspace spanned by $\{\varphi_1, \ldots, \varphi_d\}$ and $\mathcal{W}$ is chosen such that the dual norm is induced by a weighted inner product, this minimization is solved by the orthogonality condition $\langle v - \Bellman v, \varphi_i \rangle_w = 0$ for all $i$, yielding the Galerkin equations. The linear structure allows us to express the solution in closed form via a linear system.

When $\mathcal{F}$ is a nonlinear function class parameterized by $\boldsymbol{\theta} \in \mathbb{R}^p$, such as neural networks or kernel expansions, the dual pairing formulation still applies. The minimum residual principle becomes

$$
\boldsymbol{\theta}^* = \arg\min_{\boldsymbol{\theta}} \|v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}\|_{\mathcal{W}'}^2,
$$

where $v_{\boldsymbol{\theta}}$ denotes the function represented by parameters $\boldsymbol{\theta}$. This defines a nonlinear optimization problem over the parameter space. The parameterized functions $\{v_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \mathbb{R}^p\}$ form a manifold embedded in a function space. While we no longer have orthogonality conditions or closed-form solutions, the first-order optimality condition

$$
\nabla_{\boldsymbol{\theta}} \|v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}\|_{\mathcal{W}'}^2 = \mathbf{0}
$$

provides the necessary condition for a minimum. This gradient condition has a natural interpretation through the dual pairing. Expanding the gradient using the chain rule, we can write
...
The "test functions" in the nonlinear case are the partial derivatives $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i}$, which span the tangent space to the manifold $\{v_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \mathbb{R}^p\}$ at the current point. These are not fixed basis functions as in Galerkin projection, but depend on the current parameter values. The stationarity condition requires the residual to be orthogonal to all tangent directions: the best approximation within the manifold is the point where moving in any parameter direction does not decrease the residual norm to first order. This is solved numerically by gradient descent or other nonlinear optimization algorithms. The resulting solution defines an implicit nonlinear projection operator $\Pi_{\mathcal{F}}$, which replaces the linear projection $\Proj$ in fitted-value iteration.

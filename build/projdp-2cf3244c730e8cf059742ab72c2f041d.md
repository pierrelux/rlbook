...
## Beyond Monotone Approximators: Policy Evaluation
...
To see what this equation means, let $\hat{v} = \boldsymbol{\Phi} \boldsymbol{\theta}$ be the solution. We can rearrange the LSTD equation by expanding the parentheses:
...
$$
\boldsymbol{\Phi}^\top \boldsymbol{\Xi} \boldsymbol{\Phi} \boldsymbol{\theta} = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} (\mathbf{r}_\pi + \gamma \mathbf{P}_\pi \boldsymbol{\Phi} \boldsymbol{\theta}).
$$

The right side is $\boldsymbol{\Phi}^\top \boldsymbol{\Xi} (r_\pi + \gamma \mathbf{P}_\pi \hat{v}) = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} \BellmanPi \hat{v}$, where $\BellmanPi$ is the policy Bellman operator. The left side is $\boldsymbol{\Phi}^\top \boldsymbol{\Xi} \hat{v}$. Therefore:

$$
\boldsymbol{\Phi}^\top \boldsymbol{\Xi} \hat{v} = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} \BellmanPi \hat{v}.
$$

This equation says that $\hat{v} - \BellmanPi \hat{v}$ is orthogonal to every column of $\boldsymbol{\Phi}$, and therefore orthogonal to the entire subspace $\text{span}(\boldsymbol{\Phi})$, in the $\xi$-weighted inner product. This is precisely the condition that $\hat{v} = \Proj \BellmanPi \hat{v}$, where $\Proj$ is orthogonal projection onto $\text{span}(\boldsymbol{\Phi})$ with respect to the $\xi$-weighted inner product:
...
LSTD finds the fixed point of the *projected* Bellman operator, not the Bellman operator itself, so approximation error persists. But will iterative methods converge to this fixed point? For convergence of $v_{k+1} = \Proj \BellmanPi v_k$, we need $\Proj \BellmanPi$ to be a contraction. The Bellman operator $\BellmanPi = r_\pi + \gamma \mathbf{P}_\pi$ is a $\gamma$-contraction in $\|\cdot\|_\infty$, but whether it contracts in $\|\cdot\|_\xi$ depends on the relationship between $\xi$ and $\mathbf{P}_\pi$.
...
**The stationary distribution condition.** Suppose $\xi$ is the steady-state distribution: $\xi^\top \mathbf{P}_\pi = \xi^\top$. Our goal is to establish that $\Proj \BellmanPi$ is a contraction in $\|\cdot\|_\xi$. We already know $\Proj$ is non-expansive in this norm. Therefore, if $\BellmanPi$ is a $\gamma$-contraction in $\|\cdot\|_\xi$, then their composition will be a $\gamma$-contraction:

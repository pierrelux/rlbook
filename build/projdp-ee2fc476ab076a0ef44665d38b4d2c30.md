...
where $v^*(j)$ is the optimal value function satisfying the Bellman optimality equation. If $r$ and $p$ are differentiable in $a$, the action space $\mathcal{A}$ is convex, and the optimal action $\pi^*(s)$ is interior, then a necessary condition for optimality is that the first-order derivative with respect to $a$ vanishes at $a = \pi^*(s)$:

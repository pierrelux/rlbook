...
2. **Policy network amortization** (Sections 3-5): Learn a deterministic or stochastic policy network $\pi_{\boldsymbol{w}}$ that approximates $\arg\max_a q(s,a;\theta)$ or the optimal stochastic policy, enabling fast action selection via a single forward pass.
...
SAC uses the same policy amortization strategy as DDPG: learn a parametric policy $\pi_\phi$ that approximates the Boltzmann distribution. This policy plays two roles. First, it amortizes the soft value computation by providing samples to estimate $v(s) \approx q(s,a) - \alpha \log \pi_\phi(a|s)$ where $a \sim \pi_\phi(\cdot|s)$. Second, it amortizes action selection by directly outputting actions, with exploration emerging naturally from its stochasticity rather than from external noise.
...
The policy plays the dual amortization role outlined earlier. It enables efficient estimation of soft value targets by providing samples for the expectation, and it provides the action selection mechanism with exploration built into its stochasticity. Unlike DDPG and TD3, which add exploration noise externally, SAC's entropy regularization objective produces the desired exploration behavior directly.
...
The solution we developed is **amortization**: invest computational effort during training to learn mappings that replace runtime optimization with fast forward passes through neural networks. This strategy keeps us firmly within the dynamic programming framework. We still compute Bellman operators, maintain Q-functions, and perform successive approximation. The amortization simply makes these operations tractable by replacing explicit optimization with learned approximations.

Two main instantiations of this idea emerged:

**Action selection amortization** (NFQCA, DDPG, TD3): Train a policy network $\pi_{\boldsymbol{w}}(s)$ to approximate $\arg\max_a q(s,a;\theta)$ by maximizing $q(s,\pi_{\boldsymbol{w}}(s);\theta)$ over states in the dataset. Bellman targets become $r + \gamma q(s', \pi_{\boldsymbol{w}}(s'); \theta)$, replacing the intractable maximization with a single network evaluation. This applies to both deterministic policies (DDPG, TD3) that require external exploration noise and stochastic policies that provide exploration through sampling.

**Value amortization** (SAC): When using entropy regularization, the soft Bellman operator requires computing $\frac{1}{\beta}\log \int \exp(\beta q(s,a;\theta))da$, an intractable integral over continuous actions. Rather than maintaining a separate value network, modern SAC uses single-sample Monte Carlo estimation: sample $a' \sim \pi(\cdot|s')$ and approximate $v(s') \approx q(s',a') - \alpha \log \pi(a'|s')$. This reduces the integral to one policy sample and one Q-network evaluation per target.

Both approaches maintain the actor-critic structure but differ in their policy classes and target computation. Action selection amortization works for deterministic policies that directly output actions, requiring careful exploration through external noise mechanisms. Value amortization emerges naturally when using stochastic policies with entropy regularization, where the policy provides both action selection and value estimation through sampling.

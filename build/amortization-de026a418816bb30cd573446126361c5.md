...
This involves the unknown value gradient $\partial v^*/\partial s$. In a general MDP, one would need to solve jointly for both the policy and the value-function derivatives.

### The Euler class

Many control problems have additional structure that eliminates $\partial v^*/\partial s$ from the first-order condition. Rust formalizes such problems as an **Euler class** {cite}`Rust1996`: the state decomposes into $s = (x, \xi)$ where $x$ is controlled (inventory, battery charge, water level) and $\xi$ evolves independently (demand, weather, prices). The transition law factors as
...
with $f$ continuously differentiable. The **Euler-class condition** requires a function $h(x, a, \xi)$ such that

$$
\frac{\partial f}{\partial x}
=
\frac{\partial f}{\partial a} \cdot h(x, a, \xi).
$$ (EC)

In scalar problems, this means $\partial f/\partial x$ and $\partial f/\partial a$ are proportional. For affine dynamics $x' = \alpha x + \beta a + \delta$ (where $\alpha, \beta, \delta$ may depend on $\xi, \xi'$), we have $\partial f/\partial x = \alpha$ and $\partial f/\partial a = \beta$, so the condition holds with $h = \alpha/\beta$. This covers inventory ($x' = \alpha x + a - D(\xi')$), energy storage ($x' = \eta_{\mathrm{ret}} x + \eta_{\mathrm{ch}} a$), reservoir ($x' = \alpha x + I(\xi') - a$), and thermal models ($x' = \alpha(\xi) x + \beta(\xi) a + \delta(\xi)$).

Under this condition, an envelope formula expresses $\partial v^*/\partial x$ purely in terms of $r$, $h$, and $\pi^*$:
...
Substituting into the first-order condition eliminates $v^*$ entirely, yielding an equation $\mathcal{E}(\pi)(x, \xi) = 0$ depending only on primitives $r, f, h, q$ and the policy $\pi$. This transforms a coupled system (policy + value) into a closed functional equation in the policy alone.


### Discretization

For a parameterized policy $\pi_\theta(x, \xi)$, we discretize the Euler equation using weighted residuals. With collocation at points $(x_1, \xi_1), \dots, (x_N, \xi_N)$, we solve

$$
G(\theta) := \begin{bmatrix}
\mathcal{E}(\pi_\theta)(x_1, \xi_1) \\
\vdots \\
\mathcal{E}(\pi_\theta)(x_N, \xi_N)
\end{bmatrix} = 0.
$$
...
$$
\int \varphi_i(x, \xi) \mathcal{E}(\pi_\theta)(x, \xi) \mu(dx, d\xi) = 0,
\quad i = 1,\dots,M.
$$

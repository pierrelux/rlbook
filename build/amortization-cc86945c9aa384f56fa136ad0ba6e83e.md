...
### Training Objective: Residual Minimization

PCL converts path consistency into a training objective using the **least-squares residual** approach from [weighted residual methods](projection.md). Recall that the projection methods chapter contrasts two solution strategies: **function iteration** ($v_{k+1} = \Proj \Bellman v_k$) and **residual minimization** ($\min_\theta \|R(\theta)\|^2$). FQI, DQN, and actor-critic methods follow the first approach. PCL follows the second.

For a $d$-step trajectory $s_i, a_i, \ldots, s_{i+d}$, define the soft consistency error:
...
where $\pi_{\boldsymbol{\theta}}$ is a parametric policy and $v_{\boldsymbol{\phi}}$ is a value function approximation. At optimality, this residual is zero for all paths. PCL minimizes the squared residual over a set of trajectories $\mathcal{E}$:

$$
\min_{\boldsymbol{\theta}, \boldsymbol{\phi}} \sum_{s_i:s_{i+d} \in \mathcal{E}} \frac{1}{2} C(s_i:s_{i+d}, \boldsymbol{\theta}, \boldsymbol{\phi})^2
$$

This is exactly the least-squares formulation from the projection methods chapter: minimize the squared norm of the residual. The gradient updates follow from differentiating this objective:
...
These updates push both the policy and value function toward satisfying path consistency. The structure mirrors the least-squares first-order conditions: the residual $C$ multiplies the gradient of the residual with respect to each parameter group.
...
4. **while** training **do**
    ...
    7. Sample trajectory $\tau'$ from $\mathcal{R}$ $\quad$ // Prioritized by reward

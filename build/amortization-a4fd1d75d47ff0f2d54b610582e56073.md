...
Soft actor-critic (SAC) {cite}`haarnoja2018soft` adapts the NFQCA framework to entropy-regularized MDPs. The [smoothing chapter](smoothing.md) derives the smooth Bellman operator from entropy regularization and shows it admits two equivalent forms. For the value function under entropy regularization with parameter $\alpha > 0$ (equivalently, inverse temperature $\beta = 1/\alpha$):

$$
v^*(s) = \frac{1}{\beta} \log \int_A \exp\left(\beta \cdot q^*(s,a)\right) da = \mathbb{E}_{a \sim \pi^*(\cdot|s)}\left[q^*(s,a) - \alpha \log \pi^*(a|s)\right]
$$

where $\pi^*(a|s) \propto \exp(\beta \cdot q^*(s,a))$ is the Boltzmann policy. The first form (log-integral-exp) requires integrating over all actions, intractable for continuous $\mathcal{A} \subset \mathbb{R}^m$. The second form (expectation) is tractable via sampling.

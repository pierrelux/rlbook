...
### Weighted Residual Methods as Successive Approximation
...
### The Computational Obstacle: Point Evaluation vs. Function Representation

The iteration $v_{k+1} = \Proj \Bellman v_k$ has a deceptively simple mathematical form. But implementing it computationally requires confronting a fundamental obstacle. The standard value iteration assigns:

$$
v_{k+1}(s) = (\Bellman v_k)(s) \quad \text{for all } s \in \mathcal{S}.
$$

For continuous or large state spaces, we cannot store this assignment. We cannot hold a function defined at uncountably many (or very many) points. We must work with a finite-dimensional approximation $v_k \in \text{span}(\boldsymbol{\Phi})$ characterized by $n$ coefficients.

The good news: we **can evaluate the Bellman operator pointwise**. Given $v_k$ and any state $s$, we can compute:

$$
(\Bellman v_k)(s) = \max_a \left[r(s,a) + \gamma \int p(s'|s,a) v_k(s') ds'\right].
$$

This requires numerical optimization (the max) and numerical integration (the expectation), but it is a finite computation at a single point.

The obstacle is this: pointwise evaluation gives us numbers $(\Bellman v_k)(s_1), \ldots, (\Bellman v_k)(s_n)$ at sample states, but we need a **function** $v_{k+1}: \mathcal{S} \to \mathbb{R}$ defined over the entire domain. How do we go from discrete point evaluations to a continuous function representation?

### Projection as Generalization from Point Evaluations

This is where the projection operator reveals its computational meaning. The mathematical expression $\Proj \Bellman v_k$ means: take the function $\Bellman v_k$ and project it onto $\text{span}(\boldsymbol{\Phi})$. But in practice, we cannot access $\Bellman v_k$ as a complete object. We can only generate point evaluations.

The projection operation, when implemented, works as follows:

...
Under these assumptions, one can derive an envelope formula expressing $\partial V/\partial y$ purely in terms of $u$, $h$, and the optimal policy $\alpha(y,z)$. Substituting this formula into the raw first-order condition removes $V_y$ and yields an **Euler equation that is a closed functional equation in the policy** $\alpha$ alone.

...
## Connection to Classical Optimal Control and Euler Equation Methods

The gradient-based approach to action selection in NFQCA has deep roots in classical optimal control theory and econometric methods for solving dynamic programming problems. These connections reveal fundamental differences in how we can approach the coupled problem of finding both the value function and the optimal policy.

### The Euler Equation

Consider a deterministic MDP with continuous states and actions where the dynamics are given by $s' = f(s,a)$ and rewards by $r(s,a)$. The optimal value function $v^*$ solves the Bellman equation:
...
### Three Approaches to Solving the Optimality Conditions

The Euler equation reveals that optimal control is fundamentally about solving a coupled system: we need both $v^*(s)$ and $\pi^*(s)$ satisfying the Bellman equation and first-order condition simultaneously. Different computational approaches handle this coupling differently.

#### Approach 1: Eliminate the Value Function (Classical Euler Equation Methods)

In classical optimal control and econometrics, practitioners often eliminate the value function entirely from the problem. The Euler equation contains the value gradient $\frac{\partial v^*}{\partial s}$, but if we know the optimal policy $\pi^*$, we can express the value function recursively:

$$
v^*(s) = r(s,\pi^*(s)) + \gamma v^*(f(s,\pi^*(s)))
$$

Differentiating this identity with respect to $s$ (using the chain rule and Envelope Theorem):

$$
\frac{\partial v^*}{\partial s}(s) = \frac{\partial r}{\partial s}(s,\pi^*(s)) + \gamma \frac{\partial v^*}{\partial s}(f(s,\pi^*(s))) \frac{\partial f}{\partial s}(s,\pi^*(s))
$$

Substituting this back into the Euler equation yields a functional equation in $\pi^*$ alone, eliminating explicit dependence on $v^*$. This functional equation relates the policy at state $s$ to the policy at future states $f(s,\pi^*(s))$, combined with the reward and dynamics structure.

Classical methods solve this functional equation directly using weighted residual methods like collocation or Galerkin projection, as described in our chapter on projection methods for functional equations. The advantage is that we work with a lower-dimensional unknown (the policy) rather than both policy and value function. The challenge is that the resulting functional equation is typically more complex and implicit than the original Bellman equation—it involves nested compositions of $\pi^*$ with the dynamics $f$.

For certain problem structures (e.g., linear-quadratic problems), this elimination leads to tractable analytical solutions. For general nonlinear problems, numerical methods like orthogonal collocation are used to discretize the functional equation into a finite system of nonlinear equations.

#### Approach 2: Decouple Selection and Evaluation (NFQCA)

NFQCA takes a fundamentally different approach: it **assumes the Q-function is given** and treats action selection as a separate optimization problem. If we had the true Q-function $q^*(s,a) = r(s,a) + \gamma v^*(f(s,a))$, finding the optimal action would be trivial: just maximize over $a$. 

NFQCA approximates this by maintaining a parametric Q-function $q(s,a;\theta)$ and a parametric policy $\pi(s;\omega)$. The policy gradient at the current policy action $\pi(s;\omega)$ is:

$$
\nabla_a q(s,\pi(s;\omega);\theta) \approx \frac{\partial r}{\partial a}(s,\pi(s;\omega)) + \gamma \frac{\partial v}{\partial s'}(f(s,\pi(s;\omega)))\frac{\partial f}{\partial a}(s,\pi(s;\omega))
$$

Setting this gradient to zero through the policy update $\nabla_\omega q(s, \pi(s;\omega); \theta) = 0$ implements the same first-order optimality condition as the Euler equation.

The key distinction is that NFQCA **decouples** the problem:
1. Update $q(s,a;\theta)$ by fitting to Bellman targets (treating the policy as given)
2. Update $\pi(s;\omega)$ by maximizing $q(s,a;\theta)$ (treating the Q-function as given)

This alternation between fitting and maximization is computationally convenient—each step is a standard supervised learning problem. However, it's less principled than solving the coupled system directly because convergence guarantees are harder to establish when both approximators are being updated simultaneously.

#### Approach 3: Joint Projection Methods (Policy Iteration Perspective)

A third approach, implicit in policy iteration and weighted residual methods applied to Bellman equations (see our projection chapter), maintains both $v$ and $\pi$ but exploits the structure that given $\pi$, computing $v^\pi$ is a linear problem. 

Using Newton's method on the projected Bellman optimality equation amounts to:
1. Given current $v_k$, compute greedy policy $\pi_k(s) = \arg\max_a \{r(s,a) + \gamma v_k(f(s,a))\}$
2. Evaluate this policy by solving the linear Bellman equation for $\pi_k$ to get $v_{k+1}$

This is policy iteration in function approximation space. The Envelope Theorem tells us that when computing the derivative of the Bellman operator with respect to value function parameters, we can treat the greedy action as constant—the first-order effect comes only from the direct dependence. This provides the Jacobian needed for Newton's method without requiring explicit policy parameterization.

### Comparison and Modern Perspective

These three approaches reveal a spectrum:

| **Aspect** | **Euler Equation Elimination** | **NFQCA Decoupling** | **Policy Iteration** |
|:-----------|:------------------------------|:---------------------|:---------------------|
| What is parameterized? | Policy $\pi^*$ only | Both $q$ and $\pi$ separately | Value $v$ only (policy implicit) |
| Coupling | Fully coupled functional equation | Decoupled alternating updates | Partially coupled (linear subproblem) |
| Computation | Solve nonlinear functional equation | Two supervised learning problems | Greedy maximization + linear solve |
| Convergence | Well-understood for collocation/Galerkin | Requires careful analysis | Guaranteed (exact) or with projection error |

The classical Euler equation methods are arguably more principled—they solve the true optimality conditions as a coupled functional equation. But they require sophisticated numerical analysis (weighted residuals, Newton-Kantorovich theorems) and are computationally intensive for high-dimensional problems.

Modern deep RL methods like NFQCA sacrifice some of this principled foundation for computational tractability. By decoupling and using powerful function approximators (neural networks), they can handle much higher-dimensional problems. The trade-off is that convergence becomes more delicate: the alternating updates may not converge to the true solution, and we lack the clean fixed-point guarantees of classical methods.

The policy iteration perspective bridges these extremes: it maintains coupling through the alternation between policy evaluation and improvement, but exploits the linearity of evaluation to avoid solving fully coupled nonlinear systems. With exact projection (tabular case), this is provably optimal. With function approximation, it inherits the approximation error but maintains structural guarantees about monotonic improvement (under certain conditions).

Understanding these connections helps us appreciate both the heritage of modern deep RL algorithms and their limitations. The Euler equation view reminds us that gradient-based policy optimization is fundamentally about solving first-order optimality conditions—a perspective that becomes particularly relevant when we try to provide convergence guarantees or understand failure modes in modern algorithms.

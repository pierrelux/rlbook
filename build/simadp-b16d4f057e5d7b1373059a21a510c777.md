...
When we apply Monte Carlo integration to the Bellman operator, each individual expectation is unbiased. The problem arises when we maximize over these noisy estimates. For a given state-action pair $(s,a)$ and value function $v$, define the true continuation value
...
The maximization introduces systematic overestimation. To see why, let $a^*$ be the truly optimal action. Since the maximum of any collection is at least as large as any element:
...
An alternative approach modifies the estimator to break the coupling that causes bias. In the standard update $\max_a \{r(s,a) + \gamma \hat{\mu}_N(s,a)\}$, the same noisy estimate both selects the action and evaluates it. Decompose $\hat{\mu}_N(s,a) = \mu(s,a) + \varepsilon_a$ where $\varepsilon_a$ is zero-mean noise. Whichever action has the largest $\varepsilon_a$ is selected, and that same positive noise inflates the target: $Y = r(s,a^\star) + \gamma \mu(s,a^\star) + \gamma \varepsilon_{a^\star}$, producing $\mathbb{E}[Y] \ge \max_a \{r(s,a) + \gamma \mu(s,a)\}$.

Double Q-learning {cite}`van2016deep` breaks this coupling by maintaining two independent estimates $\hat{\mu}^{(1)}_N(s,a) = \mu(s,a) + \varepsilon^{(1)}_a$ and $\hat{\mu}^{(2)}_N(s,a) = \mu(s,a) + \varepsilon^{(2)}_a$, constructed from separate sample sets $\{s'_i\}$ and $\{s''_i\}$ drawn independently from $p(\cdot|s,a)$. Use the first to select, the second to evaluate:

$$
a^\star = \arg\max_{a} \left\{r(s,a) + \gamma \hat{\mu}^{(1)}_N(s,a)\right\}, \quad Y = r(s,a^\star) + \gamma \hat{\mu}^{(2)}_N(s,a^\star).
$$

Now $a^\star$ depends on $\varepsilon^{(1)}$ but the target uses $\varepsilon^{(2)}_{a^\star}$. To see why this eliminates evaluation bias, write the expectation as a nested expectation:
...
The inner expectation equals $r(s,a^\star) + \gamma \mu(s,a^\star)$ because $\varepsilon^{(2)}_{a^\star}$ is independent of $a^\star$ (selection used $\varepsilon^{(1)}$), so $\mathbb{E}[\varepsilon^{(2)}_{a^\star} \mid a^\star] = 0$. Taking the outer expectation:

$$
\mathbb{E}[Y] = \mathbb{E}_{\varepsilon^{(1)}}\big[r(s,a^\star) + \gamma \mu(s,a^\star)\big].
$$

The evaluation noise contributes zero on average. However, selection bias remains: $a^\star$ still depends on $\varepsilon^{(1)}$, so $\mathbb{E}[Y] \ne \max_a \{r(s,a) + \gamma \mu(s,a)\}$ in general. Double Q-learning eliminates evaluation bias but not selection bias, yielding much smaller total bias than the standard estimator where both compound.

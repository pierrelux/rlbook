...
### The Target Network Is Not a Design Choice

One of the most frequently cited "innovations" of DQN is the use of a target network. This framing is misleading. **Target networks are not a design choice—they are an inherent consequence of approximate value iteration's mathematical structure.** Understanding this distinction is crucial for seeing deep RL algorithms as what they truly are: implementations of classical dynamic programming principles with function approximation.

Consider the nested structure of fitted Q-iteration:

**Outer loop (Value Iteration):** At iteration $k$, use current parameters $\boldsymbol{\theta}_k$ to compute targets for all transitions:

$$
y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}_k)
$$

**Inner loop (Regression):** Solve the regression problem to convergence (or near-convergence):

$$
\boldsymbol{\theta}_{k+1} = \arg\min_{\boldsymbol{\theta}} \sum_i (q(s_i, a_i; \boldsymbol{\theta}) - y_i)^2
$$

This structure has a critical property: **the targets $\{y_i\}$ remain fixed throughout the entire inner optimization**. We compute them once using $\boldsymbol{\theta}_k$ at the start of iteration $k$, then they don't change while we run gradient descent to find $\boldsymbol{\theta}_{k+1}$. This fixed-target property is not optional—it defines what it means to apply the Bellman operator and then project.

When we flatten this nested structure into a single loop (for implementation convenience, especially in online settings), we must preserve this mathematical property. The "target network" $\boldsymbol{\theta}_{\text{target}}$ simply plays the role that $\boldsymbol{\theta}_k$ played in the nested version: it provides fixed targets for multiple gradient steps. The periodic synchronization $\boldsymbol{\theta}_{\text{target}} \leftarrow \boldsymbol{\theta}$ marks what would have been the boundary of an outer iteration.

```{prf:remark} Target Networks: Implementation Detail, Not Innovation
:class: dropdown

Compare these two equivalent formulations:

**Nested FQI (explicit structure):**
```
for k = 0, 1, 2, ...:
    # Outer iteration: compute targets using current Q-function
    for each transition (s_i, a_i, r_i, s'_i):
        y_i ← r_i + γ max_a' q(s'_i, a'; θ_k)
    
    # Inner iteration: fit to fixed targets
    θ_{k+1} ← fit(D^fit, θ_init=θ_k, K gradient steps)
```

**Flattened FQI (DQN-style):**
```
θ_target ← θ_0
for t = 0, 1, 2, ...:
    # Compute target using target network (plays role of θ_k)
    for each transition (s_i, a_i, r_i, s'_i):
        y_i ← r_i + γ max_a' q(s'_i, a'; θ_target)
    
    # One gradient step
    θ_{t+1} ← θ_t - α ∇_θ L(θ_t; D^fit)
    
    # Periodically update target (marks outer iteration boundary)
    if t mod K = 0:
        θ_target ← θ_t
```

These are **algorithmically equivalent** when the update frequency $K$ in the flattened version equals the number of gradient steps per outer iteration in the nested version. The flattened version doesn't add anything new—it simply unrolls the nested loops and makes the parameter management explicit.
```

The target network is not a "trick" to stabilize training, though it does have that effect. It's the only way to correctly implement approximate value iteration when using gradient-based optimization. **If you don't use a target network (or equivalently, if you update targets after every gradient step), you are no longer doing value iteration—you are doing something else entirely**, akin to residual minimization methods (Baird 1995) that have different convergence properties and typically require access to double sampling at each state-action pair.

The same principle applies to exponential moving average (EMA) target updates $\boldsymbol{\theta}_{\text{target}} \leftarrow \tau \boldsymbol{\theta} + (1-\tau)\boldsymbol{\theta}_{\text{target}}$ used in continuous control algorithms (DDPG, TD3, SAC). EMA targets smooth the transition between outer iterations but preserve the core principle: targets should change slowly relative to the online network, maintaining approximate separation between "applying the Bellman operator" and "fitting to its output."

This perspective clarifies the entire algorithm space. FQI, NFQI, DQN, Double DQN, and Q-learning are not fundamentally different algorithms—they are different points on a continuum defined by:

1. **Truncation parameter** $K$: How many gradient steps per outer iteration? ($K=\infty$ for FQI/NFQI, $K \in [1,4]$ for DQN, $K=1$ for Q-learning)
...
The "target network" is not on this list because it's not a choice—it's what approximate value iteration means when written in code.

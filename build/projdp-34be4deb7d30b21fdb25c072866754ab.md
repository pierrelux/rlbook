...
The weighted residual framework developed in this chapter applies to any functional equation, not just the Bellman equation for value functions. An alternative approach, widely used in economics and macroeconomics, directly approximates the **optimal policy** by solving the first-order conditions that characterize optimality {cite}`Judd1992`. This class of methods, known as Euler equation methods, treats the optimal policy $\pi^*$ as the solution to a different functional equation: the Euler equation.

### From Trajectory Optimization to the Euler Equation

Recall from continuous-time optimal control that when the Hamiltonian is differentiable in the control variable and the optimal control is interior, the first-order necessary condition for optimality is:

$$
\frac{\partial H}{\partial u}(t) = 0,
$$

which, combined with the costate equation, yields a condition relating the current control to future states and costates. For problems with smooth value functions, this can be expressed as a condition on the policy alone, without explicitly computing the value function or costate trajectory.
...
**Step 2: Eliminate $v^*$ using policy evaluation.** The first-order condition involves both the policy $\pi^*$ and the value function $v^*$. However, since $\pi^*$ is optimal, its evaluated value equals $v^*$. Under policy $\pi^*$, let $P_{\pi^*}$ denote the transition matrix with entries $P_{\pi^*}(j|s) = p(j|s,\pi^*(s))$ and let $r_{\pi^*}(s) = r(s, \pi^*(s))$. The value function satisfies:

$$
v^* = v_{\pi^*} = r_{\pi^*} + \gamma P_{\pi^*} v_{\pi^*},
$$

which has the closed-form solution (geometric series / fundamental matrix):

$$
v^* = (I - \gamma P_{\pi^*})^{-1} r_{\pi^*} = \sum_{t=0}^\infty \gamma^t P_{\pi^*}^t r_{\pi^*}.
$$

Substituting this into the first-order condition eliminates $v^*$ entirely:

$$
\frac{\partial r(s, \pi^*(s))}{\partial a} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi^*(s)} \left[(I - \gamma P_{\pi^*})^{-1} r_{\pi^*}\right](j) = 0, \quad \forall s \in \mathcal{S}.
\tag{Euler}
$$

This is the **discrete-time Euler equation**: a functional equation in the policy $\pi^*(\cdot)$ alone, expressed purely in terms of primitives $(r,p,\gamma)$ and the unknown policy. The resolvent $(I - \gamma P_{\pi^*})^{-1}$ encodes the discounted future visitation under $\pi^*$.

**Remark (Deterministic dynamics).** When transitions are deterministic, $p(\cdot|s,a) = \delta_{f(s,a)}$, and $f$ is differentiable in $a$, the Euler equation simplifies. The gradient $\nabla_a p(j|s,a)$ becomes $\nabla_a f(s,a)^\top \nabla v^*(j)\big|_{j=f(s,a)}$, yielding:

$$
\nabla_a r\big(s,\pi^*(s)\big) + \gamma \nabla_a f\big(s,\pi^*(s)\big)^\top \nabla v^*\big(f(s,\pi^*(s))\big) = 0.
$$

This is the standard discrete-time adjoint equation. The value function $v^*$ can still be eliminated via the resolvent, but in practice it's often more convenient to work with this form and use the envelope theorem to express $\nabla v^*$ in terms of the policy, yielding a differential equation in $\pi^*$.
...
We can formalize the Euler equation as a fixed-point problem on the space of policies. Define the **Euler residual operator** $\mathcal{R}_E : \Pi \to \mathbb{R}^{|\mathcal{S}|}$ by:

$$
\mathcal{R}_E(\pi)(s) = \frac{\partial r(s, \pi(s))}{\partial a} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi(s)} \left[(I - \gamma P_{\pi})^{-1} r_{\pi}\right](j).
$$

The optimal policy $\pi^*$ satisfies the functional equation:

...
where the residual is $R(s; \theta) = \Residual(\hat{f}(\cdot; \theta))(s)$ and the approximation is $\hat{f}(s; \theta) = \sum_{j=1}^n \theta_j \varphi_j(s)$. For the Bellman equation, $\Residual(v) = \mathrm{L}v - v$, so $R(s; \theta) = \mathrm{L}\hat{v}(s; \theta) - \hat{v}(s; \theta)$. This transforms the impossible task of verifying "$R(s) = 0$ for all $s$" into a finite-dimensional problem: find $n$ coefficients $\theta = (\theta_1, \ldots, \theta_n)$ satisfying $n$ weighted integral conditions.
...
where $\Residual: B_1 \to B_2$ is a continuous operator between complete normed vector spaces $B_1$ and $B_2$. For the Bellman equation, we have $\Residual(v) = \mathrm{L}v - v$, so that solving $\Residual(v) = 0$ is equivalent to finding the fixed point $v = \mathrm{L}v$.
...
When $\Contraction$ involves optimization (as in the Bellman operator $\mathrm{L}v = \max_a \{r(s,a) + \gamma \mathbb{E}[v(s')]\}$), computing this derivative appears problematic because the max operator is not differentiable. However, the **Envelope Theorem** resolves this difficulty.
...
**Newton's method** for Galerkin similarly requires integration. The residual is $R(s; \theta) = \mathrm{L}\hat{v}(s; \theta) - \hat{v}(s; \theta)$, and we need $G_i(\theta) = \int R(s; \theta) \varphi_i(s) w(s) ds = 0$. The Jacobian entry is:
...
2. **repeat**
    1. For each $i = 1, \ldots, n$:
        1. $G_i^{(k)} \leftarrow \int_{\mathcal{S}} [\mathrm{L}\hat{v}(s; \theta^{(k)}) - \hat{v}(s; \theta^{(k)})] \varphi_i(s) w(s) ds$
...
When $\xi$ is the stationary distribution of policy $\pi$ (so $\xi^\top \mathbf{P}_\pi = \xi^\top$), this system has a unique solution, and the projected Bellman operator $\Proj \mathrm{L}_\pi$ is a contraction in the weighted $L^2$ norm $\|\cdot\|_\xi$. This is the theoretical foundation for TD learning with linear function approximation.
...
$$
J_G(\theta) = \frac{\partial G}{\partial \theta} = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} \left( \frac{\partial \mathrm{L}\hat{v}(\cdot; \theta)}{\partial \theta} - \boldsymbol{\Phi} \right).
$$
...
$$
G(\theta^{(k)}) = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} (\mathrm{L}\hat{v}(\cdot; \theta^{(k)}) - \boldsymbol{\Phi} \theta^{(k)}).
$$
...
But regardless of which projection method we use, iteration takes the form $\hat{v}^{(k+1)} = \Proj \mathrm{L}\hat{v}^{(k)}$.

**Does the composition $\Proj \mathrm{L}$ inherit the contraction property of $\mathrm{L}$?** If not, the iteration may diverge, oscillate, or converge to a spurious fixed point even though the original problem is well-posed.
...
Let $\mathrm{L}: \mathcal{V} \to \mathcal{V}$ be a $\gamma$-contraction on the space $\mathcal{V}$ of bounded functions with respect to the sup norm. Let $\Proj: \mathcal{V} \to \mathcal{V}$ be a linear approximation operator satisfying monotonicity and constant preservation.

Then the composed operator $\Proj \mathrm{L}$ is a $\gamma$-contraction, and projected value iteration $v_{k+1} = \Proj \mathrm{L} v_k$ converges globally to a unique fixed point $v_\Proj \in \text{Range}(\Proj)$ with approximation error:
...
```{prf:proof}
Since $\mathrm{L}$ is a $\gamma$-contraction, we have $-\gamma\|f-g\|_\infty \leq \mathrm{L} f - \mathrm{L} g \leq \gamma\|f-g\|_\infty$ pointwise. By monotonicity of $\Proj$, $\Proj(-\gamma\|f-g\|_\infty) \leq \Proj(\mathrm{L} f - \mathrm{L} g) \leq \Proj(\gamma\|f-g\|_\infty)$. By constant preservation, $-\gamma\|f-g\|_\infty \leq \Proj(\mathrm{L} f - \mathrm{L} g) \leq \gamma\|f-g\|_\infty$, which implies $\|\Proj \mathrm{L} f - \Proj \mathrm{L} g\|_\infty \leq \gamma\|f-g\|_\infty$.

The error bound follows from fixed-point analysis: $v^* - v_\Proj = (I - \Proj \mathrm{L})^{-1}(v^* - \Proj v^*)$, and since $\Proj \mathrm{L}$ is a $\gamma$-contraction, $\|(I - \Proj \mathrm{L})^{-1}\| \leq (1-\gamma)^{-1}$.
```
...
If $\Proj$ is an averager and $\mathrm{L}$ is the Bellman operator (a $\gamma$-contraction), then $\Proj \mathrm{L}$ is a $\gamma$-contraction, and value iteration $v_{k+1} = \Proj \mathrm{L} v_k$ converges to a unique fixed point.
```
...
## Contraction in Weighted $L^2$ Norms: Policy Evaluation

The policy evaluation problem for Galerkin in discrete MDPs (covered earlier) admits a closed-form LSTD solution. But understanding when iterative algorithms converge requires a different analysis than we used for monotone approximators. The earlier theory characterized contraction preservation in the **sup norm** $\|\cdot\|_\infty$. For policy evaluation with Galerkin projection, the natural setting is a **weighted $L^2$ geometry**, where the weight is the stationary distribution.

For policy $\pi$, the Galerkin conditions gave us the LSTD equation $\boldsymbol{\Phi}^\top \boldsymbol{\Xi} (\boldsymbol{\Phi} - \gamma \mathbf{P}_\pi \boldsymbol{\Phi}) \boldsymbol{\theta} = \boldsymbol{\Phi}^\top \boldsymbol{\Xi} \mathbf{r}_\pi$. The solution satisfies $\hat{v} = \Proj \mathrm{L}_\pi \hat{v}$, where $\Proj$ is the projection operator onto $\text{span}(\boldsymbol{\Phi})$ with respect to the $\xi$-weighted inner product:
...
The solution finds the fixed point of the *projected* Bellman operator, not the Bellman operator itself. Approximation error persists even at convergence.

### When is $\Proj \mathrm{L}_\pi$ a Contraction?

For iterative projected value iteration $v_{k+1} = \Proj \mathrm{L}_\pi v_k$ to converge, we need $\Proj \mathrm{L}_\pi$ to be a contraction in $\|\cdot\|_\xi$. The projection $\Proj$ is always a non-expansion by the Pythagorean identity. The Bellman operator $\mathrm{L}_\pi = r_\pi + \gamma \mathbf{P}_\pi$ is a $\gamma$-contraction in $\|\cdot\|_\infty$, but whether it contracts in $\|\cdot\|_\xi$ depends on the relationship between $\xi$ and $\mathbf{P}_\pi$.
...
where the last equality uses $\sum_s \xi(s) p(s'|s,\pi(s)) = \xi(s')$. Therefore $\mathbf{P}_\pi$ is non-expansive in $\|\cdot\|_\xi$, making $\mathrm{L}_\pi$ a $\gamma$-contraction. Composing with the non-expansive projection:

$$
\|\Proj \mathrm{L}_\pi v - \Proj \mathrm{L}_\pi w\|_\xi \leq \|\mathrm{L}_\pi v - \mathrm{L}_\pi w\|_\xi \leq \gamma \|v - w\|_\xi.
$$

By Banach's fixed-point theorem, $\Proj \mathrm{L}_\pi$ has a unique fixed point and iterates converge from any initialization.
...
**On-policy vs off-policy.** When $\xi$ is the stationary distribution of $\pi$ (on-policy learning), convergence is guaranteed. When $\xi$ is not stationary (off-policy learning with data from a different behavior policy), the Jensen argument breaks down. The operator $\mathbf{P}_\pi$ need not be non-expansive in $\|\cdot\|_\xi$, and $\Proj \mathrm{L}_\pi$ may fail to contract. This explains off-policy divergence in linear TD learning (e.g., Baird's counterexample). Importance weighting and other corrections are designed to restore the appropriate weighting.

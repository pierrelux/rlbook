...
In the reinforcement learning setting, our gradient estimator is a sum over timesteps: $\sum_{t=0}^T Z_t$ where each $Z_t$ is the gradient contribution at time $t$. We apply control variates separately to each term. Since $\text{Var}(\sum_t Z_t) = \sum_t \text{Var}(Z_t) + \sum_{t \neq s} \text{Cov}(Z_t, Z_s)$, reducing the variance of each $Z_t$ reduces the total variance, though we do not explicitly address the cross-timestep covariance terms.

For a given state $s_t$, our estimator at time $t$ is:

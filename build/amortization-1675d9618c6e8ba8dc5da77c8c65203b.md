...
For a parameterized policy $\pi_\theta(s)$, we discretize the Euler equation using weighted residuals. With collocation at points $s_1, \dots, s_N$, we solve

$$
G(\theta) := \begin{bmatrix}
\mathcal{E}(\pi_\theta)(s_1) \\
\vdots \\
\mathcal{E}(\pi_\theta)(s_N)
\end{bmatrix} = 0.
$$
...
$$
\int \varphi_i(s) \mathcal{E}(\pi_\theta)(s) \mu(ds) = 0,
\quad i = 1,\dots,M.
$$

The mapping $G$ is nonlinear in $\theta$, requiring Newton-type methods or other root-finding schemes. Unlike the Bellman operator, this operator is not a contraction, so convergence guarantees are problem-dependent.
...
SAC uses the same policy amortization strategy as DDPG: learn a parametric policy $\pi_\phi$ that approximates the optimal stochastic policy (the Boltzmann distribution). The policy enables fast action selection through a single forward pass rather than solving an optimization problem. Exploration emerges naturally from the policy's stochasticity rather than from external noise.

## Bootstrap Targets via Single-Sample Estimation

With a learned policy $\pi_\phi$, we can compute Q-function bootstrap targets. For a transition $(s,a,r,s')$, we need the soft value at $s'$:

$$
v(s') = \mathbb{E}_{a' \sim \pi_\phi(\cdot|s')}\left[q(s',a') - \alpha \log \pi_\phi(a'|s')\right]
$$

SAC estimates this with a single Monte Carlo sample: draw $\tilde{a}' \sim \pi_\phi(\cdot|s')$ and approximate:

$$
y = r + \gamma \left[\min_{j=1,2} q^j_{\theta_{\text{target}}}(s', \tilde{a}') - \alpha \log \pi_\phi(\tilde{a}'|s')\right]
$$
...
The Q-network update assumes a policy $\pi_\phi$ that approximates the Boltzmann distribution $\pi^*(a|s) \propto \exp(\beta \cdot q^*(s,a))$. Training such a policy presents a problem: the Boltzmann distribution requires the partition function $Z(s) = \int_A \exp(\beta \cdot q(s,a))da$, the very integral we are trying to avoid. SAC sidesteps this by minimizing the KL divergence from the policy to the (unnormalized) Boltzmann distribution:

$$
\min_{\phi} \mathbb{E}_{s \sim \mathcal{D}}\left[D_{KL}\left(\pi_{\phi}(\cdot|s) \| \frac{\exp(\beta \cdot q_\theta(s,\cdot))}{Z_\theta(s)}\right)\right]
$$

Since $\log Z_\theta(s)$ does not depend on $\phi$, this reduces to:

$$
\min_{\phi} \mathbb{E}_{s \sim \mathcal{D}}\mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}\left[\alpha \log \pi_{\phi}(a|s) - q_\theta(s,a)\right]
$$

This pushes probability toward high Q-value actions while the $\log \pi_\phi$ term penalizes concentrating probability mass, maintaining entropy. The entropy bonus emerges from the KL divergence structure rather than from an explicit regularization term.

To estimate gradients of this objective, we face a technical problem: the policy parameters $\phi$ appear in the sampling distribution $\pi_\phi$, making $\nabla_\phi \mathbb{E}_{a \sim \pi_\phi}[\cdot]$ difficult to compute. SAC uses a Gaussian policy $\pi_\phi(a|s) = \mathcal{N}(\mu_\phi(s), \sigma_\phi(s)^2)$ with the reparameterization trick. Express samples as a deterministic function of parameters and independent noise:

$$
a = f_\phi(s, \epsilon) = \mu_\phi(s) + \sigma_\phi(s) \odot \epsilon, \quad \epsilon \sim \mathcal{N}(0, I)
$$

This moves $\phi$ out of the sampling distribution and into the integrand:

$$
\min_{\phi} \mathbb{E}_{s \sim \mathcal{D}}\mathbb{E}_{\epsilon \sim \mathcal{N}(0,I)}\left[\alpha \log \pi_\phi(f_\phi(s,\epsilon)|s) - q_\theta(s,f_\phi(s,\epsilon))\right]
$$

We can now differentiate through $f_\phi$ and the Q-network, as DDPG differentiates through a deterministic policy. SAC extends this by sampling noise $\epsilon$ at each gradient step rather than outputting a single deterministic action.


```{prf:algorithm} Soft Actor-Critic (SAC)
:label: sac

**Input**: MDP $(S, A, P, R, \gamma)$, twin Q-networks $q^1, q^2$, policy $\pi_{\phi}$, learning rates $\alpha_q, \alpha_\pi$, replay buffer capacity $B$, mini-batch size $b$, EMA rate $\tau$, entropy weight $\alpha$
...
5. **while** training **do**
    1. Observe state $s$
    2. Sample action: $a \sim \pi_{\phi_t}(\cdot|s)$ (stochastic policy provides exploration)
    ...
    7. For each $s'_i$, sample $\tilde{a}'_i \sim \pi_{\phi_t}(\cdot|s'_i)$
    8. $y_i \leftarrow r_i + \gamma \left[\min(q^1(s'_i, \tilde{a}'_i; \boldsymbol{\theta}^1_{\text{target}}), q^2(s'_i, \tilde{a}'_i; \boldsymbol{\theta}^2_{\text{target}})) - \alpha \log \pi_{\phi_t}(\tilde{a}'_i|s'_i)\right]$
    ...
    12. For each $s_i$, sample $\epsilon_i \sim \mathcal{N}(0,I)$ and compute $\hat{a}_i = f_{\phi_t}(s_i, \epsilon_i)$ (reparameterization)
    13. $\boldsymbol{\phi}_{t+1} \leftarrow \boldsymbol{\phi}_t - \alpha_\pi \nabla_{\boldsymbol{\phi}} \frac{1}{b}\sum_{i=1}^b \left[\alpha \log \pi_{\phi_t}(\hat{a}_i|s_i) - \min_{j=1,2} q^j(s_i, \hat{a}_i; \boldsymbol{\theta}^j_{t+1})\right]$
...
Recall from the [smoothing chapter](smoothing.md) that entropy-regularized RL optimizes:

$$
J(\pi) = \mathbb{E}_{\tau \sim \pi}\left[\sum_{t=0}^\infty \gamma^t[r(s_t, a_t) + \alpha \mathcal{H}(\pi(\cdot|s_t))]\right]
$$

where $\alpha > 0$ is the temperature parameter (entropy weight). SAC showed that the optimal policy satisfies the Boltzmann distribution $\pi^*(a|s) \propto \exp(q^*(s,a)/\alpha)$, and that optimal values satisfy soft Bellman consistency at each transition.

PCL exploits a property that becomes available when the MDP dynamics are deterministic: $s' = f(s, a)$ for some transition function $f$. Under deterministic dynamics, any observed trajectory $s_1, a_1, s_2, a_2, \ldots, s_t$ is fully determined by the initial state $s_1$ and the action sequence $a_1, \ldots, a_{t-1}$. There is no stochasticity to average over. This determinism allows us to extend the single-step soft Bellman equation into a multi-step algebraic identity.

Consider telescoping the soft Bellman equation along a path. At each step, the optimal values satisfy $v^*(s_i) = r(s_i, a_i) - \alpha \log \pi^*(a_i|s_i) + \gamma v^*(s_{i+1})$. Rearranging and summing from $i=1$ to $i=t-1$:

$$
v^*(s_1) - \gamma^{t-1} v^*(s_t) = \sum_{i=1}^{t-1} \gamma^{i-1}[r(s_i, a_i) - \alpha \log \pi^*(a_i|s_i)]
$$

This path consistency equation is not an expectation—it is an algebraic identity that must hold for any realized trajectory under the optimal policy and value function. The deterministic dynamics are essential: if transitions were stochastic, we would need to take expectations over possible next states, and the multi-step relationship would involve nested expectations that depend on the trajectory distribution. With deterministic dynamics, the trajectory is fixed once the actions are chosen, and the consistency becomes a pointwise constraint.

This deterministic structure has a striking consequence: the path consistency equation holds regardless of how the action sequence was generated. Whether the actions came from the optimal policy, a previous policy iteration, an exploration policy, or even expert demonstrations, the same algebraic relationship must be satisfied by $(v^*, \pi^*)$. The actions need not be optimal; the equation relates the endpoint state values to the rewards and log-probabilities observed along any realized path.

### From Consistency to Learning
...
$$
C(s_i:s_{i+d}, \boldsymbol{\theta}, \boldsymbol{\phi}) = -v_{\boldsymbol{\phi}}(s_i) + \gamma^d v_{\boldsymbol{\phi}}(s_{i+d}) + \sum_{j=0}^{d-1} \gamma^j[r(s_{i+j}, a_{i+j}) - \alpha \log \pi_{\boldsymbol{\theta}}(a_{i+j}|s_{i+j})]
$$

where $\pi_{\boldsymbol{\theta}}$ is a parametric policy and $v_{\boldsymbol{\phi}}$ is a value function approximation. At optimality, this error is zero for all paths. PCL minimizes the squared consistency error over a set of trajectories $\mathcal{E}$:

$$
\min_{\boldsymbol{\theta}, \boldsymbol{\phi}} \sum_{s_i:s_{i+d} \in \mathcal{E}} \frac{1}{2} C(s_i:s_{i+d}, \boldsymbol{\theta}, \boldsymbol{\phi})^2
$$
...
$$
\begin{align}
\Delta\boldsymbol{\theta} &= \eta_\pi C(s_i:s_{i+d}, \boldsymbol{\theta}, \boldsymbol{\phi}) \sum_{j=0}^{d-1} \gamma^j \nabla_{\boldsymbol{\theta}} \log \pi_{\boldsymbol{\theta}}(a_{i+j}|s_{i+j}) \\
\Delta\boldsymbol{\phi} &= \eta_v C(s_i:s_{i+d}, \boldsymbol{\theta}, \boldsymbol{\phi})\left[\nabla_{\boldsymbol{\phi}} v_{\boldsymbol{\phi}}(s_i) - \gamma^d \nabla_{\boldsymbol{\phi}} v_{\boldsymbol{\phi}}(s_{i+d})\right]
\end{align}
$$
...
### Off-Policy Learning Without Importance Correction

The deterministic dynamics that enable path consistency also eliminate the need for importance sampling. Standard off-policy methods face a distribution mismatch: data collected under behavior policy $\mu$ must be used to learn about target policy $\pi$, requiring importance weights $\prod_t \pi(a_t|s_t)/\mu(a_t|s_t)$ that can have high variance over long horizons.

PCL sidesteps this problem entirely. The path consistency equation is a constraint on the optimal $(v^*, \pi^*)$ pair that must hold for any realized trajectory—it says nothing about how that trajectory was generated. Given a path $s_1, a_1, \ldots, s_t$, the consistency error measures how far the current estimates $(v_{\boldsymbol{\phi}}, \pi_{\boldsymbol{\theta}})$ are from satisfying the optimality condition along that specific sequence of states and actions. The generating policy $\mu$ never enters the equation because deterministic dynamics mean the trajectory is fully determined by its initial state and action sequence. There is no probability distribution over trajectories to correct for.

This makes PCL effective at exploiting diverse trajectory sources. The replay buffer can contain trajectories from the current policy, from earlier policy iterations, or from entirely different sources such as expert demonstrations. Each trajectory provides an equally valid training signal because the consistency constraint is policy-agnostic.
...
**Input**: MDP $(S, A, P, R, \gamma)$, policy $\pi_{\boldsymbol{\theta}}$, value function $v_{\boldsymbol{\phi}}$, temperature $\alpha$, rollout length $d$, learning rates $\eta_\pi, \eta_v$, replay buffer capacity $B$, prioritization parameter $\lambda$
...
4. **while** training **do**
    ...
    2. Sample trajectory $\tau = (s_0, a_0, r_0, \ldots, s_T)$ from $\pi_{\boldsymbol{\theta}_t}$
    ...
    5. **for** each sub-trajectory $s_i:s_{i+d}$ in $\tau$ **do**
        1. Compute $C_i = C(s_i:s_{i+d}, \boldsymbol{\theta}_t, \boldsymbol{\phi}_t)$
        2. $\boldsymbol{\theta}_{t+1} \leftarrow \boldsymbol{\theta}_t + \eta_\pi C_i \sum_{j=0}^{d-1} \gamma^j \nabla_{\boldsymbol{\theta}} \log \pi_{\boldsymbol{\theta}_t}(a_{i+j}|s_{i+j})$
        3. $\boldsymbol{\phi}_{t+1} \leftarrow \boldsymbol{\phi}_t + \eta_v C_i[\nabla_{\boldsymbol{\phi}} v_{\boldsymbol{\phi}_t}(s_i) - \gamma^d \nabla_{\boldsymbol{\phi}} v_{\boldsymbol{\phi}_t}(s_{i+d})]$
    ...
    10. $t \leftarrow t + 1$
5. **return** $\boldsymbol{\theta}_t$, $\boldsymbol{\phi}_t$
```

### Unified PCL: Actor and Critic from a Single Network
...
Unified PCL parameterizes only Q-values $q_{\boldsymbol{\rho}}(s,a)$ and derives:

$$
\begin{align}
v_{\boldsymbol{\rho}}(s) &= \alpha \log \sum_a \exp(q_{\boldsymbol{\rho}}(s,a)/\alpha) \\
\pi_{\boldsymbol{\rho}}(a|s) &= \exp((q_{\boldsymbol{\rho}}(s,a) - v_{\boldsymbol{\rho}}(s))/\alpha)
\end{align}
$$
...
$$
C(s_i:s_{i+d}, \boldsymbol{\rho}) = -v_{\boldsymbol{\rho}}(s_i) + \gamma^d v_{\boldsymbol{\rho}}(s_{i+d}) + \sum_{j=0}^{d-1} \gamma^j[r(s_{i+j}, a_{i+j}) - \alpha \log \pi_{\boldsymbol{\rho}}(a_{i+j}|s_{i+j})]
$$

and the unified update combines both policy and value gradients:

$$
\Delta\boldsymbol{\rho} = \eta_\pi C \sum_{j=0}^{d-1} \gamma^j \nabla_{\boldsymbol{\rho}} \log \pi_{\boldsymbol{\rho}}(a_{i+j}|s_{i+j}) + \eta_v C\left[\nabla_{\boldsymbol{\rho}} v_{\boldsymbol{\rho}}(s_i) - \gamma^d \nabla_{\boldsymbol{\rho}} v_{\boldsymbol{\rho}}(s_{i+d})\right]
$$

This eliminates the actor-critic distinction entirely. The single Q-network simultaneously represents the value function (through the soft-max) and the policy (through the Boltzmann distribution). In practice, the two terms are still weighted with separate learning rates $\eta_\pi$ and $\eta_v$ to balance their contributions, but both update the same parameters $\boldsymbol{\rho}$.
...
$$
C(s_i:s_{i+d}, \boldsymbol{\theta}, \boldsymbol{\phi}) \to -v_{\boldsymbol{\phi}}(s_i) + \gamma^d v_{\boldsymbol{\phi}}(s_{i+d}) + \sum_{j=0}^{d-1} \gamma^j r(s_{i+j}, a_{i+j})
$$

which is precisely the $d$-step advantage estimate used in A3C. However, A3C requires that $v_{\boldsymbol{\phi}}$ tracks the current policy's value function $v^{\pi_{\boldsymbol{\theta}}}$ to reduce variance. PCL has no such requirement—$v_{\boldsymbol{\phi}}$ converges to the optimal value $v^*$ regardless of the policy's evolution.

With $d=1$ and using the unified parameterization, PCL becomes soft Q-learning. The consistency error simplifies to:

$$
C(s:s', \boldsymbol{\rho}) = -v_{\boldsymbol{\rho}}(s) + \gamma v_{\boldsymbol{\rho}}(s') + r - \alpha \log \pi_{\boldsymbol{\rho}}(a|s)
$$

By the Boltzmann policy definition, $-\alpha \log \pi_{\boldsymbol{\rho}}(a|s) = v_{\boldsymbol{\rho}}(s) - q_{\boldsymbol{\rho}}(s,a)$, so:

$$
C(s:s', \boldsymbol{\rho}) = r + \gamma v_{\boldsymbol{\rho}}(s') - q_{\boldsymbol{\rho}}(s,a)
$$
...
SAC uses single-sample Monte Carlo to estimate the soft value: for each transition, it samples one next action $a' \sim \pi_{\boldsymbol{\phi}}(s')$ and computes $y = r + \gamma[q(s',a') - \alpha \log \pi_{\boldsymbol{\phi}}(a'|s')]$. PCL uses multi-step rollouts with the observed action sequence, incorporating log-probabilities along the entire path.
...
The consistency error $C(s_i:s_{i+d}, \boldsymbol{\theta}, \boldsymbol{\phi})$ implicitly involves solving for the optimal trajectory-level values. By minimizing squared consistency error across many paths, PCL effectively distributes the computational cost of verifying optimality conditions across the training process. Each path update nudges both policy and value toward satisfying the consistency equation, gradually amortizing the cost of finding a globally consistent solution.

The deterministic dynamics assumption is what makes this amortization possible. With stochastic transitions, each $(s, a)$ pair could lead to many different next states, and a multi-step consistency equation would require expectations over exponentially many trajectory branches. The path consistency constraint would no longer be a pointwise algebraic identity but an integral equation. Deterministic dynamics collapse this branching structure: each path is uniquely determined, and the consistency equation becomes a constraint that can be verified and enforced on individual realized trajectories. This structural simplification is what allows PCL to use any trajectory—regardless of its generating policy—as a valid training signal.

# Summary

This chapter addressed the computational barrier that arises when extending value-based fitted methods to continuous action spaces. The core issue is tractability: computing $\max_{a \in \mathbb{R}^m} q(s,a;\theta)$ at each Bellman target evaluation requires solving a nonlinear optimization problem. For replay buffers containing millions of transitions, repeatedly solving these optimization problems becomes prohibitive.

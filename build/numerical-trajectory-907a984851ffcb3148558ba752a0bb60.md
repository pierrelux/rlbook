...
June 1, 2023, at 12:00 UTC {cite:p}`ERA52018`. Preprocessing converts the
...
in place. The initial geometric reference must pass the same validation;
if it does not, the simulation reports a planning failure instead of
returning it as a feasible flight.
...
construct the frozen plan. The two replanning variants rebuild a geometric
reference from the measured state, then perform two Gaussian mean updates.
They do not reuse the previous knot coordinates across decisions; within
one decision, each accepted mean initializes the next sampling update.
The generic shifted-sequence warm start in @mppi is an alternative students
can implement when extending this flight parameterization.
...
```{code-cell} python
:tags: [remove-input]
from pathlib import Path as _FlightPath
from IPython.display import Markdown as _FlightMarkdown, display as _flight_display
_flight_display(_FlightMarkdown(_FlightPath("artifacts/aircraft_mppi/results.md").read_text()))
```
...
validation and comparisons within its chosen trajectory family. We next
return to the representation of trajectories and examine how multiple
shooting combines simulated segments with explicit state variables.

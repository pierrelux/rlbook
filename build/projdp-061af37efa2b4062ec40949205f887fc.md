...
Consider the Galerkin approach for linear function approximation $v_{\boldsymbol{\theta}} = \sum_{i=1}^d \theta_i \varphi_i$. The orthogonality conditions $\langle v - \Bellman v, \varphi_i \rangle_w = 0$ for all $i$ produce a linear system that can be solved in closed form. These equations have an equivalent interpretation as a least-squares problem: minimize the weighted norm of the residual $\|v - \Bellman v\|_w^2$ over the subspace spanned by $\{\varphi_1, \ldots, \varphi_d\}$. At the minimum, the gradient with respect to each coefficient must vanish, which gives exactly the orthogonality conditions.

For nonlinear approximators, we keep the least-squares interpretation but lose the closed-form solution. Given a parameterized function class $v_{\boldsymbol{\theta}}$ (e.g., a neural network), we seek

$$
\boldsymbol{\theta}^* = \arg\min_{\boldsymbol{\theta}} \|v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}\|_w^2.
$$

Taking the gradient with respect to the parameters $\boldsymbol{\theta}$ and setting it to zero gives the first-order stationarity condition:

$$
\frac{\partial}{\partial \theta_i} \|v_{\boldsymbol{\theta}} - \Bellman v_{\boldsymbol{\theta}}\|_w^2 = 0 \quad \text{for all } i.
$$

By the chain rule and the definition of the weighted norm $\|f\|_w^2 = \langle f, f \rangle_w$, this is equivalent to
...
These are orthogonality conditions, just like in the linear case. The difference is that the "test functions" are no longer fixed basis functions $\varphi_i$, but are now the partial derivatives $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i}$. These partial derivatives span the tangent space to the manifold $\{v_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \mathbb{R}^p\}$ at the current parameter values. The stationarity condition requires the residual to be orthogonal to all directions in which we can move within the manifold.

When $v_{\boldsymbol{\theta}} = \sum_i \theta_i \varphi_i$ is a linear combination of fixed basis functions, we have $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} = \varphi_i$, and the nonlinear stationarity condition reduces to the Galerkin equations. For truly nonlinear parameterizations (neural networks, kernel methods), the partial derivatives change with $\boldsymbol{\theta}$, and we solve the stationarity condition numerically by gradient descent. The resulting solution defines an implicit nonlinear projection operator $\Pi_{\mathcal{F}}$, generalizing the linear projection $\Proj$.

The key insight is that minimizing a norm $\|R[v_{\boldsymbol{\theta}}]\|^2$ is equivalent to requiring orthogonality $\langle R[v_{\boldsymbol{\theta}}], \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \rangle = 0$ for all $i$, where the inner product $\langle \cdot, \cdot \rangle$ defines the norm via $\|f\|^2 = \langle f, f \rangle$. This equivalence holds for any choice of inner product: weighted $L^2$ integrals for Galerkin, sums over collocation points for collocation, or sampled expectations for neural networks.

The **dual pairing** formulation {cite}`LegrandJunca2025` provides the abstract framework unifying all these cases. Intuitively, a dual space is the collection of all ways to "measure" or "test" functions. If $\mathcal{H}$ is a space of test functions, its dual space $\mathcal{H}'$ consists of all linear functionals: mappings that take a function $w \in \mathcal{H}$ and return a number. For example, "evaluate at point $x_i$" is a functional, as is "integrate against a weight function." Formally, $\mathcal{H}'$ is the set of all continuous linear functionals on $\mathcal{H}$.

This generality is needed because the objects we test against may not be regular functions. Collocation uses Dirac delta "functions" $\delta(x - x_i)$, which are not functions at all but distributions: generalized objects defined only through their action on test functions. The dual pairing $\langle R[v], \delta(x - x_i) \rangle = R[v](x_i)$ makes this precise: the distribution $\delta(x - x_i)$ acts on the function $R[v]$ to produce the value $R[v](x_i)$. More generally, the "inner product" $\langle R[v], w \rangle$ is the pairing between a residual $R[v] \in \mathcal{H}'$ and a test function $w \in \mathcal{H}$. For weighted integrals, this is $\int R[v](x) w(x) \mu(x) dx$. For collocation, it is point evaluation. The norm induced by this pairing is $\|R[v]\|_{\mathcal{H}'} = \sup_{\|w\|=1} |\langle R[v], w \rangle|$, which measures the residual's size by its worst-case effect on test functions.

The minimum residual principle applies directly to neural network function approximation. Consider training a neural network $v_{\boldsymbol{\theta}}$ to minimize the sampled Bellman residual:

$$
\mathcal{L}(\boldsymbol{\theta}) = \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - (\Bellman v_{\boldsymbol{\theta}})(s))^2 \big].
$$

This loss measures $\|R[v_{\boldsymbol{\theta}}]\|_{L^2(\mu)}^2$ in a sampled approximation. Gradient descent on this loss implements the orthogonality condition $\langle R[v_{\boldsymbol{\theta}}], \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \rangle = 0$ where the inner product is the $L^2(\mu)$ integral approximated by sampling. The weighted residual framework clarifies what this training procedure computes: a nonlinear projection of the Bellman equation onto the neural network manifold. While deep RL more commonly uses temporal-difference methods (covered in the next chapter), residual minimization provides an alternative approach grounded in the classical numerical analysis presented here.

Weak adversarial networks make the dual structure explicit. Instead of fixing the test space, both the trial function $v_{\boldsymbol{\theta}}$ and the test function $w_{\boldsymbol{\phi}}$ are parameterized as neural networks, and training optimizes
...
This directly implements the dual norm definition: the residual's magnitude is measured by its worst-case effect over all test functions.

The minimum residual framework thus unifies classical and modern function approximation. Galerkin projection, collocation, and neural methods all seek parameters where the residual is orthogonal to a chosen set of test functions. The differences are in the approximation class (linear subspace versus nonlinear manifold), the test space (fixed basis functions, point evaluations, or learned functions), and the solution method (closed-form linear solve versus iterative gradient-based optimization). Convergence guarantees differ accordingly: linear projections with monotonicity or matched weightings yield contractions, while nonlinear methods offer no general guarantees and may have local minima. The dual pairing perspective nonetheless provides a principled foundation for understanding fitted-value iteration with expressive function classes, connecting the classical theory developed in this chapter to contemporary methods in reinforcement learning and scientific machine learning.

...
To eliminate $v^*$, apply the envelope theorem. For deterministic dynamics $s' = f(s,a)$ where $v^*(s)$ is differentiable, start with the identity at the optimal policy:

$$
v^*(s) = r\big(s,\pi^*(s)\big) + \gamma v^*\big(f(s,\pi^*(s))\big).
$$

Taking the total derivative with respect to $s$ on both sides:

$$
\frac{\partial v^*}{\partial s}(s) = \frac{\partial r}{\partial s}\big(s,\pi^*(s)\big) + \frac{\partial r}{\partial a}\big(s,\pi^*(s)\big) \frac{\partial \pi^*}{\partial s}(s) + \gamma \frac{\partial v^*}{\partial s}\big(f(s,\pi^*(s))\big) \left[\frac{\partial f}{\partial s}\big(s,\pi^*(s)\big) + \frac{\partial f}{\partial a}\big(s,\pi^*(s)\big) \frac{\partial \pi^*}{\partial s}(s)\right].
$$

The **envelope theorem** states that at the optimum, terms involving $\frac{\partial \pi^*}{\partial s}$ vanish. This occurs because the FOC holds:

$$
\frac{\partial r}{\partial a}\big(s,\pi^*(s)\big) + \gamma \frac{\partial v^*}{\partial s}\big(f(s,\pi^*(s))\big) \frac{\partial f}{\partial a}\big(s,\pi^*(s)\big) = 0.
\tag{Euler}
$$

When the FOC is satisfied, the terms with $\frac{\partial \pi^*}{\partial s}$ in the total derivative cancel, leaving:

$$
\frac{\partial v^*}{\partial s}(s) = \frac{\partial r}{\partial s}\big(s,\pi^*(s)\big) + \gamma \frac{\partial v^*}{\partial s}\big(f(s,\pi^*(s))\big) \frac{\partial f}{\partial s}\big(s,\pi^*(s)\big).
\tag{Envelope}
$$

These two equations together form the **discrete-time Euler equation**: a system of functional equations in $\pi^*(\cdot)$ and $\nabla v^*(\cdot)$. The envelope theorem allows expressing $\nabla v^*$ in terms of $\pi^*$, reward derivatives, and transition derivatives. When substituted recursively, the system depends only on the policy and primitives. As Rust {cite}`Rust1996` notes, "since $V$ drops out of the first order condition it is not necessary to consider the impact on marginal utility on all future periods $t+2, t+3, \ldots$"

For stochastic dynamics, the envelope equation becomes $\nabla v^*(s) = \nabla_s r(s,\pi^*(s)) + \gamma \mathbb{E}_{s'\sim p(\cdot|s,\pi^*(s))}[\nabla_s p(s'|s,\pi^*(s)) v^*(s') + p(s'|s,\pi^*(s)) \nabla v^*(s')]$. For constrained actions, replace the FOC with KKT conditions.
...
Euler equation methods require differentiability of $r(s,a)$ and $p(\cdot|s,a)$ in $a$, interior solutions, and that the FOC uniquely determines the optimal action. This defines a class of MDPs called the **Euler class** {cite}`Rust1996`.

### Weighted Residual Methods

Given a parameterized policy $\pi_{\boldsymbol{\theta}}(s)$, the weighted residual methods (collocation, Galerkin, least squares) from earlier apply directly to minimize the Euler residual $\mathcal{R}_E(\pi_{\boldsymbol{\theta}})$. Judd {cite}`Judd1992` uses Galerkin projection with Chebyshev polynomials for economic growth models. The resulting system $G(\boldsymbol{\theta}) = \mathbf{0}$ is generally nonlinear and requires iterative solvers.

Euler equation methods directly approximate policies, which can be more efficient when the policy is simpler than the value function. The Euler equation directly encodes first-order optimality conditions, making diagnostics transparent. However, the equations are generally nonlinear, requiring iterative solvers, and the approach is limited to problems with differentiable rewards and interior solutions. Value function methods remain more robust for discrete actions, constraints, or non-smooth objectives.

Euler equation methods provide a preview of **policy parameterization**, where we directly represent and optimize policies. Policy gradient methods in modern RL can be viewed as stochastic approximations to Euler equation residual minimization. The next chapter extends the weighted residual framework to simulation-based settings, where operators must be approximated from samples.

...
The key insight enabling soft actor-critic (SAC) {cite}`haarnoja2018soft,haarnoja2018sacapplications` is that this integral admits an equivalent expectation form. The optimal policy under entropy regularization is the Boltzmann distribution:

$$
\pi^*(a|s) = \frac{\exp(\beta \cdot q^*(s,a))}{\int_{A} \exp(\beta \cdot q^*(s,a')) da'} \propto \exp(\beta \cdot q^*(s,a))
$$

Under this policy, the soft value function can be rewritten as:

$$
v^*(s) = \mathbb{E}_{a \sim \pi^*(\cdot|s)}\left[q^*(s,a) - \alpha \log \pi^*(a|s)\right]
$$

This transformation is crucial: we've converted an intractable integral into an expectation that we can estimate by sampling. But there's a catch — we need samples from $\pi^*$, which depends on the very $q^*$ we're trying to learn.

The SAC solution mirrors the policy amortization strategy from DDPG: learn a parametric policy $\pi_\phi$ that approximates the Boltzmann distribution. This policy serves double duty:

1. **Amortizing the soft value**: Sample $a \sim \pi_\phi(\cdot|s)$ to estimate $v(s) \approx q(s,a) - \alpha \log \pi_\phi(a|s)$
2. **Amortizing action selection**: Use $\pi_\phi$ directly to select actions, with exploration naturally emerging from its stochasticity
...
The Q-network update assumes we have a policy $\pi_\phi$ that approximates the Boltzmann distribution $\pi^*(a|s) \propto \exp(\beta \cdot q^*(s,a))$. How do we train such a policy?

The challenge is that the Boltzmann distribution requires the partition function $Z(s) = \int_A \exp(\beta \cdot q(s,a))da$ — the very integral we're trying to avoid. SAC sidesteps this by minimizing the KL divergence from the policy to the (unnormalized) Boltzmann distribution:
...
The key observation is that $\log Z_\theta(s)$ doesn't depend on $\phi$, so minimizing this KL is equivalent to minimizing:

$$
\min_{\phi} \mathbb{E}_{s \sim \mathcal{D}}\mathbb{E}_{a \sim \pi_{\phi}(\cdot|s)}\left[\alpha \log \pi_{\phi}(a|s) - q_\theta(s,a)\right]
$$

This objective has a clean interpretation: push probability toward high Q-value actions while maintaining entropy (the $\log \pi_\phi$ term penalizes concentrating probability mass). The entropy bonus is automatic — it emerges from the KL divergence structure, not from an explicit regularization term.

**The reparameterization trick.** There's a technical obstacle: the policy parameters $\phi$ appear in the sampling distribution $\pi_\phi$, making the gradient $\nabla_\phi \mathbb{E}_{a \sim \pi_\phi}[\cdot]$ awkward to estimate. SAC uses a Gaussian policy $\pi_\phi(a|s) = \mathcal{N}(\mu_\phi(s), \sigma_\phi(s)^2)$ and the reparameterization trick: express samples as a deterministic function of parameters and independent noise,
...
Now we can differentiate through $f_\phi$ and the Q-network, just as DDPG differentiates through a deterministic policy. The difference is that SAC samples noise $\epsilon$ rather than outputting a single action, extending deterministic policy gradients to stochastic policies.
...
The algorithm interleaves three updates. First, the Q-networks (lines 7-10) are trained via fitted Q-iteration, but with the soft Bellman target: sample a next action $\tilde{a}'_i$ from the current policy, compute the entropy-adjusted target $y_i = r_i + \gamma[\min_j q^j_{\text{target}}(s'_i, \tilde{a}'_i) - \alpha \log \pi_\phi(\tilde{a}'_i|s'_i)]$, and regress. The minimum over twin Q-networks mitigates overestimation as in TD3. Second, the policy (lines 12-13) is updated to better match the Boltzmann distribution induced by the current Q-function, using the reparameterization trick for gradient estimation. Third, target networks are updated via EMA (lines 15-16) to stabilize training.

Note how the policy serves the dual amortization role discussed earlier: it enables efficient estimation of soft value targets (by providing samples for the expectation) while also providing the action selection mechanism (with exploration built-in from its stochasticity). This is more elegant than DDPG/TD3's separate exploration noise — the entropy regularization objective naturally produces the exploration behavior we want.

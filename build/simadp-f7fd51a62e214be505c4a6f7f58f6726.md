...
This chapter introduces **Monte Carlo integration** for approximating these expectations. Instead of using deterministic quadrature rules that require evaluating $p(s'|s,a)$ at specified points, we approximate expectations using samples drawn from the transition dynamics. This combination—projection methods for function approximation and Monte Carlo integration for expectation approximation—is what the operations research community calls **simulation-based approximate dynamic programming** and what the machine learning community calls **reinforcement learning**. By relying on samples rather than exact probabilities, we move from model-based planning to data-driven learning. The algorithms work in settings where we can simulate or interact with the system but may not have (or wish to use) explicit transition probability functions.
...
This reveals two independent approximations:
...
### Example: Piecewise Constant Approximation

Consider the simplest case: piecewise constant basis functions. Partition the state space into $N$ cells with centers $\{s_i\}_{i=1}^N$, and approximate $v(s) \approx v(s_{k(s)})$ where $k(s)$ indexes the cell containing $s$.

With this approximation, the integral decomposes into a sum over cells:

$$ \begin{aligned}
\int v(s')p(s'|s, a)ds' &= \int v(s_{k(s')})p(s'|s, a)ds' \\
&= \sum_{j=1}^{N} v(s_j) \int_{cell_j} p(s'|s, a)ds' \\
&= \sum_{j=1}^{N} v(s_j) P_j(s,a)
\end{aligned} $$

where $P_j(s,a) = \int_{cell_j} p(s'|s, a)ds'$ is the probability of transitioning into cell $j$. This reduces the continuous problem to a finite-state MDP with $N$ states.

The approach is straightforward to implement but faces the curse of dimensionality: achieving approximation error $\epsilon$ requires $O(\epsilon^{-d})$ cells in $d$ dimensions. Higher-order polynomial bases converge faster but still scale exponentially with dimension.

### Orthogonal Collocation

Orthogonal collocation from the previous chapter provides a systematic approach to coordinating basis choice and quadrature. The method proceeds as follows:

1. Choose an orthogonal polynomial basis (e.g., Chebyshev polynomials $\{T_0(s), T_1(s), \ldots, T_{n-1}(s)\}$)
2. Place collocation points at the zeros of $T_n(s)$
3. Require $R(s_i; \theta) = 0$ at each collocation point

This construction achieves natural coordination between projection and quadrature because the zeros of $T_n(s)$ are also the optimal nodes for Gauss-Chebyshev quadrature. The projection method determines where we evaluate the Bellman operator, and those same points serve as quadrature nodes for evaluating integrals inside $\Bellman v$. For smooth value functions, Gaussian quadrature integrates polynomials up to degree $2n-1$ exactly, matching the approximation order of the basis. The result is a pseudospectral method with exponential convergence $O(e^{-cn})$ for smooth problems.

Other orthogonal polynomial families yield similar pairings:
- Legendre polynomials with Gauss-Legendre quadrature
- Hermite polynomials with Gauss-Hermite quadrature (natural when shocks are Gaussian)
- Laguerre polynomials with Gauss-Laguerre quadrature (for semi-infinite domains)

For finite element methods with B-spline bases, we use piecewise Gaussian quadrature on each element. In all cases, matching the quadrature rule to the basis ensures that numerical integration does not degrade the convergence rate of the projection method.

### Quadrature Grids with Importance Sampling

The approaches above assume we can evaluate integrals $\int \varphi_i(s') p(s'|s,a) ds'$ using quadrature rules designed for the transition density $p(s'|s,a)$. However, Gaussian quadrature is typically designed for a fixed weighting function $p(s)$ independent of the current state and action. This leads to an alternative discretization strategy proposed by {cite:t}`TauchenHussey1991`.

The idea is to select quadrature nodes $\{s_1, \ldots, s_N\}$ and weights $\{w_1, \ldots, w_N\}$ for approximating integrals with respect to a weighting function $p(s)$:

$$\int f(s) p(s) ds \approx \sum_{k=1}^N f(s_k) w_k$$

The weighting function $p(s)$ might be chosen as the stationary distribution, a marginal distribution, or an importance sampling distribution reflecting prior knowledge about relevant regions of the state space. For Gaussian quadrature, the nodes and weights are chosen so this approximation is exact for polynomials of degree up to $2N-1$.

To approximate the Bellman operator integral $\int v(s') p(s'|s,a) ds'$ using quadrature designed for $p(s')$, we apply importance sampling:

$$\int v(s') p(s'|s,a) ds' = \int v(s') \frac{p(s'|s,a)}{p(s')} p(s') ds' \approx \sum_{k=1}^N v(s_k) \frac{p(s_k|s,a)}{p(s_k)} w_k$$

The terms $\frac{p(s_k|s,a)}{p(s_k)} w_k$ serve as effective transition weights, but they do not necessarily sum to one. To construct a valid discrete Markov chain approximation, we normalize:

$$p_N(s_k|s_j,a) = \frac{p(s_k|s_j,a) w_k / p(s_k)}{\sum_{i=1}^N p(s_i|s_j,a) w_i / p(s_i)}$$

This normalization creates an $N$-state Markov chain whose transition probabilities approximate the original continuous dynamics. The approach differs from the piecewise constant cell method in that the quadrature nodes are chosen globally based on $p(s)$ rather than through spatial partitioning, and normalization is required because the importance sampling correction does not preserve the probabilistic constraint.
...
The flattened version with persistent warmstarting seemed like a natural optimization - why throw away learned parameters? However, recent work {cite}`Doro2023` has shown that persistent warmstarting can actually be detrimental to learning. Neural networks tend to lose their ability to learn and generalize over the course of training, suggesting that occasionally starting fresh from $\boldsymbol{\theta}_0$ might be beneficial. In the context of DQN, this looks algorithmically as follows:

...
The Euler-class condition (EC) may look abstract, but it is easy to verify in a number of familiar models.

We illustrate with simple scalar examples, where $y,a \in \mathbb{R}$ and $z$ is an exogenous shock.
...
$$
y_{t+1} = f(y_t,a_t,z_{t+1},z_t)
= \alpha\, y_t + a_t - D(z_{t+1}),
$$

where $0 < \alpha \le 1$ models carry-over and $D(z_{t+1})$ is random demand.

Then

$$
\frac{\partial f}{\partial y}(y,a,z',z) = \alpha,
\qquad
\frac{\partial f}{\partial a}(y,a,z',z) = 1.
$$
...
y_{t+1}
=
f(y_t,a_t,z_{t+1},z_t)
=
\eta_{\mathrm{ret}}\, y_t + \eta_{\mathrm{ch}}\, a_t,
$$

where $y_t$ is state of charge, $a_t$ is charging power, and $\eta_{\mathrm{ret}},\eta_{\mathrm{ch}}$ are efficiency parameters.

Then

$$
\frac{\partial f}{\partial y}(y,a,z',z) = \eta_{\mathrm{ret}},
\qquad
\frac{\partial f}{\partial a}(y,a,z',z) = \eta_{\mathrm{ch}},
$$

so the Euler-class condition holds with

$$
h(y,a,z) = \frac{\eta_{\mathrm{ret}}}{\eta_{\mathrm{ch}}}.
$$

#### Reservoir / irrigation control

Let $y_t$ be the water level in a reservoir, $a_t$ the release, and $I(z_{t+1})$ the random inflow. A simple water-balance model is
...
with $0 < \alpha \le 1$ capturing losses.
...
In a simple RC model for a building zone, one might write
...
where $y_t$ is an internal energy or temperature proxy, $a_t$ is the HVAC input, and $z_t$ collects exogenous conditions (outside temperature, occupancy mode).

Then

$$
\frac{\partial f}{\partial y}(y,a,z',z) = \kappa(z),
\qquad
\frac{\partial f}{\partial a}(y,a,z',z) = \lambda(z),
$$

and the Euler-class condition holds with

$$
h(y,a,z) = \frac{\kappa(z)}{\lambda(z)},
$$

provided $\lambda(z) \neq 0$.

In all these examples, the stock dynamics are affine in $(y,a)$, which makes (EC) automatic. The class of Euler-type methods therefore applies well beyond textbook macro models, as long as one is willing to work with such coarse stock dynamics.

### The Euler residual operator

For Euler-class problems, the first-order optimality condition can be written (after applying the envelope formula to eliminate value-function derivatives) as a functional equation in the policy alone. Let $\mathcal{E}(\pi)(y,z)$ denote the **Euler equation expression**: the left-hand side of the first-order condition
...
where $y' = f(y,\pi(y,z),z',z)$ and $h$ is the function from the Euler-class condition (EC). This expression depends only on the policy $\pi$ and the primitives $(r,f,h,q)$—the value function has been eliminated via the envelope theorem.

We define the **Euler residual operator** $\EulerResidual$ by
...
The optimal policy $\pi^*$ satisfies $\EulerResidual(\pi^*)(y,z) = 0$ for all $(y,z)$.

In practice, we work with a parameterized policy $\pi_\theta(y,z)$. The goal is to choose $\theta$ so that $\EulerResidual(\pi_\theta)$ is "small" in some sense. For example, we can enforce the Euler equation at a finite set of collocation points.

### The nonlinear system to solve

Let $(y_1,z_1),\dots,(y_N,z_N)$ be a set of collocation points in the state space. Enforcing the Euler equation at these points gives the system

$$
\EulerResidual(\pi_\theta)(y_k,z_k) = 0,
\quad k = 1,\dots,N.
$$
...
In a Galerkin or least-squares variant, one chooses test functions $\{w_i\}_{i=1}^M$ and enforces orthogonality conditions
...
where each component of $H$ corresponds to a weighted Euler residual.

The mapping $H$ is generally nonlinear in $\theta$, so one solves $H(\theta) = 0$ using nonlinear equation solvers (Newton-type methods, fixed-point iterations, or other root-finding schemes). Unlike the Bellman operator, the Euler operator is not a contraction in a natural norm, so generic convergence guarantees are weaker and depend on the specific problem.

### Summary

- In a generic continuous-action MDP, the raw Euler equation involves $\partial v^*/\partial s$, so one would have to solve jointly for the policy and the value-function derivatives.
- **Euler-class** problems have additional structure on the stock dynamics that allows the derivative of the value function to be eliminated via an envelope formula. The resulting Euler equation is a closed functional equation in the policy alone.
- Many control problems beyond macroeconomics fit this structure, including inventory models, energy storage, reservoir control, and coarse thermal mass models.
- Euler-equation methods for such problems apply weighted residual techniques directly to the Euler residual operator $\EulerResidual(\pi)$, leading to a finite-dimensional nonlinear system $H(\theta)=0$ in the policy parameters, rather than a Bellman fixed point in $v$.

...
Assume that for each state $s$, the maximizer is unique and interior, denoted by the optimal policy $\pi^*(s)$. For a fixed state $s$, define:

$$
g_s(a) := r(s,a) + \gamma v^*(f(s,a))
$$

The optimal action at state $s$ must satisfy the first-order condition:

$$
\frac{\partial g_s}{\partial a}(a)\Big|_{a = \pi^*(s)} = 0
$$

Using the chain rule to differentiate $g_s$ and denoting $s' = f(s,a)$:

$$
\frac{\partial g_s}{\partial a}(a) = \frac{\partial r}{\partial a}(s,a) + \gamma \sum_{\ell=1}^d \frac{\partial v^*}{\partial s_\ell}(f(s,a)) \frac{\partial f_\ell}{\partial a}(s,a)
$$

Evaluating at $a = \pi^*(s)$ and setting to zero yields the Euler equation:

$$
\frac{\partial r}{\partial a}(s,\pi^*(s)) + \gamma \frac{\partial v^*}{\partial s}(f(s,\pi^*(s))) \frac{\partial f}{\partial a}(s,\pi^*(s)) = 0
$$

This is the classical first-order optimality condition for deterministic optimal control. In matrix form, $\frac{\partial v^*}{\partial s}$ is a row vector of partial derivatives and $\frac{\partial f}{\partial a}$ is the Jacobian matrix.

NFQCA's policy gradient has exactly this structure. When the Q-function $q(s,a;\theta)$ approximates the right-hand side of the Bellman equation, we have:

$$
\nabla_a q(s,a;\theta)|_{a=d(s;w)} \approx \frac{\partial r}{\partial a}(s,a) + \gamma \frac{\partial v}{\partial s'}(s')\frac{\partial f}{\partial a}(s,a)
$$
...
$$
a = \pi_{\boldsymbol{w}}(s) + \mathcal{N}
$$

...
The infinite-dimensional iteration becomes a finite-dimensional iteration in coefficient space once we choose our projection method. The key question is: given a current approximation $\hat{f}^{(k)}(x; \theta^{(k)})$, how do we find the coefficients $\theta^{(k+1)}$ for the next iterate $\hat{f}^{(k+1)}$?

Different projection methods answer this differently. For **collocation**, we proceed in two steps:
...
Why does function iteration work well when it does? When $\mathscr{T}$ is a contraction in the infinite-dimensional space with constant $\gamma < 1$, iterating it pulls any starting function toward the unique fixed point. The hope is that the finite-dimensional operator—evaluating $\mathscr{T}$ and projecting back onto the span of the basis functions—inherits this contraction property. When it does, function iteration converges globally from any initial guess, with each iteration reducing the error by a factor of roughly $\gamma$. This is computationally attractive: we only evaluate the operator and solve a linear system (for collocation) or a relatively simple system (for other methods).
...
**Computing the Jacobian:** The key challenge is computing $J_{ij} = \frac{\partial F_i}{\partial \theta_j}$. For collocation, $F_i(\theta) = \hat{f}(x_i; \theta) - (\mathscr{T}\hat{f}(\cdot; \theta))(x_i)$, so:
...
When $\mathscr{T}$ involves optimization (as in the Bellman operator $\mathrm{L}v = \max_a \{r(s,a) + \gamma \mathbb{E}[v(s')]\}$), computing this derivative appears problematic because the max operator is not differentiable. However, the **Envelope Theorem** provides an elegant solution.
...
**Advantages:** Quadratic convergence near the solution. Doesn't rely on contraction properties. Works for general nonlinear systems.

**Challenges:** Requires good initial guesses—may diverge or converge to spurious solutions from poor starts. Higher computational cost per iteration (must compute and solve a Jacobian system). More complex implementation, especially when applying the Envelope Theorem.

##### Comparison and Practical Recommendations

| **Method** | **Convergence** | **Per-iteration cost** | **Initial guess sensitivity** | **When to use** |
|:-----------|:----------------|:-----------------------|:------------------------------|:----------------|
| **Function iteration** | Linear (when contraction holds) | Low | Robust | Fixed-point problems with good contraction properties; small $\gamma$; well-conditioned bases |
| **Newton's method** | Quadratic (near solution) | Moderate (Jacobian + solve) | Requires good initial guess | Smooth problems without natural contraction; large $\gamma$; high-accuracy requirements |

**Practical strategies:**

1. **For problems with strong contraction** (small $\gamma$, well-conditioned bases, shape-preserving approximations): Function iteration is simple, robust, and often adequate.

2. **For problems with weak contraction** (large $\gamma$, high-order polynomials): Use a **hybrid approach**:
   - Run function iteration for several iterations to get into the basin of attraction
   - Switch to Newton's method for rapid convergence to high accuracy
   
3. **For problems where contraction fails** (non-monotone bases, approximation destroys structural properties): Newton's method may be necessary from the start, but requires careful initialization (e.g., from a coarser approximation or perturbation methods).

4. **Quasi-Newton methods** (BFGS, Broyden) offer a middle ground: they approximate the Jacobian using function evaluations only, avoiding explicit derivative computations while maintaining superlinear convergence.

The choice also depends on the approximation space. Linear interpolation and shape-preserving splines often preserve contraction properties, favoring function iteration. High-order polynomials may destroy contraction but provide smooth approximations suitable for Newton's method.

```{admonition} Connection to Policy Iteration
:class: note
For Bellman equations, Newton's method is equivalent to **policy iteration**. At each Newton step, we compute the optimal actions (policy) given the current coefficients, then solve for coefficients that would be optimal if that policy were held fixed. This connection explains why Newton's method often converges rapidly for dynamic programming problems: like policy iteration, it performs policy evaluation and policy improvement simultaneously, which can converge in just a few iterations for well-behaved problems.
```
...
#### Applying the Solution Methods from Step 4

The two general solution approaches from Step 4—function iteration and Newton's method—specialize naturally to the collocation case. The key insight is that collocation evaluates everything at discrete points, avoiding integration.

**Method 1: Function Iteration for Collocation**

Recall from Step 4 that function iteration exploits fixed-point structure: $\hat{f}^{(k+1)} = \mathscr{T}\hat{f}^{(k)}$. For collocation on the Bellman equation, this becomes:

$$
\theta^{(k+1)} = \boldsymbol{\Phi}^{-1} \mathrm{L}_\varphi(\theta^{(k)}).
$$

This has an intuitive two-step interpretation:

1. **Apply Bellman operator**: Compute targets $t_i^{(k)} = [\mathrm{L}_\varphi(\theta^{(k)})]_i$ at all collocation points
2. **Fit to targets**: Solve $\boldsymbol{\Phi} \theta^{(k+1)} = t^{(k)}$ to find coefficients matching the targets

This is **parametric value iteration**: we iterate the Bellman operator in coefficient space, projecting back onto the span of basis functions at each step. The advantages (global convergence when contraction is preserved, cheap iterations) and challenges (may not preserve contraction, slow for large $\gamma$) from Step 4 all apply here.

```{admonition} Handling Stochastic Expectations
:class: note
When the model includes continuous random variables (e.g., $s' = g(s, a, \epsilon)$ with random shock $\epsilon$), we approximate expectations using **numerical quadrature**. Replace continuous $\epsilon$ with discrete approximation $\{\epsilon_1, \ldots, \epsilon_m\}$ with probabilities $\{w_1, \ldots, w_m\}$:
...
Common schemes: Gauss-Hermite (normal shocks), Gauss-Legendre (uniform), sparse grids (high-dimensional).
```

**Method 2: Newton's Method for Collocation**

From Step 4, Newton's method treats the projection equations as rootfinding: $F(\theta) = \boldsymbol{\Phi} \theta - \mathrm{L}_\varphi(\theta) = 0$. The Newton update is:

$$
\theta^{(k+1)} = \theta^{(k)} - [\boldsymbol{\Phi} - J_{\mathrm{L}_\varphi}(\theta^{(k)})]^{-1} F(\theta^{(k)}).
$$

The key is computing the Jacobian $J_{\mathrm{L}_\varphi}$. Applying the **Envelope Theorem** from Step 4 to the Bellman operator: let $a_i^*(\theta)$ be the optimal action at collocation point $s_i$ given $\theta$. Then:

$$
[J_{\mathrm{L}_\varphi}]_{ij} = \frac{\partial [\mathrm{L}_\varphi(\theta)]_i}{\partial \theta_j} = \gamma \sum_{k \in \mathcal{S}} p(k|s_i, a_i^*(\theta)) \varphi_j(k).
$$

This is the **expected value of the $j$-th basis function at the next state**, evaluated at the current optimal action. We treat the optimal action as constant—the Envelope Theorem from Step 4 justifies this because the first-order condition makes the indirect effect vanish.

**Algorithm:**
1. Compute $\mathrm{L}_\varphi(\theta^{(k)})$ and optimal actions $a_i^*(\theta^{(k)})$ at all collocation points
2. Compute Jacobian: $[J_{\mathrm{L}_\varphi}]_{ij} = \gamma \sum_{s'} p(s'|s_i, a_i^*(\theta^{(k)})) \varphi_j(s')$
3. Update: $\theta^{(k+1)} = \theta^{(k)} - [\boldsymbol{\Phi} - J_{\mathrm{L}_\varphi}]^{-1} [\boldsymbol{\Phi} \theta^{(k)} - \mathrm{L}_\varphi(\theta^{(k)})]$

The advantages (quadratic convergence, doesn't need contraction) and challenges (needs good initial guess, higher cost per iteration) from Step 4 apply. As noted in Step 4, Newton's method for the Bellman equation is equivalent to **policy iteration**: each step finds coefficients optimal for a fixed policy, then improves the policy.

**Collocation-specific simplification:** Because collocation evaluates residuals pointwise (no integration), both the function values and Jacobian entries are cheap to compute compared to Galerkin or method of moments. This computational advantage—discussed in Step 4—is why collocation is the most popular projection method for Bellman equations.

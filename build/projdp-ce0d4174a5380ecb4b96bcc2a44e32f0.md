...
This "raw" Euler equation still involves the unknown derivative $\partial v^*/\partial s$. In a general MDP, any method based directly on (Euler-raw) would have to solve jointly for both the policy and the value-function derivatives.

### Euler-class structure

Many economic applications have additional structure that allows us to eliminate $\partial v^*/\partial s$ entirely from the Euler equation. Rust formalizes such problems as an **Euler class** of continuous decision processes.
...
- $y_t$ is an endogenous (controlled) "stock" state: a quantity that accumulates over time and is directly influenced by past decisions (capital, inventory, energy in storage, water level, etc.),
- $z_t$ is an exogenous shock: a random disturbance outside the decision-maker's control (demand fluctuations, weather, equipment failures, etc.).

The action is $a_t$, taking values in a compact set $\mathcal{A}(y_t,z_t) \subset \mathbb{R}^M$. The reward function is $r(y,z,a)$, typically concave in $(y,a)$ for each $z$. The transition law has the form
...
where $q$ governs the exogenous shock $z_{t+1}$ and $f$ is continuously differentiable.

The **Euler-class condition** is that there exists a matrix-valued function $h(y,a,z)$ such that
...
This condition says that the way the current stock $y$ feeds into next-period stock $y'$ is "spanned" by the directions in which the action $a$ moves $y'$. In scalar problems it simply means $\partial f/\partial y$ and $\partial f/\partial a$ are proportional.

Under these assumptions, one can derive an envelope formula expressing $\partial v/\partial y$ purely in terms of $r$, $h$, and the optimal policy $\pi(y,z)$. Substituting this formula into the raw first-order condition removes $v_y$ and yields an **Euler equation that is a closed functional equation in the policy** $\pi$ alone.
...
and the optimal policy $\pi(y,z)$ satisfies a stochastic Euler equation of the form
...
where $\mathcal{E}(\pi)(y,z)$ depends only on current and next-period $(y,z,a)$, $(y',z',a')$ and the primitives $r,f,h,q$, but not directly on $v$ or its derivatives.

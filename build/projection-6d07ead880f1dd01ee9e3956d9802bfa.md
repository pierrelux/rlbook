...
| **Method** | **Norm being minimized** | **Resulting conditions** |
|:-----------|:------------------------|:------------------------|
| Least squares | $\displaystyle\lVert R \rVert_w^2 = \int R(x; \theta)^2 w(x) dx$ | $\displaystyle\int R \cdot \frac{\partial R}{\partial \theta_j} \, w \, dx = 0$ |
| Galerkin | $\lVert R \rVert_{\mathcal{V}^*}$ (dual norm of approx. space) | $\displaystyle\int R(x; \theta) \varphi_j(x) w(x) dx = 0$ |
| Collocation | $\displaystyle\sum_i \omega_i R(x_i; \theta)^2$ (discrete) | $R(x_i; \theta) = 0$ at $n$ points |

...
provides the necessary condition for a minimum. This is solved numerically by gradient descent or other nonlinear optimization algorithms. The resulting solution defines an implicit nonlinear projection operator $\Pi_{\mathcal{F}}$, which replaces the linear projection $\Proj$ in fitted-value iteration.

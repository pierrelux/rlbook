...
We now have three distinct approaches to the same underlying problem:
...
All three correct methods address the fact that the standard Gaussian-L2 combination mismatches the true noise distribution. Bias-corrected Q explicitly adjusts for the expected overestimation. Weighted Q avoids the hard maximum that creates the bias. Extreme Q uses the correct likelihood for the noise distribution induced by the max operator.

Which assumption is correct? The answer depends on what dominates the noise. If reward stochasticity is the primary source and the central limit theorem applies, Gaussian-based corrections are appropriate. If function approximation error dominates and the max operator is applied to highly uncertain estimates, the Gumbel model may be more accurate. If the action space is small and variance estimates are reliable, weighted estimation can achieve low bias without changing the loss. In practice, modern deep RL systems have mixed error sources—reward noise, TD bootstrapping error, function approximation error, and max-operator bias all contribute—so the optimal choice may depend on the specific problem and learning regime.
...
This robustness is particularly valuable given the max-operator bias discussed earlier. When targets are systematically overestimated due to the maximum over noisy Q-values, squared error amplifies the effect: outliers dominate the gradient signal. Cross-entropy provides implicit mitigation without requiring explicit bias correction or variance estimation. The loss is more forgiving of occasional extreme targets while still pulling predictions toward the bulk of the distribution.
...
We now have four distinct approaches to the max-operator bias and noisy targets: explicit bias correction (Lee-Powell), weighted estimation (D'Eramo et al.), Gumbel regression (XQL), and classification with cross-entropy loss. Bias correction and weighted estimation address the problem at the target construction stage while preserving squared error. Gumbel regression changes the likelihood to match the true noise distribution. Classification changes the projection geometry itself, using KL divergence (cross-entropy) rather than L2 distance, which provides robustness through the logarithmic penalty structure. All four approaches address the same fundamental problem—noisy, biased targets from the max operator—but through different mechanisms. The choice depends on problem characteristics, available computational resources, and whether explicit variance estimation is feasible.

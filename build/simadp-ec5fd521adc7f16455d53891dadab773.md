...
These choices are independent but should be coordinated. Consider what happens when we apply the Bellman operator to our approximate value function $\hat{v}(s; \theta)$. We want to compute:

$$ (\Bellman \hat{v})(s) = \max_{a \in \mathcal{A}_s} \left\{r(s,a) + \gamma \sum_i \theta_i \int \varphi_i(s') p(s'|s,a) ds' \right\} $$

But we cannot evaluate these integrals exactly, so we compute instead:

$$ (\widehat{\Bellman}_Q \hat{v})(s) = \max_{a \in \mathcal{A}_s} \left\{r(s,a) + \gamma \sum_i \theta_i \sum_j w_j \varphi_i(s'_j) \right\} $$

where $\{(s'_j, w_j)\}$ are quadrature nodes and weights. The quadrature error is $\|(\Bellman \hat{v})(s) - (\widehat{\Bellman}_Q \hat{v})(s)\|$. If this error is large relative to the basis approximation quality, we waste the expressive power of the basis. For instance, degree-10 Chebyshev polynomials can represent smooth functions to $O(10^{-8})$ accuracy, but combined with rectangle-rule quadrature (error $O(h^2) \approx 10^{-2}$), we cannot exploit this accuracy. We pay the cost of storing and manipulating 10 coefficients but achieve only first-order convergence in the mesh size.

...
However, we still face the challenge of computing the integral over actions. When actions are continuous, evaluating $\frac{1}{\beta} \log \int_A \exp(\frac{1}{\beta}q_\theta(s'_i,a'))da'$ requires integrating over the entire action space—an intractable operation.

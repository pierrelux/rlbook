...
where $\rho(x; q, \theta) = \frac{p(x;\theta)}{q(x)}$ is the likelihood ratio. Taking the gradient with respect to $\theta$:

$$
\nabla_\theta J(\theta) = \mathbb{E}_{x \sim q(x)}\left[f(x) \cdot \nabla_\theta \rho(x; q, \theta)\right] = \mathbb{E}_{x \sim q(x)}\left[f(x) \cdot \rho(x; q, \theta) \cdot \nabla_\theta \log p(x;\theta)\right]
$$

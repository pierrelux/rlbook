...
For problems where we can eliminate the value function using the envelope theorem, this becomes a condition purely in terms of the policy. Consider a continuous control problem with quadratic costs and smooth dynamics. The state evolves as $s_{t+1} = f(s_t, a_t) + \epsilon_{t+1}$ where $\epsilon$ is random noise, and the reward is:

$$
r(s, a) = -\frac{1}{2}(s^\top Q s + a^\top R a),
$$

where $Q \succeq 0$ and $R \succ 0$. The Bellman equation is:

$$
v(s) = \max_{a} \left\{ -\frac{1}{2}(s^\top Q s + a^\top R a) + \gamma \mathbb{E}[v(f(s,a) + \epsilon)] \right\}.
$$

The first-order condition with respect to $a$ is:

$$
-R a^*(s) + \gamma \mathbb{E}\left[ \nabla_s v(s') \cdot \frac{\partial f(s, a^*(s))}{\partial a} \right] = 0.
$$

The envelope theorem gives $\nabla_s v(s) = -Q s + \gamma \mathbb{E}[\nabla_s v(s') \cdot \frac{\partial f(s, a^*(s))}{\partial s}]$. Substituting this into the FOC shifted forward one period and rearranging:

$$
R a(s) = \gamma \mathbb{E}\left[ \left( Q s' + \gamma \mathbb{E}[\nabla_s v(s'') \cdot \frac{\partial f(s', a(s'))}{\partial s}] \right) \cdot \frac{\partial f(s, a(s))}{\partial a} \right],
$$

where $s' = f(s, a(s)) + \epsilon$. After iterative substitution to eliminate $v$ entirely, this becomes the **discrete-time Euler equation**: a functional equation in the policy $a(\cdot)$ with no explicit appearance of the value function. For the LQR special case with linear dynamics $f(s,a) = As + Ba$, this simplifies to a linear Euler equation that can be solved analytically, recovering the Riccati recursion.

### The Euler Equation as a Functional Equation

For the quadratic cost example above, define the Euler residual operator on the space of control policies:

$$
\EulerResidual(a)(s) = R a(s) - \gamma \mathbb{E}\left[ \left( Q s' + \gamma \mathbb{E}[\nabla_s v(s'') \cdot \frac{\partial f(s', a(s'))}{\partial s}] \right) \cdot \frac{\partial f(s, a(s))}{\partial a} \right],
$$

where $s' = f(s, a(s)) + \epsilon$. The optimal policy $a^*$ satisfies the functional equation:

$$
\EulerResidual(a^*) = 0.
$$

This has the same mathematical structure as the Bellman residual $\Residual(v) = \Bellman v - v$: it is a functional equation where the unknown is an entire function (here, a policy rather than a value function). For a general MDP with action $a$ and optimal policy $\alpha^* : \mathcal{S} \to \mathcal{A}$, the Euler residual has the form:

$$
\EulerResidual(\alpha)(s) = \frac{\partial r(s, \alpha(s))}{\partial a} + \gamma \mathbb{E}_{s' \sim p(\cdot|s,\alpha(s))} \left[ \nabla_s v(s') \cdot \frac{\partial p(s'|s,a)}{\partial a}\Big|_{a=\alpha(s)} \right],
$$

where the value function gradient can itself be expressed recursively in terms of the policy using the envelope theorem. The precise form depends on the problem structure, but the pattern is the same: a first-order condition relating the current policy to its future values becomes a functional equation $\EulerResidual(\alpha) = 0$.
...
2. **Euler residual accuracy**: If the goal is to minimize the Euler residual (e.g., for econometric hypothesis testing via Hansen-Singleton tests), directly targeting this residual is more accurate than deriving the policy from an approximate value function.
3. **Economic interpretation**: The Euler equation has direct economic meaning (intertemporal optimality conditions), making diagnostics and model validation more interpretable.

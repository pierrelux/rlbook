...
The control is $c_t$, taking values in a compact set $A(y_t,z_t) \subset \mathbb{R}^M$. The one-period utility is $u(y,z,c)$, typically concave in $(y,c)$ for each $z$. The transition law has the form
...
where $q$ governs the exogenous shock $z_{t+1}$ and $r$ is continuously differentiable.

The **Euler-class condition** is that there exists a matrix-valued function $h(y,c,z)$ such that
...
This condition says that the way the current stock $y$ feeds into next-period stock $y'$ is "spanned" by the directions in which the control $c$ moves $y'$. In scalar problems it simply means $\partial r/\partial y$ and $\partial r/\partial c$ are proportional.

Under these assumptions, one can derive an envelope formula expressing $\partial v/\partial y$ purely in terms of $u$, $h$, and the optimal policy $\pi(y,z)$. Substituting this formula into the raw first-order condition removes $v_y$ and yields an **Euler equation that is a closed functional equation in the policy** $\pi$ alone.
...
u(y,z,c)
+
\gamma \int v\big(r(y,c,z',z),z'\big)\, q(dz'\mid z)
\right],
$$
...
where $\mathcal{E}(\pi)(y,z)$ depends only on current and next-period $(y,z,c)$, $(y',z',c')$ and the primitives $u,r,h,q$, but not directly on $v$ or its derivatives.
...
In all these examples, the stock dynamics are affine in $(y,c)$, which makes (EC) automatic. The class of Euler-type methods therefore applies well beyond textbook macro models, as long as one is willing to work with such coarse stock dynamics.
...
$$
\mathcal{R}_E(\pi^*)(y,z) = 0
\quad \text{for all } (y,z).
$$

In practice, we work with a parameterized policy $\pi_\theta(y,z)$. The goal is to choose $\theta$ so that $\mathcal{R}_E(\pi_\theta)$ is "small" in some sense. For example, we can enforce the Euler equation at a finite set of collocation points.
...
$$
\mathcal{R}_E(\pi_\theta)(y_k,z_k) = 0,
\quad k = 1,\dots,N.
$$
...
:=
\begin{bmatrix}
\mathcal{R}_E(\pi_\theta)(y_1,z_1) \\
\vdots \\
\mathcal{R}_E(\pi_\theta)(y_N,z_N)
\end{bmatrix},
$$
...
$$
\int w_i(y,z)\,\mathcal{R}_E(\pi_\theta)(y,z)\,\mu(dy,dz) = 0,
\quad i = 1,\dots,M,
$$

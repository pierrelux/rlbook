...
- $y_t$ is an endogenous "stock" state (capital, inventory, energy in storage, water level, etc.),
- $z_t$ is an exogenous shock.

The control is $c_t$, taking values in a compact set $A(y_t,z_t) \subset \mathbb{R}^M$. The one-period utility is $u(y,z,c)$, typically concave in $(y,c)$ for each $z$. The transition law has the special factorized form

$$
p(dy_{t+1},dz_{t+1}\mid y_t,z_t,c_t)
=
\mathbf{1}\{y_{t+1} = r(y_t,c_t,z_{t+1},z_t)\}\,q(dz_{t+1}\mid z_t),
$$

where $q$ governs the exogenous shock $z_{t+1}$ (independent of the control) and $r$ is a continuously differentiable deterministic function giving next-period stock.

The key **Euler-class condition** is that there exists a matrix-valued function $h(y,c,z)$ such that

$$
\frac{\partial r(y,c,z',z)}{\partial y}
=
\frac{\partial r(y,c,z',z)}{\partial c}\, h(y,c,z)
\quad \text{for all } (y,c,z',z).
$$ (EC)

This is a **rank-one type condition**: every column of $\partial r/\partial y$ must lie in the column space of $\partial r/\partial c$. Geometrically, this says that the way the current stock $y$ feeds into next-period stock $y'$ is "spanned" by the directions in which the control $c$ moves $y'$. In scalar problems, it simply means $\partial r/\partial y$ and $\partial r/\partial c$ are proportional.

#### Why this condition enables value elimination

Under these assumptions, one can derive an **envelope formula** expressing $\partial V/\partial y$ purely in terms of $u$, $h$, and the optimal policy $\alpha(y,z)$:

$$
\frac{\partial V(y,z)}{\partial y}
=
\frac{\partial u(y,z,\alpha(y,z))}{\partial y}
+
\frac{\partial u(y,z,\alpha(y,z))}{\partial c}\, h(y,\alpha(y,z),z).
$$

This formula expresses the value gradient entirely in terms of observable primitives (the utility function $u$ and the function $h$ from the dynamics) and the policy $\alpha$—with no explicit dependence on $V$ itself. Substituting this envelope formula into the raw first-order condition eliminates $V_y$ entirely, yielding an **Euler equation that is a closed functional equation in the policy $\alpha$ alone**.

The key insight is that the special derivative structure of the dynamics allows us to trace how changes in the current stock affect future value purely through their effect on the control problem's primitives, without needing to track the value function separately.
...
u(y,z,c)
+
\beta \int V\big(r(y,c,z',z),z'\big)\, q(dz'\mid z)
\right],
$$

and the optimal policy $\alpha(y,z)$ satisfies a stochastic Euler equation of the form

$$
\mathcal{E}(\alpha)(y,z) = 0,
\quad \forall (y,z),
$$ (Euler)

where $\mathcal{E}(\alpha)(y,z)$ depends only on current and next-period $(y,z,c)$, $(y',z',c')$ and the primitives $u,r,h,q$, but not directly on $V$ or its derivatives.

### Examples of Euler-class dynamics

The Euler-class condition may look abstract, but it is easy to verify in a number of familiar models. In particular, **affine dynamics** in $(y,c)$ automatically satisfy the condition, since the derivatives are constant.

More generally, any problem with:
- A small number of "stock" states whose dynamics are affine or simple functions of current stock and control,
- Exogenous shocks that don't depend on the control, and
- Concave stage utility/cost,

is a candidate for the Euler-class framework. This includes not just macro models, but also inventory, energy storage, reservoirs, and simple thermal storage problems.

We illustrate with simple scalar examples, where $y,c \in \mathbb{R}$ and $z$ is an exogenous shock.
...
$$
y_{t+1} = r(y_t,c_t,z_{t+1},z_t)
= \alpha\, y_t + c_t - D(z_{t+1}),
$$
...
$$
\frac{\partial r}{\partial y}(y,c,z',z) = \alpha,
\qquad
\frac{\partial r}{\partial c}(y,c,z',z) = 1.
$$
...
y_{t+1}
=
r(y_t,c_t,z_{t+1},z_t)
=
\eta_{\mathrm{ret}}\, y_t + \eta_{\mathrm{ch}}\, c_t,
$$

where $y_t$ is state of charge, $c_t$ is charging power, and $\eta_{\mathrm{ret}},\eta_{\mathrm{ch}}$ are efficiency parameters.

Then

$$
\frac{\partial r}{\partial y}(y,c,z',z) = \eta_{\mathrm{ret}},
\qquad
\frac{\partial r}{\partial c}(y,c,z',z) = \eta_{\mathrm{ch}},
$$

so the Euler-class condition holds with

$$
h(y,c,z) = \frac{\eta_{\mathrm{ret}}}{\eta_{\mathrm{ch}}}.
$$

#### Reservoir / irrigation control

Let $y_t$ be the water level in a reservoir, $c_t$ the release, and $I(z_{t+1})$ the random inflow. A simple water-balance model is
...
r(y_t,c_t,z_{t+1},z_t)
=
a(z_t)\, y_t + b(z_t)\, c_t + d(z_t),
$$

where $y_t$ is an internal energy or temperature proxy, $c_t$ is the HVAC input, and $z_t$ collects exogenous conditions (outside temperature, occupancy mode).

Then

$$
\frac{\partial r}{\partial y}(y,c,z',z) = a(z),
\qquad
\frac{\partial r}{\partial c}(y,c,z',z) = b(z),
$$

and the Euler-class condition holds with

$$
h(y,c,z) = \frac{a(z)}{b(z)},
$$
...
In all these examples, the stock dynamics are **affine** in $(y,c)$:

$$
y_{t+1} = a(z',z) \cdot y_t + b(z',z) \cdot c_t + d(z',z),
$$

where $a,b,d$ may depend on the exogenous shocks but not on $(y,c)$. For such dynamics:

$$
\frac{\partial r}{\partial y} = a(z',z), 
\qquad 
\frac{\partial r}{\partial c} = b(z',z),
$$

so the Euler-class condition holds with $h(y,c,z) = a(z',z)/b(z',z)$ (assuming $b \neq 0$).

More generally, even when dynamics are not perfectly affine, if they take the form

$$
y' = g(z',z) \cdot \big(y + \phi(c)\big),
$$

we get

$$
\frac{\partial r}{\partial y} = g(z',z), 
\qquad 
\frac{\partial r}{\partial c} = g(z',z) \cdot \phi'(c),
$$

so $h(y,c,z) = 1/\phi'(c)$.

The key insight: **affine or near-affine stock dynamics make the Euler-class condition automatic**. This is why the class of Euler-type methods applies well beyond textbook macro models, encompassing inventory, storage, reservoir, and coarse thermal models—anywhere we're willing to work with simplified stock dynamics.
...
$$
\mathcal{R}_E(\alpha)(y,z)
:=
\mathcal{E}(\alpha)(y,z),
$$

that is, $\mathcal{R}_E(\alpha)$ is the left-hand side of the Euler equation evaluated at the policy $\alpha$. The optimal policy $\alpha^*$ satisfies

$$
\mathcal{R}_E(\alpha^*)(y,z) = 0
\quad \text{for all } (y,z).
$$

In practice, we work with a parameterized policy $\alpha_\theta(y,z)$. The goal is to choose $\theta$ so that $\mathcal{R}_E(\alpha_\theta)$ is "small" in some sense. For example, we can enforce the Euler equation at a finite set of collocation points.
...
$$
\mathcal{R}_E(\alpha_\theta)(y_k,z_k) = 0,
\quad k = 1,\dots,N.
$$
...
:=
\begin{bmatrix}
\mathcal{R}_E(\alpha_\theta)(y_1,z_1) \\
\vdots \\
\mathcal{R}_E(\alpha_\theta)(y_N,z_N)
\end{bmatrix},
$$
...
$$
\int w_i(y,z)\,\mathcal{R}_E(\alpha_\theta)(y,z)\,\mu(dy,dz) = 0,
\quad i = 1,\dots,M,
$$
...
It's important to understand that the **Euler class** is not "all problems where you can write an Euler equation." Almost any smooth MDP with continuous actions has first-order conditions that could be called "Euler equations."

What makes the Euler class special is the structural condition that allows **complete elimination** of the value function from the system. In a general MDP, the raw Euler equation $\frac{\partial r}{\partial a} + \gamma \frac{\partial v^*}{\partial s} \frac{\partial f}{\partial a} = 0$ still involves $\partial v^*/\partial s$, so you'd need to approximate both the policy and the value gradient jointly.

The Euler-class condition—that $\partial r/\partial y = (\partial r/\partial c) h$—is what enables the envelope formula that expresses $\partial V/\partial y$ purely in terms of primitives and the policy. This transforms a coupled system (policy + value) into a **closed functional equation in the policy alone**.

This is what allows Rust and others to define an **Euler operator** on policies and apply projection methods without carrying around a separate approximation to $V$ or its derivatives—a significant computational advantage when the dimension of the policy space is smaller than the dimension of the value function space.
```
...
- Euler-equation methods for such problems apply weighted residual techniques directly to the Euler residual operator $\mathcal{R}_E(\alpha)$, leading to a finite-dimensional nonlinear system $G(\theta)=0$ in the policy parameters, rather than a Bellman fixed point in $V$.

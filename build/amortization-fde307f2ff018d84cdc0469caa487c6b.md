...
Three instantiations of this idea emerged:
...
**Value amortization** (SAC): When using entropy regularization, the soft Bellman operator requires computing $\frac{1}{\beta}\log \int \exp(\beta q(s,a;\theta))da$, an intractable integral over continuous actions. Train a value network $v_\psi(s)$ to approximate $\mathbb{E}_{a \sim \pi}[q(s,a) - \alpha \log \pi(a|s)]$, reducing the integral to a single-sample Monte Carlo estimate plus one forward pass.

**Temporal amortization** (SVG): Use a learned dynamics model to unroll future trajectories, reducing the number of Bellman backups needed by directly estimating multi-step returns. This amortizes value computation across time rather than across the action space.

Each approach trades different forms of computational cost. Action selection amortization requires training an actor network alongside the critic, introducing additional gradient computations but preserving the FQI structure. Value amortization adds a third network but eliminates the need to evaluate the Q-function at sampled actions during target computation. Temporal amortization reduces the number of Bellman updates needed but requires learning an accurate dynamics model.

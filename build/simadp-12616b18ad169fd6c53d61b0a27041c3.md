...
1. Choose a subset of states $\mathcal{S}^\star$ using a space-filling design over the state space.
...
3. Fit a regression model $y \approx g_\eta(\phi)$ using least squares or another fitting method.
...
The Keane-Wolpin approach treated maximization bias as a quantity to be learned and subtracted. There is an alternative strategy that modifies the estimator itself so that the maximization step does not introduce systematic overestimation. The bias arises when we use the same noisy estimate both to select an action and to evaluate it. If we maintain two independent value estimates and use one to choose the action while the other evaluates it, we can eliminate the coupling that produces upward drift.

Consider the Monte Carlo Bellman operator $\widehat{\Bellman}_N$ applied at state $s$. For each action $a$, we compute

$$
\hat{q}_N(s,a) = r(s,a) + \frac{\gamma}{N}\sum_{i=1}^N v(s'_i),
$$

where $s'_i \sim p(\cdot \mid s,a)$. The standard Monte Carlo update forms the target $\max_a \hat{q}_N(s,a)$. This maximum both selects the best action according to the noisy estimates and uses that same noise to compute the value. Write $\hat{q}_N(s,a) = q(s,a) + \varepsilon_a$ where $\mathbb{E}[\varepsilon_a] = 0$. Then

$$
\mathbb{E}\Big[\max_a \hat{q}_N(s,a)\Big] = \mathbb{E}\Big[\max_a (q(s,a) + \varepsilon_a)\Big] \geq \max_a q(s,a),
$$
...
This is the principle behind double Q-learning {cite}`van2016deep`. For parametric Q-function approximation with two sets of parameters $\theta^{(1)}$ and $\theta^{(2)}$, we maintain two Q-functions $q^{(1)}(s,a; \theta^{(1)})$ and $q^{(2)}(s,a; \theta^{(2)})$. During value iteration, we alternate which function selects actions and which evaluates them. The following algorithm implements this for simulation-based approximate dynamic programming.

```{prf:algorithm} Double Q Value Iteration with Monte Carlo
:label: double-q-value-iteration

**Input:** MDP, Q-function approximators $q^{(1)}(s,a; \theta^{(1)})$ and $q^{(2)}(s,a; \theta^{(2)})$, state sample $\mathcal{S}$, Monte Carlo sample size $N$, maximum iterations $K$, tolerance $\varepsilon$

**Output:** Parameters $\theta^{(1)}$, $\theta^{(2)}$

1. Initialize $\theta^{(1)}_0$ and $\theta^{(2)}_0$
2. $k \leftarrow 0$
3. **repeat**
4. $\quad$ $\mathcal{D}^{(1)} \leftarrow \emptyset$, $\mathcal{D}^{(2)} \leftarrow \emptyset$
5. $\quad$ **for** each $s \in \mathcal{S}$ **do**
6. $\quad\quad$ **for** each $a \in \mathcal{A}(s)$ **do**
7. $\quad\quad\quad$ Draw $N$ next states: $s'_1, \ldots, s'_N \sim p(\cdot \mid s,a)$
8. $\quad\quad\quad$ **// Compute targets using decoupled selection-evaluation**
9. $\quad\quad\quad$ **for** $i = 1, \ldots, N$ **do**
10. $\quad\quad\quad\quad$ $a^{(1)}_i \leftarrow \arg\max_{a'} q^{(1)}(s'_i, a'; \theta^{(1)}_k)$
11. $\quad\quad\quad\quad$ $a^{(2)}_i \leftarrow \arg\max_{a'} q^{(2)}(s'_i, a'; \theta^{(2)}_k)$
12. $\quad\quad\quad$ **end for**
13. $\quad\quad\quad$ $Y^{(1)}(s,a) \leftarrow r(s,a) + \frac{\gamma}{N}\sum_{i=1}^N q^{(2)}(s'_i, a^{(1)}_i; \theta^{(2)}_k)$
14. $\quad\quad\quad$ $Y^{(2)}(s,a) \leftarrow r(s,a) + \frac{\gamma}{N}\sum_{i=1}^N q^{(1)}(s'_i, a^{(2)}_i; \theta^{(1)}_k)$
15. $\quad\quad\quad$ $\mathcal{D}^{(1)} \leftarrow \mathcal{D}^{(1)} \cup \{((s,a), Y^{(1)}(s,a))\}$
16. $\quad\quad\quad$ $\mathcal{D}^{(2)} \leftarrow \mathcal{D}^{(2)} \cup \{((s,a), Y^{(2)}(s,a))\}$
17. $\quad\quad$ **end for**
18. $\quad$ **end for**
19. $\quad$ **// Fit both Q-functions to their respective targets**
20. $\quad$ $\theta^{(1)}_{k+1} \leftarrow \arg\min_{\theta} \sum_{((s,a),y) \in \mathcal{D}^{(1)}} (q^{(1)}(s,a;\theta) - y)^2$
21. $\quad$ $\theta^{(2)}_{k+1} \leftarrow \arg\min_{\theta} \sum_{((s,a),y) \in \mathcal{D}^{(2)}} (q^{(2)}(s,a;\theta) - y)^2$
22. $\quad$ $k \leftarrow k+1$
23. **until** convergence or $k \geq K$
24. **return** $\theta^{(1)}_k$, $\theta^{(2)}_k$
```

At lines 10-11, we use $q^{(1)}$ to select the action for computing the target for $q^{(1)}$, but then at line 13 we evaluate that action using $q^{(2)}$. This decouples the noise in the selection step from the noise in the evaluation step. The same principle applies symmetrically for $q^{(2)}$ at lines 11 and 14. The Monte Carlo averages at lines 13-14 are unbiased for the expected values of the selected actions, and the maximization bias is removed because selection and evaluation use independent noise sources.

The contrast with Keane-Wolpin is instructive. Both methods address the same phenomenon: the upward bias introduced by $\max_a$ applied to noisy estimates. The econometric approach learns a correction term that can be subtracted from the noisy maximum. The double Q approach designs the estimator so that the coupling between selection and evaluation noise does not occur. The former learns to undo the bias; the latter avoids creating it.

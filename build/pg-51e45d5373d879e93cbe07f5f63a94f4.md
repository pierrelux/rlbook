...
This is precisely the coefficient from ordinary least squares regression of $Z$ on $C$. Substituting back yields the variance reduction:

$$
\text{Var}(Z) - \text{Var}(Z_{\text{cv}}) = \frac{[\text{Cov}(Z,C)]^2}{\text{Var}(C)} = R^2 \cdot \text{Var}(Z)
$$

where $R^2 = \frac{[\text{Cov}(Z,C)]^2}{\text{Var}(Z)\text{Var}(C)}$ is the coefficient of determination from regressing $Z$ on $C$. The control variate method is equivalent to fitting a linear model to predict the estimator, then subtracting the predicted component. The variance reduction equals the $R^2$ of this regression multiplied by the original variance.

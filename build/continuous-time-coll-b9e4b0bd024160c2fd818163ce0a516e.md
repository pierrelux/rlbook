...
=2\tau^2-3\tau+1,\\
\ell_m(\tau)
&=\frac{\ta\mathbf{u}(\tau-1)}{(\frac12-0)(\frac12-1)}
=4\ta\mathbf{u}(1-\tau),\\
\ell_1(\tau)
&=\frac{\ta\mathbf{u}(\tau-\frac12)}{(1-0)(1-\frac12)}
=2\tau^2-\tau.
\end{aligned}
$$
...
$$
\mathbf{\mathbf{z}}=\left(\mathbf{x}_0,\{\mathbf{x}_{k,j},\mathbf{u}_{k,j}\}_{k,j},\mathbf{x}_N\right)\,.
$$

Here $\operatorname{col}$ stacks the listed vectors into one column, omitting
duplicate variables when a stage shares a mesh endpoint. If $t_f$ is optimized,
...
c_f(\mathbf{x}_N,t_f)+\sum_{k=0}^{N-1}J_k\\
\text{subject to}\quad&
\text{stage equations},\\
...
state polynomial. Write $\ell_r^{\mathrm{state}}$ for their cardinal
...
=\sum_{r=0}^{d}\widehat{\mathbf x}_{k,r}\lambda_\mathbf{h}(\tau).
$$
...
matrix shorthand, all nodal constraints are $D\mathbf{x}_k=h_k\mathbf{f}_k$. The right endpoint
is evaluated with another fixed row. Here $\mathsf X_k\in\mathbb R^{(d+1)\times n}$
has support states as rows, and $\mathsf F_k\in\mathbb R^{s\times n}$ has
stage slopes as rows; neither is a single state vector:

$$
\mathbf{x}_{k+1}=\sum_{r=0}^{d}\lambda_\mathbf{h}(1)\widehat{\mathbf x}_{k,r}.
$$
...
Consider $\dot x=\mathbf{f}(x,u,t)$ on $[t_k,t_{k+1}]$, with duration $h_k$ and
...
4. **Match the stored right endpoint.** Setting $\mathbf{p}_k(1)=\mathbf{x}_{k+1}$ gives the
   defect $\mathbf{x}_{k+1}-\mathbf{x}_k-h_k\mathbf{f}_{k,1}=0$. Substituting the two slope choices yields

   $$
   \begin{aligned}
   \mathbf{x}_{k+1}-\mathbf{x}_k-h_k\mathbf{f}(\mathbf{x}_k,\mathbf{u}_k,t_k)&=0
   &&\text{(explicit Euler)},\\
   \mathbf{x}_{k+1}-\mathbf{x}_k-h_k\mathbf{f}(\mathbf{x}_{k+1},\mathbf{u}_{k+1},t_{k+1})&=0
   &&\text{(implicit Euler)}.
   \end{aligned}
   $$
...
5. **Apply the same integration rule to the cost rate.** The single sampled
   ...
   $$
   J_k=\begin{cases}
   h_kc(\mathbf{x}_k,\mathbf{u}_k,t_k),&\tau_1=0,\\
   h_kc(\mathbf{x}_{k+1},\mathbf{u}_{k+1},t_{k+1}),&\tau_1=1.
   \end{cases}
   $$
...
\ell_m(\tau)=4\ta\mathbf{u}(1-\tau),\qquad
\ell_1(\tau)=2\tau^2-\tau.
$$
...
1. explicit Euler from the single slope node $\tau_c=0$;
2. implicit Euler from the single slope node $\tau_c=1$;
3. the trapezoidal defect from the slope nodes $\tau_c=0,1$.

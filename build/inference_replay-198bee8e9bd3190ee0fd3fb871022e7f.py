...
_VIEW_TITLES = {
    ...
    "open_loop": "Open-loop frequency schedule under trace replay",
    "mpc": "Receding-horizon control under trace replay",
    "scheduling": "Reduced-model prefill and decode scheduling",
    "fqi": "Reduced-model scheduling policy comparison",
}
...
def _profile_provenance(
    ...
    *,
    ...
    if normalized == "measured_l4" and measurement_verified:
        return (source_label or "Measured NVIDIA L4 profile", "stands")
    if normalized == "measured_l4":
        return ("Unverified measured-profile claim", "caveat")
...
def _validate_profile_freshness(artifact: Mapping[str, Any], artifact_path: Path) -> None:
    metadata = artifact.get("metadata")
    if not isinstance(metadata, Mapping):
        return
    expected = str(metadata.get("profile_csv_sha256", ""))
    if not expected:
        return
    if re.fullmatch(r"[0-9a-fA-F]{64}", expected) is None:
        raise ReplayDataError("Replay artifact contains an invalid profile CSV checksum.")
    relative = str(
        metadata.get("profile_csv_path", "data/inference_serving/l4_profile.csv")
    )
    if Path(relative).is_absolute():
        raise ReplayDataError("Replay artifact profile_csv_path must be repository-relative.")
    candidates = [parent / relative for parent in artifact_path.parents]
    profile_path = next((candidate for candidate in candidates if candidate.is_file()), None)
    if profile_path is None:
        raise ReplayDataError(
            "Cannot verify the replay artifact against its recorded profile CSV."
        )
    if _sha256(profile_path) != expected.lower():
        raise ReplayDataError(
            "Replay artifact is stale: the inference profile CSV has changed. "
            "Rebuild textbook_results.json and its SVG fallbacks."
        )
    validation = metadata.get("measurement_validation", {})
    manifest_expected = (
    ...
    if manifest_expected:
        if re.fullmatch(r"[0-9a-fA-F]{64}", manifest_expected) is None:
            raise ReplayDataError(
                "Replay artifact contains an invalid profile manifest checksum."
            )
        manifest_relative = str(
            metadata.get(
                "profile_manifest_path",
                "data/inference_serving/profile_manifest.json",
            )
        )
        if Path(manifest_relative).is_absolute():
            raise ReplayDataError(
                "Replay artifact profile_manifest_path must be repository-relative."
            )
        manifest_candidates = [
            parent / manifest_relative for parent in artifact_path.parents
        ]
        manifest_path = next(
            (candidate for candidate in manifest_candidates if candidate.is_file()),
            None,
        )
        if manifest_path is None or _sha256(manifest_path) != manifest_expected.lower():
            raise ReplayDataError(
                "Replay artifact is stale: the inference profile manifest has changed. "
                "Rebuild textbook_results.json and its SVG fallbacks."
            )
...
def _load_source(source: Path | str | Mapping[str, Any] | Any) -> dict[str, Any]:
    if isinstance(source, (Path, str)):
        ...
        if isinstance(loaded, Mapping):
            _validate_profile_freshness(loaded, path.resolve())
    else:
...
def _downsample_indices(
    ...
    *,
    ...
    required_indices = np.unique(
        np.asarray([0, length - 1, *required], dtype=int)
    )
    if np.any((required_indices < 0) | (required_indices >= length)):
        raise ReplayDataError("A required replay frame index is out of bounds.")
    if required_indices.size > maximum:
        raise ReplayDataError(
            "maximum_frames is smaller than the number of recorded plan updates; "
            "increase it so no control interval is dropped."
        )
    if length <= maximum:
        return np.arange(length, dtype=int)
    remaining = maximum - required_indices.size
    candidates = np.setdiff1d(np.arange(length, dtype=int), required_indices)
    if remaining <= 0:
        return required_indices
    sampled = np.unique(
        np.rint(np.linspace(0, candidates.size - 1, remaining)).astype(int)
    )
    return np.sort(np.concatenate([required_indices, candidates[sampled]]))
...
def _normalize_controller(
    ...
    raw_plan_starts = mapping.get("planned_clock_start_time_s")
    if raw_plan_starts is None:
        plan_starts: list[float | None] = [
            float(time_s[index]) if row else None
            for index, row in enumerate(normalized_plan)
        ]
    else:
        if (
            not isinstance(raw_plan_starts, Sequence)
            or isinstance(raw_plan_starts, (str, bytes))
            or len(raw_plan_starts) != time_s.size
        ):
            raise ReplayDataError(
                "'planned_clock_start_time_s' must align with the recorded time points."
            )
        plan_starts = []
        for row, value in zip(normalized_plan, raw_plan_starts):
            if not row:
                plan_starts.append(None)
                continue
            try:
                start = float(value)
            except (TypeError, ValueError) as error:
                raise ReplayDataError(
                    "Every non-empty plan must record a numeric start time."
                ) from error
            if not np.isfinite(start):
                raise ReplayDataError("A recorded plan start time is non-finite.")
    ...
    if not np.isfinite(plan_dt_s) or plan_dt_s <= 0.0:
        raise ReplayDataError("'plan_dt_s' must be finite and positive.")

    requests = _request_records(mapping)
    plan_update_indices = [
        index for index, row in enumerate(normalized_plan) if row
    ]
    indices = _downsample_indices(
        time_s.size,
        maximum_frames,
        required=plan_update_indices,
    )
    frames: list[dict[str, Any]] = []
    latest_plan_index: int | None = None
    for source_index in indices:
        eligible_updates = [
            index for index in plan_update_indices if index <= source_index
        ]
        if eligible_updates:
            latest_plan_index = eligible_updates[-1]
        ...
        frame["planned_clock_mhz"] = (
            normalized_plan[latest_plan_index]
            if latest_plan_index is not None
            else []
        )
        frame["plan_start_time_s"] = (
            plan_starts[latest_plan_index]
            if latest_plan_index is not None
            else None
        )
        frame["plan_update"] = source_index in plan_update_indices
        frames.append(frame)
...
def _normalise_replay(
    ...
    *,
    ...
    validation = root_metadata.get("measurement_validation", {})
    measurement_verified = bool(
        isinstance(validation, Mapping)
        and validation.get("validated") is True
        and validation.get("manifest_status") == "complete"
        and re.fullmatch(
            r"[0-9a-fA-F]{64}", str(validation.get("manifest_sha256", ""))
        )
    )
...
<section id="__ROOT__" class="inference-replay" aria-labelledby="__TITLE__" aria-describedby="__DESCRIPTION__">
  ...
  <div class="visuals">
    <svg data-policy-view viewBox="0 0 520 270" role="img" hidden aria-labelledby="__ROOT__-policy-title __ROOT__-policy-description">
      ...
      <desc id="__ROOT__-policy-description">A seven by seven grid shows the selected action for each prefill and decode queue length. A white dot appears only when the log contains that cell's displayed state-action choice, and grows with its visit count.</desc>
      ...
      <g aria-hidden="true">
        ...
        <circle class="coverage-dot" cx="390" cy="143" r="5"></circle><text class="annotation" x="402" y="147">logged visits to shown action</text>
      </g>
...
(() => {
  ...
  const renderPolicy = () => {
    ...
    if (coverage) {
      ...
      coverageLabel.textContent = "state-action coverage " + (100 * (finite(fraction) ? fraction : observed)).toFixed(1) + "%";
    }
  };
  ...
  const renderFrame = (announce) => {
    ...
    if (Array.isArray(plan) && plan.length && lanes[1].key.includes("clock")) {
      const planDomainFrames = frames.concat(plan.map((value) => ({[lanes[1].key]: value})));
      const domain = extent(planDomainFrames, lanes[1].key);
      const planStart = finite(frame.plan_start_time_s) ? frame.plan_start_time_s : frame.time_s;
      const samples = plan.map((value, index) => {
        const planTime = planStart + index * run.plan_dt_s;
        const x = 54 + 344 * ((planTime - t0) / Math.max(t1 - t0, 1e-9));
        const y = 230 - 79 * ((value - domain[0]) / (domain[1] - domain[0]));
        return [Math.max(54, Math.min(398, x)), Math.max(151, Math.min(230, y))];
      });
      const pieces = samples.length ? ["M" + samples[0][0].toFixed(2) + " " + samples[0][1].toFixed(2)] : [];
      for (let index = 1; index < samples.length; index += 1) {
        pieces.push("H" + samples[index][0].toFixed(2));
        pieces.push("V" + samples[index][1].toFixed(2));
      }
      const planEnd = planStart + plan.length * run.plan_dt_s;
      const endX = 54 + 344 * ((planEnd - t0) / Math.max(t1 - t0, 1e-9));
      if (samples.length) pieces.push("H" + Math.max(54, Math.min(398, endX)).toFixed(2));
      planPath.setAttribute("d", pieces.join(" "));
    } else {
  ...
  renderFrame(false);
  const fallbackIds = {
    modeling: "fig-inference-serving-model-fallback",
    open_loop: "fig-inference-open-loop-fallback",
    mpc: "fig-inference-mpc-fallback",
    scheduling: "fig-inference-scheduling-dp-fallback",
    fqi: "fig-inference-scheduling-fqi-fallback"
  };
  const fallbackId = fallbackIds[replay.view];
  let fallbackDocument = document;
  try {
    if (window.parent && window.parent !== window && window.parent.document) {
      fallbackDocument = window.parent.document;
    }
  ...
  if (!hideFallback() && typeof MutationObserver !== "undefined") {
    const fallbackObserver = new MutationObserver(() => {
      if (hideFallback()) fallbackObserver.disconnect();
    });
        ...
        if any(coverage is not None for _, _, coverage in panels):
            handles.append(
                plt.Line2D(
                    ...
                    markeredgecolor=INK,
                    markersize=5,
                    label="logged visits to displayed action",
                )
            )

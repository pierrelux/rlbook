...
$$
\begin{aligned}
\underset{x(\cdot),\mathbf{u}(\cdot),t_f}{\operatorname{minimize}}
\quad&
c_f(\mathbf{x}(t_\mathbf{f}),t_\mathbf{f})
+\int_{t_0}^{t_f} c(\mathbf{x}(t),\mathbf{u}(t),t)\,dt\\
\text{subject to}\quad&
\dot \mathbf{x}(t)=\mathbf{f}(\mathbf{x}(t),\mathbf{u}(t),t),\\
&\mathbf{h}(\mathbf{x}(t_0),\mathbf{x}(t_\mathbf{f}),t_\mathbf{f})=0,\\
...
$\mathbf{h}=0$ imposes endpoint conditions, while $\mathbf{g}\leq 0$ represents constraints that
must hold along the path. Setting $\tau=0$ gives the Mayer special case, while
...
$$
t_0<t_1<\cdots<t_N=t_\mathbf{f},\qquad h_k=t_{k+1}-t_k,
$$
...
$[t_k,t_{k+1}]$. The full approximation $\mathbf{x}_h$ is therefore **piecewise
...
node $\tau_j$ corresponds to the physical time $t_k+h_kc_j$. The state and
...
$$
\mathbf{f}_{k,j}
=\mathbf{f}(\mathbf{x}_{k,j},\mathbf{u}_{k,j},t_k+h_kc_j)
$$
...
acting from $\tau=0$ to $\tau=\tau_i$ acts for $h_kc_i$ units of time and
...
\boxed{
\mathbf{x}_{k+1}
=\mathbf{x}_k+h_k\sum_{j=1}^{s}b_jF_{k,j},
\qquad
...
states (squares). The middle piece $\mathbf{p}_k$ is shown in blue in both panels; only
...
$\mathbf{p}_k'(\tau_j)=h_kF_{k,j}$ because that axis uses normalized time. The stage
equations place the dots on the piece, and the endpoint defect makes it reach
$\mathbf{x}_{k+1}$, where the next piece begins. This schematic uses quadratic state
pieces and linear slope interpolants.
:::

The running-cost integral can be treated by exactly the same construction.
Introduce a scalar state $y(t)$ that records the cost accumulated since
$t_0$:
...
The original objective is now $c_f(\mathbf{x}(t_\mathbf{f}),t_\mathbf{f})+y(t_\mathbf{f})$. The augmented state
$(\mathbf{x},y)$ has two rates of change: $\mathbf{f}$ supplies the physical state rate and $c$
...
$$
c_{k,j}:=c(\mathbf{x}_{k,j},\mathbf{u}_{k,j},t_k+h_kc_j).
$$
...
\qquad
y_{k+1}=y_k+h_k\sum_{j=1}^{s}b_jL_{k,j}.
$$
...
Here $\mathbf{u}_h$ is the piecewise control approximation. The weighted sum integrates
...
$y_N=\sum_{k=0}^{N-1}J_k$. The accumulated-cost variables can therefore be
...
$$
\mathbf{z}=\left(\mathbf{x}_0,\{\mathbf{x}_{k,j},\mathbf{u}_{k,j}\}_{k,j},\mathbf{x}_N\right)\,.
$$
...
c_f(\mathbf{x}_N,t_\mathbf{f})+\sum_{k=0}^{N-1}J_k\\
\text{subject to}\quad&
\text{stage equations},\\
...
confusing them with the running cost $c$:

$$
\mathbf{x}_h(t_k+h_k\tau)
=\sum_{\mathbf{h}=0}^{d}\mathbf{x}_{k,r}\lambda_\mathbf{h}(\tau).
$$
...
$$
\boxed{
\sum_{\mathbf{h}=0}^{d}D_{ir}\mathbf{x}_{k,r}
=h_k \mathbf{f}\left(
\sum_{\mathbf{h}=0}^{d}E_{ir}\mathbf{x}_{k,r},
\mathbf{u}_{k,i},
t_k+h_kc_i
\right).
}
...
$$
\mathbf{x}_{k+1}=\sum_{\mathbf{h}=0}^{d}\lambda_\mathbf{h}(1)\mathbf{x}_{k,r}.
$$
...
1. **Choose the stage and evaluate its slope.** The left node uses the
   ...
   \mathbf{f}(\mathbf{x}_k,\mathbf{u}_k,t_k),&\tau_1=0,\\
   \mathbf{f}(\mathbf{x}_{k+1},\mathbf{u}_{k+1},t_{k+1}),&\tau_1=1.
   \end{cases}
   $$
...
3. **Integrate from the left state.** The reference-time derivative is
   $\mathbf{p}_k'(\tau)=h_kF_{k,1}$. Integrating and imposing $\mathbf{p}_k(0)=\mathbf{x}_k$ gives
...
4. **Match the stored right endpoint.** Setting $\mathbf{p}_k(1)=\mathbf{x}_{k+1}$ gives the
   defect $\mathbf{x}_{k+1}-\mathbf{x}_k-h_kF_{k,1}=0$. Substituting the two slope choices yields

   $$
   \begin{aligned}
   \mathbf{x}_{k+1}-\mathbf{x}_k-h_kf(\mathbf{x}_k,\mathbf{u}_k,t_k)&=0
   &&\text{(explicit Euler)},\\
   \mathbf{x}_{k+1}-\mathbf{x}_k-h_kf(\mathbf{x}_{k+1},\mathbf{u}_{k+1},t_{k+1})&=0
   &&\text{(implicit Euler)}.
   \end{aligned}
   $$
...
5. **Apply the same integration rule to the cost rate.** The single sampled
   ...
   $$
   J_k=\begin{cases}
   h_kL(\mathbf{x}_k,\mathbf{u}_k,t_k),&\tau_1=0,\\
   h_kL(\mathbf{x}_{k+1},\mathbf{u}_{k+1},t_{k+1}),&\tau_1=1.
   \end{cases}
   $$
...
Choose the endpoint collocation nodes $\tau_0=0$ and $\tau_1=1$, and abbreviate the
two ODE slopes by
...
state $\mathbf{x}_{k+\frac12}$, control $\mathbf{u}_{k+\frac12}$, and ODE slope
...
$$
\mathbf{x}_k=(\mathbf{p}_k,v_k,\theta_k,\omega_k)
$$
...
$$
y_k=6\theta_k^2+0.15\omega_k^2+0.035a_k^2.
$$
...
$$
J
=h\left(\frac12y_0+\sum_{k=1}^{N-1}y_k+\frac12y_N\right)
+0.002h\sum_{k=0}^{N-1}
\left(\frac{a_{k+1}-a_k}{h}\right)^2.
$$

The boundary conditions impose $\mathbf{x}_0=(0,0,0,0)$ and $\mathbf{x}_N=(4,0,0,0)$, so both
...
$$
\dot \mathbf{x}_h(t_k+h_k\tau)=\sum_jF_{k,j}\ell_j(\tau),
$$

derive:

1. explicit Euler from the single slope node $\tau=0$;
2. implicit Euler from the single slope node $\tau=1$;
3. the trapezoidal defect from the slope nodes $\tau=0,1$.

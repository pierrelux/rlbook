...
The `fit` operation abstracts the supervised learning step that appears in every value-based algorithm. Given a regression dataset $\mathcal{D}^{\text{fit}} = \{((s_i, a_i), y_i)\}$ of state-action pairs with target values, it returns parameters $\boldsymbol{\theta}$ that minimize a loss:

$$
\boldsymbol{\theta} = \texttt{fit}(\mathcal{D}^{\text{fit}}, \boldsymbol{\theta}_{\text{init}}, K) \approx \arg\min_{\boldsymbol{\theta}} \sum_{((s,a), y) \in \mathcal{D}^{\text{fit}}} \ell(q(s,a; \boldsymbol{\theta}), y)
$$

The three parameters control how this optimization is performed:
...
- **Truncation** $K \in \mathbb{N} \cup \{\infty\}$: Number of optimization steps. $K=\infty$ runs to convergence (sample average approximation), $K \in \{10, \ldots, 100\}$ performs partial optimization, $K=1$ makes a single update (stochastic approximation)
- **Loss function** $\ell$: Typically squared error $(q - y)^2$, but can be Huber loss, cross-entropy for classification-based Q-learning, or other regression losses

The implementation depends on the function approximator. For neural networks, optimization uses gradient descent or variants (Adam, RMSprop). For tree ensembles, it builds trees to minimize prediction error. For linear models with squared loss, it solves the normal equations $\boldsymbol{\theta} = (X^\top X)^{-1}X^\top y$ directly. The abstraction allows different approximators while maintaining a unified algorithmic framework.

...
Define the Euler residual operator $\EulerResidual: \mathcal{C} \to \mathcal{C}$ on the space of consumption policies:

$$
\EulerResidual(c)(k) = u'(c(k)) - \gamma \mathbb{E}[u'(c(f(k) - c(k))) f'(f(k) - c(k))].
$$

The optimal policy $c^*$ satisfies the functional equation:

$$
\EulerResidual(c^*) = 0.
$$

This has the same mathematical structure as the Bellman residual $\Residual(v) = \Bellman v - v$: it is a functional equation where the unknown is an entire function (here, a policy rather than a value function). More generally, for an MDP with action $a$ and optimal policy $\alpha^* : \mathcal{S} \to \mathcal{A}$, we can write:

$$
\EulerResidual(\alpha)(s) = \frac{\partial r(s, \alpha(s))}{\partial a} + \gamma \mathbb{E}_{s' \sim p(\cdot|s,\alpha(s))} \left[ \frac{\partial r(s', \alpha(s'))}{\partial a} \frac{\partial s'}{\partial a}\Big|_{a=\alpha(s)} \right] + \ldots
$$

The precise form depends on the problem structure, but the pattern is the same: a first-order condition relating the current policy to its future values becomes a functional equation $\EulerResidual(\alpha) = 0$.
...
This is the approach used by Judd (1992) for the stochastic growth model, using Chebyshev polynomial bases and Gauss-Chebyshev quadrature to approximate the integrals.
...
**Empirical performance**: In numerical comparisons on smooth economic models like the Brock-Mirman stochastic growth problem, Euler equation projection methods (particularly orthogonal collocation) achieve high accuracy with relatively few basis functions and fast computation {cite}`Judd1992`. However, value function iteration remains more robust and broadly applicable across different problem structures.

...
The same step has an optimization interpretation. **Sequential Quadratic
Programming (SQP)** chooses a displacement $\Delta\mathbf z$ by solving the
quadratic program
...
\end{aligned}}
\qquad
B_k=\nabla^2_{\mathbf z\mathbf z}
\mathcal L(\mathbf z^k,\boldsymbol\lambda^k).
$$

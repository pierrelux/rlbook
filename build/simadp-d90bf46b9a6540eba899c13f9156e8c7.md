...
Target networks are not on this list. They are what approximate value iteration means when implemented with gradient-based optimization, not a tunable design choice. DQN's contribution was demonstrating that this classical structure (already present in NFQI) could scale to high-dimensional problems with deep networks and careful engineering, not inventing a new algorithmic principle.

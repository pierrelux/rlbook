...
The Keane-Wolpin approach treated maximization bias as a quantity to be learned and subtracted. An alternative strategy is to modify the estimator itself to break the coupling that causes the bias. The key observation is that the bias arises from a specific interaction: when we take $\max_a \hat{q}(s,a)$, the maximization both selects an action and uses the noise in that same estimate to compute the value.

To see the mechanism more precisely, decompose the Monte Carlo estimate as $\hat{q}(s,a) = q^\star(s,a) + \varepsilon_a$, where $q^\star$ is the true Q-value and $\mathbb{E}[\varepsilon_a] = 0$. The standard estimator is

$$
\max_a \hat{q}(s,a) = \max_a (q^\star(s,a) + \varepsilon_a).
$$

The maximization picks the action with the largest $q^\star(s,a) + \varepsilon_a$. If action $a$ has a positive noise realization $\varepsilon_a > 0$, it becomes more likely to be selected, and the selected value includes that same positive noise. This coupling produces upward bias:

$$
\mathbb{E}\Big[\max_a (q^\star(s,a) + \varepsilon_a)\Big] \geq \max_a q^\star(s,a).
$$

Double Q-learning {cite}`van2016deep` decouples selection from evaluation by maintaining two independent Q-function estimates, $q^{(1)}$ and $q^{(2)}$. Write

$$
q^{(1)}(s,a) = q^\star(s,a) + \varepsilon^{(1)}_a, \quad q^{(2)}(s,a) = q^\star(s,a) + \varepsilon^{(2)}_a,
$$

where $\varepsilon^{(1)}_a$ and $\varepsilon^{(2)}_a$ are independent zero-mean errors. When updating $q^{(1)}$, we select the action using $q^{(1)}$ but evaluate it using $q^{(2)}$:

$$
Y^{(1)} = r(s,a) + \gamma \, q^{(2)}(s', a^\star), \quad \text{where } a^\star = \arg\max_{a'} q^{(1)}(s',a').
$$

Now the selected action is $a^\star = \arg\max_{a'} (q^\star(s',a') + \varepsilon^{(1)}_{a'})$, determined by the noise $\varepsilon^{(1)}$. But the evaluated quantity is $q^{(2)}(s', a^\star) = q^\star(s', a^\star) + \varepsilon^{(2)}_{a^\star}$, which depends on the independent noise $\varepsilon^{(2)}$. Because $\varepsilon^{(2)}_{a^\star}$ is independent of $a^\star$, we have

$$
\mathbb{E}[q^{(2)}(s', a^\star) \mid a^\star] = q^\star(s', a^\star).
$$

The evaluation noise $\varepsilon^{(2)}_{a^\star}$ does not contribute to bias because it was not used in the selection. The coupling is broken.

However, double Q-learning does not eliminate all bias. The selected action $a^\star$ was chosen based on $q^\star(s',a') + \varepsilon^{(1)}_{a'}$, not the true $q^\star(s',a')$. If an action has a positive noise realization in $\varepsilon^{(1)}$, it is more likely to be selected. Thus

$$
\mathbb{E}[q^\star(s', a^\star)] = \mathbb{E}_{a^\star}\big[q^\star(s', a^\star)\big],
$$

where the expectation is over $a^\star = \arg\max_{a'} (q^\star(s',a') + \varepsilon^{(1)}_{a'})$. This is not equal to $\max_a q^\star(s,a)$ in general. The selection noise still introduces bias, but it is typically much smaller than the bias from coupling selection and evaluation. In the standard estimator, both sources of noise (selection and evaluation) contribute to the bias. In double Q-learning, only the selection noise contributes. The update symmetrically applies to $q^{(2)}$, using $q^{(2)}$ for selection and $q^{(1)}$ for evaluation.
...
At lines 9-10, we use $q^{(1)}$ to select the action for updating $q^{(1)}$, but then at line 12 we evaluate that action using $q^{(2)}$. This decouples the noise in the selection step from the noise in the evaluation step. The same principle applies symmetrically for $q^{(2)}$ at lines 10 and 13. The Monte Carlo averages are unbiased for the expected values of the selected actions, and the maximization bias is removed because selection and evaluation use independent noise sources. This principle applies whether we store Q-values in a table or use function approximation.

...
The logsumexp operator provides one way to soften the hard maximum, but other approaches exist that explicitly account for uncertainty in value estimates. D'Eramo et al. {cite}`deramo2016estimating` proposed replacing the hard max with a weighted average where weights reflect the probability that each action is optimal under uncertainty.

When Q-value estimates are noisy, we can model this uncertainty using a Gaussian approximation. Suppose that for a given state $s'$, each Q-value estimate $q(s', a')$ has sampling distribution approximately $\mathcal{N}(\hat{\mu}_{a'}, \hat{\sigma}_{a'}^2/n)$, where $n$ is the number of samples used to estimate the Q-value. Under this model, the weight for action $a'$ is the probability that it achieves the maximum:
...
where $\phi$ and $\Phi$ are the standard normal PDF and CDF. This integral computes the probability that Q-value $a'$ is the largest by integrating over all possible values $x$, weighted by the probability that $a'$ takes value $x$ (the $\phi$ term) and that all other actions have values below $x$ (the $\Phi$ product).

The soft Bellman target becomes:

$$
v(s') = \sum_{a' \in \mathcal{A}_{s'}} w_{a'} \, q(s', a')
$$

This is a probability-weighted expectation rather than a hard maximum. The weights $w_{a'}$ naturally concentrate on actions with high estimated values and low uncertainty, while spreading probability mass across multiple plausible candidates when uncertainty is high.

**Comparison with logsumexp:** The Gaussian-weighted approach and the logsumexp operator both soften the hard max but arise from different assumptions:

- **Logsumexp**: Assumes Gumbel noise on rewards (or equivalently, entropy regularization). The temperature parameter $\beta$ controls smoothness uniformly across all states. The resulting policy is the Boltzmann distribution over Q-values.

- **Gaussian weights**: Assumes Gaussian sampling distributions for Q-value estimates with potentially different variances per action. Weights adapt to heterogeneous uncertainty. Actions with tighter confidence intervals (lower $\hat{\sigma}_{a'}$) receive more weight.

The Gaussian-weighted estimator has lower bias than the simple maximum (which overestimates) and can be computed in closed form for small action spaces, though the integral becomes expensive when $|\mathcal{A}|$ is large. For approximation with function approximation error rather than sampling uncertainty, the variance estimates $\hat{\sigma}_{a'}$ must be obtained through auxiliary mechanisms such as ensemble disagreement or bootstrapped estimates.

Both approaches replace the discontinuous, non-differentiable hard max with smooth operators suitable for gradient-based optimization, but the logsumexp formulation has become dominant in modern deep reinforcement learning due to its connection to entropy regularization and its computational simplicity.

...
**The problem:** This Euler equation still involves $\partial v^*/\partial s$, the gradient of the value function. If we want to solve for the policy $\pi^*$, we must also keep track of the value-function derivatives. For a general MDP, Euler-based methods would thus have to solve jointly for $\pi$ and $\nabla v$, which offers no clear advantage over working with the Bellman equation directly.

### The Euler-class condition: when the value function disappears

**The remarkable fact:** For a broad class of problems, we *can* eliminate $\partial v^*/\partial s$ from the Euler equation using an envelope theorem argument. The resulting equation involves only the policy $\pi$ and the problem primitives $(r, f)$. {cite:t}`Rust1996` formalizes this class as **Euler-class** continuous decision processes.
...
**What does this mean intuitively?** The condition (EC) says that the way today's stock $y$ affects tomorrow's stock $y'$ is "aligned" with the way the action $a$ affects $y'$. More precisely: the Jacobian $\partial f/\partial y$ lies in the column space of $\partial f/\partial a$, with the matrix $h$ providing the linear combination.
...
where $\mathcal{E}(\pi)(y,z)$ can be computed from $(r,f,h,q)$ and the policy evaluated at current and next-period states. Crucially, $\mathcal{E}(\pi)$ does *not* involve the value function or its derivatives. The value function has been completely eliminated.
...
Now that we have established that Euler-class problems admit a closed functional equation in the policy alone, we can apply the same weighted residual framework we used for the Bellman equation—except now the unknown is $\pi$, not $v$.
...
where $y' = f(y,\pi(y,z),z',z)$ and $h$ comes from the Euler-class condition (EC). This expression depends only on $\pi$ and the primitives $(r,f,h,q)$—the value function is gone.

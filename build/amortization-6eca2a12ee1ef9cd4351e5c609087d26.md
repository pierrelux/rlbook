...
**Input:** MDP $(S, \mathcal{A}, P, R, \gamma)$, policy $\pi_{\boldsymbol{\theta}}$, value function $v_{\boldsymbol{\phi}}$, temperature $\alpha$, rollout length $d$, learning rates $\eta_\pi, \eta_v$, replay buffer capacity $B$, prioritization parameter $\lambda$

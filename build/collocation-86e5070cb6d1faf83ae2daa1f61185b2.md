...
swinging. The oscillation is predictable when the cable length is known, so
this example compares two uses of that structure: modify a baseline command to
cancel the known oscillation, or include the payload dynamics directly in
a constrained trajectory optimization. All three commands are open loop: they
are fixed before the move and do not respond to measurements during execution.
...
The deterministic task starts from rest and moves the trolley $4$ m. The
nominal cable length is $\ell=1.20$ m. Every controller is limited to
...
increases the cable length by $10\%$ without redesigning any controller. This
...
The third command is computed rather than shaped. On $N=28$ intervals with
step $h$, the NLP decision vector contains the state
...
Before inspecting the result, predict which method should have the smallest
nominal residual sway and which should be least affected by the cable-length
change. The ZV shaper is tailored to one frequency. The collocation solution
uses the full nonlinear nominal model but is also open loop.
...
together. Both commands remain open loop here. Feedback or receding-horizon

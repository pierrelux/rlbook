...
The weighted residual framework applies to any functional equation, not just the Bellman equation for value functions. An alternative approach, widely used in economics and macroeconomics, directly approximates the **optimal policy** by solving the first-order conditions that characterize optimality {cite}`Judd1992`. These Euler equation methods treat the optimal policy $\pi^*$ as the solution to a functional equation derived from the FOC rather than from the Bellman equation.

### From Trajectory Optimization to the Euler Equation

In continuous-time optimal control, when the Hamiltonian is differentiable in the control variable and the optimal control is interior, the first-order necessary condition for optimality is:

$$
\frac{\partial H}{\partial u}(t) = 0,
$$

which, combined with the costate equation, yields a condition relating the current control to future states and costates. For smooth value functions, this can be expressed as a condition on the policy alone.

The discrete-time analog arises when the Bellman operator involves smooth optimization over continuous actions. Consider an MDP with continuous action space where the Bellman equation is:

$$
v(s) = \max_{a \in \mathcal{A}} \left\{ r(s, a) + \gamma \sum_{j \in \mathcal{S}} p(j|s,a) v(j) \right\}.
$$

The optimal policy $\pi^*(s)$ at each state $s$ solves the maximization problem:

$$
\pi^*(s) = \arg\max_{a \in \mathcal{A}} \left\{ r(s, a) + \gamma \sum_{j \in \mathcal{S}} p(j|s,a) v^*(j) \right\},
$$

where $v^*(j)$ is the optimal value function satisfying the Bellman optimality equation. 

**Assumptions.** We assume:
1. $r(s,a)$ is continuously differentiable in $a$
2. $p(\cdot|s,a)$ is differentiable in $a$ with $\sum_j |\frac{\partial p(j|s,a)}{\partial a}| < \infty$
3. The action space $\mathcal{A}$ is convex
4. The optimal policy $\pi^*(s)$ is interior (no active constraints) for all $s$

**Step 1: First-order necessary condition (Danskin).** For fixed $s$ and $v^*$, define $Q^*(s, a) := r(s, a) + \gamma \sum_{j \in \mathcal{S}} p(j|s,a) v^*(j)$. The map $a \mapsto Q^*(s, a)$ is differentiable in $a$ with:

$$
\frac{\partial Q^*(s, a)}{\partial a} = \frac{\partial r(s, a)}{\partial a} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a} v^*(j).
$$

By Danskin's theorem, if $\pi^*(s)$ is an interior maximizer of $a \mapsto Q^*(s,a)$ over the convex set $\mathcal{A}$, then a **necessary condition** for optimality is:

$$
\frac{\partial Q^*(s, a)}{\partial a}\Big|_{a=\pi^*(s)} = 0 \quad \Leftrightarrow \quad \frac{\partial r(s, a)}{\partial a}\Big|_{a=\pi^*(s)} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi^*(s)} v^*(j) = 0.
$$

This holds for every state $s \in \mathcal{S}$, giving the first-order condition:

$$
\frac{\partial r(s, \pi^*(s))}{\partial a} + \gamma \sum_{j \in \mathcal{S}} \frac{\partial p(j|s,a)}{\partial a}\Big|_{a=\pi^*(s)} v^*(j) = 0, \quad \forall s \in \mathcal{S}.
\tag{Euler-FOC}
$$

This condition is **sufficient** if, in addition, $a \mapsto Q^*(s,a)$ is concave for each $s$.

**Step 2: Eliminate $v^*$ using the envelope theorem.** The first-order condition from Step 1 involves both $\pi^*$ and $v^*$. To obtain a policy-only equation, we use the **envelope theorem** for the value function. For continuous state spaces where $v^*(s)$ is differentiable in $s$, the envelope theorem states:

$$
\frac{\partial v^*(s)}{\partial s} = \frac{\partial r(s, \pi^*(s))}{\partial s} + \gamma \mathbb{E}_{s' \sim p(\cdot|s,\pi^*(s))}\left[\frac{\partial v^*(s')}{\partial s'} \frac{\partial s'}{\partial s}\right].
$$

This expresses $\nabla_s v^*$ in terms of immediate reward derivatives and the expected derivative of $v^*$ at the next state, not the value function $v^*$ itself.

Substituting the envelope theorem into the first-order condition creates a telescoping effect. When we express $v^*(j)$ via its defining Bellman equation and use the envelope theorem to substitute for $\nabla_s v^*$, future periods $t+2, t+3, \ldots$ drop out. The result involves only marginal rewards at time $t$ and expected marginal rewards at time $t+1$. This gives the **discrete-time Euler equation**:

$$
\frac{\partial r(s, \pi^*(s))}{\partial a} + \gamma \mathbb{E}_{s' \sim p(\cdot|s,\pi^*(s))}\left[\frac{\partial r(s', \pi^*(s'))}{\partial s'} - \frac{\partial r(s', \pi^*(s'))}{\partial a'} \frac{\partial s'}{\partial a'}\right] \frac{\partial s'}{\partial a}\Big|_{a=\pi^*(s)} = 0.
\tag{Euler}
$$

This is a functional equation in the policy $\pi^*(\cdot)$ alone. The value function has been eliminated: only derivatives of the reward function appear.

As Rust {cite}`Rust1996` emphasizes, "the impact on expected discounted utility can be evaluated only in terms of the value of marginal utility in period $t$ and the expected discounted value of marginal utility in period $t+1$: since $V$ drops out of the first order condition it is not necessary to consider the impact on marginal utility on all future periods $t+2, t+3, \ldots$" The FOC involves $v^*(s')$ at the next state. The envelope theorem expresses $\frac{\partial v^*(s')}{\partial s'}$ in terms of reward derivatives. When we substitute, the $v^*(s'')$ terms at $t+2$ also get the envelope treatment, and so on. The recursive structure causes all future value derivatives to collapse into current and next-period reward derivatives.

This is the discrete-time analog of the continuous-time Euler equation in optimal control, where the costate equation allows expressing optimality conditions without solving for the entire value function trajectory.

**Remark (Deterministic dynamics).** When transitions are deterministic, $s' = f(s,a)$, and $f$ is differentiable in $a$, the FOC simplifies to:

$$
\nabla_a r\big(s,\pi^*(s)\big) + \gamma \nabla_a f\big(s,\pi^*(s)\big)^\top \nabla v^*\big(f(s,\pi^*(s))\big) = 0.
$$

This is the standard discrete-time adjoint equation. Applying the envelope theorem to express $\nabla_s v^*(s')$ in terms of reward derivatives at $s'$ gives the policy-only form shown in equation (Euler) above. The deterministic case is particularly common in economic applications (consumption-savings problems, growth models, etc.) where the state evolution is governed by an accounting identity like $k' = f(k,c)$ (capital accumulation).

**Remark (Constrained actions).** If the action space has binding constraints or the optimal action lies on the boundary of $\mathcal{A}$, the interior FOC must be replaced by the **Karush-Kuhn-Tucker (KKT) conditions**. For constraints $g_k(s,a) \leq 0$:

$$
\nabla_a Q^*(s,\pi^*(s)) + \sum_k \lambda_k(s) \nabla_a g_k(s,\pi^*(s)) = 0, \quad \lambda_k(s) \geq 0, \quad \lambda_k(s) g_k(s,\pi^*(s)) = 0.
$$

### The Euler Equation as a Functional Equation and Operator

We can formalize the Euler equation as a fixed-point problem on the space of policies. Define the **Euler residual operator** $\mathcal{R}_E : \Pi \to \mathcal{F}(\mathcal{S})$ (mapping policies to functions on the state space) by evaluating the left-hand side of the Euler equation. For continuous state spaces with deterministic dynamics $s' = f(s,a)$:

$$
\mathcal{R}_E(\pi)(s) = \frac{\partial r(s, \pi(s))}{\partial a} + \gamma \mathbb{E}_{s' = f(s,\pi(s))}\left[\frac{\partial r(s', \pi(s'))}{\partial s'} - \frac{\partial r(s', \pi(s'))}{\partial a'} \frac{\partial s'}{\partial a'}\right] \frac{\partial f(s,a)}{\partial a}\Big|_{a=\pi(s)}.
$$

For discrete state spaces, the expectation becomes a sum. The optimal policy $\pi^*$ satisfies the functional equation:

$$
\mathcal{R}_E(\pi^*) = 0.
$$

This has analogous structure to the Bellman residual $\mathcal{R}(v) = \mathcal{T} v - v$: it is a functional equation where the unknown is an entire function. However, there is a **critical difference**: whereas the Bellman operator $\mathcal{T}$ is a $\gamma$-contraction in the supremum norm (guaranteeing convergence of value iteration and well-understood approximation error bounds), the **Euler operator is generally not a contraction**. 

This means:
- Iterative Euler-equation methods (solving $\mathcal{R}_E(\pi)=0$ by successive approximation) may not converge without additional structure
- Approximation error bounds must be derived problem-by-problem rather than following from general contraction theory
- The weighted-residual framework applies (we can still project onto finite-dimensional policy classes), but convergence guarantees differ from Bellman methods

Despite these challenges, Euler-equation methods are widely used in economics and continuous-state MDPs where they often admit analytic or semi-analytic solutions.

**Connection to the policy gradient theorem.** The derivation above closely resembles the policy gradient theorem, but for the Bellman **optimality** equation rather than policy evaluation. Recall that the policy gradient theorem differentiates the objective $J(\theta) = \mathbb{E}[v_\pi(s_0)]$ with respect to policy parameters, yielding:

$$
\nabla_\theta J(\theta) \propto \mathbb{E}_\pi \left[ \nabla_\theta \log \pi_\theta(a|s) \, Q_\pi(s,a) \right].
$$

This applies to policy evaluation: computing the value of a **given** policy $\pi_\theta$. The Euler equation, in contrast, characterizes the **optimal** policy by differentiating with respect to the **action** at each state. Danskin's theorem justifies ignoring how the optimal action might change locally when taking the derivative—we simply evaluate the gradient at the maximizer. The fundamental matrix $(I - \gamma P_{\pi^*})^{-1}$ then sums the discounted visitation, exactly as in the policy gradient proof. The key difference is that we're characterizing optimality (first-order conditions) rather than evaluating a given policy (gradient of the value).

### When Are Euler Equation Methods Applicable?

Euler equation methods require:

1. **Differentiability**: The reward function $r(s,a)$ must be differentiable in $a$, and transition dynamics (if state-dependent) must be smooth.
2. **Interior solutions**: The optimal action must satisfy $\pi^*(s) \in \text{interior}(\mathcal{A})$ so that first-order conditions are necessary. Constrained or bang-bang solutions require complementarity conditions or switching logic.
3. **Uniqueness**: The first-order condition must uniquely determine the optimal action. If the Hamiltonian is non-convex in $a$, first-order conditions are insufficient.

These conditions are analogous to those required for using Pontryagin's maximum principle in continuous-time trajectory optimization. When they hold, the optimal policy can be characterized without explicitly computing the value function. This defines a class of MDPs called the **Euler class** {cite}`Rust1996`.

### Weighted Residual Methods for the Euler Equation

Given a parameterized policy $\pi_{\boldsymbol{\theta}}(s) = \sum_{i=1}^n \theta_i \varphi_i(s)$, we seek parameters $\boldsymbol{\theta}$ such that the Euler residual $\mathcal{R}_E(\pi_{\boldsymbol{\theta}})$ is close to zero. The three weighted residual methods from earlier in this chapter apply directly:

**Collocation.** Choose states $\{s_1, \ldots, s_n\}$ and require the Euler equation to hold exactly at these points:

$$
\mathcal{R}_E(\pi_{\boldsymbol{\theta}})(s_i) = 0, \quad i = 1, \ldots, n.
$$

This gives $n$ (generally nonlinear) equations in $n$ unknowns. The collocation points can be chosen as zeros of orthogonal polynomials for spectral accuracy, just as in value function collocation.

**Galerkin.** Require the residual to be orthogonal to the basis functions with respect to a weighted inner product:

$$
\langle \mathcal{R}_E(\pi_{\boldsymbol{\theta}}), \varphi_i \rangle_w = \int_{\mathcal{S}} \mathcal{R}_E(\pi_{\boldsymbol{\theta}})(s) \varphi_i(s) w(s) ds = 0, \quad i = 1, \ldots, n.
$$

This is the approach used by {cite}`Judd1992` for economic growth models, using Chebyshev polynomial bases and Gauss-Chebyshev quadrature to approximate the integrals. The same framework applies to continuous control problems in robotics and other RL domains.

**Least Squares.** Minimize the weighted norm of the Euler residual:

$$
\boldsymbol{\theta}^* = \arg\min_{\boldsymbol{\theta}} \int_{\mathcal{S}} \mathcal{R}_E(\pi_{\boldsymbol{\theta}})(s)^2 w(s) ds.
$$

The first-order conditions for this minimization problem yield orthogonality conditions with test functions $\frac{\partial \mathcal{R}_E(\pi_{\boldsymbol{\theta}})}{\partial \theta_i}$, just as in the nonlinear value function case.

In each case, we obtain a system of $n$ equations $G(\boldsymbol{\theta}) = \mathbf{0}$, which can be solved by Newton's method or other rootfinding algorithms. The system is generally nonlinear because the Euler residual involves the policy evaluated at future states that depend on current actions: $j = f(s, \pi_{\boldsymbol{\theta}}(s))$.

### Computational Considerations and Tradeoffs

**Advantages of Euler equation methods:**

1. **Direct policy approximation**: For problems where the policy is simpler than the value function (lower-dimensional, smoother), directly approximating $\pi$ can be more efficient than approximating $v$ and extracting $\pi$ via the argmax.
2. **Euler residual accuracy**: If the goal is to minimize the Euler residual (e.g., for testing whether a policy satisfies optimality conditions), directly targeting this residual is more accurate than deriving the policy from an approximate value function.
3. **Interpretable optimality conditions**: The Euler equation directly encodes first-order optimality conditions, making diagnostics and model validation more transparent than checking Bellman residuals.

**Disadvantages:**

1. **Nonlinearity**: Even for problems where the Bellman operator has structure (e.g., linearity in the value function for policy evaluation), the Euler equation is generally nonlinear in the policy, requiring iterative solvers like Newton's method at each function evaluation.
2. **Limited applicability**: Requires differentiability and interior solutions. Many RL problems involve discrete actions, constraints, or non-smooth rewards where first-order conditions do not apply.
3. **No global convergence guarantee**: Unlike value iteration with monotone approximators, successive approximation of the Euler equation $\pi_{k+1} = \Psi(\pi_k)$ does not generally define a contraction mapping, though convergence can be established under problem-specific conditions.

**Empirical performance**: In numerical comparisons on smooth control problems with differentiable rewards and dynamics, Euler equation projection methods (particularly orthogonal collocation) can achieve high accuracy with relatively few basis functions {cite}`Judd1992`. However, value function iteration remains more robust and broadly applicable, especially for problems with discrete actions, constraints, or non-smooth objectives common in many RL applications.

### Connection to Policy Parameterization

Euler equation methods provide a preview of **policy parameterization**, where we directly represent and optimize policies rather than value functions. The next chapter extends the weighted residual framework to simulation-based settings, where expectations in both the Bellman operator and the Euler operator must be approximated from samples. Policy gradient methods in modern reinforcement learning can be viewed as stochastic approximations to Euler equation residual minimization, though typically without explicitly forming the Euler equation. A later chapter will develop policy parameterization systematically, showing how actor-critic architectures combine value function approximation (for variance reduction) with policy parameterization (for direct optimization).

The generality of weighted residual methods is now clear: they apply to any functional equation arising from an MDP, whether that equation characterizes the value function (Bellman equation), the policy (Euler equation), or other objects like Q-functions or advantage functions. The unifying principle remains orthogonality of residuals to test functions. The next chapter extends this framework to the simulation-based setting, where operators themselves must be approximated from samples rather than computed exactly.

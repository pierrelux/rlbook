...
What does this bias mean operationally? If we sample $N$ next states for each action and compute $\max_a \hat{\mu}_N(s,a)$, we get some estimate. If we repeat this entire process many times—each time drawing fresh samples and taking the max—and then average all those estimates together, that average will not converge to the true Bellman value $(\Bellman v)(s)$. Instead, it will systematically land above it. This is fundamentally different from having noisy but unbiased estimates, where repeated sampling and averaging would eventually get us arbitrarily close to the truth. Here, no amount of repetition fixes the problem: the maximization step ensures our estimates are systematically optimistic, and this optimism compounds across iterations as overestimates feed into future target computations.

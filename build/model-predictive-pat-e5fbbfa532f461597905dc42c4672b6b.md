...
in parallel. We first work through this calculation, then derive its
connection to stochastic optimal control. A Brownian particle gives us an
analytic check; competing passages and uncertain aircraft winds show how
physical uncertainty affects planning. Executing the first action and
planning again from the next observation will connect these computations
to the following chapter on receding-horizon control.
...
Suppose a current plan steers a particle toward a target but stops short.
We can simulate nearby plans that push more strongly, push less strongly,
or change when the force is applied. How should their outcomes change the
current plan? Single shooting already supplies the computation we need:
each candidate control sequence produces a trajectory and a cost. MPPI
uses a batch of these evaluations to decide the update.

Write the current sequence as $\mathbf U=(\mathbf u_0,\ldots,\mathbf u_{N-1})$.
Draw $M$ Gaussian perturbation sequences $\boldsymbol\epsilon^{(i)}$ and
...
$$
\mathbf U^+=\sum_{i=1}^M w_i\mathbf V^{(i)}
=\mathbf U+\sum_{i=1}^M w_i\boldsymbol\epsilon^{(i)},
\qquad w_i\geq0,\qquad \sum_{i=1}^M w_i=1.
$$

The same trajectory weight multiplies every action in its sequence. If the
plans that push harder tend to finish closer to the target, they receive
more weight, and the average shifts toward a stronger push. The update
combines candidates instead of simply selecting the best sampled plan.

The weights decrease exponentially with the trajectory cost. For an initial
batch drawn from the zero-mean Gaussian reference used below, a rollout
with state and terminal cost $S_i$ receives

$$
w_i=\frac{\exp(-S_i/\lambda)}{\sum_{j=1}^M\exp(-S_j/\lambda)},
\qquad \lambda>0.
$$

A cost advantage of $\lambda\log2$ therefore gives twice the weight.
Smaller $\lambda$ concentrates the average on fewer candidates. In this
formulation the Gaussian reference accounts for control effort; we will
derive that relationship after the worked example. Sampling around a
nonzero plan also requires an importance correction to the weights, derived
in @sec-mppi-distribution.
...
guarantee that a modest number of samples will find useful plans: long
...
using the update in the passage and aircraft examples.

The sampling procedure can be used with deterministic dynamics. Its random
perturbations explore possible controls. A stochastic physical system adds
another reason to simulate many trajectories: even a fixed control can
produce different outcomes. The particle example below connects these two
uses of randomness under assumptions that we can verify explicitly.

## A Brownian Particle and One Control Update

We now reduce the plan to a single constant force applied for one interval.
Can the weighted average recover a force that we can also calculate
analytically? A particle with Brownian motion and a quadratic target cost
lets us answer this question by computing three trajectories, their weights,
and the resulting update. This first example checks the algorithm; the
later passages will show uncertainty changing the preferred action.

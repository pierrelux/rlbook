...
The weighted residual framework applies to any functional equation arising from an MDP. An alternative to approximating the value function is to directly approximate the **optimal policy** by solving the first-order conditions that characterize optimality {cite}`Judd1992`. In dynamic programming, these first-order conditions are often called **Euler equations**.

Consider an MDP with a continuous action space $\mathcal{A}$ where the optimal policy $\pi^*(s)$ solves
$$
\pi^*(s)
= \arg\max_{a \in \mathcal{A}}
\left\{ r(s, a) + \gamma \sum_{j \in \mathcal{S}} p(j \mid s,a)\, v^*(j) \right\}.
$$
Define
$$
Q^*(s,a)
:= r(s,a) + \gamma \sum_{j \in \mathcal{S}} p(j \mid s,a)\, v^*(j).
$$
Assume $\mathcal{A}$ is convex, $r(s,a)$ and $p(\cdot \mid s,a)$ are differentiable in $a$, and that $\pi^*(s)$ is an interior maximizer. By Danskin's theorem, if $a \mapsto Q^*(s,a)$ has an interior maximizer at $a = \pi^*(s)$, then the first-order condition is
$$
\frac{\partial Q^*(s,a)}{\partial a}\Big|_{a = \pi^*(s)}
=
\frac{\partial r(s, \pi^*(s))}{\partial a}
\;+\;
\gamma \sum_{j \in \mathcal{S}} 
\frac{\partial p(j \mid s,a)}{\partial a}\Big|_{a=\pi^*(s)} v^*(j)
= 0,
\quad \forall s \in \mathcal{S}.
\tag{FOC}
$$
This condition is necessary, and it is sufficient if $Q^*(s,\cdot)$ is concave in $a$. In this sense, the Euler equation is simply the first-order condition of the dynamic optimization problem.

To see more clearly how derivatives of the value function enter, and how the envelope theorem is used in practice, it is convenient to specialize to a continuous state with deterministic dynamics.

### Deterministic dynamics and the Euler equation

Suppose that the next state is given by a differentiable mapping
$$
s' = f(s,a),
$$
and that the optimal value function is differentiable in $s$. The Bellman optimality equation at the optimal policy can be written as
$$
v^*(s) = r\big(s,\pi^*(s)\big) + \gamma\, v^*\big(f(s,\pi^*(s))\big).
$$

Assume:
- $r$ and $f$ are continuously differentiable in both $s$ and $a$,
- $\mathcal{A}$ is convex and $\pi^*(s)$ is an interior maximizer for each $s$,
- $v^*$ is differentiable in $s$.

For each fixed $s$, the optimal policy maximizes
$$
a \mapsto r(s,a) + \gamma\, v^*(f(s,a)).
$$
The first-order condition at $a = \pi^*(s)$ is
$$
...
\tag{Euler}
$$
This is the discrete-time **Euler equation** for the optimal policy. It relates the current marginal payoff to the marginal continuation value through the dynamics.

### Envelope form

In many economic applications, one also wants an expression for the derivative of the value function with respect to the state. This is where the envelope theorem enters.

Differentiate the Bellman identity with respect to $s$ and apply the chain rule:
$$
\begin{aligned}
\frac{\partial v^*}{\partial s}(s)
&=
\frac{\partial r}{\partial s}\big(s,\pi^*(s)\big)
+
\frac{\partial r}{\partial a}\big(s,\pi^*(s)\big)\,
\frac{\partial \pi^*}{\partial s}(s) \\
&\quad
+
\gamma\, \frac{\partial v^*}{\partial s}\big(f(s,\pi^*(s))\big)
\left[
\frac{\partial f}{\partial s}\big(s,\pi^*(s)\big)
+
\frac{\partial f}{\partial a}\big(s,\pi^*(s)\big)\,
\frac{\partial \pi^*}{\partial s}(s)
\right].
...
The total derivative (TD) contains terms involving $\frac{\partial \pi^*}{\partial s}(s)$. At the optimum, the Euler condition (Euler) holds:
$$
\frac{\partial r}{\partial a}\big(s,\pi^*(s)\big)
+
\gamma\, \frac{\partial v^*}{\partial s}\big(f(s,\pi^*(s))\big)\,
...
0.
$$
Multiply this equation on the right by $\frac{\partial \pi^*}{\partial s}(s)$:
$$
\frac{\partial r}{\partial a}\big(s,\pi^*(s)\big)\,
\frac{\partial \pi^*}{\partial s}(s)
+
...
= 0.
$$
These are exactly the terms in (TD) that involve $\frac{\partial \pi^*}{\partial s}(s)$. Substituting this identity into (TD) cancels the policy-derivative terms and yields the **envelope equation**
$$
\frac{\partial v^*}{\partial s}(s)
=
\frac{\partial r}{\partial s}\big(s,\pi^*(s)\big)
...
The pair of equations (Euler) and (Envelope) forms a system of functional equations in $\pi^*(\cdot)$ and $\nabla v^*(\cdot)$. The envelope theorem has removed derivatives of the policy from the expression for $\nabla v^*$, so the value function enters only through its derivative with respect to the state. As Rust {cite}`Rust1996` notes, one does not need to track the impact of current decisions on all future periods one by one. The envelope condition summarizes those effects in $\nabla v^*$.

For stochastic dynamics, an analogous pair of equations can be derived. Suppose 
$$
s' = g(s,a,\xi)
$$
with exogenous noise $\xi$, and write the Bellman equation as
$$
v^*(s) = r\big(s,\pi^*(s)\big) + \gamma\, \mathbb{E}\big[v^*\big(g(s,\pi^*(s),\xi)\big)\big].
$$
Under the same smoothness and interiority assumptions, the Euler equation becomes
$$
\frac{\partial r}{\partial a}\big(s,\pi^*(s)\big)
+
\gamma\, \mathbb{E}\!\left[
\frac{\partial v^*}{\partial s}\big(g(s,\pi^*(s),\xi)\big)\,
\frac{\partial g}{\partial a}\big(s,\pi^*(s),\xi\big)
\right]
= 0,
$$
...
\gamma\, \mathbb{E}\!\left[
\frac{\partial v^*}{\partial s}\big(g(s,\pi^*(s),\xi)\big)\,
\frac{\partial g}{\partial s}\big(s,\pi^*(s),\xi\big)
\right].
$$
For constrained actions, the first-order condition is replaced by the KKT conditions.

### The Euler Residual Operator

Define the **Euler residual operator** $\mathcal{R}_E : \Pi \to \mathcal{F}(\mathcal{S})$ by the left-hand side of equation (Euler). The optimal policy satisfies $\mathcal{R}_E(\pi^*) = 0$. Unlike the Bellman operator, which is a $\gamma$-contraction in the sup norm, the Euler residual operator does not have a comparable global contraction property. Iterative methods may fail to converge without additional structure, and error bounds must be derived problem by problem. Despite these limitations, Euler equation methods are widely used in economics where they often admit analytic or low-dimensional numerical solutions.

Euler equation methods require differentiability of $r(s,a)$ and the dynamics in $a$, interior solutions, and that the first-order condition uniquely determines the optimal action. This defines a class of MDPs called the **Euler class** {cite}`Rust1996`.

### Weighted Residual Methods

Given a parameterized policy $\pi_{\boldsymbol{\theta}}(s)$, the weighted residual methods from earlier (collocation, Galerkin, least squares) apply directly to minimize the Euler residual $\mathcal{R}_E(\pi_{\boldsymbol{\theta}})$. Judd {cite}`Judd1992` uses Galerkin projection with Chebyshev polynomials for economic growth models. The resulting system
$$
G(\boldsymbol{\theta}) = \mathbf{0}
$$
is generally nonlinear and is solved by iterative methods.

Euler equation methods directly approximate policies, which can be more efficient when the policy is simpler than the value function. The Euler condition encodes first-order optimality directly, and violations of the Euler equation provide natural diagnostics. On the other hand, the resulting equations are nonlinear, rely on differentiability and interior solutions, and are less robust to discrete actions or non-smooth objectives. Value-function methods remain more flexible in those settings.

Euler equation methods also provide a bridge to **policy parameterization** in modern reinforcement learning. One can view policy gradient methods as stochastic approximation schemes that drive an approximate policy toward satisfying Euler-type first-order conditions on average. The next chapter extends the weighted residual framework to simulation-based settings, where operators must be approximated from samples.

...
The iteration $v_{k+1} = \Proj \Bellman v_k$ is written in function space, but we actually work with finite-dimensional representations $v_k = \boldsymbol{\Phi} \boldsymbol{\theta}_k$, where $\boldsymbol{\Phi}$ is the matrix of basis function evaluations at sample states and $\boldsymbol{\theta}_k$ is the vector of coefficients. What does the function iteration become when we translate it to parameter space?

Consider the Galerkin case. We want to find $v_{k+1} = \boldsymbol{\Phi} \boldsymbol{\theta}_{k+1}$ such that:

$$
\langle v_{k+1} - \Bellman v_k, \varphi_j \rangle_w = 0, \quad j = 1, \ldots, d.
$$

Recall from earlier in the chapter that the projection operator is $\Proj = \boldsymbol{\Phi}(\boldsymbol{\Phi}^\top \mathbf{W} \boldsymbol{\Phi})^{-1} \boldsymbol{\Phi}^\top \mathbf{W}$, where $\mathbf{W}$ is a diagonal matrix with weights $w(s_i)$ on the diagonal. The iteration $v_{k+1} = \Proj \Bellman v_k$ becomes:

$$
\boldsymbol{\Phi} \boldsymbol{\theta}_{k+1} = \boldsymbol{\Phi}(\boldsymbol{\Phi}^\top \mathbf{W} \boldsymbol{\Phi})^{-1} \boldsymbol{\Phi}^\top \mathbf{W} (\Bellman v_k).
$$

The key is the term $(\Bellman v_k)$ on the right. This is a vector containing the evaluations $[(\Bellman v_k)(s_1), \ldots, (\Bellman v_k)(s_n)]^\top$. Let's call this vector $\mathbf{y}$:
...
This is exactly the weighted least-squares formula for fitting $\boldsymbol{\Phi} \boldsymbol{\theta}$ to data $\mathbf{y}$. The matrix $\boldsymbol{\Phi}$ contains the basis function evaluations at sample points (the "design matrix" in regression). The vector $\mathbf{y}$ contains the targets. The operator $(\boldsymbol{\Phi}^\top \mathbf{W} \boldsymbol{\Phi})^{-1} \boldsymbol{\Phi}^\top \mathbf{W}$ is the pseudoinverse that solves the regression problem.

So here is what the projection operator is doing computationally:

1. Evaluate the Bellman operator at sample states: $y_i = (\Bellman v_k)(s_i)$ for $i = 1, \ldots, n$
2. Solve the regression problem: find $\boldsymbol{\theta}_{k+1}$ that minimizes $\sum_i w(s_i) [y_i - \sum_j \theta_j \varphi_j(s_i)]^2$
3. The new value function is $v_{k+1}(s) = \sum_j \theta_{k+1,j} \varphi_j(s)$, now defined everywhere

The "data" being fit is the vector $\mathbf{y}$ of Bellman operator evaluations. The projection operator $\Proj$ is the regression operator that fits our approximation to this data. Function iteration in the abstract becomes **successive regression** in the computational implementation: generate targets, fit to targets, repeat.

For collocation, the story is similar. We require $v_{k+1}(s_i) = y_i$, which in matrix form is $\boldsymbol{\Phi} \boldsymbol{\theta}_{k+1} = \mathbf{y}$. If $\boldsymbol{\Phi}$ is square and invertible, $\boldsymbol{\theta}_{k+1} = \boldsymbol{\Phi}^{-1} \mathbf{y}$. This is exact interpolation: fit the basis functions to pass through the data points.

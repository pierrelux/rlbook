...
### Convergence Analysis in Weighted $L^2$

LSTD finds the fixed point of the *projected* Bellman operator, not the Bellman operator itself, so approximation error persists. But will iterative methods converge to this fixed point? For convergence of $v_{k+1} = \Proj \BellmanPi v_k$, we need the composed operator $\Proj \BellmanPi$ to be a contraction in $\|\cdot\|_\xi$. The Bellman operator $\BellmanPi = r_\pi + \gamma \mathbf{P}_\pi$ is a $\gamma$-contraction in $\|\cdot\|_\infty$, but we will see that whether it contracts in $\|\cdot\|_\xi$ depends on the relationship between $\xi$ and $\mathbf{P}_\pi$.
...
### Contraction Under Stationary Distribution

Suppose $\xi$ is the steady-state distribution: $\xi^\top \mathbf{P}_\pi = \xi^\top$. Our goal is to establish that $\Proj \BellmanPi$ is a contraction in $\|\cdot\|_\xi$. We already know $\Proj$ is non-expansive in this norm. Therefore, if $\BellmanPi$ is a $\gamma$-contraction in $\|\cdot\|_\xi$, then their composition will be a $\gamma$-contraction:

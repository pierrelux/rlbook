...
Consider the interval $[t_k,t_{k+1}]$, with length
$h_k=t_{k+1}-t_k$. Its normalized coordinate is
...
The unknown state and control are functions, but a nonlinear-programming solver
accepts only finitely many numbers. A transcription must therefore choose
finite coordinates for those functions and replace the differential equation
by algebraic equality constraints, producing a finite nonlinear program (NLP).

Polynomial values at selected points provide the finite coordinates. They lead
to the following construction:

$$
\begin{array}{ccc}
\text{nodal values} & \longrightarrow & \text{polynomial trajectory}\\
&&\downarrow\\
\text{finite NLP} & \longleftarrow &
\substack{\text{dynamics and}\\\text{quadrature constraints}}
\end{array}
$$

Euler and trapezoidal transcription instantiate each step of this construction.
Hermite--Simpson applies the same operators at higher polynomial degree.
...
Here $x(t)\in\mathbb R^{n_x}$ and $u(t)\in\mathbb R^{n_u}$. The terminal term $\Phi$ and running term $L$ may both be present. Setting $L=0$ gives the Mayer special case, while setting $\Phi=0$ gives the Lagrange special case. All three forms use the same transcription machinery.
...
$\tau=(t-t_k)/h_k$. Normalization defines the differentiation and integration
operators once on $[0,1]$; the interval length $h_k$ supplies the physical-time
scaling.
...
and the operators built from it. Nodes derived from Legendre polynomials can
still use Lagrange nodal coordinates in the NLP; no recurrence for
orthogonal-polynomial coefficients is needed.
...
residual vanishes at the collocation nodes. An NLP solver may use a squared
constraint violation in an internal merit function, but the collocation
...
Start by representing the state derivative at $s$ collocation nodes
$c_1,\ldots,c_s$ on $[0,1]$. Their Lagrange cardinal functions are
$\ell_j$. Let

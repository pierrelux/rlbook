...
Deep Q-Network (DQN) relaxes the buffer constraint by maintaining a circular replay buffer of capacity $B$. We still collect one transition per step, but now we can sample mini-batches from recent history.

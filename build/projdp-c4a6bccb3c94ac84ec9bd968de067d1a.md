...
These are orthogonality conditions with test functions $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i}$. These partial derivatives span the tangent space to the manifold $\{v_{\boldsymbol{\theta}} : \boldsymbol{\theta} \in \mathbb{R}^p\}$ at the current parameters. When $v_{\boldsymbol{\theta}} = \sum_i \theta_i \varphi_i$, we have $\frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} = \varphi_i$, recovering the Galerkin equations. For nonlinear parameterizations, the partial derivatives change with $\boldsymbol{\theta}$, and we solve by gradient descent. The solution defines an implicit nonlinear projection operator $\Proj_{\mathcal{F}}$, generalizing $\Proj$.

Neural network function approximation in reinforcement learning instantiates this framework. When training a neural network $v_{\boldsymbol{\theta}}$ by minimizing squared temporal-difference errors or Bellman residuals, we are performing gradient descent on the sampled loss

$$
\mathcal{L}(\boldsymbol{\theta}) = \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - (\Bellman v_{\boldsymbol{\theta}})(s))^2 \big].
$$

This can be interpreted as implementing the orthogonality condition $\langle \Residual(v_{\boldsymbol{\theta}}), \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \rangle = 0$ in a Monte Carlo approximation of the $L^2(\mu)$ inner product, assuming gradient descent converges to a stationary point. Standard algorithms like Q-learning and TD learning with neural networks are thus computing nonlinear projections of the Bellman equation onto the neural network manifold, exactly as the minimum residual framework prescribes.

This equivalence between norm minimization and orthogonality holds for any inner product: weighted $L^2$ integrals for Galerkin, sums over collocation points for collocation, or sampled expectations for neural networks. The **dual pairing** formulation {cite}`LegrandJunca2025` provides the unifying framework. A dual space is the collection of all linear functionals on a function space: mappings that take a function and return a number. For example, "evaluate at point $x_i$" and "integrate against a weight" are both functionals. This generality handles test objects that are not regular functions. Collocation uses Dirac delta "functions" $\delta(x - x_i)$, which are distributions defined only through their action on test functions via the dual pairing $\langle \Residual(v), \delta(x - x_i) \rangle = (\Residual v)(x_i)$. The induced dual norm $\|\Residual(v)\|_* = \sup_{\|w\|=1} |\langle \Residual(v), w \rangle|$ measures the residual by its worst-case effect on test functions. This worst-case perspective has inspired recent work {cite}`Zang2020` that literally poses residual minimization as a two-player game, with one network representing the trial function and another representing adversarial test functions.

The minimum residual framework unifies classical projection methods and modern function approximation across several dimensions. Linear methods (polynomials, splines) use fixed test functions and yield linear systems. Nonlinear methods (neural networks) use parameter-dependent test functions and require iterative optimization. The approximation class, test space, and solution algorithm all vary, but the underlying principle remains: find parameters where the residual is orthogonal to a chosen test space. Convergence guarantees differ accordingly. Linear projections with monotonicity or matched weightings yield contractions with rigorous error bounds. Nonlinear methods offer greater representational power but lack general convergence guarantees, making their analysis an active research frontier. The next chapter completes this picture by showing how to implement these methods when the Bellman operator itself must be approximated from samples rather than computed exactly.

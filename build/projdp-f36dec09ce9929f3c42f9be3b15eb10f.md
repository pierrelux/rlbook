...
If $r$ is differentiable in $a$ and the optimal action $a^*(s)$ is interior, the first-order condition at the optimum is:

$$
\frac{\partial r(s, a)}{\partial a}\Big|_{a=a^*(s)} + \gamma \mathbb{E}_{s' \sim p(\cdot|s,a^*(s))} \left[ v'(s') \frac{\partial s'}{\partial a} \right] = 0.
$$

For problems where we can eliminate the value function using the envelope theorem, this becomes a condition purely in terms of the policy. Consider a continuous control problem with smooth reward $r(s,a)$ and dynamics $s' = f(s,a) + \epsilon$. Taking the derivative of the Bellman optimality equation with respect to $a$ and using the envelope theorem to express $\nabla_s v$ in terms of the policy, we obtain a functional equation of the form:

$$
\frac{\partial r(s, a(s))}{\partial a} + \gamma \mathbb{E}\left[ \nabla_s v(s') \cdot \frac{\partial f(s, a(s))}{\partial a} \right] = 0,
$$

where the envelope theorem gives $\nabla_s v(s) = \frac{\partial r(s, a(s))}{\partial s} + \gamma \mathbb{E}[\nabla_s v(s') \cdot \frac{\partial f(s, a(s))}{\partial s}]$. Through recursive substitution, the value function can be eliminated entirely, leaving a **discrete-time Euler equation**: a functional equation in the policy $a(\cdot)$ alone.

### The Euler Equation as a Functional Equation

Define the Euler residual operator $\EulerResidual$ on the space of policies:

$$
\EulerResidual(\alpha)(s) = \frac{\partial r(s, \alpha(s))}{\partial a} + \gamma \mathbb{E}_{s' \sim p(\cdot|s,\alpha(s))} \left[ \nabla_s v(s') \cdot \frac{\partial f(s, \alpha(s))}{\partial a} \right].
$$

The optimal policy $\alpha^*$ satisfies the functional equation:

$$
\EulerResidual(\alpha^*) = 0.
$$
...
2. **Interior solutions**: The optimal action must satisfy $a^*(s) \in \text{interior}(\mathcal{A})$ so that first-order conditions are necessary. Constrained or bang-bang solutions require complementarity conditions or switching logic.
...
Given a parameterized policy $\alpha_{\boldsymbol{\theta}}(s) = \sum_{i=1}^n \theta_i \varphi_i(s)$, we seek parameters $\boldsymbol{\theta}$ such that the Euler residual $\EulerResidual(\alpha_{\boldsymbol{\theta}})$ is close to zero. The three weighted residual methods from earlier in this chapter apply directly:

**Collocation.** Choose states $\{s_1, \ldots, s_n\}$ and require the Euler equation to hold exactly at these points:

$$
\EulerResidual(\alpha_{\boldsymbol{\theta}})(s_i) = 0, \quad i = 1, \ldots, n.
$$
...
$$
\langle \EulerResidual(\alpha_{\boldsymbol{\theta}}), \varphi_i \rangle_w = \int_{\mathcal{S}} \EulerResidual(\alpha_{\boldsymbol{\theta}})(s) \varphi_i(s) w(s) ds = 0, \quad i = 1, \ldots, n.
$$
...
$$
\boldsymbol{\theta}^* = \arg\min_{\boldsymbol{\theta}} \int_{\mathcal{S}} \EulerResidual(\alpha_{\boldsymbol{\theta}})(s)^2 w(s) ds.
$$

The first-order conditions for this minimization problem yield orthogonality conditions with test functions $\frac{\partial \EulerResidual(\alpha_{\boldsymbol{\theta}})}{\partial \theta_i}$, just as in the nonlinear value function case.

In each case, we obtain a system of $n$ equations $G(\boldsymbol{\theta}) = \mathbf{0}$, which can be solved by Newton's method or other rootfinding algorithms. The system is generally nonlinear because the Euler residual involves the policy evaluated at future states that depend on current actions: $s' = g(s, \alpha_{\boldsymbol{\theta}}(s))$.

### Computational Considerations and Tradeoffs

**Advantages of Euler equation methods:**

1. **Direct policy approximation**: For problems where the policy is simpler than the value function (lower-dimensional, smoother), directly approximating $\alpha$ can be more efficient than approximating $v$ and extracting $\alpha$ via the argmax.
...
3. **No global convergence guarantee**: Unlike value iteration with monotone approximators, successive approximation of the Euler equation $\alpha_{k+1} = \Psi(\alpha_k)$ does not generally define a contraction mapping, though convergence can be established under problem-specific conditions.

...
expanding and cooling as it moves away. Exchanging heat with the plate at each
extreme of its travel, the parcel acts as one link in a bucket brigade that
carries heat from the far end of the stack toward the loudspeaker end. A cold
heat exchanger at the far end connects to the payload being cooled, and a hot
heat exchanger at the loudspeaker end rejects the heat to ambient air. There
are no moving parts other than the loudspeaker, and the only knob is how hard
it drives. The control problem is a pull-down: starting from ambient
temperature, choose a driver-amplitude schedule that brings the payload to a
target temperature by a fixed deadline while spending little acoustic energy.
...
The replay reads recorded plans and integrates only the constant-amplitude
comparison; it does not rerun the optimizer.
...
lowers the efficiency of every later step. At positive work, the cooling
coefficient of performance (COP) is $\dot Q_c/\dot W$. All coefficients are

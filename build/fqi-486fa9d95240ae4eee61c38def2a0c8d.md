...
where $\delta_\tau$ is the Dirac mass at transition $\tau$. For any integrand $g(\tau)$ (loss, TD error, gradient term), expectations under $P$ are approximated by expectations under $\hat{P}_{\mathcal{B}_t}$ using the **sample average estimator**:
...
This is exactly the sample average estimator from the Monte Carlo chapter, applied now to transitions. Conceptually, fitted Q-iteration performs **plug-in approximate dynamic programming**: we replace the unknown transition law $P$ by $\hat{P}_{\mathcal{B}_t}$ and run value iteration using this empirical Bellman operator:
...
depending on whether we use a hard or soft Bellman operator. Composing the empirical distribution with this map provides an empirical distribution over regression pairs:

$$
\hat{P}_t^{\text{fit}} \triangleq (\mathrm{id},T_q)_\# \hat{P}_{\mathcal{B}_t}
$$

That is, the distribution of $((s,a), y)$ when $(s,a,r,s')\sim\hat{P}_{\mathcal{B}_t}$ and $y = T_{q}(s,a,r,s')$. The inner regression problem at time $t$ is therefore:
...
**Offline, replay, and online as special cases.** Different algorithms correspond to different ways of evolving the buffer $\mathcal{B}_t$ and different replay ratios:
...
Crucially, flattening replaces the pair $(n,k)$ by a single global step index $t$, but the underlying empirical distribution $\hat{P}_{\mathcal{D}}$ remains the same—we still sample from the fixed offline dataset throughout.

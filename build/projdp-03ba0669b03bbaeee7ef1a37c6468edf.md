...
$$
\int_{\mathcal{S}} \varphi_i(x) R(x; \theta) dx = 0, \quad i = 1, \ldots, n.
$$

To understand what this means, recall that in finite dimensions, two vectors are orthogonal when their inner product is zero. For functions, $\langle R, \varphi_i \rangle = \int R(x) \varphi_i(x) dx = 0$ expresses the same concept: $R$ and $\varphi_i$ are orthogonal as functions. But there's more to this than just testing against individual basis functions.
...
$$
\langle R(\cdot; \theta), I_{D_i} \rangle = \int_{D_i} R(x; \theta) dx = 0, \quad i = 1, \ldots, n.
$$
...
$$
\min_\theta \int_{\mathcal{S}} R(x; \theta)^2 dx = \min_\theta \langle R(\cdot; \theta), R(\cdot; \theta) \rangle.
$$
...
- **Galerkin and moments**: More expensive due to integration. Computing $P_i(\theta) = \int R(x; \theta) p_i(x) dx$ requires numerical quadrature. Each Jacobian entry requires integrating $\frac{\partial R}{\partial \theta_j} p_i$.

...
token and of per-request mean inter-token latency. Confirmation compares this
...
already streaming, and its transition model is fitted on mixed-serving data.
These measurements test queue drainage under the workload the controller will
actually encounter.
...
record. Completed confirmation measurements are still pending.

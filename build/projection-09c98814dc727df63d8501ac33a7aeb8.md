...
| **Method** | **Test functions** | **Equivalent optimization** |
|:-----------|:-------------------|:---------------------------|
| Minimum residual (full) | $p_j = \frac{\partial R(\cdot; \theta)}{\partial \theta_j}$ | $\displaystyle\min_\theta \tfrac{1}{2}\|R(\cdot; \theta)\|_w^2$ |
| Projection (FQI step) | $p_j = \frac{\partial \hat{v}}{\partial \theta_j}$ on $(\hat{v} - f)$ | $\displaystyle\min_\theta \tfrac{1}{2}\|\hat{v}(\cdot; \theta) - f\|_w^2$ |
| Galerkin | $p_j = \varphi_j$ (trial basis) | min residual in dual norm of $\text{span}\{\varphi_j\}$ |
| Petrov–Galerkin | $p_j \neq \varphi_j$ | weighted LS with custom metric (stability knob) |
| Collocation | $p_i = \delta(s - s_i)$ (point masses) | $\displaystyle\min_\theta \sum_i \omega_i \, R(s_i; \theta)^2$ |

To see why this correspondence holds, consider the **minimum residual** objective $\min_\theta \frac{1}{2}\|R(\cdot; \theta)\|_w^2 = \min_\theta \frac{1}{2}\langle R(\cdot; \theta), R(\cdot; \theta) \rangle_w$. Setting the gradient to zero:
...
The test functions are the gradients $\partial R / \partial \theta_j$. This is a **true minimum-residual** method: it directly minimizes how far $\hat{v}$ is from satisfying the Bellman equation.

Now consider **projection least-squares**. Fix a target function $f$ and minimize $\frac{1}{2}\|\hat{v}(\cdot; \theta) - f\|_w^2$. The first-order conditions are:

$$
\left\langle \hat{v}(\cdot; \theta) - f, \frac{\partial \hat{v}}{\partial \theta_j} \right\rangle_w = 0,
$$

which means the error $(\hat{v} - f)$ is orthogonal to the tangent space of the trial manifold. For linear approximation $\hat{v} = \sum_j \theta_j \varphi_j$, the partial derivative $\frac{\partial \hat{v}}{\partial \theta_j} = \varphi_j$ is just the $j$-th basis function, so the test functions are the basis functions themselves. This is **orthogonal projection** of $f$ onto the approximation space: $\hat{v} = \Pi_w f$.

The connection to **Fitted Q-Iteration (FQI)** is now immediate. Taking $f = \Bellman \hat{v}_k$ (the Bellman image of the current iterate), projection least-squares gives:

$$
\hat{v}_{k+1} = \Pi_w \, \Bellman \hat{v}_k.
$$

This is **not** minimum-residual iteration. FQI does not minimize $\|R\|^2 = \|\Bellman \hat{v} - \hat{v}\|^2$ directly. Instead, it **projects**, then **iterates**: apply $\Bellman$, project the result back onto the approximation space, repeat. The distinction matters because minimum-residual requires differentiating through the Bellman operator (the "full gradient"), while projection uses only the gradient of $\hat{v}$ with respect to $\theta$ (the "semi-gradient"). We return to this distinction when discussing temporal difference learning.

...
where:
- $y_t \in \mathbb{R}^n$ is the **stock variable**: an endogenous, controlled quantity that accumulates over time (e.g., capital, inventory, water in a reservoir, energy in a battery).
- $z_t \in \mathbb{R}^p$ is the **exogenous shock**: a random disturbance outside the agent's control (e.g., demand, weather, equipment failures).
...
so the stock evolves via a known function $f$ (which may depend on the next shock $z_{t+1}$), while the shock follows an exogenous Markov law $q$.
...
The condition (EC) says that the way today's stock $y$ affects tomorrow's stock $y'$ is "aligned" with the way the action $a$ affects $y'$. More precisely: the Jacobian $\partial f/\partial y$ lies in the column space of $\partial f/\partial a$, with the matrix $h$ providing the linear combination.
...
The condition (EC) may look abstract at first, but it is straightforward to verify in concrete models. The key insight: **affine dynamics automatically satisfy (EC)**. Since many practical control problems use linear or affine state updates (at least as first-order approximations), the Euler-class structure is quite common.
...
For students coming from an RL background, Euler methods may feel unfamiliar. But if you are working on a problem with Euler-class structure—linear stock dynamics, differentiable rewards, continuous actions—this approach can be more efficient and interpretable than discretizing the state space and iterating on the Bellman equation.

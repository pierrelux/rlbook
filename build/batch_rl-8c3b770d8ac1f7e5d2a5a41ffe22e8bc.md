...
All value-based reinforcement learning algorithms share the same two-level structure. The outer loop applies the Bellman operator by computing targets from the current Q-function and observed transitions, producing a supervised learning dataset $\mathcal{D}^{\text{fit}} = \{((s_i, a_i), y_i)\}$ where each target is $y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}_n)$ using the single observed next state $s'_i$. The inner loop solves a regression problem: find parameters $\boldsymbol{\theta}$ that make $q(s_i, a_i; \boldsymbol{\theta}) \approx y_i$. We can write this abstractly as:

$$
\begin{aligned}
&\textbf{repeat } n = 0, 1, 2, \ldots \\
&\quad \text{Construct } \mathcal{D}_n^{\text{fit}} = \{((s_i, a_i), y_i)\} \text{ from transitions and current } q(\cdot, \cdot; \boldsymbol{\theta}_n) \\
&\quad \boldsymbol{\theta}_{n+1} \leftarrow \texttt{fit}(\mathcal{D}_n^{\text{fit}}, \boldsymbol{\theta}_{\text{init}}, K) \\
...
$$
\texttt{fit}(\mathcal{D}^{\text{fit}}, \boldsymbol{\theta}_{\text{init}}, K) \to \boldsymbol{\theta}
$$

and minimizes $\mathcal{L}(\boldsymbol{\theta}) = \frac{1}{|\mathcal{D}^{\text{fit}}|}\sum_{((s,a), y) \in \mathcal{D}^{\text{fit}}} \ell(q(s,a; \boldsymbol{\theta}), y)$ using $K$ gradient steps starting from $\boldsymbol{\theta}_{\text{init}}$. This resembles scikit-learn's API but with additional controls that matter for reinforcement learning. Standard supervised learning uses random initialization and runs to convergence ($K = \infty$). Reinforcement learning algorithms vary these: warm starting from the previous iteration's parameters ($\boldsymbol{\theta}_{\text{init}} = \boldsymbol{\theta}_n$), taking only $K \in \{1, \ldots, 10\}$ gradient steps, or somewhere in between.

The apparent diversity of methods (FQI, NFQI, Q-learning, DQN, Double DQN, Soft Q) emerges from independent choices about how to construct $\mathcal{D}_n^{\text{fit}}$, what initialization and truncation to pass to `fit`, and which loss function $\ell$ and function approximator $q(s,a; \boldsymbol{\theta})$ to use. This section maps that design space.

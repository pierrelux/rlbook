...
showing that $\Proj \mathrm{L}$ is a $\gamma$-contraction. The error bound follows from fixed-point analysis: $v^* - v_\Proj = (I - \Proj \mathrm{L})^{-1}(v^* - \Proj v^*)$, and since $\Proj \mathrm{L}$ is a $\gamma$-contraction, $\|(I - \Proj \mathrm{L})^{-1}\| \leq (1-\gamma)^{-1}$.
```

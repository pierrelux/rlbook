...
The target network is not a stabilization trick invented for deep RL. It's the natural byproduct of flattening the nested FQI structure. When you see DQN presented with a target network that updates every $K$ steps, you're looking at approximate value iteration in flattened form. The algorithm is still performing outer loop (value iteration) and inner loop (regression), but the presentation obscures this. Target networks exist because we've flattened the nested structure. They update periodically to mark the boundaries between outer iterations. DQN is batch approximate DP in flattened form, not online stochastic approximation.

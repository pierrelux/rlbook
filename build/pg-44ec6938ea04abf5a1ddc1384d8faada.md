...
To reuse data across multiple updates, recall the likelihood ratio method from the beginning of this chapter. There, we showed that for any proposal distribution $q(x)$, we can write an expectation under $p(x;\theta)$ as:

$$
\mathbb{E}_{x \sim p(x;\theta)}[f(x)] = \mathbb{E}_{x \sim q(x)}\left[f(x) \frac{p(x;\theta)}{q(x)}\right]
$$

The REINFORCE estimator corresponds to the special case $q(x) = p(x;\theta)$, where the ratio equals 1 and we recover the score function estimator. But the likelihood ratio framework is more general: we can use any $q$ whose support contains the support of $p$, and the importance weight $p/q$ corrects for the distribution mismatch.

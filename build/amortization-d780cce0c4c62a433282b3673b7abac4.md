...
The V-network itself is trained by sampling actions from the policy at state $s'_i$ and fitting to the entropy-adjusted Q-values. This three-way interaction—Q-network, V-network, and policy—exists only because the entropy-regularized formulation provides the mathematical relationship that lets us replace the intractable integral with a tractable expectation.

...
When DQN combined neural networks with replay buffers and periodic "target network updates," it appeared to introduce a novel stabilization mechanism. In reality, DQN is simply NFQI adapted for online data collection. The target network is not a design innovation—it is the direct consequence of approximate value iteration's mathematical structure.

...
We have developed weighted residual methods through the lens of abstract functional equations: choose test functions, impose orthogonality conditions $\langle R, p_i \rangle_w = 0$, solve for coefficients. This mathematical framework is elegant, but it can obscure the computational intuition. What are we actually doing when we apply these methods to solve dynamic programming problems? The answer is simpler than the formalism suggests: we are performing **function iteration with a fitting step**.

The weighted residual framework provides a systematic way to solve functional equations. Given an equation $\Residual(v) = 0$, we choose test functions $\{p_1, \ldots, p_n\}$ and impose orthogonality conditions:

$$
\langle \Residual(v), p_i \rangle_w = 0, \quad i = 1, \ldots, n.
$$

For the Bellman optimality equation, the residual is $\Residual(v) = v - \Bellman v$. The weighted residual conditions become:

$$
\langle v - \Bellman v, p_i \rangle_w = 0, \quad i = 1, \ldots, n.
$$

These conditions define a nonlinear system of equations for the coefficients of $v$ in our approximation space. As we showed earlier, this system has a fixed-point interpretation. When we restrict $v \in \text{span}(\boldsymbol{\Phi})$ and choose appropriate test functions, the conditions are equivalent to requiring that $v$ satisfies:

$$
v = \Proj \Bellman v,
$$

where $\Proj$ is a projection operator onto $\text{span}(\boldsymbol{\Phi})$ with respect to the weighted inner product $\langle \cdot, \cdot \rangle_w$. For Galerkin methods, $\Proj$ is orthogonal projection. For collocation, $\Proj$ is interpolation.

Since this is a fixed-point problem, we can apply successive approximation:

$$
v_{k+1} = \Proj \Bellman v_k.
$$

Under appropriate conditions (monotonicity of $\Proj$, or matching the weight to the operator for policy evaluation), this iteration converges to a solution of the weighted residual conditions.

### From Function Iteration to Parameter Updates

The iteration $v_{k+1} = \Proj \Bellman v_k$ is written in function space, but we actually work with finite-dimensional representations $v_k = \boldsymbol{\Phi} \boldsymbol{\theta}_k$, where $\boldsymbol{\theta}_k$ is the vector of coefficients. What does the function iteration become when we translate it to parameter space? The answer depends on whether we have a discrete or continuous state space.

**Finite state spaces.** When the state space $\mathcal{S}$ is finite with $|\mathcal{S}| = n$ states, we can enumerate all states as $s_1, \ldots, s_n$. The matrix $\boldsymbol{\Phi}$ has $n$ rows and $d$ columns, where entry $\Phi_{ij} = \varphi_j(s_i)$ is the evaluation of the $j$-th basis function at state $s_i$. The weighted inner product is:

$$
\langle u, v \rangle_w = \sum_{i=1}^n w(s_i) u(s_i) v(s_i) = u^\top \mathbf{W} v,
$$

where $\mathbf{W}$ is a diagonal matrix with $w(s_1), \ldots, w(s_n)$ on the diagonal. The Galerkin projection operator is $\Proj = \boldsymbol{\Phi}(\boldsymbol{\Phi}^\top \mathbf{W} \boldsymbol{\Phi})^{-1} \boldsymbol{\Phi}^\top \mathbf{W}$. The iteration $v_{k+1} = \Proj \Bellman v_k$ becomes:

$$
\boldsymbol{\theta}_{k+1} = (\boldsymbol{\Phi}^\top \mathbf{W} \boldsymbol{\Phi})^{-1} \boldsymbol{\Phi}^\top \mathbf{W} \mathbf{y},
$$

where $\mathbf{y}$ is the vector of Bellman operator evaluations at all states:

$$
\mathbf{y} = \begin{bmatrix} (\Bellman v_k)(s_1) \\ \vdots \\ (\Bellman v_k)(s_n) \end{bmatrix}.
$$

This is exactly the weighted least-squares formula. The matrix $\boldsymbol{\Phi}$ contains basis function evaluations at all states (the "design matrix"), $\mathbf{y}$ contains the targets, and the operator $(\boldsymbol{\Phi}^\top \mathbf{W} \boldsymbol{\Phi})^{-1} \boldsymbol{\Phi}^\top \mathbf{W}$ is the pseudoinverse that solves the regression problem. We fit $\boldsymbol{\Phi} \boldsymbol{\theta}$ to minimize $\sum_{i=1}^n w(s_i) [y_i - (\boldsymbol{\Phi} \boldsymbol{\theta})_i]^2$.

**Collocation with discrete points.** For collocation, we choose $n$ specific states $s_1, \ldots, s_n$ (the collocation points) and require $v_{k+1}(s_i) = y_i = (\Bellman v_k)(s_i)$ at those points. In matrix form: $\boldsymbol{\Phi} \boldsymbol{\theta}_{k+1} = \mathbf{y}$. If $\boldsymbol{\Phi}$ is square and invertible, $\boldsymbol{\theta}_{k+1} = \boldsymbol{\Phi}^{-1} \mathbf{y}$. This is exact interpolation: fit the basis functions to pass through the data points. If we have more collocation points than basis functions ($n > d$), we can use a least-squares fit instead.

**Continuous or large state spaces.** For Galerkin projection with continuous state spaces, the orthogonality conditions are integrals:

$$
\int_{\mathcal{S}} w(s) [v_{k+1}(s) - (\Bellman v_k)(s)] \varphi_j(s) ds = 0, \quad j = 1, \ldots, d.
$$

We cannot evaluate this integral analytically. Instead, we approximate it using numerical quadrature or Monte Carlo sampling. If we sample states $s_1, \ldots, s_n$ with appropriate weights, the integral becomes a weighted sum, and we are back in the finite-dimensional setting. The "data" being fit is now the set of Bellman operator evaluations at the sampled states.

In all cases, the projection operator $\Proj$ implements a fitting procedure: given target values $y_i = (\Bellman v_k)(s_i)$ at evaluation points (all states, collocation points, or sampled states), find coefficients $\boldsymbol{\theta}_{k+1}$ such that $v_{k+1} = \boldsymbol{\Phi} \boldsymbol{\theta}_{k+1}$ approximates these targets according to some criterion (weighted least squares for Galerkin, exact matching for collocation). Function iteration in the abstract becomes **successive fitting** in the computational implementation: generate targets, fit to targets, repeat.

This interpretation extends immediately to non-linear approximators. Neural networks, decision trees, and kernel methods are all procedures for constructing a function from sample data. They implement different forms of generalization:

- **Neural networks**: Fit parameters $\boldsymbol{\theta}$ by gradient descent on $\sum_i [y_i - v_{\boldsymbol{\theta}}(s_i)]^2$
- **Decision trees**: Partition the state space and assign constant values to regions based on the samples
- **Kernel methods**: Construct $v(s) = \sum_i \alpha_i k(s, s_i)$ with kernel-weighted contributions from all samples

Each method takes the dataset $\{(s_1, y_1), \ldots, (s_n, y_n)\}$ where $y_i = (\Bellman v_k)(s_i)$ and produces a function $v_{k+1}: \mathcal{S} \to \mathbb{R}$. This is the same conceptual operation as $\Proj$, just implemented with different approximation classes and optimization procedures.
...
The abstraction $\mathtt{fit}$ encapsulates all the complexity of function approximation, whether that involves solving a linear system, running gradient descent for thousands of steps, growing a decision tree, or training an ensemble. The projection operator $\Proj$ from earlier in the chapter is simply one particular instantiation of $\mathtt{fit}$: when $\mathcal{F}$ is a linear subspace and we minimize weighted squared error, we recover Galerkin or collocation methods. Neural networks, decision trees, and kernel methods extend this framework beyond linear approximations, trading theoretical tractability for representational flexibility.

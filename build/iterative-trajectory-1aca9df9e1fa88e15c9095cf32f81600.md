...
**DDP**. Both are local trajectory optimizers; their names need not determine
the order in which their ingredients are learned. Here the backward recursion
is derived by substitution and quadratic minimization.
...
control limits, and the same stopping settings. The following table reports

...
$$
(\mathrm{L}_\alpha v)(s) = \max_{\pi(\cdot|s) \in \Delta(\mathcal{A})}\left\{\sum_a \pi(a|s)[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]] + \alpha \mathcal{H}(\pi(\cdot|s))\right\}
$$

where $\mathcal{H}(\pi(\cdot|s)) = -\sum_a \pi(a|s)\log \pi(a|s)$ represents the entropy of the policy at state $s$, $\alpha > 0$ is the entropy regularization weight, and $\Delta(\mathcal{A})$ denotes the probability simplex over actions.
...
$$
J(\pi) = \sum_a \pi(a|s)\left[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]\right] + \alpha \sum_a \pi(a|s)\log \pi(a|s)
$$

subject to the normalization constraint $\sum_a \pi(a|s) = 1$. 

We form the Lagrangian by introducing the multiplier $\lambda$ for the normalization constraint:

$$
\mathcal{L}(\pi, \lambda) = \sum_a \pi(a|s)\left[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]\right] - \alpha \sum_a \pi(a|s)\log \pi(a|s) - \lambda\left(\sum_a \pi(a|s) - 1\right)
$$

Taking the partial derivative with respect to $\pi(a|s)$ and setting it to zero yields the first-order optimality condition:

$$
\frac{\partial \mathcal{L}}{\partial \pi(a|s)} = r(s,a) + \gamma \mathbb{E}_{s'}[v(s')] - \alpha(1 + \log \pi(a|s)) - \lambda = 0
$$
...
Using the normalization constraint $\sum_a \pi^*(a|s) = 1$ to eliminate $\lambda$, we obtain the softmax policy:

$$
\pi^*(a|s) = \frac{\exp\left(\frac{1}{\alpha}(r(s,a) + \gamma \mathbb{E}_{s'}[v(s')])\right)}{\sum_{a'} \exp\left(\frac{1}{\alpha}(r(s,a') + \gamma \mathbb{E}_{s'}[v(s')])\right)}
$$
...
$$
\begin{align*}
v(s) &= \sum_a \pi^*(a|s)[r(s,a) + \gamma \mathbb{E}_{s'}[v(s')]] + \alpha \mathcal{H}(\pi^*(\cdot|s)) \\

...
mean away from zero. Let $S_i$ be the accumulated state cost along rollout $i$, including
its terminal cost. MPPI assigns it the positive score $\exp(-S_i/\lambda)$
...
The Gaussian reference already favors small controls. In this formulation,
that preference accounts for control effort, which is why $S_i$ includes
state and terminal costs without adding the effort penalty again. The
derivation after the worked example makes this relationship explicit. It
also explains why sampling around a nonzero $\mathbf U$ requires an
importance correction to the displayed weights; see @sec-mppi-distribution.
In the stochastic-control interpretation developed later, $\lambda$ is tied
to the control-effort penalty and the physical noise, so changing it can
change the control problem rather than only the numerical averaging rule.
...
We now reduce the control sequence to a single constant force applied for one interval.

...
We know that standard value iteration $v_{k+1} = \Bellman v_k$ converges because $\mathrm{L}$ is a $\gamma$-contraction in the sup norm. But now we're iterating with the **composed operator** $\Proj \Bellman$ instead of $\Bellman$ alone.
...
If $\Proj$ is an averager and $\mathrm{L}$ is the Bellman operator (a $\gamma$-contraction), then $\Proj \Bellman$ is a $\gamma$-contraction, and value iteration $v_{k+1} = \Proj \Bellman v_k$ converges to a unique fixed point.
```

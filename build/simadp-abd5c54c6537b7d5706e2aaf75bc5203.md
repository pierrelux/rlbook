...
These choices are independent but should be coordinated. The total error in evaluating the Bellman operator decomposes as:

$$ \text{Error} = \underbrace{\|\Bellman v - \Bellman \hat{v}\|}_{\text{projection error}} + \underbrace{\|\Bellman \hat{v} - \widehat{\Bellman}_Q \hat{v}\|}_{\text{quadrature error}} $$

where $\widehat{\Bellman}_Q$ denotes the Bellman operator with numerical quadrature. If the quadrature error dominates the projection error, we gain nothing from using a high-order basis. For example, using degree-10 Chebyshev polynomials (projection error $\approx 10^{-8}$ for smooth functions) with rectangle-rule quadrature (quadrature error $\approx 10^{-2}$) produces total error $\approx 10^{-2}$. We pay the cost of storing and manipulating 10 basis functions but achieve only first-order accuracy.

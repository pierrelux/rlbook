...
This is exactly the sample average estimator from the Monte Carlo chapter, applied now to transitions. Conceptually, fitted Q-iteration performs **plug-in approximate dynamic programming**. The plug-in principle is a term from statistics meaning: replace unknown quantities with whatever estimates we can construct from data. Here, we substitute the empirical distribution $\hat{P}_{\mathcal{B}_t}$ for the unknown transition law $P$ and run value iteration using this empirical Bellman operator:

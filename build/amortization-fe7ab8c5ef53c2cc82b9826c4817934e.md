...
For a single observed trajectory, $R = \sum_t \gamma^t \varepsilon_t$ (accumulated noise). In expectation, $\mathbb{E}[R(s_0:s_d)] = 0$ when actions are sampled from $\pi^*$.

This constraint holds for any observed action sequence, regardless of which policy generated it. The behavior policy never appears—we only evaluate $\pi^*$ at the observed actions. This enables off-policy learning without importance sampling.

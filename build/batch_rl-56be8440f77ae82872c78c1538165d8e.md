...
All value-based reinforcement learning algorithms share the same two-level structure:

```
repeat:
    construct dataset D_fit from transitions
    θ_new ← fit(D_fit, θ_init, K)
```

The outer loop applies the Bellman operator by computing targets from the current Q-function and observed transitions. This produces a supervised learning dataset $\mathcal{D}^{\text{fit}} = \{((s_i, a_i), y_i)\}$ where each target is $y_i = r_i + \gamma \max_{a'} q(s'_i, a'; \theta)$ using the single observed next state $s'_i$. The inner loop solves a regression problem: find parameters $\theta$ that make $q(s_i, a_i; \theta) \approx y_i$.

The `fit` operation resembles scikit-learn's API but with additional controls that matter for reinforcement learning:

$$
\texttt{fit}(\mathcal{D}^{\text{fit}}, \boldsymbol{\theta}_{\text{init}}, K) \to \boldsymbol{\theta}
$$

This takes a regression dataset, an initialization point, and a truncation parameter $K$ controlling how many optimization steps to take. Standard supervised learning uses random initialization and runs to convergence ($K = \infty$). Reinforcement learning algorithms vary these: warm starting from the previous iteration's parameters ($\boldsymbol{\theta}_{\text{init}} = \boldsymbol{\theta}_n$), taking only $K \in \{1, \ldots, 10\}$ gradient steps, or somewhere in between.

The apparent diversity of methods (FQI, NFQI, Q-learning, DQN, Double DQN, Soft Q) emerges from independent choices about how to construct $\mathcal{D}^{\text{fit}}$, what to pass to `fit`, and how to implement the regression. This section maps that design space.

...
This is the principle behind double Q-learning {cite}`van2016deep`. In the tabular setting with independent Monte Carlo updates to two separate Q-tables, the algorithm alternates which table selects and which evaluates at each step.

```{prf:algorithm} Double Q-Learning (Tabular)
:label: double-q-learning

**Input:** Step sizes $\alpha_k$, discount $\gamma$, exploration parameter $\varepsilon$

**Output:** Two Q-function estimates $Q^{(1)}$, $Q^{(2)}$

1. Initialize $Q^{(1)}(s,a)$ and $Q^{(2)}(s,a)$ for all $s,a$
2. **for** each episode or interaction step **do**
3. $\quad$ Observe current state $s$
4. $\quad$ With probability $\varepsilon$, select a random action $a$; otherwise select
   $$a \leftarrow \arg\max_{a'} \big(Q^{(1)}(s,a') + Q^{(2)}(s,a')\big)$$
5. $\quad$ Execute $a$, observe reward $r$ and next state $s'$
6. $\quad$ With probability $1/2$, update $Q^{(1)}$:
7. $\quad\quad$ $a^\star \leftarrow \arg\max_{a'} Q^{(1)}(s',a')$
8. $\quad\quad$ $Y^{(1)} \leftarrow r + \gamma \, Q^{(2)}(s', a^\star)$
9. $\quad\quad$ $Q^{(1)}(s,a) \leftarrow Q^{(1)}(s,a) + \alpha_k (Y^{(1)} - Q^{(1)}(s,a))$
10. $\quad$ Otherwise, update $Q^{(2)}$ symmetrically:
11. $\quad\quad$ $a^\star \leftarrow \arg\max_{a'} Q^{(2)}(s',a')$
12. $\quad\quad$ $Y^{(2)} \leftarrow r + \gamma \, Q^{(1)}(s', a^\star)$
13. $\quad\quad$ $Q^{(2)}(s,a) \leftarrow Q^{(2)}(s,a) + \alpha_k (Y^{(2)} - Q^{(2)}(s,a))$
14. **end for**
```

The behavior policy at step 4 uses the sum or average of both Q-functions to select actions. This combines information from both estimates and reduces variance. The update step alternates which function selects and which evaluates, maintaining symmetry between the two estimates.

For function approximation with parametric Q-functions $q^{(1)}(s,a; \theta^{(1)})$ and $q^{(2)}(s,a; \theta^{(2)})$, the same principle applies. Each update uses gradient descent on the squared error between the target and the current estimate:

$$
\begin{aligned}
\theta^{(1)} &\leftarrow \theta^{(1)} - \alpha \nabla_{\theta^{(1)}} \big(q^{(1)}(s,a; \theta^{(1)}) - Y^{(1)}\big)^2, \\
\theta^{(2)} &\leftarrow \theta^{(2)} - \alpha \nabla_{\theta^{(2)}} \big(q^{(2)}(s,a; \theta^{(2)}) - Y^{(2)}\big)^2,
\end{aligned}
$$

where the targets $Y^{(1)}$ and $Y^{(2)}$ follow the decoupled selection-evaluation pattern. If the two function approximators have approximately independent errors at the next state (through independent initialization, different update schedules, or different data), then the action selection and evaluation steps use effectively independent noise, removing the main source of maximization bias.

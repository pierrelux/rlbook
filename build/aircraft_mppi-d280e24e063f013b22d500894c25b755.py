...
def evaluate_tapes(aircraft, initial, tapes, dts, gusts, config):
    ...
    burn = initial[3]-x[..., 3]
    cost = burn + 80*(horizontal/config.horizontal_tolerance)**2 + 80*(vertical/config.vertical_tolerance)**2 + 1e5*accumulated
...
def optimize(aircraft, state, observed_gust, config, clock_fraction, remaining, stochastic, rng):
    ...
    max_steps = int(np.ceil(remaining*1.12/config.prediction_dt))+2
    ...
    for _ in range(config.iterations):
        ...
        zero = observed_gust[None, None, :]*np.exp(-np.arange(max_steps)[None, :, None]*config.prediction_dt/config.gust_tau)
        _, check = evaluate_tapes(aircraft, state, proposed_tape, proposed_dt, zero, config)
        feasible = check['horizontal'][0, 0] <= config.horizontal_tolerance and check['vertical'][0, 0] <= config.vertical_tolerance and check['worst'][0, 0] <= 1e-7
...
def simulate(aircraft, config, mode, actual_gust, seed=None):
    ...
    while x[4] < scheduled_end-1e-6 and tick < 100:
        ...
        while x[4] < execution_end-1e-6:
            ...
            tape_elapsed += dt
        tick += 1
    return {'mode': mode, 'states': np.asarray(states), 'controls': np.asarray(controls), 'gusts': np.asarray(gust_record), 'dts': np.asarray(dt_record), 'replans': replans}
...
def audit(aircraft, run, config, fine_dt=5.):
    ...
    return {
        ...
        'min_vertical_speed_mps': float(controls[:, 1].min()),
        'max_vertical_speed_mps': float(controls[:, 1].max()),
        'cruise_samples': int(np.sum(np.abs(controls[:, 1]) < .5)),
        'planning_seconds': float(sum(p['runtime_s'] for p in run['replans'])),
        'mean_ess': float(np.mean([p['ess'] for p in run['replans']])),

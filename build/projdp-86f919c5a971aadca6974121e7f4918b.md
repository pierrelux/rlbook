...
From this analytical viewpoint, we recognize that the solution satisfies $\hat{v} = \Proj \BellmanPi \hat{v}$, where $\Proj$ is orthogonal projection onto $\text{span}(\boldsymbol{\Phi})$ with respect to the $\xi$-weighted inner product:
...
The weighting by $\xi$ is not arbitrary. Temporal difference learning performs stochastic updates using individual transitions: $\theta_{k+1} = \theta_k + \alpha_k (r + \gamma v_{\theta_k}(s') - v_{\theta_k}(s)) \nabla v_{\theta_k}(s)$, with states sampled from $\xi$. The ODE analysis of this stochastic process (Borkar-Meyn theory) shows convergence to the fixed point of the $\xi$-weighted projected Bellman operator. LSTD solves for this fixed point in closed form.
...
$$
\|v\|_\xi^2 = \|\Proj v\|_\xi^2 + \|v - \Proj v\|_\xi^2 \geq \|\Proj v\|_\xi^2.
$$

Therefore $\|\Proj v\|_\xi \leq \|v\|_\xi$ for all $v$, so $\Proj$ is non-expansive in $\|\cdot\|_\xi$.

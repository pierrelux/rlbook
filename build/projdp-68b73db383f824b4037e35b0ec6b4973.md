...
### What $\Proj$ Is Actually Doing: Filling in the Blanks

The iteration $v_{k+1} = \Proj \Bellman v_k$ appears to be a function space operation: apply the Bellman operator to get a new function, then project it onto the approximation space. But this is not how it works computationally. We cannot evaluate $\Bellman v_k$ at every state in a continuous or large state space. We cannot form the complete function $\Bellman v_k: \mathcal{S} \to \mathbb{R}$ and then project it.

What we can do is evaluate $\Bellman v_k$ at sample points. For any state $s$, we can compute:

$$
(\Bellman v_k)(s) = \max_a \left[r(s,a) + \gamma \int p(s'|s,a) v_k(s') ds'\right].
$$

This is a finite computation: numerical optimization and numerical integration at one point. So we evaluate at sample states $s_1, \ldots, s_n$ to get numbers $y_1 = (\Bellman v_k)(s_1), \ldots, y_n = (\Bellman v_k)(s_n)$.

Now we need a function $v_{k+1}: \mathcal{S} \to \mathbb{R}$ defined everywhere. This is what the projection operator $\Proj$ is doing computationally: **it fills in the blanks**. It takes the values we computed at sample points and constructs a complete function by generalizing from those samples. The projection operation is:

1. Sample states $s_1, \ldots, s_n$
2. Evaluate targets: $y_i = (\Bellman v_k)(s_i)$
3. Find $v_{k+1} \in \text{span}(\boldsymbol{\Phi})$ that satisfies the weighted residual conditions with respect to these samples

For collocation, step 3 means: find $v_{k+1} = \sum_j \theta_j \varphi_j$ such that $v_{k+1}(s_i) = y_i$ for all $i$. This is interpolation. We generalize from the point evaluations to the entire domain by constructing a function that passes through the sample points.

For Galerkin, step 3 means: find $v_{k+1} = \sum_j \theta_j \varphi_j$ that minimizes $\sum_i w(s_i) [v_{k+1}(s_i) - y_i]^2$. This is weighted least-squares regression. We generalize from the point evaluations by finding the best fit in the approximation space.

In both cases, the projection operator $\Proj$ is the mechanism that takes discrete samples $\{(s_i, y_i)\}$ and produces a function defined everywhere on $\mathcal{S}$. **Projection is generalization from samples.** The choice of projection method—collocation, Galerkin, or others—determines how we generalize: exact interpolation, least-squares fit, or some other criterion.

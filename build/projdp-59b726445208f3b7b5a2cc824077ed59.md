...
This is still expressed in terms of both the policy $\pi$ and the value function $v^*$. However, for problems where the envelope theorem applies, we can eliminate $v^*$ entirely. Taking the derivative of the Bellman optimality equation with respect to $s$ gives $\nabla_s v^*(s)$ in terms of the policy and value function; recursive substitution then eliminates $v^*$, leaving a **discrete-time Euler equation**: a functional equation in the policy $\pi(\cdot)$ alone. The precise form depends on whether transitions are deterministic or stochastic, and whether the state space is discrete or continuous.

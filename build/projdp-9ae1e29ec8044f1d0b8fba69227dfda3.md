...
Physics-informed neural networks (PINNs) instantiate this for Bellman equations. A neural network $v_{\boldsymbol{\theta}}$ approximates the value function by minimizing the sampled residual norm:

$$
\mathcal{L}(\boldsymbol{\theta}) = \mathbb{E}_{s \sim \mu} \big[ (v_{\boldsymbol{\theta}}(s) - (\Bellman v_{\boldsymbol{\theta}})(s))^2 \big].
$$

Minimizing this by gradient descent implements the orthogonality condition $\langle R[v_{\boldsymbol{\theta}}], \frac{\partial v_{\boldsymbol{\theta}}}{\partial \theta_i} \rangle = 0$ in a Monte Carlo approximation of the $L^2(\mu)$ inner product. Weak adversarial networks make the dual structure explicit: one network represents the trial function $v_{\boldsymbol{\theta}}$, another represents the test function $w_{\boldsymbol{\phi}}$, and training optimizes

...
All value-based reinforcement learning algorithms share the same two-level structure. The outer loop applies the Bellman operator $\mathcal{L}$ to the current Q-function at observed state-action pairs, producing a sequence of supervised learning problems. At iteration $n$, we construct a regression dataset $(X_n, y_n)$ where $X_n = \{(s_i, a_i)\}$ are the state-action pairs and $y_n = \{\widehat{\mathcal{L}}_N q(s_i, a_i; \boldsymbol{\theta}_n)\}$ are targets computed as Monte Carlo approximations of $(\mathcal{L}q)(s_i, a_i)$ using $N$ samples from the transition distribution. The inner loop solves the regression problem: find parameters $\boldsymbol{\theta}$ that make $q(s_i, a_i; \boldsymbol{\theta}) \approx y_{n,i}$. We can write this abstractly as:

$$
\begin{aligned}
&\textbf{repeat } n = 0, 1, 2, \ldots \\
&\quad \text{Construct } (X_n, y_n) \text{ where } y_{n,i} = \widehat{\mathcal{L}}_N q(s_i, a_i; \boldsymbol{\theta}_n) \text{ for each } (s_i, a_i) \in X_n \\
&\quad \boldsymbol{\theta}_{n+1} \leftarrow \texttt{fit}(X_n, y_n, \boldsymbol{\theta}_{\text{init}}, K) \\
...
The key insight is that we solve a **sequence** of regression problems $(X_0, y_0), (X_1, y_1), (X_2, y_2), \ldots$, where the input points $X_n$ may stay fixed (batch setting) or change (online setting), but the targets $y_n$ always change because they depend on the current Q-function $q(\cdot; \boldsymbol{\theta}_n)$. Each outer iteration recomputes targets using the updated Q-function, then fits a new Q-function to these targets.

The apparent diversity of methods (FQI, NFQI, Q-learning, DQN, Double DQN, Soft Q) emerges from independent choices about how to construct $(X_n, y_n)$, what initialization and truncation to pass to `fit`, and which loss function $\ell$ and function approximator $q(s,a; \boldsymbol{\theta})$ to use. In practice, we typically have $N=1$ (single observed next state per transition) and $\widehat{\mathcal{L}}_1 q(s_i, a_i; \boldsymbol{\theta}_n) = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}_n)$ where $(s_i, a_i, r_i, s'_i)$ is the observed transition tuple. This section maps that design space.

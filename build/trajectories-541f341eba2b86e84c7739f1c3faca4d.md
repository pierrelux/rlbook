...
with tolerances of $0.1$ degree and $0.002$ degree per day. The primary linear
program minimizes the worst final extra altitude loss:

These are unwrapped directed differences. Because the rows of $G$ sum to zero,
...
This lexicographic step chooses a low-variation member of the primary optimal
set. It does not materially worsen the primary worst-loss objective merely to
make a prettier command.

### One immutable plan, two plant models
...
nonlinear replay asks a different question: whether that one-shot plan survives
the declared model change. It does not. We deliberately do not add an MPC
controller to this chapter. [Closing the loop by replanning](mpc.md#closing-the-loop-by-replanning)
will show how new state information can alter the remaining controls.
...
The opening CubeSat example made that finite-horizon structure concrete. A
daily linear model turned three complete high-drag schedules into one linear
program with terminal phasing constraints and an altitude-loss objective. Its
nominal feasibility certificate applied only to that planning model. Replaying
the unchanged schedule through the nonlinear variable-density plant exposed a
large accumulated phase miss, separating optimization accuracy from model
validity.

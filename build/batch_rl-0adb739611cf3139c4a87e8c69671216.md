...
The apparent diversity of methods (FQI, NFQI, Q-learning, DQN, Double DQN, Soft Q) emerges from independent choices about how to construct $(X^{(n)}, y^{(n)})$, what initialization and truncation to pass to `fit`, and which loss function $\ell$ and function approximator $q(s,a; \boldsymbol{\theta})$ to use. In practice, we typically have $N=1$ (single observed next state per transition) and $\widehat{\mathcal{L}}_1 q(s_i, a_i; \boldsymbol{\theta}^{(n)}) = r_i + \gamma \max_{a'} q(s'_i, a'; \boldsymbol{\theta}^{(n)})$ where $(s_i, a_i, r_i, s'_i)$ is the observed transition tuple. This section maps that design space.

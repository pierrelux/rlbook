...
$$
\mathcal \mathcal L(\mathbf x,\boldsymbol\lambda,\boldsymbol\mu)
...
=\inf_{\mathbf x}\mathcal \mathcal L(\mathbf x,\boldsymbol\lambda,\boldsymbol\mu).
$$
...
$$
q(\boldsymbol\lambda,\boldsymbol\mu)
\le\mathcal \mathcal L(\bar{\mathbf x},\boldsymbol\lambda,\boldsymbol\mu)
\le f(\bar{\mathbf x}).
$$
...
$$
\mathcal \mathcal L(\mathbf x^\star,\boldsymbol\lambda,\boldsymbol\mu)
\le
\mathcal \mathcal L(\mathbf x^\star,\boldsymbol\lambda^\star,\boldsymbol\mu^\star)
\le
\mathcal \mathcal L(\mathbf x,\boldsymbol\lambda^\star,\boldsymbol\mu^\star)
$$
...
$$
\begin{aligned}
\nabla_{\mathbf x}\mathcal \mathcal L(\mathbf x^\star,\boldsymbol\lambda^\star,\boldsymbol\mu^\star)
...
\qquad \mathcal \mathcal L(x,\lambda)=-x^2+\lambda x.
$$
...
Yet $\mathcal \mathcal L(x,0)=-x^2$ has a strict maximum at zero. For every fixed
...
\begin{aligned}
\mathcal \mathcal L(\mathbf x,\boldsymbol\lambda^\star,\boldsymbol\mu^\star)
&\ge \mathcal \mathcal L(\mathbf x^\star,\boldsymbol\lambda^\star,\boldsymbol\mu^\star)\\
&\quad+\nabla_{\mathbf x}\mathcal \mathcal L(\mathbf x^\star,\boldsymbol\lambda^\star,\boldsymbol\mu^\star)^\top
...
$\mathcal \mathcal L(\mathbf x^\star,\boldsymbol\lambda,\boldsymbol\mu)\le f(\mathbf x^\star)$,
while complementary slackness gives
$\mathcal \mathcal L(\mathbf x^\star,\boldsymbol\lambda^\star,\boldsymbol\mu^\star)=f(\mathbf x^\star)$.
...
$$
\begin{bmatrix}
\nabla^2_{\mathbf{x}\mathbf{x}} \mathcal L(\mathbf{x}^k, \boldsymbol{\lambda}^k) & \nabla \mathbf{h}(\mathbf{x}^k)^T \\
...
def solve_qp_subproblem(x, lambda_, mu):
    ...
    jac_g_np = np.array(jac_h(x))
    jac_h_np = np.array(jac_g(x))
    h_np = np.array(h(x))
    g_np = np.array(g(x))
    ...
    constraints = [
        jac_g_np @ delta_x + h_np == 0,
        jac_h_np @ delta_x + g_np <= 0
    ]
...
plt.fill_between(x1_ineq, x2_lower, 2.5, color='gray', alpha=0.2, hatch='\\/...', label='Feasible Region')

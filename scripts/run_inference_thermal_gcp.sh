#!/usr/bin/env bash
# Explicit maintainer workflow for the isolated L4 thermal identification run.
# It never promotes or replaces the completed full inference profile.

set -euo pipefail

PROJECT="potent-arcade-491015-g7"
ACCOUNT="pierreluc@carbonforge.ai"
VM_NAME="pierreluc-l4-rlbook-thermal"
MACHINE_TYPE="g2-standard-8"
IMAGE_FAMILY="pytorch-2-9-cu129-ubuntu-2204-nvidia-580"
IMAGE_PROJECT="deeplearning-platform-release"
REMOTE_SCRIPT="/var/tmp/profile_inference_gpu.py"
REMOTE_OUTPUT="/var/tmp/inference-serving-thermal"
RUN_STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
LOCAL_OUTPUT="${1:-data/inference_serving/thermal-identification-${RUN_STAMP}}"
STAGING_ROOT="${TMPDIR:-/private/tmp}"
STAGING_OUTPUT="${STAGING_ROOT%/}/rlbook-inference-thermal-${VM_NAME}-${RUN_STAMP}"
MAX_RUNTIME_HOURS="4"
MAX_RUNTIME="4h"
...
if [[ "${RLBOOK_RUN_THERMAL_IDENTIFICATION:-}" != "YES" ]]; then
  echo "Set RLBOOK_RUN_THERMAL_IDENTIFICATION=YES to authorize this bounded paid run." >&2
  exit 2
fi
...
ESTIMATED_MAX_EXPOSURE_USD="$(python3 - \
  ...
  "${PRIOR_FAILED_TRIAL_USD}" <<'PY'
from decimal import Decimal
import sys

hours, hourly, headroom, prior = map(Decimal, sys.argv[1:])
...
create_thermal_vm() {
  ...
  if create_output="$(gcloud compute instances create "${VM_NAME}" \
    ...
    --labels purpose=rlbook-thermal,owner=pierreluc,provisioning=standard \
    2>&1)"; then
    printf '%s\n' "${create_output}"
    return 0
  fi
...
START_COMMAND="mkdir -p '${REMOTE_OUTPUT}'; rm -f '${REMOTE_OUTPUT}/thermal.complete' '${REMOTE_OUTPUT}/thermal.failed'; nohup python3 '${REMOTE_SCRIPT}' --mode thermal-identification --output-directory '${REMOTE_OUTPUT}' --source-git-revision '${SOURCE_GIT_REVISION}' --source-worktree-state '${SOURCE_WORKTREE_STATE}' --launcher-script-sha256 '${LAUNCHER_SCRIPT_SHA256}' --cloud-project '${PROJECT}' --cloud-zone '${SELECTED_ZONE}' --provisioning-model STANDARD --machine-type '${MACHINE_TYPE}' >'${REMOTE_OUTPUT}/driver.log' 2>&1 </dev/null & thermal_pid=\$!; echo \$thermal_pid >'${REMOTE_OUTPUT}/thermal.pid'"
gcloud compute ssh "${VM_NAME}" \
  --project "${PROJECT}" \
  --zone "${SELECTED_ZONE}" \
...
copy_progress() {
  local remote_file
  for remote_file in l4_thermal_telemetry.csv l4_thermal_requests.csv thermal_manifest.json thermal_vllm.log driver.log bootstrap.log thermal.complete thermal.failed; do
  ...
  if ! marker_list="$(gcloud compute ssh "${VM_NAME}" \
    --project "${PROJECT}" \
    --zone "${SELECTED_ZONE}" \
    --command "sudo -n find '${REMOTE_OUTPUT}' -maxdepth 1 -type f -name 'thermal-block-*' -printf '%f\\n' 2>/dev/null | sort" \
    2>/dev/null)"; then
    return 1
  fi
...
while true; do
  ...
  if ! REMOTE_BLOCKS="$(gcloud compute ssh "${VM_NAME}" \
    --project "${PROJECT}" \
    --zone "${SELECTED_ZONE}" \
    --command "sudo -n find '${REMOTE_OUTPUT}' -maxdepth 1 -name 'thermal-block-*.done' -print 2>/dev/null | sort" \
    2>/dev/null)"; then
  ...
  if gcloud compute ssh "${VM_NAME}" \
    --project "${PROJECT}" \
    --zone "${SELECTED_ZONE}" \
    --command "sudo -n test -f '${REMOTE_OUTPUT}/thermal.complete'" >/dev/null 2>&1; then
    if copy_progress; then
      break
    fi
    ...
    --project "${PROJECT}" \
    --zone "${SELECTED_ZONE}" \
    --command "sudo -n test -f '${REMOTE_OUTPUT}/thermal.failed'" >/dev/null 2>&1; then
    copy_progress || true
...
python3 - \
  ...
  "$(pwd)/scripts/run_inference_thermal_gcp.sh" <<'PY'
import csv
import hashlib
import json
import math
...
manifest = json.loads((root / "thermal_manifest.json").read_text(encoding="utf-8"))
if manifest.get("status") != "complete" or manifest.get("mode") != "thermal-identification":
    raise SystemExit("thermal manifest is not a complete thermal-identification run")
if manifest.get("schema_version") != 2 or manifest.get("protocol") != "cold-start-pulses-v2":
    raise SystemExit("thermal manifest is not the safe cold-start pulse protocol")
for required in ("l4_thermal_telemetry.csv", "l4_thermal_requests.csv"):
...
if completed != scheduled:
    raise SystemExit("thermal block completion order does not match the fixed schedule")
if [sequence["split"] for sequence in manifest["schedule"]] != [
    "training",
    "training",
    "validation",
]:
    raise SystemExit("thermal train/validation split changed")
if not all(
...
with (root / "l4_thermal_telemetry.csv").open(newline="", encoding="utf-8") as stream:
    telemetry_data = list(csv.DictReader(stream))
telemetry_rows = len(telemetry_data)
with (root / "l4_thermal_requests.csv").open(newline="", encoding="utf-8") as stream:
...
for completed_file in l4_thermal_telemetry.csv l4_thermal_requests.csv thermal_vllm.log driver.log bootstrap.log thermal.complete thermal_manifest.json; do
  if [[ -f "${STAGING_OUTPUT}/${completed_file}" ]]; then
    mv "${STAGING_OUTPUT}/${completed_file}" "${LOCAL_OUTPUT}/${completed_file}"
  fi
done
for marker in "${STAGING_OUTPUT}"/thermal-block-*; do
  if [[ -f "${marker}" ]]; then
    mv "${marker}" "${LOCAL_OUTPUT}/$(basename "${marker}")"
  fi
done

echo "Verified thermal-identification data are in ${LOCAL_OUTPUT}. The VM will now be deleted."
